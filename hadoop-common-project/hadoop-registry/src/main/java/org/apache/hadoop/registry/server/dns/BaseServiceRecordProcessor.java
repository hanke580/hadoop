...
public abstract class BaseServiceRecordProcessor implements ServiceRecordProcessor {
    ...
    abstract class ContainerRecordDescriptor<T> extends RecordDescriptor<T> {
        ...
        protected Name getContainerName() throws PathNotFoundException, TextParseException {
            String service = RegistryPathUtils.lastPathEntry(RegistryPathUtils.parentOf(RegistryPathUtils.parentOf(getPath())));
            String description = getRecord().description.toLowerCase();
            String user = RegistryPathUtils.getUsername(getPath());
            return Name.fromString(MessageFormat.format("{0}.{1}.{2}.{3}", description, service, user, domain));
        }
        ...
        protected Name getComponentName() throws PathNotFoundException, TextParseException {
            ...
            String user = RegistryPathUtils.getUsername(getPath());
            return Name.fromString(MessageFormat.format("{0}.{1}.{2}.{3}", component, service, user, domain));
        }
    }
    ...
    abstract class ApplicationRecordDescriptor<T> extends RecordDescriptor<T> {
        ...
        protected Name getServiceName() throws TextParseException {
            String user = RegistryPathUtils.getUsername(getPath());
            String service = String.format("%s.%s.%s", RegistryPathUtils.lastPathEntry(getPath()), user, domain);
