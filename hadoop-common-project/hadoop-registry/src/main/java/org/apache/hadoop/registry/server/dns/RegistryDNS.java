...
public class RegistryDNS extends AbstractService implements DNSOperations, ZoneSelector {
    ...
    private ConcurrentMap<Name, DNSKEYRecord> dnsKeyRecs = new ConcurrentHashMap<>();
    ...
    public RegistryDNS(String name) {
        ...
        executor = HadoopExecutors.newCachedThreadPool(new ThreadFactory() {
            ...
            public Thread newThread(Runnable r) {
                return new Thread(r, "RegistryDNS " + counter.getAndIncrement());
            }
        });
    }
    ...
    private void addNXT(Message response, int flags) throws DNSSEC.DNSSECException, IOException {
        Record nxtRecord = getNXTRecord(response.getSectionArray(Section.QUESTION)[0]);
    ...
    private final RegistryCommand addRecordCommand = new RegistryCommand() {
                ...
                try (CloseableLock lock = writeLock.lock()) {
                    ...
                    if (isDNSSECEnabled()) {
                        ...
                        try {
                            ...
                            RRSIGRecord rrsigRecord = DNSSEC.sign(rRset, dnskeyRecord, privateKey, inception, expiration);
        ...
        private void addDSRecord(Zone zone, Name name, int dClass, long dsTtl, Date inception, Date expiration) throws DNSSEC.DNSSECException {
            ...
            DNSKEYRecord dnskeyRecord = dnsKeyRecs.get(zone.getOrigin());
            DSRecord dsRecord = new DSRecord(name, dClass, dsTtl, DSRecord.Digest.SHA1, dnskeyRecord);
            zone.addRecord(dsRecord);
            LOG.info("Adding {}", dsRecord);
            rRset = zone.findExactMatch(dsRecord.getName(), dsRecord.getType());
            rrsigRecord = DNSSEC.sign(rRset, dnskeyRecord, privateKey, inception, expiration);
            rRset.addRR(rrsigRecord);
        }
