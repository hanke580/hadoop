...
public class TestRegistryDNS extends Assert {
    ...
    Record[] assertDNSQuery(String lookup, int type, int numRecs) throws IOException {
        ...
        assertEquals("wrong number of answer records", isSecure() ? numRecs * 2 : numRecs, recs.length);
        if (isSecure()) {
    ...
    public void testAAAALookup() throws Exception {
        ...
        getRegistryDNS().register("/registry/users/root/services/org-apache-slider/test1/components/" + "ctr-e50-1451931954322-0016-01-000002", record);
        // start assessing whether correct records are available
        Record[] recs = assertDNSQuery("ctr-e50-1451931954322-0016-01-000002.dev.test.", Type.AAAA, 1);
        assertEquals("wrong result", "172.17.0.19", ((AAAARecord) recs[0]).getAddress().getHostAddress());
