...
public class TestTFile {
    ...
    // write them twice
    private int writeSomeRecords(Writer writer, int start, int n) throws IOException {
        String value = "value";
    ...
    private int readAndCheckbytes(Scanner scanner, int start, int n) throws IOException {
        String value = "value";
        for (int i = start; i < (start + n); i++) {
            ...
            assertTrue("bytes for keys do not match " + keyStr + " " + new String(key), Arrays.equals(keyStr.getBytes(), key));
            ...
            assertTrue("bytes for keys do not match", Arrays.equals(keyStr.getBytes(), key));
    ...
    // write them twice
    private int writeLargeRecords(Writer writer, int start, int n) throws IOException {
    ...
    private int readLargeRecords(Scanner scanner, int start, int n) throws IOException {
        for (int i = start; i < (start + n); i++) {
            byte[] key = readKey(scanner);
            String keyStr = String.format(localFormatter, i);
            assertTrue("bytes for keys do not match", Arrays.equals(keyStr.getBytes(), key));
    ...
    private int writePrepWithKnownLength(Writer writer, int start, int n) throws IOException {
    ...
    private int readPrepWithKnownLength(Scanner scanner, int start, int n) throws IOException {
        for (int i = start; i < (start + n); i++) {
            String key = String.format(localFormatter, i);
    ...
    private int writePrepWithUnkownLength(Writer writer, int start, int n) throws IOException {
        for (int i = start; i < (start + n); i++) {
    ...
    private int readPrepWithUnknownLength(Scanner scanner, int start, int n) throws IOException {
    ...
    private void writeNumMetablocks(Writer writer, String compression, int n) throws IOException {
        for (int i = 0; i < n; i++) {
    ...
    private void someTestingWithMetaBlock(Writer writer, String compression) throws IOException {
    ...
    private void readNumMetablocks(Reader reader, int n) throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            ...
            byte[] b = new byte[len];
            din.readFully(b);
            assertTrue("faield to match metadata", Arrays.equals(("something to test" + i).getBytes(), b));
            din.close();
        }
    }
