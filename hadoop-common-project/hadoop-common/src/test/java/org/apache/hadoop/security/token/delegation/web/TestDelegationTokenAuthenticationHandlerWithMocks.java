...
public class TestDelegationTokenAuthenticationHandlerWithMocks {
    ...
    private Token<DelegationTokenIdentifier> testGetToken(String renewer, String service, Text expectedTokenKind) throws Exception {
        ...
        String queryString = DelegationTokenAuthenticator.OP_PARAM + "=" + op.toString() + "&" + DelegationTokenAuthenticator.RENEWER_PARAM + "=" + renewer;
        if (service != null) {
            queryString += "&" + DelegationTokenAuthenticator.SERVICE_PARAM + "=" + service;
        }
    ...
    private void testRenewToken(String testRenewer) throws Exception {
        Token<DelegationTokenIdentifier> dToken = (Token<DelegationTokenIdentifier>) handler.getTokenManager().createToken(UserGroupInformation.getCurrentUser(), testRenewer);
