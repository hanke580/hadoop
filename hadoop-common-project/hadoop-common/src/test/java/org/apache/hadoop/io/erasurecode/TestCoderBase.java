...
public abstract class TestCoderBase {
    ...
    protected void prepareBufferAllocator(boolean usingSlicedBuffer) {
        if (usingSlicedBuffer) {
            int roughEstimationSpace = chunkSize * (numDataUnits + numParityUnits) * 10;
            allocator = new SlicedBufferAllocator(usingDirectBuffer, roughEstimationSpace);
        } else {
    ...
    protected void prepare(Configuration conf, int numDataUnits, int numParityUnits, int[] erasedDataIndexes, int[] erasedParityIndexes, boolean usingFixedData) {
        ...
        this.erasedDataIndexes = erasedDataIndexes != null ? erasedDataIndexes : new int[] { 0 };
        this.erasedParityIndexes = erasedParityIndexes != null ? erasedParityIndexes : new int[] { 0 };
    ...
    protected void prepare(Configuration conf, int numDataUnits, int numParityUnits, int[] erasedDataIndexes, int[] erasedParityIndexes) {
        prepare(conf, numDataUnits, numParityUnits, erasedDataIndexes, erasedParityIndexes, false);
    }
    ...
    protected void prepare(int numDataUnits, int numParityUnits, int[] erasedDataIndexes, int[] erasedParityIndexes) {
        prepare(null, numDataUnits, numParityUnits, erasedDataIndexes, erasedParityIndexes, false);
    }
    ...
    protected int[] getErasedIndexesForDecoding() {
        int[] erasedIndexesForDecoding = new int[erasedDataIndexes.length + erasedParityIndexes.length];
        int idx = 0;
    ...
    protected ECChunk[] backupAndEraseChunks(ECChunk[] dataChunks, ECChunk[] parityChunks) {
        ECChunk[] toEraseChunks = new ECChunk[erasedDataIndexes.length + erasedParityIndexes.length];
        int idx = 0;
        for (int i = 0; i < erasedDataIndexes.length; i++) {
    ...
    protected ECChunk[] prepareOutputChunksForDecoding() {
        ECChunk[] chunks = new ECChunk[erasedDataIndexes.length + erasedParityIndexes.length];
        for (int i = 0; i < chunks.length; i++) {
            chunks[i] = allocateOutputChunk();
        }
        return chunks;
    ...
    protected void dumpSetting() {
        if (allowDump) {
            ...
            sb.append(" erasedDataIndexes=").append(Arrays.toString(erasedDataIndexes));
            sb.append(" erasedParityIndexes=").append(Arrays.toString(erasedParityIndexes));
