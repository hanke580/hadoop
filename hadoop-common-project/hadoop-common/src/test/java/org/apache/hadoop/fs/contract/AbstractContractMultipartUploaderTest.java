...
public abstract class AbstractContractMultipartUploaderTest extends AbstractFSContractTestBase {
    ...
    public void testSingleUpload() throws Exception {
        ...
        if (finalizeConsumesUploadIdImmediately()) {
            intercept(FileNotFoundException.class, () -> mpu.complete(file, partHandles, uploadHandle));
        } else {
     ...
     */
    protected PartHandle buildAndPutPart(final Path file, final UploadHandle uploadHandle, final int index, final MessageDigest origDigest) throws IOException {
     ...
     * @throws IOException IO failure.
     */
    protected PartHandle putPart(final Path file, final UploadHandle uploadHandle, final int index, final byte[] payload) throws IOException {
        ContractTestUtils.NanoTimer timer = new ContractTestUtils.NanoTimer();
        PartHandle partHandle = mpu(index).putPart(file, new ByteArrayInputStream(payload), index, uploadHandle, payload.length);
        timer.end("Uploaded part %s", index);
        LOG.info("Upload bandwidth {} MB/s", timer.bandwidthDescription(payload.length));
        return partHandle;
    }
     ...
     * @throws IOException IO failure
     */
    private PathHandle completeUpload(final Path file, final UploadHandle uploadHandle, final Map<Integer, PartHandle> partHandles, final MessageDigest origDigest, final int expectedLength) throws IOException {
        PathHandle fd = complete(file, uploadHandle, partHandles);
        FileStatus status = verifyPathExists(getFileSystem(), "Completed file", file);
        assertEquals("length of " + status, expectedLength, status.getLen());
        if (origDigest != null) {
     ...
     * @throws IOException IO failure
     */
    protected void verifyContents(final Path file, final MessageDigest origDigest, final int expectedLength) throws IOException {
        ContractTestUtils.NanoTimer timer2 = new ContractTestUtils.NanoTimer();
        assertArrayEquals("digest of source and " + file + " differ", origDigest.digest(), digest(file));
        timer2.end("Completed digest", file);
        LOG.info("Download bandwidth {} MB/s", timer2.bandwidthDescription(expectedLength));
    }
     ...
     * @throws IOException IO failure
     */
    private PathHandle complete(final Path file, final UploadHandle uploadHandle, final Map<Integer, PartHandle> partHandles) throws IOException {
        ContractTestUtils.NanoTimer timer = new ContractTestUtils.NanoTimer();
     ...
     */
    private void abortUpload(final Path file, UploadHandle uploadHandle) throws IOException {
    ...
    public void testMultipartUploadAbort() throws Exception {
        ...
        if (finalizeConsumesUploadIdImmediately()) {
            intercept(FileNotFoundException.class, () -> abortUpload(file, uploadHandle));
        } else {
            abortUpload(file, uploadHandle);
        }
    }
    ...
    public void testAbortEmptyUploadHandle() throws Exception {
        ...
        UploadHandle uploadHandle = BBUploadHandle.from(byteBuffer);
        intercept(IllegalArgumentException.class, () -> abortUpload(methodPath(), uploadHandle));
    }
    ...
    public void testCompleteEmptyUpload() throws Exception {
        ...
        intercept(IllegalArgumentException.class, () -> mpu.complete(dest, new HashMap<>(), handle));
    ...
    public void testCompleteEmptyUploadID() throws Exception {
        ...
        UploadHandle emptyHandle = BBUploadHandle.from(ByteBuffer.wrap(new byte[0]));
        Map<Integer, PartHandle> partHandles = new HashMap<>();
        PartHandle partHandle = putPart(dest, realHandle, 1, generatePayload(1, SMALL_FILE));
        partHandles.put(1, partHandle);
    ...
    public void testDirectoryInTheWay() throws Exception {
        ...
        partHandles.put(1, partHandle);
        fs.mkdirs(file);
        intercept(IOException.class, () -> completeUpload(file, uploadHandle, partHandles, null, size));
    ...
    public void testConcurrentUploads() throws Throwable {
        ...
        eventually(timeToBecomeConsistentMillis(), 500, () -> {
            ...
            assertEquals("File length in " + status, size2, status.getLen());
        });
