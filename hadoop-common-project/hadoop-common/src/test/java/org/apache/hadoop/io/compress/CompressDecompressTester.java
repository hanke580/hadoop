...
public class CompressDecompressTester<T extends Compressor, E extends Decompressor> {
    ...
    public CompressDecompressTester<T, E> withCompressDecompressPair(T compressor, E decompressor) {
        addPair(compressor, decompressor, Joiner.on("_").join(compressor.getClass().getCanonicalName(), decompressor.getClass().getCanonicalName()));
        return this;
    }
    ...
    public void test() throws InstantiationException, IllegalAccessException {
        ...
        for (TesterPair<T, E> pair : pairs) {
            for (CompressionTestStrategy strategy : stateges) {
                strategy.getTesterStrategy().assertCompression(pair.getName(), pair.getCompressor(), pair.getDecompressor(), Arrays.copyOf(originalRawData, originalRawData.length));
            }
        }
        endAll(pairs);
    }
    ...
    private void endAll(ImmutableList<TesterPair<T, E>> pairs) {
        for (TesterPair<T, E> pair : pairs) pair.end();
    }
        ...
        COMPRESS_DECOMPRESS_ERRORS(new TesterCompressionStrategy() {
            ...
            private boolean checkSetInputNullPointerException(Compressor compressor) {
                try {
                    ...
                    logger.error(joiner.join(compressor.getClass().getCanonicalName(), "checkSetInputNullPointerException error !!!"));
                }
                return false;
            }
            ...
            private boolean checkCompressNullPointerException(Compressor compressor, byte[] rawData) {
                try {
                    ...
                    logger.error(joiner.join(compressor.getClass().getCanonicalName(), "checkCompressNullPointerException error !!!"));
                }
                return false;
            }
            ...
            private boolean checkCompressNullPointerException(Decompressor decompressor, byte[] rawData) {
                try {
                    ...
                    logger.error(joiner.join(decompressor.getClass().getCanonicalName(), "checkCompressNullPointerException error !!!"));
                }
                return false;
            }
            ...
            private boolean checkSetInputNullPointerException(Decompressor decompressor) {
                try {
                    ...
                    logger.error(joiner.join(decompressor.getClass().getCanonicalName(), "checkSetInputNullPointerException error !!!"));
                }
                return false;
            }
            ...
            private boolean checkSetInputArrayIndexOutOfBoundsException(Compressor compressor) {
                try {
                    ...
                    logger.error(joiner.join(compressor.getClass().getCanonicalName(), "checkSetInputArrayIndexOutOfBoundsException error !!!"));
                }
                return false;
            }
            ...
            private boolean checkCompressArrayIndexOutOfBoundsException(Compressor compressor, byte[] rawData) {
                try {
                    ...
                    logger.error(joiner.join(compressor.getClass().getCanonicalName(), "checkCompressArrayIndexOutOfBoundsException error !!!"));
                }
                return false;
            }
            ...
            private boolean checkCompressArrayIndexOutOfBoundsException(Decompressor decompressor, byte[] rawData) {
                try {
                    ...
                    logger.error(joiner.join(decompressor.getClass().getCanonicalName(), "checkCompressArrayIndexOutOfBoundsException error !!!"));
                }
                return false;
            }
            ...
            private boolean checkSetInputArrayIndexOutOfBoundsException(Decompressor decompressor) {
                try {
                    ...
                    logger.error(joiner.join(decompressor.getClass().getCanonicalName(), "checkNullPointerException error !!!"));
                }
                return false;
            }
            ...
            void assertCompression(String name, Compressor compressor, Decompressor decompressor, byte[] originalRawData) {
                ...
                try {
                    compressor.reset();
                    // decompressor.end();
                    BlockCompressorStream blockCompressorStream = new BlockCompressorStream(bytesOut, compressor, 1024, 0);
                    blockCompressorStream.close();
                    // check compressed output
                    buf = bytesOut.toByteArray();
                    int emSize = emptySize.get(compressor.getClass());
                    Assert.assertEquals(joiner.join(name, "empty stream compressed output size != " + emSize), emSize, buf.length);
                    // use compressed output as input for decompression
                    bytesIn = new ByteArrayInputStream(buf);
                    // create decompression stream
                    blockDecompressorStream = new BlockDecompressorStream(bytesIn, decompressor, 1024);
                    // no byte is available because stream was closed
                    assertEquals(joiner.join(name, " return value is not -1"), -1, blockDecompressorStream.read());
                } catch (IOException e) {
                ...
                try {
                    if (originalRawData.length > maxSize) {
                        do {
                            ...
                            compressor.finish();
                            while (!compressor.finished()) {
                                compresSize = compressor.compress(operationBlock, 0, operationBlock.length);
                    ...
                    for (Integer step : blockLabels) {
                        ...
                        while (!decompressor.finished()) {
                            int dSize = decompressor.decompress(operationBlock, 0, operationBlock.length);
                            decompressOut.write(operationBlock, 0, dSize);
                        }
                        decompressor.reset();
                        off = off + step;
                    }
                    assertArrayEquals(joiner.join(name, "byte arrays not equals error !!!"), originalRawData, decompressOut.toByteArray());
                } catch (Exception ex) {
                    fail(joiner.join(name, ex.getMessage()));
                } finally {
                    try {
    ...
    private static <T extends Compressor, E extends Decompressor> boolean isAvailable(TesterPair<T, E> pair) {
        ...
        if (compressor.getClass().isAssignableFrom(Lz4Compressor.class) && (NativeCodeLoader.isNativeCodeLoaded()))
            return true;
        else if (compressor.getClass().isAssignableFrom(BuiltInZlibDeflater.class) && NativeCodeLoader.isNativeCodeLoaded())
            return true;
        else if (compressor.getClass().isAssignableFrom(ZlibCompressor.class)) {
            return ZlibFactory.isNativeZlibLoaded(new Configuration());
        } else if (compressor.getClass().isAssignableFrom(SnappyCompressor.class) && isNativeSnappyLoadable())
            return true;
        return false;
    }
    ...
    abstract static class TesterCompressionStrategy {
        ...
        abstract void assertCompression(String name, Compressor compressor, Decompressor decompressor, byte[] originalRawData);
    }
}
