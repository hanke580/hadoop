...
public class TestWinUtils {
    ...
    public void setUp() throws IOException {
        ...
        assumeWindows();
        TEST_DIR.mkdirs();
        assertTrue("Failed to create Test directory " + TEST_DIR, TEST_DIR.isDirectory());
    ...
    private void chmod(String mask, File file) throws IOException {
        Shell.execCommand(winutils, "chmod", mask, file.getCanonicalPath());
    }

    private void chmodR(String mask, File file) throws IOException {
        Shell.execCommand(winutils, "chmod", "-R", mask, file.getCanonicalPath());
    }

    private String ls(File file) throws IOException {
        return Shell.execCommand(winutils, "ls", file.getCanonicalPath());
    }

    private String lsF(File file) throws IOException {
        return Shell.execCommand(winutils, "ls", "-F", file.getCanonicalPath());
    }
    ...
    private void chown(String userGroup, File file) throws IOException {
        Shell.execCommand(winutils, "chown", userGroup, file.getCanonicalPath());
    }
    ...
    private void assertOwners(File file, String expectedUser, String expectedGroup) throws IOException {
        ...
        assertEquals(StringUtils.toLowerCase(expectedUser), StringUtils.toLowerCase(args[2]));
        assertEquals(StringUtils.toLowerCase(expectedGroup), StringUtils.toLowerCase(args[3]));
    }
