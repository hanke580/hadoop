...
public class TestTFileSeqFileComparison {
   ...
   */
    ...
    private void createTFile(String parameters, String compress) throws IOException {
        ...
        KVAppendable appendable = new TFileAppendable(fs, path, compress, options.minBlockSize, options.osOutputBufferSize, conf);
    ...
    private void readTFile(String parameters, boolean delFile) throws IOException {
        ...
        {
            Path path = new Path(options.rootDir, "TFile.Performance");
            KVReadable readable = new TFileReadable(fs, path, options.osInputBufferSize, conf);
    ...
    private void createSeqFile(String parameters, String compress) throws IOException {
        ...
        KVAppendable appendable = new SeqFileAppendable(fs, path, options.osOutputBufferSize, compress, options.minBlockSize);
        timeWrite(path, appendable, options.keyLength, options.valueLength, options.fileSize);
    }
    ...
    private void readSeqFile(String parameters, boolean delFile) throws IOException {
        ...
        Path path = new Path(options.rootDir, "SeqFile.Performance");
        KVReadable readable = new SeqFileReadable(fs, path, options.osInputBufferSize);
        timeRead(path, readable);
        if (delFile) {
            if (fs.exists(path)) {
                fs.delete(path, true);
    ...
    private static String parameters2String(MyOptions options) {
        return String.format("KLEN: %d-%d... VLEN: %d-%d...MinBlkSize: %.2fKB...Target Size: %.2fMB...Compression: ...%s", options.keyLength, options.keyLength * 2, options.valueLength, options.valueLength * 2, (double) options.minBlockSize / 1024, (double) options.fileSize / 1024 / 1024, options.compress);
    }
