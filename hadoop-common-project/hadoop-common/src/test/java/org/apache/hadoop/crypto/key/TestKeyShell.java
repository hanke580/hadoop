...
public class TestKeyShell {
    ...
    private void deleteKey(KeyShell ks, String keyName) throws Exception {
        int rc;
        outContent.reset();
        final String[] delArgs = { "delete", keyName, "-f", "-provider", jceksProvider };
        rc = ks.run(delArgs);
        assertEquals(0, rc);
        assertTrue(outContent.toString().contains(keyName + " has been " + "successfully deleted."));
    }
    ...
    public void testKeySuccessfulKeyLifecycle() throws Exception {
        ...
        outContent.reset();
        final String[] args3 = { "invalidateCache", keyName, "-provider", jceksProvider };
        rc = ks.run(args3);
        assertEquals(0, rc);
        assertTrue(outContent.toString().contains("key1 has been successfully " + "invalidated."));
