...
public class Crc32PerformanceTest {
    ...
    interface Crc32 {
        ...
        void verifyChunked(ByteBuffer data, int bytesPerCrc, ByteBuffer crcs, String filename, long basePos) throws ChecksumException;
        ...
        final class Native implements Crc32 {
            ...
            public void verifyChunked(ByteBuffer data, int bytesPerSum, ByteBuffer sums, String fileName, long basePos) throws ChecksumException {
                NativeCrc32.verifyChunkedSums(bytesPerSum, DataChecksum.Type.CRC32.id, sums, data, fileName, basePos);
            }
        ...
        final class NativeC implements Crc32 {
            ...
            public void verifyChunked(ByteBuffer data, int bytesPerSum, ByteBuffer sums, String fileName, long basePos) throws ChecksumException {
                if (data.isDirect()) {
                    NativeCrc32.verifyChunkedSums(bytesPerSum, DataChecksum.Type.CRC32C.id, sums, data, fileName, basePos);
                } else {
                    final int dataOffset = data.arrayOffset() + data.position();
                    final int crcsOffset = sums.arrayOffset() + sums.position();
                    NativeCrc32.verifyChunkedSumsByteArray(bytesPerSum, DataChecksum.Type.CRC32C.id, sums.array(), crcsOffset, data.array(), dataOffset, data.remaining(), fileName, basePos);
                }
            }
        ...
        abstract class AbstractCrc32<T extends Checksum> implements Crc32 {
            ...
            public void verifyChunked(ByteBuffer data, int bytesPerCrc, ByteBuffer sums, String filename, long basePos) throws ChecksumException {
                ...
                if (data.hasArray() && sums.hasArray()) {
                    DataChecksum.verifyChunked(type, algorithm, data.array(), data.position(), data.remaining(), bytesPerCrc, sums.array(), sums.position(), filename, basePos);
                } else {
                    DataChecksum.verifyChunked(type, algorithm, data, bytesPerCrc, sums, filename, basePos);
                }
            }
        }
    ...
    Crc32PerformanceTest(final int dataLengthMB, final int trials, final boolean direct) {
        ...
        if (NativeCrc32.isAvailable()) {
            ...
            GenericTestUtils.setLogLevel(getLogger(NativeCodeLoader.class), Level.TRACE);
        }
    }
    ...
    private ByteBuffer computeCrc(ByteBuffer dataBufs, int bytePerCrc, DataChecksum.Type type) {
        ...
        final DataChecksum checksum = DataChecksum.newDataChecksum(type, bytePerCrc);
    ...
    private ByteBuffer computeCrc(Class<? extends Crc32> clazz, ByteBuffer dataBufs, int bytePerCrc) throws Exception {
        ...
        final int size = 4 * (dataBufs.remaining() - 1) / bytePerCrc + 1;
        final ByteBuffer crcBufs = allocateByteBuffer(size);
        final DataChecksum checksum = DataChecksum.newDataChecksum(crc.crcType(), bytePerCrc);
        checksum.calculateChunkedSums(dataBufs, crcBufs);
        return crcBufs;
    }

    private void doBench(final List<Class<? extends Crc32>> crcTargets) throws Exception {
    ...
    private void doBench(final List<Class<? extends Crc32>> crcTargets, final ByteBuffer[] dataBufs, final int bytePerCrc, final PrintStream outCrc) throws Exception {
        final ByteBuffer[] crcBufs = new ByteBuffer[dataBufs.length];
        final ByteBuffer[] crcBufsC = new ByteBuffer[dataBufs.length];
        for (int i = 0; i < dataBufs.length; i++) {
            crcBufs[i] = computeCrc(dataBufs[i], bytePerCrc, DataChecksum.Type.CRC32);
            crcBufsC[i] = computeCrc(dataBufs[i], bytePerCrc, DataChecksum.Type.CRC32C);
        }
        ...
        for (int numThreads = 1; numThreads <= dataBufs.length; numThreads <<= 1) {
            ...
            for (Class<? extends Crc32> c : crcTargets) {
                ...
                printCell(String.format("%9.1f", result.mbps), c.getSimpleName().length() + 1, outCrc);
    ...
    private BenchResult doBench(Class<? extends Crc32> clazz, final int numThreads, final ByteBuffer[] dataBufs, final ByteBuffer[] crcBufs, final int bytePerCrc) throws Exception {
