...
public abstract class CryptoStreamsTestBase {
    ...
    protected static final byte[] key = { 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16 };

    protected static final byte[] iv = { 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08 };
    ...
    private int preadAll(PositionedReadable in, byte[] b, int off, int len) throws IOException {
        int n = 0;
        int total = 0;
        while (n != -1) {
            total += n;
    ...
    private int byteBufferPreadAll(ByteBufferPositionedReadable in, ByteBuffer buf) throws IOException {
        int n = 0;
        int total = 0;
        while (n != -1) {
            total += n;
    ...
    private void byteBufferPreadCheck(ByteBufferPositionedReadable in) throws Exception {
    ...
    protected abstract OutputStream getOutputStream(int bufferSize, byte[] key, byte[] iv) throws IOException;

    protected InputStream getInputStream(int bufferSize) throws IOException {
        return getInputStream(bufferSize, key, iv);
    }

    protected abstract InputStream getInputStream(int bufferSize, byte[] key, byte[] iv) throws IOException;
    ...
    private int readAll(InputStream in, long pos, ByteBuffer buf) throws IOException {
        int n = 0;
        int total = 0;
        while (n != -1) {
            total += n;
    ...
    private void positionedReadCheckWithByteBuffer(InputStream in, int pos) throws Exception {
        ByteBuffer result = ByteBuffer.allocate(dataLen);
    ...
    private void byteBufferReadFullyCheck(InputStream in, int pos) throws Exception {
        ...
        // Exceeds maximum length
        result = ByteBuffer.allocate(dataLen);
        try {
            ((ByteBufferPositionedReadable) in).readFully(pos, result);
            Assert.fail("Read fully exceeds maximum length should fail.");
        } catch (EOFException e) {
    ...
    private void byteBufferPreadCheck(InputStream in, ByteBuffer buf, int bufPos) throws Exception {
