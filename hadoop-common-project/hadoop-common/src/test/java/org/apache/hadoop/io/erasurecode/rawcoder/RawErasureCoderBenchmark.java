...
public final class RawErasureCoderBenchmark {
    ...
    enum CODER {
        ...
        DUMMY_CODER("Dummy coder"), LEGACY_RS_CODER("Legacy Reed-Solomon Java coder"), RS_CODER("Reed-Solomon Java coder"), ISAL_CODER("ISA-L coder");
    ...
    private static RawErasureEncoder getRawEncoder(int index) throws IOException {
        ...
        encoder.encode(getBufferForInit(BenchData.NUM_DATA_UNITS, 1, isDirect), getBufferForInit(BenchData.NUM_PARITY_UNITS, 1, isDirect));
        return encoder;
    }
    ...
    private static RawErasureDecoder getRawDecoder(int index) throws IOException {
        ...
        ByteBuffer[] inputs = getBufferForInit(BenchData.NUM_ALL_UNITS, 1, isDirect);
        for (int erasedIndex : BenchData.ERASED_INDEXES) {
            inputs[erasedIndex] = null;
        }
        decoder.decode(inputs, BenchData.ERASED_INDEXES, getBufferForInit(BenchData.ERASED_INDEXES.length, 1, isDirect));
        return decoder;
    }
    ...
    private static void printThreadStatistics(List<Long> durations, DecimalFormat df) {
        ...
        Double percentile = durations.get((int) Math.ceil(durations.size() * 0.9) - 1) / 1000.0;
