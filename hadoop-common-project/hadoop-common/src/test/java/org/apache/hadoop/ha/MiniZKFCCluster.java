...
public class MiniZKFCCluster {
    ...
    private void addSvcs(List<DummyHAService> svcs, int i) {
        svcs.add(new DummyHAService(HAServiceState.INITIALIZING, new InetSocketAddress("svc" + (i + 1), 1234)));
     ...
     */
    public void waitForHAState(int idx, HAServiceState state) throws Exception {
     ...
     */
    public void waitForHealthState(int idx, State state) throws Exception {
     ...
     */
    public void expireActiveLockHolder(int idx) throws NoNodeException {
        Stat stat = new Stat();
        byte[] data = zks.getZKDatabase().getData(DummyZKFC.LOCK_ZNODE, stat, null);
