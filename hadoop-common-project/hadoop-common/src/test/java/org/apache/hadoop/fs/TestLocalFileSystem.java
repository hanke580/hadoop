...
public class TestLocalFileSystem {
    ...
    protected Statistics getFileStatistics() {
        ...
        final List<Statistics> fileStats = all.stream().filter(s -> s.getScheme().equals("file")).collect(Collectors.toList());
        assertEquals("Number of statistics counters for file://", 1, fileStats.size());
    ...
    private void assertWritesCRC(String operation, Path path, LambdaTestUtils.VoidCallable callable, boolean delete) throws Exception {
        ...
        try {
            callable.call();
            assertEquals("Bytes written in " + operation + "; stats=" + stats, CRC_SIZE + DATA.length, stats.getBytesWritten() - bytesOut0);
        } finally {
            if (delete) {
                // clean up
                try {
