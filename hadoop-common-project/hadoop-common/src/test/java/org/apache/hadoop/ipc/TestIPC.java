...
public class TestIPC {
    ...
    static ConnectionId getConnectionId(InetSocketAddress addr, int rpcTimeout, Configuration conf) throws IOException {
        return ConnectionId.getConnectionId(addr, null, UserGroupInformation.getCurrentUser(), rpcTimeout, null, conf);
    }
    ...
    static LongWritable call(Client client, long param, InetSocketAddress addr, Configuration conf) throws IOException {
        return call(client, new LongWritable(param), addr, 0, conf);
    }

    static LongWritable call(Client client, LongWritable param, InetSocketAddress addr, int rpcTimeout, Configuration conf) throws IOException {
     ...
     */
    private void doErrorTest(Class<? extends LongWritable> clientParamClass, Class<? extends LongWritable> serverParamClass, Class<? extends LongWritable> serverResponseClass, Class<? extends LongWritable> clientResponseClass) throws IOException, InstantiationException, IllegalAccessException {
        // start server
    ...
    private Socket checkConnect(String addr, boolean asProxy) throws Exception {
        ...
        UserGroupInformation ugi = spy(UserGroupInformation.createRemoteUser(principal.toString()));
        ...
        SocketException expectedConnectEx = new SocketException("Expected connect failure");
        Socket s = Mockito.mock(Socket.class);
        SocketFactory mockFactory = Mockito.mock(SocketFactory.class);
        Mockito.doReturn(s).when(mockFactory).createSocket();
        doThrow(expectedConnectEx).when(s).connect(any(SocketAddress.class), Mockito.anyInt());
        ...
        try (Client client = new Client(LongWritable.class, conf, mockFactory)) {
            ...
            final LongWritable param = new LongWritable(RANDOM.nextLong());
            final ConnectionId remoteId = new ConnectionId(sockAddr, TestBindingProtocol.class, ugi, 0, RetryPolicies.TRY_ONCE_THEN_FAIL, conf);
