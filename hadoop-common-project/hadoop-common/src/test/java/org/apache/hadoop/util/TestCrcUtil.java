...
public class TestCrcUtil {
    ...
    private static void doTestComposeCrc(byte[] data, DataChecksum.Type type, int chunkSize, boolean useMonomial) throws IOException {
        ...
        DataChecksum checksum = DataChecksum.newDataChecksum(type, Integer.MAX_VALUE);
        ...
        int compositeCrc = 0;
        int crcMonomial = useMonomial ? CrcUtil.getMonomial(chunkSize, crcPolynomial) : 0;
        for (int offset = 0; offset + chunkSize <= data.length; offset += chunkSize) {
            ...
            if (useMonomial) {
                compositeCrc = CrcUtil.composeWithMonomial(compositeCrc, partialCrc, crcMonomial, crcPolynomial);
            } else {
                compositeCrc = CrcUtil.compose(compositeCrc, partialCrc, chunkSize, crcPolynomial);
            }
        }
        ...
        if (partialChunkSize > 0) {
            ...
            int partialCrc = (int) checksum.getValue();
            compositeCrc = CrcUtil.compose(compositeCrc, partialCrc, partialChunkSize, crcPolynomial);
        }
        assertEquals(String.format("Using CRC type '%s' with crcPolynomial '0x%08x' and chunkSize '%d'" + ", expected '0x%08x', got '0x%08x'", type, crcPolynomial, chunkSize, fullCrc, compositeCrc), fullCrc, compositeCrc);
    }
    ...
    private static void doTestComposeCrcZerolength(DataChecksum.Type type) throws IOException {
        ...
        assertEquals(crcA, CrcUtil.composeWithMonomial(crcA, crcB, monomial, crcPolynomial));
    }
