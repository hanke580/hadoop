...
class DummyHAService extends HAServiceTarget {
    ...
    boolean failToBecomeActive, failToBecomeStandby, failToBecomeObserver, failToFence;
    ...
    DummyHAService(HAServiceState state, InetSocketAddress address, boolean testWithProtoBufRPC) {
        this.state = state;
        ...
        this.healthMonitorProxy = makeHealthMonitorMock(conf, HA_HM_RPC_TIMEOUT_DEFAULT, HA_HM_RPC_CONNECT_MAX_RETRIES_DEFAULT);
        try {
    ...
    DummyHAService(HAServiceState state, InetSocketAddress address, InetSocketAddress healthMonitorAddress, boolean testWithProtoBufRPC) {
        this(state, address, testWithProtoBufRPC);
        if (testWithProtoBufRPC) {
            this.healthMonitorAddress = startAndGetRPCServerAddress(healthMonitorAddress);
        } else {
    ...
    private HAServiceProtocol makeHealthMonitorMock(Configuration conf, int timeoutMs, int retries) {
        HAServiceProtocol service;
        if (!testWithProtoBufRPC) {
            service = new MockHAProtocolImpl();
        } else {
    ...
    @Override
    public HAServiceProtocol getHealthMonitorProxy(Configuration conf, int timeout, int retries) throws IOException {
        if (testWithProtoBufRPC) {
    ...
    private class MockHAProtocolImpl implements HAServiceProtocol, Closeable {
        ...
        @Override
        public void transitionToObserver(StateChangeRequestInfo req) throws ServiceFailedException, AccessControlException, IOException {
            checkUnreachable();
