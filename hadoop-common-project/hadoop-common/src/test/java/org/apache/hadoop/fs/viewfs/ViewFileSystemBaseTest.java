...
abstract public class ViewFileSystemBaseTest {
    ...
    private void testOperationsThroughMountLinksInternal(boolean located) throws IOException {
        ...
        Assert.assertTrue("Delete should succeed", fsView.delete(new Path("/user/foo"), false));
        ...
        // Delete the created file
        Assert.assertTrue("Delete should succeed", fsView.delete(new Path("/internalDir/linkToDir2/foo"), false));
   ...
   */
    ...
    public void testInternalMkdirExisting1() throws IOException {
        Assert.assertTrue("mkdir of existing dir should succeed", fsView.mkdirs(fileSystemTestHelper.getTestRootPath(fsView, "/internalDir")));
    }

    public void testInternalMkdirExisting2() throws IOException {
        Assert.assertTrue("mkdir of existing dir should succeed", fsView.mkdirs(fileSystemTestHelper.getTestRootPath(fsView, "/internalDir/linkToDir2")));
    }
    ...
    public void testCloseChildrenFileSystem() throws Exception {
        ...
        ConfigUtil.addLink(config, clusterName, "/user", new Path(targetTestRoot, "user").toUri());
        ...
        assertTrue("viewfs should have at least one child fs.", viewFs.getChildFileSystems().length > 0);
    ...
    public void testChildrenFileSystemLeak() throws Exception {
        ...
        ViewFileSystem viewFs = (ViewFileSystem) FileSystem.get(new URI("viewfs://" + clusterName + "/"), config);
