...
public class TestRPC extends TestRpcBase {
    ...
    private void testCallsInternal(Configuration myConf) throws Exception {
        ...
        try {
            ...
            assertEquals("" + server.getPort(), server.getRpcMetrics().getTag("port").value());
            assertEquals("TestProtobufRpcProto", server.getRpcMetrics().getTag("serverName").value());
    ...
    private <T> ExternalCall<T> newExtCall(UserGroupInformation ugi, PrivilegedExceptionAction<T> callable) {
    ...
    public static class TestReaderException extends IOException {
        ...
        public boolean equals(Object t) {
            return (t.getClass() == TestReaderException.class) && getMessage().equals(((TestReaderException) t).getMessage());
        }
    }
