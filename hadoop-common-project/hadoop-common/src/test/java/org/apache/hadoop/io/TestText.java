...
public class TestText {
    ...
    private static String getTestString(int len) throws Exception {
        ...
        while (buffer.length() < length) {
            ...
            if (Character.isDefined(codePoint)) {
                //unpaired surrogate
                if (codePoint < Character.MIN_SUPPLEMENTARY_CODE_POINT && !Character.isHighSurrogate((char) codePoint) && !Character.isLowSurrogate((char) codePoint)) {
    ...
    public static String getLongString() throws Exception {
        ...
        StringBuilder buffer = new StringBuilder();
        while (buffer.length() < length) buffer.append(str);
        return buffer.toString();
    }
    ...
    public void testCompare() throws Exception {
        ...
        for (int i = 0; i < NUM_ITERATIONS; i++) {
            ...
            assertEquals("Equivalence of different txt objects, same content", 0, txt1.compareTo(txt3));
            assertEquals("Equvalence of data output buffers", 0, comparator.compare(out1.getData(), 0, out3.getLength(), out3.getData(), 0, out3.getLength()));
        }
    }
