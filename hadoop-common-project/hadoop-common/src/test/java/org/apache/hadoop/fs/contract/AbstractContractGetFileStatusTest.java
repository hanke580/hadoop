...
public abstract class AbstractContractGetFileStatusTest extends AbstractFSContractTestBase {
    ...
    private void listFilesOnEmptyDir(boolean recursive) throws IOException {
        ...
        fs.delete(path, true);
        Path subfolder = createDirWithEmptySubFolder();
        new TreeScanResults(fs.listFiles(path, recursive)).assertSizeEquals("listFiles(test dir, " + recursive + ")", 0, 0, 0);
        describe("Test on empty subdirectory");
    ...
    public void testListLocatedStatusEmptyDirectory() throws IOException {
        ...
        fs.delete(path, true);
        Path subfolder = createDirWithEmptySubFolder();
        new TreeScanResults(fs.listLocatedStatus(path)).assertSizeEquals("listLocatedStatus(test dir)", 0, 1, 0);
        describe("Test on empty subdirectory");
    ...
    public void testListStatusIteratorNoDir() throws Throwable {
        ...
        intercept(FileNotFoundException.class, () -> getFileSystem().listStatusIterator(path("missing")));
    }
    ...
    public void testListStatusNoDir() throws Throwable {
        ...
        intercept(FileNotFoundException.class, () -> getFileSystem().listStatus(path("missing")));
    }

    @Test
    public void testListStatusFilteredNoDir() throws Throwable {
        describe("test the listStatus(path, filter) call on a missing path");
        intercept(FileNotFoundException.class, () -> getFileSystem().listStatus(path("missing"), ALL_PATHS));
    }
    ...
    private FileStatus[] verifyListStatus(int expected, Path path, PathFilter filter) throws IOException {
        ...
        assertEquals("length of listStatus(" + path + ", " + filter + " ) " + Arrays.toString(result), expected, result.length);
        return result;
    }
