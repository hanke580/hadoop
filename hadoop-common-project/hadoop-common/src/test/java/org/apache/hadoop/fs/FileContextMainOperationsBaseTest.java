...
public abstract class FileContextMainOperationsBaseTest {
    ...
    private static final byte[] data = getFileData(numBlocks, getDefaultBlockSize());
    ...
    protected Path getDefaultWorkingDirectory() throws IOException {
        return getTestRootPath(fc, "/user/" + System.getProperty("user.name")).makeQualified(fc.getDefaultFileSystem().getUri(), fc.getWorkingDirectory());
    }
    ...
    private void writeReadAndDelete(int len) throws IOException {
        ...
        FSDataOutputStream out = fc.create(path, EnumSet.of(CREATE), CreateOpts.repFac((short) 1), CreateOpts.blockSize(getDefaultBlockSize()));
    ...
    private void testRenameDirectoryAsNonExistentDirectory(Rename... options) throws Exception {
        ...
        Assert.assertFalse("Nested file1 exists", exists(fc, getTestRootPath(fc, "test/hadoop/dir/file1")));
        Assert.assertFalse("Nested file2 exists", exists(fc, getTestRootPath(fc, "test/hadoop/dir/subdir/file2")));
        Assert.assertTrue("Renamed nested file1 exists", exists(fc, getTestRootPath(fc, "test/new/newdir/file1")));
        Assert.assertTrue("Renamed nested exists", exists(fc, getTestRootPath(fc, "test/new/newdir/subdir/file2")));
    }
     ...
     */
    void testUnsupportedSymlink() throws IOException {
    ...
    protected void rename(Path src, Path dst, boolean srcExists, boolean dstExists, Rename... options) throws IOException {
        try {
   ...
   */
    void testGetFileContext1() throws IOException {
        final Path rootPath = getTestRootPath(fc, "test");
    ...
    public void testOpenFileRead() throws Exception {
        ...
        createFile(path);
        final long length = fc.getFileStatus(path).getLen();
        try (FSDataInputStream fsdis = fc.openFile(path).opt("fs.test.something", true).opt("fs.test.something2", 3).opt("fs.test.something3", "3").build().get()) {
            byte[] bb = new byte[(int) length];
            fsdis.readFully(bb);
            assertArrayEquals(data, bb);
        }
    ...
    public void testOpenFileUnknownOption() throws Throwable {
        ...
        FutureDataInputStreamBuilder builder = fc.openFile(path).opt("fs.test.something", true).must("fs.test.something", true);
        intercept(IllegalArgumentException.class, () -> builder.build());
    }

    @Test
    public void testOpenFileLazyFail() throws Throwable {
        describe("openFile fails on a missing file in the get() and not before");
        FutureDataInputStreamBuilder builder = fc.openFile(path("testOpenFileUnknownOption")).opt("fs.test.something", true);
    ...
    public void testOpenFileApplyRead() throws Throwable {
        ...
        createFile(path);
        CompletableFuture<Long> readAllBytes = fc.openFile(path).build().thenApply(ContractTestUtils::readStream);
        assertEquals("Wrong number of bytes read from stream", data.length, (long) readAllBytes.get());
    }
    ...
    public void testOpenFileApplyAsyncRead() throws Throwable {
        ...
        assertTrue("async accept operation not invoked", accepted.get());
    }
}
