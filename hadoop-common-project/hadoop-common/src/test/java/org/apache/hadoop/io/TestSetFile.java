...
public class TestSetFile {
    ...
    private void writeData(FileSystem fs, int elementSize) throws IOException {
        ...
        for (int i = 0; i < elementSize; i++) writer.append(new IntWritable(i));
        writer.close();
    }
    ...
    private static void writeTest(FileSystem fs, RandomDatum[] data, String file, CompressionType compress) throws IOException {
        MapFile.delete(fs, file);
