...
public final class FileContextTestWrapper extends FSTestWrapper {
    ...
    public Path getDefaultWorkingDirectory() throws IOException {
        return getTestRootPath("/user/" + System.getProperty("user.name")).makeQualified(fc.getDefaultFileSystem().getUri(), fc.getWorkingDirectory());
    }
   ...
   */
    ...
    public long createFile(Path path, int numBlocks, int blockSize) throws IOException {
        return createFile(path, numBlocks, CreateOpts.blockSize(blockSize), CreateOpts.createParent());
    }
    ...
    public void writeFile(Path path, byte[] b) throws IOException {
        FSDataOutputStream out = fc.create(path, EnumSet.of(CreateFlag.CREATE), CreateOpts.createParent());
        out.write(b);
        out.close();
    }
    ...
    public void checkFileStatus(String path, fileType expectedType) throws IOException {
    ...
    public void checkFileLinkStatus(String path, fileType expectedType) throws IOException {
    ...
    @Override
    public void rename(Path src, Path dst, Rename... options) throws AccessControlException, FileAlreadyExistsException, FileNotFoundException, ParentNotDirectoryException, UnsupportedFileSystemException, IOException {
