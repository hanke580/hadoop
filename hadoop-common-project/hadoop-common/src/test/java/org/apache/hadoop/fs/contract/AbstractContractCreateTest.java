...
public abstract class AbstractContractCreateTest extends AbstractFSContractTestBase {
    ...
    private void testCreateFileOverExistingFileNoOverwrite(boolean useBuilder) throws Throwable {
        describe("Verify overwriting an existing file fails, using builder API=" + useBuilder);
        ...
        try {
            writeDataset(getFileSystem(), path, data2, data2.length, 1024, false, useBuilder);
    ...
    private void testOverwriteExistingFile(boolean useBuilder) throws Throwable {
        describe("Overwrite an existing file and verify the new data is there, " + "use builder API=" + useBuilder);
        Path path = path("testOverwriteExistingFile", useBuilder);
        byte[] data = dataset(256, 'a', 'z');
        writeDataset(getFileSystem(), path, data, data.length, 1024, false, useBuilder);
        ContractTestUtils.verifyFileContents(getFileSystem(), path, data);
        byte[] data2 = dataset(10 * 1024, 'A', 'Z');
        writeDataset(getFileSystem(), path, data2, data2.length, 1024, true, useBuilder);
    ...
    private void testOverwriteEmptyDirectory(boolean useBuilder) throws Throwable {
        describe("verify trying to create a file over an empty dir fails, " + "use builder API=" + useBuilder);
        Path path = path("testOverwriteEmptyDirectory");
        mkdirs(path);
        assertIsDirectory(path);
        byte[] data = dataset(256, 'a', 'z');
        try {
            writeDataset(getFileSystem(), path, data, data.length, 1024, true, useBuilder);
            assertIsDirectory(path);
    ...
    private void testOverwriteNonEmptyDirectory(boolean useBuilder) throws Throwable {
        describe("verify trying to create a file over a non-empty dir fails, " + "use builder API=" + useBuilder);
    ...
    private void validateBlockSize(FileSystem fs, Path path, int minValue) throws IOException, InterruptedException {
        ...
        assertTrue("File status block size too low:  " + statusDetails + " min value: " + minValue, status.getBlockSize() >= minValue);
        long defaultBlockSize = fs.getDefaultBlockSize(path);
        assertTrue("fs.getDefaultBlockSize(" + path + ") size " + defaultBlockSize + " is below the minimum of " + minValue, defaultBlockSize >= minValue);
    }
    ...
    public void testCreateFileUnderFile() throws Throwable {
        ...
        Path parent = new Path(grandparent, "parent");
        expectCreateUnderFileFails("creating a file under a file", grandparent, parent);
    }
    ...
    public void testCreateUnderFileSubdir() throws Throwable {
        ...
        Path parent = new Path(grandparent, "parent");
        Path child = new Path(parent, "child");
        expectCreateUnderFileFails("creating a file under a subdirectory of a file", grandparent, child);
    }
    ...
    public void testMkdirUnderFile() throws Throwable {
        ...
        Path grandparent = methodPath();
        Path parent = new Path(grandparent, "parent");
        expectMkdirsUnderFileFails("mkdirs() under a file", grandparent, parent);
    }
    ...
    public void testMkdirUnderFileSubdir() throws Throwable {
        ...
        Path parent = new Path(grandparent, "parent");
        Path child = new Path(parent, "child");
        expectMkdirsUnderFileFails("mkdirs() file/dir", grandparent, child);
        try {
            // create the child
            mkdirs(child);
            ...
            handleRelaxedException("creating a file under a subdirectory of a file ", "FileAlreadyExistsException", e);
        }
    }
    ...
    protected void expectCreateUnderFileFails(String action, Path file, Path descendant) throws Exception {
        ...
        try {
            ...
            handleRelaxedException(action, "ParentNotDirectoryException", e);
        }
    }
