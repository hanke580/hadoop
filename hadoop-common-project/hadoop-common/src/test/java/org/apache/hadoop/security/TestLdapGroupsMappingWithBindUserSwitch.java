...
public class TestLdapGroupsMappingWithBindUserSwitch extends TestLdapGroupsMappingBase {
    ...
    private void createCredentialForAlias(Configuration conf, String alias, String password) throws Exception {
        ...
        assertArrayEquals(bindpass, provider.getCredentialEntry(alias).getCredential());
    }
    ...
    private void doTestBindUserSwitch(Configuration conf, Integer numFailures, List<String> expectedBindUsers, List<String> expectedBindPasswords) throws NamingException {
        doTestBindUserSwitch(conf, numFailures, Iterators.cycle(expectedBindUsers), Iterators.cycle(expectedBindPasswords));
    }
    ...
    private void doTestBindUserSwitch(Configuration conf, Integer numFailures, Iterator<String> expectedBindUsers, Iterator<String> expectedBindPasswords) throws NamingException {
        ...
        when(getContext().search(anyString(), anyString(), any(Object[].class), any(SearchControls.class))).thenAnswer(invocationOnMock -> {
        ...
        verify(getContext(), times(numExpectedSearchCalls)).search(anyString(), anyString(), any(Object[].class), any(SearchControls.class));
    }
}
