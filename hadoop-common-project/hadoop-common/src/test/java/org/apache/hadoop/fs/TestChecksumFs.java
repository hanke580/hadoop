...
public class TestChecksumFs extends HadoopTestBase {
    ...
    private void verifyRename(Path srcPath, Path dstPath, boolean overwrite) throws Exception {
        ...
        assertTrue("Checksum file doesn't exist for source file - " + srcPath, fc.util().exists(fs.getChecksumFile(srcPath)));
        fs.rename(srcPath, dstPath, renameOpt);
        assertTrue("Checksum file doesn't exist for dest file - " + srcPath, fc.util().exists(fs.getChecksumFile(dstPath)));
    ...
    private void createTestFile(ChecksumFs fs, Path path, int content) throws IOException {
        try (FSDataOutputStream fout = fs.create(path, EnumSet.of(CREATE, OVERWRITE), Options.CreateOpts.perms(FsPermission.getDefault()))) {
