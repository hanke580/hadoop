...
public class HttpServerFunctionalTest extends Assert {
        ...
        try {
            if (!testWebappDir.exists()) {
                if (!testWebappDir.mkdirs()) {
                    fail("Test webapp dir " + testWebappDir.getCanonicalPath() + " can not be created");
                }
            }
        } catch (IOException e) {
        }
    ...
    public static HttpServer2 createServer(boolean xFrameEnabled, String xFrameOptionValue, Configuration conf) throws IOException {
        return localServerBuilder(TEST).setFindPort(true).configureXFrame(xFrameEnabled).setXFrameOption(xFrameOptionValue).setConf(conf).build();
    }
