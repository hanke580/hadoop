...
public final class DelegatingSSLSocketFactory extends SSLSocketFactory {
    ...
    public enum SSLChannelMode {
        ...
        OpenSSL, Default, Default_JSSE, Default_JSSE_with_GCM
    }
    ...
    private DelegatingSSLSocketFactory(SSLChannelMode preferredChannelMode) throws IOException {
        ...
        ciphers = (channelMode == SSLChannelMode.Default_JSSE && version.startsWith("1.8")) ? alterCipherList(defaultCiphers) : defaultCiphers;
        providerName = ctx.getProvider().getName() + "-" + ctx.getProvider().getVersion();
    }
    ...
    private void initializeSSLContext(SSLChannelMode preferredChannelMode) throws NoSuchAlgorithmException, KeyManagementException, IOException {
        ...
        switch(preferredChannelMode) {
            ...
            case Default_JSSE_with_GCM:
                ...
                break;
            default:
                throw new IOException("Unknown channel mode: " + preferredChannelMode);
        }
    }
    ...
    public Socket createSocket(Socket s, String host, int port, boolean autoClose) throws IOException {
        SSLSocketFactory factory = ctx.getSocketFactory();
        return configureSocket(factory.createSocket(s, host, port, autoClose));
    }
    ...
    private String[] alterCipherList(String[] defaultCiphers) {
        ...
        for (int i = 0; i < defaultCiphers.length; i++) {
            if (defaultCiphers[i].contains("_GCM_")) {
                LOG.debug("Removed Cipher - {} from list of enabled SSLSocket ciphers", defaultCiphers[i]);
            } else {
