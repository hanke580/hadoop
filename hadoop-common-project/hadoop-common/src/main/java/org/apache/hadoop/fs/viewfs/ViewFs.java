...
ViewFs extends AbstractFileSystem {
    ...
    ViewFs(final URI theUri, final Configuration conf) throws IOException, URISyntaxException {
        ...
        renameStrategy = ViewFileSystem.RenameStrategy.valueOf(conf.get(Constants.CONFIG_VIEWFS_RENAME_STRATEGY, ViewFileSystem.RenameStrategy.SAME_MOUNTPOINT.toString()));
    }
    ...
    public void renameInternal(final Path src, final Path dst, final boolean overwrite) throws IOException, UnresolvedLinkException {
        ...
        ViewFileSystem.verifyRenameStrategy(srcUri, dstUri, resSrc.targetFileSystem == resDst.targetFileSystem, renameStrategy);
    ...
    public void unsetStoragePolicy(final Path src) throws IOException {
        InodeTree.ResolveResult<AbstractFileSystem> res = fsState.resolve(getUriPath(src), true);
    ...
    private abstract class WrappingRemoteIterator<T extends FileStatus> implements RemoteIterator<T> {
        ...
        public T next() throws IOException {
            ...
            String suffix = targetFs.stripOutRoot(status.getPath());
            Path newPath = makeQualified(suffix.length() == 0 ? originalPath : new Path(resolvedPath, suffix));
   ...
   */
    static class InternalDirOfViewFs extends AbstractFileSystem {
        ...
        public FileStatus getFileStatus(final Path f) throws IOException {
            checkPathIsSlash(f);
            return new FileStatus(0, true, 0, 0, creationTime, creationTime, PERMISSION_555, ugi.getShortUserName(), ugi.getPrimaryGroupName(), new Path(theInternalDir.fullPath).makeQualified(myUri, null));
        }

        @Override
        public FileStatus getFileLinkStatus(final Path f) throws IOException {
            // look up i internalDirs children - ignore first Slash
            INode<AbstractFileSystem> inode = theInternalDir.getChildren().get(f.toUri().toString().substring(1));
            if (inode == null) {
            ...
            if (inode.isLink()) {
                ...
                result = new FileStatus(0, false, 0, 0, creationTime, creationTime, PERMISSION_555, ugi.getShortUserName(), ugi.getPrimaryGroupName(), inodelink.getTargetLink(), new Path(inode.fullPath).makeQualified(myUri, null));
            } else {
                result = new FileStatus(0, true, 0, 0, creationTime, creationTime, PERMISSION_555, ugi.getShortUserName(), ugi.getPrimaryGroupName(), new Path(inode.fullPath).makeQualified(myUri, null));
            }
            return result;
        }
        ...
        public FileStatus[] listStatus(final Path f) throws AccessControlException, IOException {
            ...
            int i = 0;
            for (Entry<String, INode<AbstractFileSystem>> iEntry : theInternalDir.getChildren().entrySet()) {
