...
public class ReadWriteDiskValidatorMetrics {
    ...
    public ReadWriteDiskValidatorMetrics() {
        ...
        for (int i = 0; i < fileReadQuantiles.length; i++) {
            ...
            fileReadQuantiles[i] = registry.newQuantiles("readLatency" + interval + "s", "File read latency", "Ops", "latencyMicros", interval);
        }
        fileWriteQuantiles = new MutableQuantiles[quantileIntervals.length];
        for (int i = 0; i < fileWriteQuantiles.length; i++) {
            int interval = quantileIntervals[i];
            fileWriteQuantiles[i] = registry.newQuantiles("writeLatency" + interval + "s", "File write latency", "Ops", "latencyMicros", interval);
        }
    }
     ...
     * metrics.
     */
    protected final static Map<String, ReadWriteDiskValidatorMetrics> DIR_METRICS = new HashMap<>();
     ...
     * @return the metric
     */
    public synchronized static ReadWriteDiskValidatorMetrics getMetric(String dirName) {
