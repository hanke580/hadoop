...
abstract class InodeTree<T> {
    ...
    static class INodeDir<T> extends INode<T> {
        ...
        //filesystem of this internal directory
    ...
    private void createLink(final String src, final String target, final LinkType linkType, final String settings, final UserGroupInformation aUgi, final Configuration config) throws URISyntaxException, IOException, FileAlreadyExistsException, UnsupportedFileSystemException {
        ...
        switch(linkType) {
            ...
            case NFLY:
                ...
                newLink = new INodeLink<T>(fullPath, aUgi, getTargetFileSystem(settings, targetUris), targetUris);
                break;
            default:
    ...
    protected abstract T getTargetFileSystem(INodeDir<T> dir) throws URISyntaxException;
    ...
    protected InodeTree(final Configuration config, final String viewName) throws UnsupportedFileSystemException, URISyntaxException, FileAlreadyExistsException, IOException {
        ...
        final String mountTablePrefix = Constants.CONFIG_VIEWFS_PREFIX + "." + mountTableName + ".";
        ...
        final String linkMergeSlashPrefix = Constants.CONFIG_VIEWFS_LINK_MERGE_SLASH;
            ...
            if (key.startsWith(mountTablePrefix)) {
                ...
                if (src.startsWith(linkPrefix)) {
                    ...
                    if (src.length() != linkFallbackPrefix.length()) {
                        throw new IOException("ViewFs: Mount points initialization error." + " Invalid " + Constants.CONFIG_VIEWFS_LINK_FALLBACK + " entry in config: " + src);
                    }
                    ...
                    if (src.length() != linkMergeSlashPrefix.length()) {
                        throw new IOException("ViewFs: Mount points initialization error." + " Invalid " + Constants.CONFIG_VIEWFS_LINK_MERGE_SLASH + " entry in config: " + src);
                    }
        ...
        if (isMergeSlashConfigured) {
            ...
            root = new INodeLink<T>(mountTableName, ugi, getTargetFileSystem(new URI(mergeSlashTarget)), new URI(mergeSlashTarget));
            ...
            for (LinkEntry le : linkEntries) {
                if (le.isLinkType(LinkType.SINGLE_FALLBACK)) {
                    ...
                    fallbackLink = new INodeLink<T>(mountTableName, ugi, getTargetFileSystem(new URI(le.getTarget())), new URI(le.getTarget()));
                } else {
                    createLink(le.getSrc(), le.getTarget(), le.getLinkType(), le.getSettings(), le.getUgi(), le.getConfig());
                }
            }
    ...
    ResolveResult<T> resolve(final String p, final boolean resolveLastComponent) throws FileNotFoundException {
        ...
        if (path.length <= 1) {
            ...
            T targetFs = root.isInternalDir() ? getRootDir().getInternalDirFs() : getRootLink().getTargetFileSystem();
            ResolveResult<T> res = new ResolveResult<T>(ResultKind.INTERNAL_DIR, targetFs, root.fullPath, SlashPath);
            return res;
        }
        /**
        ...
        if (root.isLink()) {
            ...
            ResolveResult<T> res = new ResolveResult<T>(ResultKind.EXTERNAL_DIR, getRootLink().getTargetFileSystem(), root.fullPath, remainingPath);
            return res;
        }
        Preconditions.checkState(root.isInternalDir());
        INodeDir<T> curInode = getRootDir();
        ...
        for (i = 1; i < path.length - (resolveLastComponent ? 0 : 1); i++) {
            ...
            if (nextInode == null) {
                if (hasFallbackLink()) {
                    return new ResolveResult<T>(ResultKind.EXTERNAL_DIR, getRootFallbackLink().getTargetFileSystem(), root.fullPath, new Path(p));
                } else {
            ...
            if (nextInode.isLink()) {
                ...
                final ResolveResult<T> res = new ResolveResult<T>(ResultKind.EXTERNAL_DIR, link.getTargetFileSystem(), nextInode.fullPath, remainingPath);
                return res;
        ...
        final ResolveResult<T> res = new ResolveResult<T>(ResultKind.INTERNAL_DIR, curInode.getInternalDirFs(), curInode.fullPath, remainingPath);
        return res;
    }
