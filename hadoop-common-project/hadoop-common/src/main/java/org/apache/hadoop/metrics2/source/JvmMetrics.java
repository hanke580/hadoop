...
public class JvmMetrics implements MetricsSource {
    ...
    public static JvmMetrics create(String processName, String sessionId, MetricsSystem ms) {
        ...
        boolean useThreadMXBean = new Configuration().getBoolean(CommonConfigurationKeys.HADOOP_METRICS_JVM_USE_THREAD_MXBEAN, CommonConfigurationKeys.HADOOP_METRICS_JVM_USE_THREAD_MXBEAN_DEFAULT);
        return ms.register(JvmMetrics.name(), JvmMetrics.description(), new JvmMetrics(processName, sessionId, useThreadMXBean));
    }
