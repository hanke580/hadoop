...
public class DecayRpcScheduler implements RpcScheduler, DecayRpcSchedulerMXBean, MetricsSource {
    ...
    private final ConcurrentHashMap<Object, List<AtomicLong>> callCosts = new ConcurrentHashMap<Object, List<AtomicLong>>();
    ...
    // e.g., report top 10 users' metrics
    ...
    public DecayRpcScheduler(int numLevels, String ns, Configuration conf) {
        ...
        decayRpcSchedulerDetailedMetrics = DecayRpcSchedulerDetailedMetrics.create(ns);
    ...
    private CostProvider parseCostProvider(String ns, Configuration conf) {
        List<CostProvider> providers = conf.getInstances(ns + "." + CommonConfigurationKeys.IPC_COST_PROVIDER_KEY, CostProvider.class);
        if (providers.size() < 1) {
    ...
    private static double parseDecayFactor(String ns, Configuration conf) {
        ...
        if (factor == 0.0) {
            factor = conf.getDouble(ns + "." + IPC_SCHEDULER_DECAYSCHEDULER_FACTOR_KEY, IPC_SCHEDULER_DECAYSCHEDULER_FACTOR_DEFAULT);
        } else if ((factor > 0.0) && (factor < 1)) {
            LOG.warn(IPC_FCQ_DECAYSCHEDULER_FACTOR_KEY + " is deprecated. Please use " + IPC_SCHEDULER_DECAYSCHEDULER_FACTOR_KEY + ".");
        }
    ...
    private static long parseDecayPeriodMillis(String ns, Configuration conf) {
        long period = conf.getLong(ns + "." + IPC_FCQ_DECAYSCHEDULER_PERIOD_KEY, 0);
        if (period == 0) {
            period = conf.getLong(ns + "." + IPC_SCHEDULER_DECAYSCHEDULER_PERIOD_KEY, IPC_SCHEDULER_DECAYSCHEDULER_PERIOD_DEFAULT);
        } else if (period > 0) {
            LOG.warn((IPC_FCQ_DECAYSCHEDULER_PERIOD_KEY + " is deprecated. Please use " + IPC_SCHEDULER_DECAYSCHEDULER_PERIOD_KEY));
        }
        if (period <= 0) {
    ...
    private static double[] parseThresholds(String ns, Configuration conf, int numLevels) {
        ...
        if (percentages.length == 0) {
            ...
            LOG.warn(IPC_FCQ_DECAYSCHEDULER_THRESHOLDS_KEY + " is deprecated. Please use " + IPC_DECAYSCHEDULER_THRESHOLDS_KEY);
        }
    ...
    private void decayCurrentCosts() {
        ...
        try {
            ...
            long totalRawCost = 0;
            Iterator<Map.Entry<Object, List<AtomicLong>>> it = callCosts.entrySet().iterator();
            while (it.hasNext()) {
                ...
                LOG.debug("Decaying costs for the user: {}, its decayedCost: {}, rawCost: {}", entry.getKey(), nextValue, rawCost.get());
                if (nextValue == 0) {
            ...
            LOG.debug("After decaying the stored costs, totalDecayedCost: {}, " + "totalRawCallCost: {}.", totalDecayedCost, totalRawCost);
            // Now refresh the cache of scheduling decisions
            recomputeScheduleCache();
            // Update average response time with decay
            updateAverageResponseTime(true);
        } catch (Exception ex) {
            LOG.error("decayCurrentCosts exception: " + ExceptionUtils.getStackTrace(ex));
            throw ex;
        }
    }
        ...
        if (backOffByResponseTimeEnabled) {
            ...
            if (LOG.isDebugEnabled()) {
                ...
                for (int i = 0; i < numLevels; i++) {
                    LOG.debug("Queue: {} responseTime: {} backoffThreshold: {}", i, responseTimes[i], backOffResponseTimeThresholds[i]);
                }
            }
            // High priority rpc over threshold triggers back off of low priority rpc
            for (int i = 0; i < priorityLevel + 1; i++) {
                if (responseTimeAvgInLastWindow.get(i) > backOffResponseTimeThresholds[i]) {
                    backOff = true;
                    break;
                }
            }
    ...
    public void addResponseTime(String callName, Schedulable schedulable, ProcessingDetails details) {
        ...
        this.decayRpcSchedulerDetailedMetrics.addQueueTime(priorityLevel, queueTime);
        this.decayRpcSchedulerDetailedMetrics.addProcessingTime(priorityLevel, processingTime);
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("addResponseTime for call: {}  priority: {} queueTime: {} " + "processingTime: {} ", callName, priorityLevel, queueTime, processingTime);
        }
    }
    ...
    private void addUniqueIdentityCount(MetricsRecordBuilder rb) {
        rb.addCounter(Interns.info("UniqueCallers", "Total unique callers"), getUniqueIdentityCount());
    }

    // Key: DecayedCallVolume
    private void addDecayedCallVolume(MetricsRecordBuilder rb) {
        rb.addCounter(Interns.info("DecayedCallVolume", "Decayed Total " + "incoming Call Volume"), getTotalCallVolume());
    }

    private void addRawCallVolume(MetricsRecordBuilder rb) {
        rb.addCounter(Interns.info("CallVolume", "Raw Total " + "incoming Call Volume"), getTotalRawCallVolume());
    }

    // Key: Priority.0.CompletedCallVolume
    private void addCallVolumePerPriority(MetricsRecordBuilder rb) {
        for (int i = 0; i < responseTimeCountInLastWindow.length(); i++) {
            rb.addGauge(Interns.info("Priority." + i + ".CompletedCallVolume", "Completed Call volume " + "of priority " + i), responseTimeCountInLastWindow.get(i));
        }
    }
    ...
    private void addAvgResponseTimePerPriority(MetricsRecordBuilder rb) {
        for (int i = 0; i < responseTimeAvgInLastWindow.length(); i++) {
            rb.addGauge(Interns.info("Priority." + i + ".AvgResponseTime", "Average" + " response time of priority " + i), responseTimeAvgInLastWindow.get(i));
        }
    }
