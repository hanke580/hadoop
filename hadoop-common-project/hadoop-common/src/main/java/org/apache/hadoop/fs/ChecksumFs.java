...
ChecksumFs extends FilterFs {
    ...
    public ChecksumFs(AbstractFileSystem theFs) throws IOException, URISyntaxException {
        super(theFs);
        defaultBytesPerChecksum = getMyFs().getServerDefaults(new Path("/")).getBytesPerChecksum();
    }
    ...
    private int getSumBufferSize(int bytesPerSum, int bufferSize, Path file) throws IOException {
        int defaultBufferSize = getMyFs().getServerDefaults(file).getFileBufferSize();
        ...
        protected int readChunk(long pos, byte[] buf, int offset, int len, byte[] checksum) throws IOException {
            ...
            if (needChecksum()) {
                ...
                if (sumLenRead >= 0 && sumLenRead % CHECKSUM_SIZE != 0) {
                    throw new EOFException("Checksum file not a length multiple of checksum size " + "in " + file + " at " + pos + " checksumpos: " + checksumPos + " sumLenread: " + sumLenRead);
                }
                if (sumLenRead <= 0) {
    ...
    @Override
    public FSDataOutputStream createInternal(Path f, EnumSet<CreateFlag> createFlag, FsPermission absolutePermission, int bufferSize, short replication, long blockSize, Progressable progress, ChecksumOpt checksumOpt, boolean createParent) throws IOException {
    ...
    public RemoteIterator<LocatedFileStatus> listLocatedStatus(final Path f) throws AccessControlException, FileNotFoundException, UnresolvedLinkException, IOException {
        final RemoteIterator<LocatedFileStatus> iter = getMyFs().listLocatedStatus(f);
