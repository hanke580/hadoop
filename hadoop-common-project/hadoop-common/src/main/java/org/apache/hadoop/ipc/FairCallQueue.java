...
public class FairCallQueue<E extends Schedulable> extends AbstractQueue<E> implements BlockingQueue<E> {
   ...
   */
    ...
    public FairCallQueue(int priorityLevels, int capacity, String ns, Configuration conf) {
        ...
        this.serverFailOverEnabled = conf.getBoolean(ns + "." + CommonConfigurationKeys.IPC_CALLQUEUE_SERVER_FAILOVER_ENABLE, CommonConfigurationKeys.IPC_CALLQUEUE_SERVER_FAILOVER_ENABLE_DEFAULT);
     ...
     */
    private static final class MetricsProxy implements FairCallQueueMXBean, MetricsSource {
        ...
        public void getMetrics(MetricsCollector collector, boolean all) {
            MetricsRecordBuilder rb = collector.addRecord("FairCallQueue").setContext("rpc").tag(Interns.info("namespace", "Namespace"), namespace);
            ...
            for (int i = 0; i < currentQueueSizes.length; i++) {
                rb.addGauge(Interns.info("FairCallQueueSize_p" + i, "FCQ Queue Size"), currentQueueSizes[i]);
                rb.addCounter(Interns.info("FairCallQueueOverflowedCalls_p" + i, "FCQ Overflowed Calls"), currentOverflowedCalls[i]);
            }
        }
    }
