...
public class HHXORErasureEncodingStep extends HHErasureCodingStep {
    ...
    public HHXORErasureEncodingStep(ECBlock[] inputBlocks, ECBlock[] outputBlocks, RawErasureEncoder rsRawEncoder, RawErasureEncoder xorRawEncoder) {
        ...
        piggyBackIndex = HHUtil.initPiggyBackIndexWithoutPBVec(rsRawEncoder.getNumDataUnits(), rsRawEncoder.getNumParityUnits());
    }
    ...
    private void doEncode(ByteBuffer[][] inputs, ByteBuffer[][] outputs) throws IOException {
        ...
        ByteBuffer[] piggyBacks = HHUtil.getPiggyBacksFromInput(inputs[0], piggyBackIndex, numParityUnits, 0, xorRawEncoder);
        ...
        encodeWithPiggyBacks(piggyBacks, outputs, numParityUnits, inputs[0][0].isDirect());
    }
