...
public class NetworkTopology {
    ...
    public static NetworkTopology getInstance(Configuration conf, InnerNode.Factory factory) {
        NetworkTopology nt = ReflectionUtils.newInstance(conf.getClass(CommonConfigurationKeysPublic.NET_TOPOLOGY_IMPL_KEY, NetworkTopology.class, NetworkTopology.class), conf);
    ...
    protected Node chooseRandom(final String scope, String excludedScope, final Collection<Node> excludedNodes) {
        ...
        LOG.debug("Choosing random from {} available nodes on node {}," + " scope={}, excludedScope={}, excludeNodes={}. numOfDatanodes={}.", availableNodes, innerNode, scope, excludedScope, excludedNodes, numOfDatanodes);
        Node ret = null;
        if (availableNodes > 0) {
            ret = chooseRandom(innerNode, node, excludedNodes, numOfDatanodes, availableNodes);
        }
    ...
    private Node chooseRandom(final InnerNode parentNode, final Node excludedScopeNode, final Collection<Node> excludedNodes, final int totalInScopeNodes, final int availableNodes) {
        if (totalInScopeNodes < availableNodes) {
            LOG.warn("Total Nodes in scope : {} are less than Available Nodes : {}", totalInScopeNodes, availableNodes);
            return null;
        }
        ...
        if (ret == null && lastValidNode != null) {
            LOG.error("BUG: Found lastValidNode {} but not nth valid node. " + "parentNode={}, excludedScopeNode={}, excludedNodes={}, " + "totalInScopeNodes={}, availableNodes={}, nthValidToReturn={}.", lastValidNode, parentNode, excludedScopeNode, excludedNodes, totalInScopeNodes, availableNodes, nthValidToReturn);
            ret = lastValidNode;
        }
        return ret;
    }
    ...
    public String toString() {
        ...
        for (int i = 0; i < numOfLeaves; i++) {
            tree.append(NodeBase.getPath(clusterMap.getLeaf(i, null))).append("\n");
        }
    ...
    protected int getWeight(Node reader, Node node) {
        ...
        if (reader != null && node != null) {
            ...
            int currentLevelToCompare = maxReaderLevel > maxNodeLevel ? maxNodeLevel : maxReaderLevel;
            Node r = reader;
            Node n = node;
            weight = 0;
    ...
    protected static int getWeightUsingNetworkLocation(Node reader, Node node) {
        ...
        int weight = Integer.MAX_VALUE;
        if (reader != null && node != null) {
            String readerPath = normalizeNetworkLocationPath(reader.getNetworkLocation());
            String nodePath = normalizeNetworkLocationPath(node.getNetworkLocation());
            //same rack
            if (readerPath.equals(nodePath)) {
                ...
                int maxLevelToCompare = readerPathToken.length > nodePathToken.length ? nodePathToken.length : readerPathToken.length;
                int currentLevel = 1;
                //traverse through the path and calculate the distance
                while (currentLevel < maxLevelToCompare) {
                    if (!readerPathToken[currentLevel].equals(nodePathToken[currentLevel])) {
                        break;
                    }
                    currentLevel++;
                }
    ...
    private static String normalizeNetworkLocationPath(String path) {
        ...
        if (path.charAt(0) != PATH_SEPARATOR) {
            throw new IllegalArgumentException("Network Location" + "path doesn't start with " + PATH_SEPARATOR + ": " + path);
        }
        int len = path.length();
     ...
     */
        sortByDistanceUsingNetworkLocation(reader, nodes, activeLen, list -> Collections.shuffle(list));
    }
