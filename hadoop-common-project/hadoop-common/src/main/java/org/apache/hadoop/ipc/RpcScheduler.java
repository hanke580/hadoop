...
public interface RpcScheduler {
    ...
    default void addResponseTime(String name, int priorityLevel, int queueTime, int processingTime) {
    ...
    default void addResponseTime(String callName, Schedulable schedulable, ProcessingDetails details) {
        ...
        int queueTimeMs = (int) details.get(ProcessingDetails.Timing.QUEUE, TimeUnit.MILLISECONDS);
        int processingTimeMs = (int) details.get(ProcessingDetails.Timing.PROCESSING, TimeUnit.MILLISECONDS);
        addResponseTime(callName, schedulable.getPriorityLevel(), queueTimeMs, processingTimeMs);
    }
