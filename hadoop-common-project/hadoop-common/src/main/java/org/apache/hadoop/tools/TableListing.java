...
public class TableListing {
    ...
    private static class Column {
        ...
        private void setWrapWidth(int width) {
            ...
            if (maxWidth > wrapWidth) {
                maxWidth = wrapWidth;
            } else // Else we need to traverse through and find the real maxWidth
            {
                maxWidth = 0;
    ...
    public void addRow(String... row) {
        if (row.length != columns.length) {
            throw new RuntimeException("trying to add a row with " + row.length + " columns, but we have " + columns.length + " columns.");
        }
