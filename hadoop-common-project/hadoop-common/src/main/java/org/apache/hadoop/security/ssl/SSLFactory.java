...
public class SSLFactory implements ConnectionConfigurator {
    ...
    public SSLFactory(Mode mode, Configuration conf) {
        ...
        requireClientCert = sslConf.getBoolean(SSL_REQUIRE_CLIENT_CERT_KEY, SSL_REQUIRE_CLIENT_CERT_DEFAULT);
        ...
        enabledProtocols = conf.getStrings(SSL_ENABLED_PROTOCOLS_KEY, SSL_ENABLED_PROTOCOLS_DEFAULT);
        excludeCiphers = Arrays.asList(sslConf.getTrimmedStrings(SSL_SERVER_EXCLUDE_CIPHER_LIST));
        if (LOG.isDebugEnabled()) {
            LOG.debug("will exclude cipher suites: {}", StringUtils.join(",", excludeCiphers));
        }
    }
