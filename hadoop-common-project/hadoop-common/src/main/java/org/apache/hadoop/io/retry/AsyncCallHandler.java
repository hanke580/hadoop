...
public class AsyncCallHandler {
    ...
    static class ConcurrentQueue<T> {
        ...
        boolean isEmpty(long time) {
            return Time.monotonicNow() - emptyStartTime.get() > time && queue.isEmpty();
        }
    ...
    class AsyncCallQueue {
        ...
        long checkCalls() {
            ...
            for (final Iterator<AsyncCall> i = queue.iterator(); i.hasNext(); ) {
                ...
                if (c.isDone()) {
                    // the call is done, remove it from the queue.
                    i.remove();
                    queue.checkEmpty();
                } else {
