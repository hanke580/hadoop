...
public class CryptoInputStream extends FilterInputStream implements Seekable, PositionedReadable, ByteBufferReadable, HasFileDescriptor, CanSetDropBehind, CanSetReadahead, HasEnhancedByteBufferAccess, ReadableByteChannel, CanUnbuffer, StreamCapabilities, ByteBufferPositionedReadable {
     ...
     */
    @Override
    public void readFully(long position, final ByteBuffer buf) throws IOException {
        checkStream();
        if (!(in instanceof ByteBufferPositionedReadable)) {
            throw new UnsupportedOperationException(in.getClass().getCanonicalName() + " does not support positioned reads with byte buffers.");
        }
    ...
    private void decrypt(long filePosition, ByteBuffer buf, int length, int start) throws IOException {
        ...
        try {
            ...
            while (decryptedBytes < length) {
                ...
                buf.limit(start + decryptedBytes + Math.min(length - decryptedBytes, localInBuffer.remaining()));
                ...
                try {
                    ...
                    localPadding = afterDecryption(localDecryptor, localInBuffer, filePosition + length, localIV);
                }
            }
        } finally {
            returnBuffer(localInBuffer);
    ...
    public void readFully(long position, byte[] buffer, int offset, int length) throws IOException {
        checkStream();
        if (!(in instanceof PositionedReadable)) {
            throw new UnsupportedOperationException(in.getClass().getCanonicalName() + " does not support positioned readFully.");
        }
     ...
     */
        if (pos <= streamOffset && pos >= (streamOffset - outBuffer.remaining())) {
            ...
            if (!(in instanceof Seekable)) {
                throw new UnsupportedOperationException(in.getClass().getCanonicalName() + " does not support seek.");
            }
     ...
     */
    private void decrypt(ByteBuffer buf, int length, int start) throws IOException {
        buf = buf.duplicate();
        int decryptedBytes = 0;
        while (decryptedBytes < length) {
            buf.position(start + decryptedBytes);
            buf.limit(start + decryptedBytes + Math.min(length - decryptedBytes, inBuffer.remaining()));
            inBuffer.put(buf);
            // Do decryption
            try {
                decrypt(decryptor, inBuffer, outBuffer, padding);
                ...
                padding = afterDecryption(decryptor, inBuffer, streamOffset - (length - decryptedBytes), iv);
            }
        }
    }
    ...
    public boolean seekToNewSource(long targetPos) throws IOException {
        ...
        checkStream();
        if (!(in instanceof Seekable)) {
            throw new UnsupportedOperationException(in.getClass().getCanonicalName() + " does not support seekToNewSource.");
        }
    ...
    public ByteBuffer read(ByteBufferPool bufferPool, int maxLength, EnumSet<ReadOption> opts) throws IOException, UnsupportedOperationException {
        ...
        if (!(in instanceof HasEnhancedByteBufferAccess)) {
            throw new UnsupportedOperationException(in.getClass().getCanonicalName() + " does not support enhanced byte buffer access.");
        }
    ...
    public void releaseBuffer(ByteBuffer buffer) {
        if (!(in instanceof HasEnhancedByteBufferAccess)) {
            throw new UnsupportedOperationException(in.getClass().getCanonicalName() + " does not support release buffer.");
        }
    ...
    public void setReadahead(Long readahead) throws IOException, UnsupportedOperationException {
        if (!(in instanceof CanSetReadahead)) {
            throw new UnsupportedOperationException(in.getClass().getCanonicalName() + " does not support setting the readahead caching strategy.");
        }
    ...
    public void setDropBehind(Boolean dropCache) throws IOException, UnsupportedOperationException {
        if (!(in instanceof CanSetReadahead)) {
            throw new UnsupportedOperationException(in.getClass().getCanonicalName() + " stream does not support setting the drop-behind caching" + " setting.");
        }
        ...
        switch(StringUtils.toLowerCase(capability)) {
            ...
            case StreamCapabilities.PREADBYTEBUFFER:
                if (!(in instanceof StreamCapabilities)) {
                    throw new UnsupportedOperationException(in.getClass().getCanonicalName() + " does not expose its stream capabilities.");
                }
    ...
    private long right_2;

    private long right_1;
    ...
    private long right_6;

    private long right_5;

    private long right_4;

    private long right_3;
}
