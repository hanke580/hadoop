...
public class LogThrottlingHelper {
    ...
    public LogAction record(String recorderName, long currentTimeMs, double... values) {
        ...
        if (primaryRecorderName.equals(recorderName) && currentTimeMs - minLogPeriodMs >= lastLogTimestampMs) {
    ...
    private static class LoggingAction implements LogAction {
        ...
        public SummaryStatistics getStats(int idx) {
            if (idx < 0 || idx >= stats.length) {
                throw new IllegalArgumentException("Requested stats at idx " + idx + " but this log only maintains " + stats.length + " stats");
            }
            return stats[idx];
        }
        ...
        private void recordValues(double... values) {
            if (values.length != stats.length) {
                throw new IllegalArgumentException("received " + values.length + " values but expected " + stats.length);
            }
            count++;
