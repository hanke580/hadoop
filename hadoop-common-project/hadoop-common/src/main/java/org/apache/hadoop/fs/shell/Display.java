...
class Display extends FsCommand {
    ...
    public static class Checksum extends Display {
        ...
        protected void processPath(PathData item) throws IOException {
            ...
            String outputChecksum = checksum == null ? "NONE" : String.format("%s\t%s", checksum.getAlgorithmName(), StringUtils.byteToHexString(checksum.getBytes(), 0, checksum.getLength()));
            if (displayBlockSize) {
                FileStatus fileStatus = item.fs.getFileStatus(item.path);
                out.printf("%s\t%s\tBlockSize=%s%n", item.toString(), outputChecksum, fileStatus != null ? fileStatus.getBlockSize() : "NONE");
            } else {
    ...
    protected class TextRecordInputStream extends InputStream {
        ...
        public TextRecordInputStream(FileStatus f) throws IOException {
            ...
            key = ReflectionUtils.newInstance(r.getKeyClass().asSubclass(Writable.class), lconf);
