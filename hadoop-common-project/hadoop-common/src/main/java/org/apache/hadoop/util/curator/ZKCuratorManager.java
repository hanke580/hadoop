...
public final class ZKCuratorManager {
    ...
    public static List<ACL> getZKAcls(Configuration conf) throws IOException {
        ...
        String zkAclConf = conf.get(CommonConfigurationKeys.ZK_ACL, CommonConfigurationKeys.ZK_ACL_DEFAULT);
        try {
    ...
    public void start(List<AuthInfo> authInfos) throws IOException {
        ...
        if (zkHostPort == null) {
            throw new IOException(CommonConfigurationKeys.ZK_ADDRESS + " is not configured.");
        }
        int numRetries = conf.getInt(CommonConfigurationKeys.ZK_NUM_RETRIES, CommonConfigurationKeys.ZK_NUM_RETRIES_DEFAULT);
        int zkSessionTimeout = conf.getInt(CommonConfigurationKeys.ZK_TIMEOUT_MS, CommonConfigurationKeys.ZK_TIMEOUT_MS_DEFAULT);
        int zkRetryInterval = conf.getInt(CommonConfigurationKeys.ZK_RETRY_INTERVAL_MS, CommonConfigurationKeys.ZK_RETRY_INTERVAL_MS_DEFAULT);
        ...
        CuratorFramework client = CuratorFrameworkFactory.builder().connectString(zkHostPort).sessionTimeoutMs(zkSessionTimeout).retryPolicy(retryPolicy).authorization(authInfos).build();
        client.start();
        this.curator = client;
    }
    ...
    public boolean create(final String path, List<ACL> zkAcl) throws Exception {
        ...
        if (!exists(path)) {
            curator.create().withMode(CreateMode.PERSISTENT).withACL(zkAcl).forPath(path, null);
            created = true;
        }
        return created;
    }
    ...
    public void safeCreate(String path, byte[] data, List<ACL> acl, CreateMode mode, List<ACL> fencingACL, String fencingNodePath) throws Exception {
        if (!exists(path)) {
            SafeTransaction transaction = createTransaction(fencingACL, fencingNodePath);
     ...
     */
    public void safeDelete(final String path, List<ACL> fencingACL, String fencingNodePath) throws Exception {
        if (exists(path)) {
            SafeTransaction transaction = createTransaction(fencingACL, fencingNodePath);
    ...
    public class SafeTransaction {
        ...
        SafeTransaction(List<ACL> fencingACL, String fencingNodePath) throws Exception {
            ...
            curatorOperations.add(curator.transactionOp().create().withMode(CreateMode.PERSISTENT).withACL(fencingACL).forPath(fencingNodePath, new byte[0]));
        }

        public void commit() throws Exception {
            curatorOperations.add(curator.transactionOp().delete().forPath(fencingNodePath));
        ...
        public void create(String path, byte[] data, List<ACL> acl, CreateMode mode) throws Exception {
            curatorOperations.add(curator.transactionOp().create().withMode(mode).withACL(acl).forPath(path, data));
        }

        public void delete(String path) throws Exception {
            curatorOperations.add(curator.transactionOp().delete().forPath(path));
        }

        public void setData(String path, byte[] data, int version) throws Exception {
            curatorOperations.add(curator.transactionOp().setData().withVersion(version).forPath(path, data));
        }
    }
}
