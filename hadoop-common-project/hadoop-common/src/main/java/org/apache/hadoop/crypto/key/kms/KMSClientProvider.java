...
public class KMSClientProvider extends KeyProvider implements CryptoExtension, KeyProviderDelegationTokenExtension.DelegationTokenExtension {

    static final Logger LOG = LoggerFactory.getLogger(KMSClientProvider.class);
   ...
   */
    ...
    private KeyProviderDelegationTokenExtension.DelegationTokenExtension clientTokenProvider = this;
    ...
    private class EncryptedQueueRefiller implements ValueQueue.QueueRefiller<EncryptedKeyVersion> {
        ...
        @Override
        public void fillQueueForKey(String keyName, Queue<EncryptedKeyVersion> keyQueue, int numEKVs) throws IOException {
            ...
            List<EncryptedKeyVersion> ekvs = parseJSONEncKeyVersions(keyName, response);
    ...
    private static void writeJson(Object obj, OutputStream os) throws IOException {
    ...
    public static class Factory extends KeyProviderFactory {
        ...
        public KeyProvider createProvider(URI providerUri, Configuration conf) throws IOException {
            if (SCHEME_NAME.equals(providerUri.getScheme())) {
                ...
                KMSClientProvider[] providers = createProviders(conf, origUrl, port, hostsPart);
                return new LoadBalancingKMSClientProvider(providerUri, providers, conf);
            }
            return null;
        }

        private KMSClientProvider[] createProviders(Configuration conf, URL origUrl, int port, String hostsPart) throws IOException {
    ...
    public KMSClientProvider(URI uri, Configuration conf) throws IOException {
        ...
        int timeout = conf.getInt(CommonConfigurationKeysPublic.KMS_CLIENT_TIMEOUT_SECONDS, CommonConfigurationKeysPublic.KMS_CLIENT_TIMEOUT_DEFAULT);
        ...
        LOG.debug("KMSClientProvider created for KMS url: {} delegation token " + "service: {} canonical service: {}.", kmsUrl, dtService, canonicalService);
    }
        ...
        try {
            ...
            conn = getActualUgi().doAs(new PrivilegedExceptionAction<HttpURLConnection>() {
                ...
                public HttpURLConnection run() throws Exception {
                    DelegationTokenAuthenticatedURL authUrl = createAuthenticatedURL();
    ...
    private <T> T call(HttpURLConnection conn, Object jsonOutput, int expectedResponse, Class<T> klass) throws IOException {
        return call(conn, jsonOutput, expectedResponse, klass, authRetry);
    }

    private <T> T call(HttpURLConnection conn, Object jsonOutput, int expectedResponse, Class<T> klass, int authRetryCount) throws IOException {
        T ret = null;
    ...
    private KeyVersion createKeyInternal(String name, byte[] material, Options options) throws NoSuchAlgorithmException, IOException {
        checkNotEmpty(name, "name");
    ...
    public void invalidateCache(String name) throws IOException {
        checkNotEmpty(name, "name");
        final URL url = createURL(KMSRESTConstants.KEY_RESOURCE, name, KMSRESTConstants.INVALIDATECACHE_RESOURCE, null);
    ...
    private KeyVersion rollNewVersionInternal(String name, byte[] material) throws NoSuchAlgorithmException, IOException {
        checkNotEmpty(name, "name");
    ...
    public void reencryptEncryptedKeys(List<EncryptedKeyVersion> ekvs) throws IOException, GeneralSecurityException {
        ...
        for (EncryptedKeyVersion ekv : ekvs) {
            ...
            if (keyName == null) {
                ...
                Preconditions.checkArgument(keyName.equals(ekv.getEncryptionKeyName()), "All EncryptedKey must have the same key name.");
            }
            jsonPayload.add(KMSUtil.toJSON(ekv));
        }
        final URL url = createURL(KMSRESTConstants.KEY_RESOURCE, keyName, KMSRESTConstants.REENCRYPT_BATCH_SUB_RESOURCE, null);
        final HttpURLConnection conn = createConnection(url, HTTP_POST);
        conn.setRequestProperty(CONTENT_TYPE, APPLICATION_JSON_MIME);
        final List<Map> response = call(conn, jsonPayload, HttpURLConnection.HTTP_OK, List.class);
        Preconditions.checkArgument(response.size() == ekvs.size(), "Response size is different than input size.");
    ...
    // sub-providers.
    protected void setClientTokenProvider(KeyProviderDelegationTokenExtension.DelegationTokenExtension provider) {
    ...
    DelegationTokenAuthenticatedURL createAuthenticatedURL() {
        return new DelegationTokenAuthenticatedURL(configurator) {
            ...
            public org.apache.hadoop.security.token.Token<? extends TokenIdentifier> selectDelegationToken(URL url, Credentials creds) {
                if (LOG.isDebugEnabled()) {
                    LOG.debug("Looking for delegation token. creds: {}", creds.getAllTokens());
                }
    ...
    protected static Token<?> selectDelegationToken(Credentials creds, Text service) {
    ...
    public long renewDelegationToken(final Token<?> dToken) throws IOException {
        try {
            ...
            final DelegationTokenAuthenticatedURL authUrl = createAuthenticatedURL();
    ...
    public Void cancelDelegationToken(final Token<?> dToken) throws IOException {
        try {
            final String doAsUser = getDoAsUser();
            final DelegationTokenAuthenticatedURL.Token token = generateDelegationToken(dToken);
            return getActualUgi().doAs(new PrivilegedExceptionAction<Void>() {
    ...
    private DelegationTokenAuthenticatedURL.Token generateDelegationToken(final Token<?> dToken) {
        ...
        Token<AbstractDelegationTokenIdentifier> dt = new Token<>(dToken.getIdentifier(), dToken.getPassword(), dToken.getKind(), dToken.getService());
    ...
    private boolean containsKmsDt(UserGroupInformation ugi) throws IOException {
        ...
        if (!creds.getAllTokens().isEmpty()) {
            LOG.debug("Searching for KMS delegation token in user {}'s credentials", ugi);
    ...
    UserGroupInformation getActualUgi() throws IOException {
        ...
        if (UserGroupInformation.isSecurityEnabled() && !containsKmsDt(actualUgi) && !actualUgi.shouldRelogin()) {
