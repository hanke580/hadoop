...
public class Globber {
    ...
    private Globber(FileSystem fs, Path pathPattern, PathFilter filter, boolean resolveSymlinks) {
        ...
        this.tracer = FsTracer.get(fs.getConf());
        LOG.debug("Created Globber for path={}, symlinks={}", pathPattern, resolveSymlinks);
    }
    ...
    private Globber(FileContext fc, Path pathPattern, PathFilter filter, boolean resolveSymlinks) {
        ...
        this.resolveSymlinks = resolveSymlinks;
        this.tracer = fc.getTracer();
        LOG.debug("Created Globber path={}, symlinks={}", pathPattern, resolveSymlinks);
    }
    ...
    public FileStatus[] glob() throws IOException {
        ...
        try (DurationInfo ignored = new DurationInfo(LOG, false, "glob %s", pathPattern)) {
            return doGlob();
        } finally {
            scope.close();
        }
    ...
    private FileStatus[] doGlob() throws IOException {
        ...
        ArrayList<FileStatus> results = new ArrayList<>(flattenedPatterns.size());
        ...
        for (String flatPattern : flattenedPatterns) {
            ...
            for (int componentIdx = 0; componentIdx < components.size(); componentIdx++) {
                ArrayList<FileStatus> newCandidates = new ArrayList<>(candidates.size());
                ...
                for (FileStatus candidate : candidates) {
                    if (globFilter.hasPattern()) {
                        ...
                        if (children.length == 1) {
                            ...
                            // disambiguate.
                            if (resolveSymlinks) {
                                LOG.debug("listStatus found one entry; disambiguating {}", children[0]);
                                Path path = candidate.getPath();
                                FileStatus status = getFileStatus(path);
                                if (status == null) {
                                    // null means the file was not found
                                    LOG.warn("File/directory {} not found:" + " it may have been deleted." + " If this is an object store, this can be a sign of" + " eventual consistency problems.", path);
                                    continue;
                                }
     ...
     */
        if ((!sawWildcard) && results.isEmpty() && (flattenedPatterns.size() <= 1)) {
            LOG.debug("No matches found and there was no wildcard in the path {}", pathPattern);
            return null;
        }
        /*
    ...
    public static class GlobBuilder {
        ...
        public Globber build() {
            return fs != null ? new Globber(fs, pathPattern, filter, resolveSymlinks) : new Globber(fc, pathPattern, filter, resolveSymlinks);
        }
    }
}
