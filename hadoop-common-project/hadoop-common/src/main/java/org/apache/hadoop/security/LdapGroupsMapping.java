...
public class LdapGroupsMapping implements GroupMappingServiceProvider, Configurable {
   ...
   */
     ...
     */
    public static final String LDAP_TRUSTSTORE_KEY = LDAP_CONFIG_PREFIX + ".ssl.truststore";

    /**
     * The key of the credential entry containing the password for
     * the LDAP SSL truststore
     */
    public static final String LDAP_TRUSTSTORE_PASSWORD_KEY = LDAP_CONFIG_PREFIX + ".ssl.truststore.password";

    /**
     * The path to a file containing the password for
     * the LDAP SSL truststore
     */
    public static final String LDAP_TRUSTSTORE_PASSWORD_FILE_KEY = LDAP_TRUSTSTORE_PASSWORD_KEY + ".file";
   ...
   */
    public static final String BIND_USERS_KEY = LDAP_CONFIG_PREFIX + ".bind.users";
   ...
   */
    ...
    public static final String BIND_USER_KEY = LDAP_CONFIG_PREFIX + BIND_USER_SUFFIX;
   ...
   */
    ...
    public static final String BIND_PASSWORD_KEY = LDAP_CONFIG_PREFIX + BIND_PASSWORD_SUFFIX;

    public static final String BIND_PASSWORD_DEFAULT = "";

    public static final String BIND_PASSWORD_FILE_SUFFIX = BIND_PASSWORD_SUFFIX + ".file";

    public static final String BIND_PASSWORD_FILE_KEY = LDAP_CONFIG_PREFIX + BIND_PASSWORD_FILE_SUFFIX;

    public static final String BIND_PASSWORD_FILE_DEFAULT = "";

    public static final String BIND_PASSWORD_ALIAS_SUFFIX = BIND_PASSWORD_SUFFIX + ".alias";

    public static final String BIND_PASSWORD_ALIAS_KEY = LDAP_CONFIG_PREFIX + BIND_PASSWORD_ALIAS_SUFFIX;
   ...
   */
    public static final String USER_BASE_DN_KEY = LDAP_CONFIG_PREFIX + ".userbase";

    /*
   * Base DN used in group search.
   */
    public static final String GROUP_BASE_DN_KEY = LDAP_CONFIG_PREFIX + ".groupbase";
   ...
   */
    public static final String GROUP_HIERARCHY_LEVELS_KEY = LDAP_CONFIG_PREFIX + ".search.group.hierarchy.levels";
   ...
   */
    ...
    // 60 seconds
    public static final int CONNECTION_TIMEOUT_DEFAULT = 60 * 1000;
    ...
    public static final String LDAP_NUM_ATTEMPTS_KEY = LDAP_CONFIG_PREFIX + ".num.attempts";

    public static final int LDAP_NUM_ATTEMPTS_DEFAULT = 3;

    public static final String LDAP_NUM_ATTEMPTS_BEFORE_FAILOVER_KEY = LDAP_CONFIG_PREFIX + ".num.attempts.before.failover";

    public static final int LDAP_NUM_ATTEMPTS_BEFORE_FAILOVER_DEFAULT = LDAP_NUM_ATTEMPTS_DEFAULT;

    public static final String LDAP_CTX_FACTORY_CLASS_KEY = LDAP_CONFIG_PREFIX + ".ctx.factory.class";
   ...
   */
     ...
     */
        ...
        for (int attempt = 1; attempt <= numAttempts; attempt++, atemptsBeforeFailover++) {
            try {
                return doGetGroups(user, groupHierarchyLevels);
            } catch (AuthenticationException e) {
                switchBindUser(e);
            } catch (NamingException e) {
                LOG.warn("Failed to get groups for user {} (attempt={}/{}) using {}. " + "Exception: ", user, attempt, numAttempts, currentLdapUrl, e);
                LOG.trace("TRACE", e);
    ...
    private NamingEnumeration<SearchResult> lookupPosixGroup(SearchResult result, DirContext c) throws NamingException {
        ...
        if (uidNumber != null && gidNumber != null) {
            return c.search(groupbaseDN, "(&" + groupSearchFilter + "(|(" + posixGidAttr + "={0})" + "(" + groupMemberAttr + "={1})))", new Object[] { gidNumber, uidNumber }, SEARCH_CONTROLS);
        }
    ...
    List<String> doGetGroups(String user, int goUpHierarchy) throws NamingException {
        ...
        NamingEnumeration<SearchResult> results = c.search(userbaseDN, userSearchFilter, new Object[] { user }, SEARCH_CONTROLS);
        // return empty list if the user can not be found.
        if (!results.hasMoreElements()) {
            LOG.debug("doGetGroups({}) returned no groups because the " + "user is not found.", user);
  ...
  */
    ...
    protected boolean failover(int attemptsMadeWithSameLdap, int maxAttemptsBeforeFailover) {
        if (attemptsMadeWithSameLdap >= maxAttemptsBeforeFailover) {
            ...
            LOG.info("Reached {} attempts on {}, failing over to {}", attemptsMadeWithSameLdap, previousLdapUrl, currentLdapUrl);
            return true;
        }
        return false;
    }
    ...
    protected void switchBindUser(AuthenticationException e) {
        ...
        if (!oldBindUser.equals(currentBindUser)) {
            LOG.info("Switched from {} to {} after an AuthenticationException: {}", oldBindUser, currentBindUser, e.getMessage());
        }
    }
    ...
    private DirContext getDirContext() throws NamingException {
        if (ctx == null) {
            ...
            if (useSsl) {
                ...
                LdapSslSocketFactory.setConfigurations(keystore, keystorePass, truststore, truststorePass);
    ...
    public synchronized void setConf(Configuration conf) {
        ...
        isPosix = groupSearchFilter.contains(POSIX_GROUP) && userSearchFilter.contains(POSIX_ACCOUNT);
        ...
        groupHierarchyLevels = conf.getInt(GROUP_HIERARCHY_LEVELS_KEY, GROUP_HIERARCHY_LEVELS_DEFAULT);
        posixUidAttr = conf.get(POSIX_UID_ATTR_KEY, POSIX_UID_ATTR_DEFAULT);
        posixGidAttr = conf.get(POSIX_GID_ATTR_KEY, POSIX_GID_ATTR_DEFAULT);
        ...
        if (useOneQuery) {
            returningAttributes = new String[] { groupNameAttr, posixUidAttr, posixGidAttr, memberOfAttr };
        } else {
            returningAttributes = new String[] { groupNameAttr, posixUidAttr, posixGidAttr };
        }
        ...
        Class<? extends InitialContextFactory> ldapCtxFactoryClass = conf.getClass(LDAP_CTX_FACTORY_CLASS_KEY, null, InitialContextFactory.class);
        ...
        this.numAttempts = conf.getInt(LDAP_NUM_ATTEMPTS_KEY, LDAP_NUM_ATTEMPTS_DEFAULT);
        this.numAttemptsBeforeFailover = conf.getInt(LDAP_NUM_ATTEMPTS_BEFORE_FAILOVER_KEY, LDAP_NUM_ATTEMPTS_BEFORE_FAILOVER_DEFAULT);
    }
    ...
    private void loadSslConf(Configuration sslConf) {
        ...
        keystorePass = getPassword(sslConf, LDAP_KEYSTORE_PASSWORD_KEY, LDAP_KEYSTORE_PASSWORD_DEFAULT);
        if (keystorePass.isEmpty()) {
            keystorePass = extractPassword(sslConf.get(LDAP_KEYSTORE_PASSWORD_FILE_KEY, LDAP_KEYSTORE_PASSWORD_FILE_DEFAULT));
        }
        truststore = sslConf.get(LDAP_TRUSTSTORE_KEY, "");
        truststorePass = getPasswordFromCredentialProviders(sslConf, LDAP_TRUSTSTORE_PASSWORD_KEY, "");
        if (truststorePass.isEmpty()) {
            truststorePass = extractPassword(sslConf.get(LDAP_TRUSTSTORE_PASSWORD_FILE_KEY, ""));
        }
    }
    ...
    String extractPassword(String pwFile) {
        ...
        try (Reader reader = new InputStreamReader(Files.newInputStream(Paths.get(pwFile)), StandardCharsets.UTF_8)) {
            int c = reader.read();
            while (c > -1) {
    ...
    private String getPasswordForBindUser(String keyPrefix) {
        String password;
        String alias = conf.get(keyPrefix + BIND_PASSWORD_ALIAS_SUFFIX, BIND_PASSWORD_ALIAS_DEFAULT);
        password = getPasswordFromCredentialProviders(conf, alias, "");
        if (password.isEmpty()) {
            password = getPassword(conf, keyPrefix + BIND_PASSWORD_SUFFIX, BIND_PASSWORD_DEFAULT);
            if (password.isEmpty()) {
                password = extractPassword(conf.get(keyPrefix + BIND_PASSWORD_FILE_SUFFIX, BIND_PASSWORD_FILE_DEFAULT));
            }
        }
        return password;
    }
    ...
    public static class LdapSslSocketFactory extends SocketFactory {
        ...
        public static synchronized SocketFactory getDefault() {
            if (defaultSslFactory == null) {
                try {
                    ...
                    LOG.info("Successfully instantiated LdapSslSocketFactory with " + "keyStoreLocation = {} and trustStoreLocation = {}", keyStoreLocation, trustStoreLocation);
        ...
        private static KeyManager[] createKeyManagers() throws IOException, GeneralSecurityException {
            ...
            keyMgrFactory.init(createKeyStore(keyStoreLocation, keyStorePassword), getPasswordCharArray(keyStorePassword));
        ...
        private static TrustManager[] createTrustManagers() throws IOException, GeneralSecurityException {
            ...
            trustMgrFactory.init(createKeyStore(trustStoreLocation, trustStorePassword));
