...
public class MapFile {
    ...
    public static class Reader implements java.io.Closeable {
        ...
        private synchronized int seekInternal(WritableComparable key, final boolean before) throws IOException {
            ...
            seekIndex != -1 && seekIndex + 1 < count && comparator.compare(key, keys[seekIndex + 1]) < 0 && comparator.compare(key, nextKey) >= 0) {
                ...
                if (// decode insertion point
                seekIndex < 0)
                    seekIndex = -seekIndex - 2;
                if (// belongs before first entry
                seekIndex == -1)
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            try (MapFile.Writer writer = new MapFile.Writer(conf, fs, out, reader.getKeyClass().asSubclass(WritableComparable.class), reader.getValueClass())) {
