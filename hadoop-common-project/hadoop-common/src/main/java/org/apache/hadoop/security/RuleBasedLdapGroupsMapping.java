...
public class RuleBasedLdapGroupsMapping extends LdapGroupsMapping {
    ...
    public static final String CONVERSION_RULE_KEY = LDAP_CONFIG_PREFIX + ".conversion.rule";
    ...
    public synchronized void setConf(Configuration conf) {
        ...
        try {
            ...
            LOG.warn("Invalid {} configured: '{}'. Using default value: '{}'", CONVERSION_RULE_KEY, value, CONVERSION_RULE_DEFAULT);
        }
    }
    ...
    public synchronized List<String> getGroups(String user) {
        ...
        switch(rule) {
            case TO_UPPER:
                return groups.stream().map(StringUtils::toUpperCase).collect(Collectors.toList());
            case TO_LOWER:
                return groups.stream().map(StringUtils::toLowerCase).collect(Collectors.toList());
            case NONE:
            default:
                return groups;
        }
