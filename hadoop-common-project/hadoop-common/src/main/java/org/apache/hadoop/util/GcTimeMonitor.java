...
public class GcTimeMonitor extends Thread {
    ...
    public static class Builder {
        ...
        public GcTimeMonitor build() {
            return new GcTimeMonitor(observationWindowMs, sleepIntervalMs, maxGcTimePercentage, handler);
        }
    }
    ...
    private void calculateGCTimePercentageWithinObservedInterval() {
        ...
        long gcTimeWithinObservationWindow = Math.min(gcDataBuf[startIdx].gcPause, gcDataBuf[startIdx].ts - startObsWindowTs);
        if (startIdx != endIdx) {
            for (int i = (startIdx + 1) % bufSize; i != endIdx; i = (i + 1) % bufSize) {
                gcTimeWithinObservationWindow += gcDataBuf[i].gcPause;
            }
        }
        curData.update(ts, gcMonitorRunTime, totalGcTime, totalGcCount, (int) (gcTimeWithinObservationWindow * 100 / Math.min(observationWindowMs, gcMonitorRunTime)));
    }
