...
abstract class CommandWithDestination extends FsCommand {
    ...
    protected void processArguments(LinkedList<PathData> args) throws IOException {
        ...
        if (args.size() > 1) {
            ...
            throw new PathNotFoundException(dst.toString()).withFullyQualifiedPath(dst.path.toUri().toString());
        }
    ...
    protected PathData getTargetPath(PathData src) throws IOException {
        ...
        } else if (dst.representsDirectory()) {
    ...
    private boolean checkPathsForReservedRaw(Path src, Path target) throws PathOperationException {
        ...
        if (srcIsRR && !dstIsRR) {
            final String s = "' copy from '" + RESERVED_RAW + "' to non '" + RESERVED_RAW + "'. Either both source and target must be in '" + RESERVED_RAW + "' or neither.";
            throw new PathOperationException("'" + src.toString() + s);
        } else if (!srcIsRR && dstIsRR) {
            final String s = "' copy from non '" + RESERVED_RAW + "' to '" + RESERVED_RAW + "'. Either both source and target must be in '" + RESERVED_RAW + "' or neither.";
    ...
    private static class TargetFileSystem extends FilterFileSystem {
        ...
        FSDataOutputStream create(PathData item, boolean lazyPersist) throws IOException {
            if (lazyPersist) {
                ...
                return create(item.path, FsPermission.getFileDefault().applyUMask(FsPermission.getUMask(getConf())), createFlags, getConf().getInt(IO_FILE_BUFFER_SIZE_KEY, IO_FILE_BUFFER_SIZE_DEFAULT), (short) 1, defaultBlockSize, null, null);
            } else {
