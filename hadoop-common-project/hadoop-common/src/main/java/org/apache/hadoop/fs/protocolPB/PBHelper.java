...
public final class PBHelper {
    ...
    public static FileStatus convert(FileStatusProto proto) throws IOException {
        ...
        FileStatus fileStatus = new FileStatus(length, isdir, blockReplication, blocksize, mtime, atime, permission, owner, group, symlink, path, FileStatus.attributes((flags & FileStatusProto.Flags.HAS_ACL_VALUE) != 0, (flags & FileStatusProto.Flags.HAS_CRYPT_VALUE) != 0, (flags & FileStatusProto.Flags.HAS_EC_VALUE) != 0, (flags & FileStatusProto.Flags.SNAPSHOT_ENABLED_VALUE) != 0));
        return fileStatus;
    }
    ...
    public static FileStatusProto convert(FileStatus stat) throws IOException {
        ...
        if (stat.isDirectory()) {
            ...
            bld.setFileType(FileStatusProto.FileType.FT_SYMLINK).setSymlink(stat.getSymlink().toString());
        } else {
            bld.setFileType(FileStatusProto.FileType.FT_FILE).setLength(stat.getLen()).setBlockReplication(stat.getReplication()).setBlockSize(stat.getBlockSize());
        }
        ...
        flags |= stat.isSnapshotEnabled() ? FileStatusProto.Flags.SNAPSHOT_ENABLED_VALUE : 0;
        bld.setFlags(flags);
        return bld.build();
    }
}
