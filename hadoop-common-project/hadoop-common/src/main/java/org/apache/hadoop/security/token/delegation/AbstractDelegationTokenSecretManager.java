...
public abstract class AbstractDelegationTokenSecretManager<TokenIdent extends AbstractDelegationTokenIdentifier> extends SecretManager<TokenIdent> {
    ...
    public synchronized void addPersistedDelegationToken(TokenIdent identifier, long renewDate) throws IOException {
        ...
        if (dKey == null) {
            LOG.warn("No KEY found for persisted identifier " + formatTokenId(identifier));
            return;
        }
    ...
    protected synchronized byte[] createPassword(TokenIdent identifier) {
        ...
        org.zlab.ocov.tracker.Runtime.update(identifier, 28, identifier);
    ...
    public synchronized long renewToken(Token<TokenIdent> token, String renewer) throws InvalidToken, IOException {
        ...
        id.readFields(in);
        org.zlab.ocov.tracker.Runtime.update(id, 122, token, renewer);
    ...
    private class ExpiredTokenRemover extends Thread {
        ...
        public void run() {
            ...
            try {
                while (running) {
                    long now = Time.now();
                    if ((org.zlab.ocov.tracker.Runtime.updateBranch(lastMasterKeyUpdate + keyUpdateInterval, now, "<", 27))) {
                        try {
                            rollMasterKey();
