...
public final class ShutdownHookManager {
        ...
        try {
            Runtime.getRuntime().addShutdownHook(new Thread() {
                ...
                public void run() {
                    ...
                    LOG.debug(String.format("Completed shutdown in %.3f seconds; Timeouts: %d", (ended - started) / 1000.0, timeoutCount));
    ...
    int executeShutdown() {
        ...
        for (HookEntry entry : getShutdownHooksInOrder()) {
            ...
            try {
                ...
                timeouts++;
                future.cancel(true);
                LOG.warn("ShutdownHook '" + entry.getHook().getClass().getSimpleName() + "' timeout, " + ex.toString(), ex);
            } catch (Throwable ex) {
    ...
    static long getShutdownTimeout(Configuration conf) {
        long duration = conf.getTimeDuration(SERVICE_SHUTDOWN_TIMEOUT, SERVICE_SHUTDOWN_TIMEOUT_DEFAULT, TIME_UNIT_DEFAULT);
