...
public abstract class Server {
    ...
    static class ExceptionsHandler {
         ...
         * @return
         */
        private static Set<String> addExceptions(final Set<String> exceptionsSet, Class<?>[] exceptionClass) {
    ...
    void logSlowRpcCalls(String methodName, Call call, ProcessingDetails details) {
        ...
        if ((rpcMetrics.getProcessingSampleCount() > minSampleSize) && (processingTime > threeSigma)) {
            LOG.warn("Slow RPC : {} took {} {} to process from client {}," + " the processing detail is {}", methodName, processingTime, RpcMetrics.TIMEUNIT, call, details.toString());
    ...
    void updateMetrics(Call call, long startTime, boolean connDropped) {
        ...
        details.set(Timing.QUEUE, startTime - timestampNanos - details.get(Timing.ENQUEUE));
   ...
   */
    ...
    public static class Call implements Schedulable, PrivilegedExceptionAction<Void> {
        ...
        // the HTrace scope on the server side
        ...
        Call() {
            this(RpcConstants.INVALID_CALL_ID, RpcConstants.INVALID_RETRY_COUNT, RPC.RpcKind.RPC_BUILTIN, RpcConstants.DUMMY_CLIENT_ID);
        }
    ...
    private class RpcCall extends Call {
        ...
        void doResponse(Throwable t, RpcStatusProto status) throws IOException {
            ...
            if (t != null) {
                ...
                setupResponse(call, status, RpcErrorCodeProto.ERROR_RPC_SERVER, null, t.getClass().getName(), StringUtils.stringifyException(t));
            } else {
                setupResponse(call, call.responseParams.returnStatus, call.responseParams.detailedErr, call.rv, call.responseParams.errorClass, call.responseParams.error);
            }
        ...
        private void sendDeferedResponse() {
            try {
                ...
                LOG.error("Failed to send deferred response. ThreadName=" + Thread.currentThread().getName() + ", CallId=" + callId + ", hostname=" + getHostAddress());
            }
        }
        ...
        public void setDeferredError(Throwable t) {
            if (this.connection.getServer().running) {
                ...
                try {
                    ...
                    setupResponse(this, responseParams.returnStatus, responseParams.detailedErr, null, responseParams.errorClass, responseParams.error);
                } catch (IOException e) {
                    // For synchronous calls, application code is done once it has
                    // returned from a method. It does not expect to receive an error.
                    // This is equivalent to what happens in synchronous calls when the
    ...
    private class Listener extends Thread {
        ...
        private int backlogLength = conf.getInt(CommonConfigurationKeysPublic.IPC_SERVER_LISTEN_QUEUE_SIZE_KEY, CommonConfigurationKeysPublic.IPC_SERVER_LISTEN_QUEUE_SIZE_DEFAULT);

        private boolean reuseAddr = conf.getBoolean(CommonConfigurationKeysPublic.IPC_SERVER_REUSEADDR_KEY, CommonConfigurationKeysPublic.IPC_SERVER_REUSEADDR_DEFAULT);
        ...
        void doAccept(SelectionKey key) throws InterruptedException, IOException, OutOfMemoryError {
            ...
            while ((channel = server.accept()) != null) {
                ...
                Connection c = connectionManager.register(channel, this.listenPort, this.isOnAuxiliaryPort);
    ...
    private final static long PURGE_INTERVAL_NANOS = TimeUnit.NANOSECONDS.convert(15, TimeUnit.MINUTES);
    ...
    public class Connection {
        ...
        private void saslReadAndProcess(RpcWritable.Buffer buffer) throws RpcServerException, IOException, InterruptedException {
            final RpcSaslProto saslMessage = getMessage(RpcSaslProto.getDefaultInstance(), buffer);
            switch(saslMessage.getState()) {
                case WRAP:
                    {
                        if (!saslContextEstablished || !useWrap) {
                            throw new FatalRpcServerException(RpcErrorCodeProto.FATAL_INVALID_RPC_HEADER, new SaslException("Server is not wrapping data"));
                        }
         ...
         */
        private void saslProcess(RpcSaslProto saslMessage) throws RpcServerException, IOException, InterruptedException {
            if (saslContextEstablished) {
                throw new FatalRpcServerException(RpcErrorCodeProto.FATAL_INVALID_RPC_HEADER, new SaslException("Negotiation is already complete"));
            }
            ...
            try {
                ...
                throw new FatalRpcServerException(RpcErrorCodeProto.FATAL_UNAUTHORIZED, ioe);
            }
         ...
         * @throws InterruptedException
         */
        private RpcSaslProto processSaslMessage(RpcSaslProto saslMessage) throws SaslException, IOException, AccessControlException, InterruptedException {
        ...
        private void doSaslReply(Exception ioe) throws IOException {
            setupResponse(authFailedCall, RpcStatusProto.FATAL, RpcErrorCodeProto.FATAL_UNAUTHORIZED, null, ioe.getClass().getName(), ioe.toString());
        ...
        private void checkDataLength(int dataLength) throws IOException {
            if (dataLength < 0) {
                ...
                String error = "Requested data length " + dataLength + " is longer than maximum configured RPC length " + maxDataLength + ".  RPC came from " + getHostAddress();
                LOG.warn(error);
                throw new IOException(error);
            }
        }
        ...
        public int readAndProcess() throws IOException, InterruptedException {
            while (!shouldClose()) {
                ...
                int count = -1;
                try {
                    this.left_33 = (dataLengthBuffer.remaining());
                    this.right_33 = (0);
                } catch (Exception e) {
                    this.left_33 = 0L;
                    this.right_33 = 0L;
                ...
                if (dataLengthBuffer.remaining() > 0) {
                    ...
                    try {
                        this.left_35 = (dataLengthBuffer.remaining());
                        this.right_35 = (0);
                    } catch (Exception e) {
                        this.left_35 = 0L;
                        this.right_35 = 0L;
                ...
                if (!connectionHeaderRead) {
                    ...
                    try {
                        this.left_37 = (connectionHeaderBuf.remaining());
                        this.right_37 = (0);
                    } catch (Exception e) {
                        this.left_37 = 0L;
                        this.right_37 = 0L;
                    ...
                    if (!RpcConstants.HEADER.equals(dataLengthBuffer)) {
                        LOG.warn("Incorrect RPC Header length from {}:{} " + "expected length: {} got length: {}", hostAddress, remotePort, RpcConstants.HEADER, dataLengthBuffer);
                        setupBadVersionResponse(version);
                        return -1;
                    }
                    if (version != CURRENT_VERSION) {
                        //Warning is ok since this is not supposed to happen.
                        LOG.warn("Version mismatch from " + hostAddress + ":" + remotePort + " got version " + version + " expected version " + CURRENT_VERSION);
                        setupBadVersionResponse(version);
                        return -1;
                    }
                    // this may switch us into SIMPLE
                    authProtocol = initializeAuthContext(connectionHeaderBuf.get(2));
                    // clear to next read rpc packet len
                    dataLengthBuffer.clear();
                    connectionHeaderBuf = null;
                    connectionHeaderRead = true;
                    // connection header read, now read  4 bytes rpc packet len
                    continue;
                }
                if (data == null) {
                    // just read 4 bytes -  length of RPC packet
                    dataLengthBuffer.flip();
                ...
                if (data.remaining() == 0) {
                    // to read length of future rpc packets
                    dataLengthBuffer.clear();
                    data.flip();
                    ByteBuffer requestData = data;
                    // null out in case processOneRpc throws.
                    data = null;
         ...
         */
        private RpcSaslProto buildSaslNegotiateResponse() throws InterruptedException, SaslException, IOException {
        ...
        private SaslServer createSaslServer(AuthMethod authMethod) throws IOException, InterruptedException {
            final Map<String, ?> saslProps = saslPropsResolver.getServerProperties(addr, ingressPort);
        ...
        private void setupHttpRequestOnIpcPortResponse() throws IOException {
            RpcCall fakeCall = new RpcCall(this, 0);
            fakeCall.setResponse(ByteBuffer.wrap(RECEIVED_HTTP_REQ_RESPONSE.getBytes(StandardCharsets.UTF_8)));
            sendResponse(fakeCall);
        }
        ...
        private void processConnectionContext(RpcWritable.Buffer buffer) throws RpcServerException {
            ...
            if (connectionContextRead) {
                throw new FatalRpcServerException(RpcErrorCodeProto.FATAL_INVALID_RPC_HEADER, "Connection context already processed");
            }
            ...
            if (authProtocol == AuthProtocol.NONE) {
                ...
                if ((protocolUser != null) && (!protocolUser.getUserName().equals(user.getUserName()))) {
                    if (authMethod == AuthMethod.TOKEN) {
                        ...
                        throw new FatalRpcServerException(RpcErrorCodeProto.FATAL_UNAUTHORIZED, new AccessControlException("Authenticated user (" + user + ") doesn't match what the client claims to be (" + protocolUser + ")"));
                    } else {
        ...
        private void processOneRpc(ByteBuffer bb) throws IOException, InterruptedException {
            ...
            try {
                ...
                if (callId < 0) {
                    ...
                    throw new FatalRpcServerException(RpcErrorCodeProto.FATAL_INVALID_RPC_HEADER, "Connection context not established");
                } else {
                ...
                setupResponse(call, rse.getRpcStatusProto(), rse.getRpcErrorCodeProto(), null, t.getClass().getName(), t.getMessage() != null ? t.getMessage() : t.toString());
                sendResponse(call);
            }
        }
         ...
         */
        private void checkRpcHeaders(RpcRequestHeaderProto header) throws RpcServerException {
            if (!header.hasRpcOp()) {
         ...
         * @throws InterruptedException
         */
        private void processRpcRequest(RpcRequestHeaderProto header, RpcWritable.Buffer buffer) throws RpcServerException, InterruptedException {
            ...
            try {
                ...
                throw new FatalRpcServerException(RpcErrorCodeProto.FATAL_DESERIALIZING_REQUEST, err);
            }
            ...
            if (alignmentContext != null && call.rpcRequest != null && (call.rpcRequest instanceof ProtobufRpcEngine2.RpcProtobufRequest)) {
            ...
            try {
                ...
                throw new FatalRpcServerException(RpcErrorCodeProto.ERROR_RPC_SERVER, ioe);
            }
         ...
         * @throws InterruptedException
         */
        private void processRpcOutOfBandRequest(RpcRequestHeaderProto header, RpcWritable.Buffer buffer) throws RpcServerException, IOException, InterruptedException {
            ...
            if (callId == CONNECTION_CONTEXT_CALL_ID) {
                ...
                if (authProtocol == AuthProtocol.SASL && !saslContextEstablished) {
                    throw new FatalRpcServerException(RpcErrorCodeProto.FATAL_INVALID_RPC_HEADER, "Connection header sent during SASL negotiation");
                }
                ...
                if (authProtocol != AuthProtocol.SASL) {
                    throw new FatalRpcServerException(RpcErrorCodeProto.FATAL_INVALID_RPC_HEADER, "SASL protocol not requested by client");
                }
                saslReadAndProcess(buffer);
            } else if (callId == PING_CALL_ID) {
                LOG.debug("Received ping message");
            } else {
                throw new FatalRpcServerException(RpcErrorCodeProto.FATAL_INVALID_RPC_HEADER, "Unknown out of band call #" + callId);
            }
        }
        ...
        private void authorizeConnection() throws RpcServerException {
            try {
                ...
                throw new FatalRpcServerException(RpcErrorCodeProto.FATAL_UNAUTHORIZED, ae);
            }
        }
        ...
        <T extends Message> T getMessage(Message message, RpcWritable.Buffer buffer) throws RpcServerException {
            try {
                ...
                throw new FatalRpcServerException(RpcErrorCodeProto.FATAL_DESERIALIZING_REQUEST, "Error decoding " + protoClass.getSimpleName() + ": " + ioe);
            }
        }
        ...
        private long right_37;

        private long right_36;

        private long right_35;

        private long right_34;

        private long right_33;

        private long right_39;

        private long right_38;

        private long left_38;

        private long left_37;

        private long left_39;

        private long left_34;

        private long left_33;

        private long left_36;

        private long left_35;
    }
    ...
    private void internalQueueCall(Call call, boolean blocking) throws IOException, InterruptedException {
        try {
            ...
            call.getProcessingDetails().set(Timing.ENQUEUE, deltaNanos, TimeUnit.NANOSECONDS);
    ...
    private class Handler extends Thread {
        ...
        public Handler(int instanceNumber) {
            this.setDaemon(true);
            this.setName("IPC Server handler " + instanceNumber + " on default port " + port);
        }
        ...
        public void run() {
            ...
            while (running) {
                ...
                try {
                    ...
                    if (alignmentContext != null && call.isCallCoordinated() && call.getClientStateId() > alignmentContext.getLastSeenStateId()) {
                        /*
             ...
             */
                    ...
                    if (call != null) {
                        ...
                        ProcessingDetails.LOG.debug("Served: [{}]{} name={} user={} details={}", call, (call.isResponseDeferred() ? ", deferred" : ""), call.getDetailedMetricsName(), call.getRemoteUser(), call.getProcessingDetails());
                    }
                }
            }
    ...
    protected Server(String bindAddress, int port, Class<? extends Writable> rpcRequestClass, int handlerCount, int numReaders, int queueSizePerHandler, Configuration conf, String serverName, SecretManager<? extends TokenIdentifier> secretManager, String portRangeConfig) throws IOException {
        ...
        this.callQueue = new CallQueueManager<Call>(getQueueClass(prefix, conf), getSchedulerClass(prefix, conf), getClientBackoffEnable(prefix, conf), maxQueueSize, prefix, conf);
        ...
        this.setLogSlowRPC(conf.getBoolean(CommonConfigurationKeysPublic.IPC_SERVER_LOG_SLOW_RPC, CommonConfigurationKeysPublic.IPC_SERVER_LOG_SLOW_RPC_DEFAULT));
    ...
    private void setupResponse(RpcCall call, RpcResponseHeaderProto header, Writable rv) throws IOException {
        final byte[] response;
    ...
    private byte[] setupResponseForWritable(RpcResponseHeaderProto header, Writable rv) throws IOException {
    ...
    private class ConnectionManager {
        ...
        ConnectionManager() {
            ...
            this.idleScanThreshold = conf.getInt(CommonConfigurationKeysPublic.IPC_CLIENT_IDLETHRESHOLD_KEY, CommonConfigurationKeysPublic.IPC_CLIENT_IDLETHRESHOLD_DEFAULT);
            this.idleScanInterval = conf.getInt(CommonConfigurationKeys.IPC_CLIENT_CONNECTION_IDLESCANINTERVAL_KEY, CommonConfigurationKeys.IPC_CLIENT_CONNECTION_IDLESCANINTERVAL_DEFAULT);
            this.maxIdleTime = 2 * conf.getInt(CommonConfigurationKeysPublic.IPC_CLIENT_CONNECTION_MAXIDLETIME_KEY, CommonConfigurationKeysPublic.IPC_CLIENT_CONNECTION_MAXIDLETIME_DEFAULT);
            this.maxIdleToClose = conf.getInt(CommonConfigurationKeysPublic.IPC_CLIENT_KILL_MAX_KEY, CommonConfigurationKeysPublic.IPC_CLIENT_KILL_MAX_DEFAULT);
            this.maxConnections = conf.getInt(CommonConfigurationKeysPublic.IPC_SERVER_MAX_CONNECTIONS_KEY, CommonConfigurationKeysPublic.IPC_SERVER_MAX_CONNECTIONS_DEFAULT);
            // create a set with concurrency -and- a thread-safe iterator, add 2
            // for listener and idle closer threads
            this.connections = Collections.newSetFromMap(new ConcurrentHashMap<Connection, Boolean>(maxQueueSize, 0.75f, readThreads + 2));
