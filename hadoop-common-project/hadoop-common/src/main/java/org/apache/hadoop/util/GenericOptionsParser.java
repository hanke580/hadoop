...
public class GenericOptionsParser {
    ...
    protected Options buildGeneralOptions(Options opts) {
        synchronized (OptionBuilder.class) {
            Option fs = OptionBuilder.withArgName("file:///|hdfs://namenode:port").hasArg().withDescription("specify default filesystem URL to use, " + "overrides 'fs.defaultFS' property from configurations.").create("fs");
    ...
    public static URL[] getLibJars(Configuration conf) throws IOException {
        ...
        for (String file : files) {
            ...
            if (tmp.getFileSystem(conf).equals(FileSystem.getLocal(conf))) {
                ...
                LOG.warn("The libjars file " + tmp + " is not on the local " + "filesystem. It will not be added to the local classpath.");
            }
        }
     ...
     * if the input files argument is null
     */
    private String validateFiles(String files, boolean expandWildcard) throws IOException {
        if (files == null) {
            return null;
        }
    ...
    private boolean matchesCurrentDirectory(String path) {
        return path.isEmpty() || path.equals(Path.CUR_DIR) || path.equals(Path.CUR_DIR + File.separator);
    }
