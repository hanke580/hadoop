...
public class HealthMonitor {
    ...
    private List<ServiceStateCallback> serviceStateCallbacks = Collections.synchronizedList(new LinkedList<ServiceStateCallback>());
    ...
    HealthMonitor(Configuration conf, HAServiceTarget target) {
        ...
        this.rpcConnectRetries = conf.getInt(HA_HM_RPC_CONNECT_MAX_RETRIES_KEY, HA_HM_RPC_CONNECT_MAX_RETRIES_DEFAULT);
        ...
        while (shouldRun) {
            ...
            try {
                ...
                if (isHealthCheckFailedException(t)) {
                    ...
                    LOG.warn("Transport-level exception trying to monitor health of {}", targetToMonitor, t);
                    RPC.stopProxy(proxy);
                    proxy = null;
                    enterState(State.SERVICE_NOT_RESPONDING);
