...
public class SysInfoLinux extends SysInfo {
    ...
    // inactive cache memory, -1 if not there
    private long inactiveFileSize = -1;

    // RAM corrupt and not available
    private long hardwareCorruptSize = 0;

    // # of hugepages reserved
    private long hugePagesTotal = 0;

    // # size of each hugepage
    private long hugePageSize = 0;
    ...
    // aggregated bytes read from network
    private long numNetBytesRead = 0L;

    // aggregated bytes written to network
    private long numNetBytesWritten = 0L;

    // aggregated bytes read from disks
    private long numDisksBytesRead = 0L;

    // aggregated bytes written to disks
    ...
    public static final long JIFFY_LENGTH_IN_MILLIS = Math.max(Math.round(1000D / getConf("CLK_TCK")), -1);
    ...
    private void readProcMemInfoFile(boolean readAgain) {
        ...
        try {
            fReader = new InputStreamReader(Files.newInputStream(Paths.get(procfsMemFile)), Charset.forName("UTF-8"));
    ...
    private void readProcCpuInfoFile() {
        ...
        InputStreamReader fReader;
        try {
            fReader = new InputStreamReader(Files.newInputStream(Paths.get(procfsCpuFile)), Charset.forName("UTF-8"));
            in = new BufferedReader(fReader);
        } catch (IOException f) {
            // shouldn't happen....
    ...
    private void readProcStatFile() {
        ...
        InputStreamReader fReader;
        try {
            fReader = new InputStreamReader(Files.newInputStream(Paths.get(procfsStatFile)), Charset.forName("UTF-8"));
            in = new BufferedReader(fReader);
        } catch (IOException f) {
            // shouldn't happen....
            return;
    ...
    private void readProcNetInfoFile() {
        ...
        InputStreamReader fReader;
        try {
            fReader = new InputStreamReader(Files.newInputStream(Paths.get(procfsNetFile)), Charset.forName("UTF-8"));
            in = new BufferedReader(fReader);
        } catch (IOException f) {
            return;
        }
    ...
    private void readProcDisksInfoFile() {
        ...
        BufferedReader in;
        try {
            in = new BufferedReader(new InputStreamReader(Files.newInputStream(Paths.get(procfsDisksFile)), Charset.forName("UTF-8")));
        } catch (IOException f) {
            return;
        }
        Matcher mat;
    ...
    int readDiskBlockInformation(String diskName, int defSector) {
        ...
        BufferedReader in;
        try {
            in = new BufferedReader(new InputStreamReader(Files.newInputStream(Paths.get(procfsDiskSectorFile)), Charset.forName("UTF-8")));
        } catch (IOException f) {
            return defSector;
        }
        Matcher mat;
