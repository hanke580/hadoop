...
class FsUsage extends FsCommand {
    ...
    public static class Df extends FsUsage {
        ...
        protected void processArguments(LinkedList<PathData> args) throws IOException {
            setUsagesTable(new TableBuilder("Filesystem", "Size", "Used", "Available", "Use%", "Mounted on"));
        ...
        private void addToUsagesTable(URI uri, FsStatus fsStatus, String mountedOnPath) {
            ...
            getUsagesTable().addRow(uri, formatSize(size), formatSize(used), formatSize(free), StringUtils.formatPercent((double) used / (double) size, 0), mountedOnPath);
        }
        ...
        protected void processPath(PathData item) throws IOException {
            if (ViewFileSystemUtil.isViewFileSystem(item.fs)) {
                ...
                for (Map.Entry<ViewFileSystem.MountPoint, FsStatus> entry : fsStatusMap.entrySet()) {
                    ...
                    addToUsagesTable(mountPointFileSystemURIs[0], fsStatus, viewFsMountPoint.getMountedOnPath().toString());
                }
            } else {
    ...
    public static class Du extends FsUsage {
        ...
        protected void processArguments(LinkedList<PathData> args) throws IOException {
            if (showHeaderLine) {
                setUsagesTable(new TableBuilder("SIZE", "DISK_SPACE_CONSUMED_WITH_ALL_REPLICAS", "FULL_PATH_NAME"));
            } else {
        ...
        protected void processPath(PathData item) throws IOException {
            ...
            getUsagesTable().addRow(formatSize(length), formatSize(spaceConsumed), item);
        }
    }
