...
public abstract class FileSystem extends Configured implements Closeable, DelegationTokenIssuer, PathCapabilities {
    ...
    @VisibleForTesting
    static void removeFileSystemForTesting(URI uri, Configuration conf, FileSystem fs) throws IOException {
    ...
    public void initialize(URI name, Configuration conf) throws IOException {
        ...
        resolveSymlinks = conf.getBoolean(CommonConfigurationKeysPublic.FS_CLIENT_RESOLVE_REMOTE_SYMLINKS_KEY, CommonConfigurationKeysPublic.FS_CLIENT_RESOLVE_REMOTE_SYMLINKS_DEFAULT);
    }
    ...
    public static FileSystem get(URI uri, Configuration conf) throws IOException {
        ...
        if (scheme != null && authority == null) {
            ...
            if (// if scheme matches default
            scheme.equals(defaultUri.getScheme()) && defaultUri.getAuthority() != null) {
    ...
    private static void debugLogFileSystemClose(String methodName, String additionalInfo) {
        if (LOGGER.isDebugEnabled()) {
            ...
            LOGGER.debug("FileSystem.{}() by method: {}); {}", methodName, throwable.getStackTrace()[2], additionalInfo);
    ...
    public BlockLocation[] getFileBlockLocations(FileStatus file, long start, long len) throws IOException {
        ...
        if ((org.zlab.ocov.tracker.Runtime.updateBranch(file.getLen(), start, "<=", 54))) {
    ...
    public FsServerDefaults getServerDefaults() throws IOException {
        ...
        return new FsServerDefaults(getDefaultBlockSize(), config.getInt("io.bytes.per.checksum", 512), 64 * 1024, getDefaultReplication(), config.getInt(IO_FILE_BUFFER_SIZE_KEY, IO_FILE_BUFFER_SIZE_DEFAULT), false, FS_TRASH_INTERVAL_DEFAULT, DataChecksum.Type.CRC32, "");
    }
    ...
    public FSDataOutputStream create(Path f, boolean overwrite, int bufferSize, short replication, long blockSize, Progressable progress) throws IOException {
        return this.create(f, FsCreateModes.applyUMask(FsPermission.getFileDefault(), FsPermission.getUMask(getConf())), overwrite, bufferSize, replication, blockSize, progress);
    }
    ...
    public FSDataOutputStream createNonRecursive(Path f, FsPermission permission, boolean overwrite, int bufferSize, short replication, long blockSize, Progressable progress) throws IOException {
        return createNonRecursive(f, permission, overwrite ? EnumSet.of(CreateFlag.CREATE, CreateFlag.OVERWRITE) : EnumSet.of(CreateFlag.CREATE), bufferSize, replication, blockSize, progress);
    }
    ...
    public FSDataOutputStream append(Path f, boolean appendToNewBlock) throws IOException {
        return append(f, getConf().getInt(IO_FILE_BUFFER_SIZE_KEY, IO_FILE_BUFFER_SIZE_DEFAULT), null, appendToNewBlock);
    }
    ...
    public FileStatus[] globStatus(Path pathPattern) throws IOException {
        return Globber.createGlobber(this).withPathPattern(pathPattern).withPathFiltern(DEFAULT_FILTER).withResolveSymlinks(true).build().glob();
    }
    ...
    protected class DirListingIterator<T extends FileStatus> implements RemoteIterator<T> {
        ...
        public boolean hasNext() throws IOException {
            return i < entries.getEntries().length || entries.hasMore();
        }
            ...
            private void handleFileStat(LocatedFileStatus stat) throws IOException {
                if (stat.isFile()) {
                    ...
                    try {
                        ...
                        LOGGER.debug("Directory {} deleted while attempting for recursive listing", stat.getPath());
                    }
                }
            }
    ...
    public Path getHomeDirectory() {
        ...
        try {
            ...
            LOGGER.warn("Unable to get user name. Fall back to system property " + "user.name", ex);
            username = System.getProperty("user.name");
        }
        return this.makeQualified(new Path(USER_HOME_PREFIX + "/" + username));
    }
    ...
    public void close() throws IOException {
        debugLogFileSystemClose("close", "Key: " + key + "; URI: " + getUri() + "; Object Identity Hash: " + Integer.toHexString(System.identityHashCode(this)));
                ...
                while (it.hasNext()) {
                    ...
                    try {
                        ...
                        try {
                            ...
                            if (LOGGER.isDebugEnabled()) {
                                LOGGER.debug("{}:// = {} from {}", fs.getScheme(), fs.getClass(), ClassUtil.findContainingJar(fs.getClass()));
                            }
                        } catch (Exception e) {
    ...
    private static FileSystem createFileSystem(URI uri, Configuration conf) throws IOException {
        ...
        try (TraceScope scope = tracer.newScope("FileSystem#createFileSystem");
            ...
            try {
                ...
                org.zlab.ocov.tracker.Runtime.update(fs, 220);
    ...
    static final class Cache {
        ...
        Cache(final Configuration conf) {
            int permits = conf.getInt(FS_CREATION_PARALLEL_COUNT, FS_CREATION_PARALLEL_COUNT_DEFAULT);
            checkArgument(permits > 0, "Invalid value of %s: %s", FS_CREATION_PARALLEL_COUNT, permits);
        ...
        private FileSystem getInternal(URI uri, Configuration conf, Key key) throws IOException {
            ...
            try (DurationInfo d = new DurationInfo(LOGGER, false, "Acquiring creator semaphore for %s", uri)) {
            ...
            try {
                ...
                final long timeout = conf.getTimeDuration(SERVICE_SHUTDOWN_TIMEOUT, SERVICE_SHUTDOWN_TIMEOUT_DEFAULT, ShutdownHookManager.TIME_UNIT_DEFAULT);
                // any FS to close outside of the synchronized section
                synchronized (this) {
                    // lock on the Cache object
                    ...
                    if (oldfs != null) {
                        ...
                        if (map.isEmpty() && !ShutdownHookManager.get().isShutdownInProgress()) {
                            ShutdownHookManager.get().addShutdownHook(clientFinalizer, SHUTDOWN_HOOK_PRIORITY, timeout, ShutdownHookManager.TIME_UNIT_DEFAULT);
                        }
            ...
            public void run() {
                while (!Thread.interrupted()) {
                    try {
                        ...
                        LOGGER.warn("Exception in the cleaner thread but it will" + " continue to run", th);
                    }
                }
            }
        }
    ...
    protected static FSDataOutputStreamBuilder createDataOutputStreamBuilder(@Nonnull final FileSystem fileSystem, @Nonnull final Path path) {
        ...
        public FSDataOutputStream build() throws IOException {
            ...
            if (getFlags().contains(CreateFlag.CREATE) || getFlags().contains(CreateFlag.OVERWRITE)) {
                if (isRecursive()) {
                    ...
                    return getFS().createNonRecursive(getPath(), getPermission(), getFlags(), getBufferSize(), getReplication(), getBlockSize(), getProgress());
                }
     ...
     */
    protected CompletableFuture<FSDataInputStream> openFileWithOptions(final PathHandle pathHandle, final OpenFileParameters parameters) throws IOException {
        AbstractFSBuilderImpl.rejectUnknownMandatoryKeys(parameters.getMandatoryKeys(), Options.OpenFileOptions.FS_OPTION_OPENFILE_STANDARD_OPTIONS, "");
    ...
    private void methodNotSupported() {
        ...
        throw new UnsupportedOperationException(getClass().getCanonicalName() + " does not support method " + name);
    }
    ...
    @InterfaceStability.Unstable
    protected static FSDataInputStreamBuilder createDataInputStreamBuilder(@Nonnull final FileSystem fileSystem, @Nonnull final Path path) {
    ...
    @InterfaceAudience.LimitedPrivate("Filesystems")
    @InterfaceStability.Unstable
    protected static FSDataInputStreamBuilder createDataInputStreamBuilder(@Nonnull final FileSystem fileSystem, @Nonnull final PathHandle pathHandle) {
     ...
     */
    private static class FSDataInputStreamBuilder extends FutureDataInputStreamBuilderImpl implements FutureDataInputStreamBuilder {
         ...
         */
        protected FSDataInputStreamBuilder(@Nonnull final FileSystem fileSystem, @Nonnull final Path path) {
         ...
         * @param pathHandle path handle of file to open.
         */
        protected FSDataInputStreamBuilder(@Nonnull final FileSystem fileSystem, @Nonnull final PathHandle pathHandle) {
        ...
        public CompletableFuture<FSDataInputStream> build() throws IOException {
            ...
            OpenFileParameters parameters = new OpenFileParameters().withMandatoryKeys(getMandatoryKeys()).withOptionalKeys(getOptionalKeys()).withOptions(getOptions()).withStatus(super.getStatus()).withBufferSize(getOptions().getInt(FS_OPTION_OPENFILE_BUFFER_SIZE, getBufferSize()));
            if (optionalPath.isPresent()) {
                return getFS().openFileWithOptions(optionalPath.get(), parameters);
            } else {
                return getFS().openFileWithOptions(getPathHandle(), parameters);
            }
        }
    }
