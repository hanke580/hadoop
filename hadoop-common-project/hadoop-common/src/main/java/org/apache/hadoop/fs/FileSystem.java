...
public abstract class FileSystem extends Configured implements Closeable {
     ...
     */
    private void collectDelegationTokens(final String renewer, final Credentials credentials, final List<Token<?>> tokens) throws IOException {
    ...
    public BlockLocation[] getFileBlockLocations(FileStatus file, long start, long len) throws IOException {
        ...
        if ((org.zlab.ocov.tracker.Runtime.updateBranch(file.getLen(), start, "<=", 28))) {
    ...
    public FSDataOutputStream create(Path f, boolean overwrite, int bufferSize, short replication, long blockSize, Progressable progress) throws IOException {
        return this.create(f, FsPermission.getFileDefault().applyUMask(FsPermission.getUMask(getConf())), overwrite, bufferSize, replication, blockSize, progress);
    }
    ...
    private static FileSystem createFileSystem(URI uri, Configuration conf) throws IOException {
        ...
        try (TraceScope scope = tracer.newScope("FileSystem#createFileSystem")) {
            ...
            org.zlab.ocov.tracker.Runtime.update(fs, 110, uri, conf);
            return fs;
        }
    }
