...
public class XORRawEncoder extends RawErasureEncoder {
    ...
    protected void doEncode(ByteBufferEncodingState encodingState) {
        ...
        for (int i = 1; i < encodingState.inputs.length; i++) {
            for (iIdx = encodingState.inputs[i].position(), oIdx = output.position(); iIdx < encodingState.inputs[i].limit(); iIdx++, oIdx++) {
