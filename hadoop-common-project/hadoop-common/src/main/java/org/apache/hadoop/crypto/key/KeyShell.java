...
public class KeyShell extends CommandShell {
        ...
        public void execute() throws NoSuchAlgorithmException, IOException {
            try {
                ...
                try {
                    ...
                    getOut().println("Cannot roll key: " + keyName + " within KeyProvider: " + provider + ".");
                    throw e;
                }
    ...
    private class DeleteCommand extends Command {
        ...
        public boolean validate() {
            ...
            if (interactive) {
                try {
                    ...
                    getOut().println(keyName + " will not be deleted. " + prettifyException(e));
                }
            }
            return true;
        }
        ...
        public void execute() throws IOException {
            warnIfTransientProvider();
            getOut().println("Deleting key: " + keyName + " from KeyProvider: " + provider);
            if (cont) {
                try {
    ...
    private class InvalidateCacheCommand extends Command {
        ...
        public void execute() throws NoSuchAlgorithmException, IOException {
            try {
                warnIfTransientProvider();
                getOut().println("Invalidating cache on KeyProvider: " + provider + "\n  for key name: " + keyName);
                provider.invalidateCache(keyName);
                getOut().println("Cached keyversions of " + keyName + " has been successfully invalidated.");
                printProviderWritten();
            } catch (IOException e) {
                getOut().println("Cannot invalidate cache for key: " + keyName + " within KeyProvider: " + provider + ".");
                throw e;
            }
        }
    ...
    protected void printException(Exception e) {
        getErr().println("Executing command failed with " + "the following exception: " + prettifyException(e));
    }
