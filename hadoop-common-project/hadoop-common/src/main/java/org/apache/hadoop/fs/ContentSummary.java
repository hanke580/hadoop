...
public class ContentSummary extends QuotaUsage implements Writable {
    ...
    @Override
    public int hashCode() {
        long result = getLength() ^ getFileCount() ^ getDirectoryCount() ^ getSnapshotLength() ^ getSnapshotFileCount() ^ getSnapshotDirectoryCount() ^ getSnapshotSpaceConsumed() ^ getErasureCodingPolicy().hashCode();
    ...
    public String toString(boolean qOption, boolean hOption, boolean tOption, boolean xOption, List<StorageType> types) {
        ...
        if (xOption) {
            return prefix + String.format(SUMMARY_FORMAT, formatSize(directoryCount - snapshotDirectoryCount, hOption), formatSize(fileCount - snapshotFileCount, hOption), formatSize(length - snapshotLength, hOption));
        } else {
