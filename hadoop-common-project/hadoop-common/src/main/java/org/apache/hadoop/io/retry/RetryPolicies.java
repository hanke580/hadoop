...
public class RetryPolicies {
    ...
    static abstract class RetryLimited implements RetryPolicy {
        ...
        public RetryAction shouldRetry(Exception e, int retries, int failovers, boolean isIdempotentOrAtMostOnce) throws Exception {
            ...
            return new RetryAction(RetryAction.RetryDecision.RETRY, timeUnit.toMillis(calculateSleepTime(retries)), getReason());
        }
        ...
        public String toString() {
            if (myString == null) {
                myString = getClass().getSimpleName() + "(maxRetries=" + maxRetries + ", sleepTime=" + sleepTime + " " + timeUnit + ")";
            }
            return myString;
        }
    }
    ...
    public static class MultipleLinearRandomRetry implements RetryPolicy {
        ...
        @Override
        public RetryAction shouldRetry(Exception e, int curRetry, int failovers, boolean isIdempotentOrAtMostOnce) throws Exception {
    ...
    static class FailoverOnNetworkExceptionRetry implements RetryPolicy {
        ...
        public RetryAction shouldRetry(Exception e, int retries, int failovers, boolean isIdempotentOrAtMostOnce) throws Exception {
            ...
            if (e instanceof ConnectException || e instanceof EOFException || e instanceof NoRouteToHostException || e instanceof UnknownHostException || e instanceof StandbyException || e instanceof ConnectTimeoutException || shouldFailoverOnException(e)) {
            ...
            } else if (e instanceof AccessControlException || hasWrappedAccessControlException(e)) {
                return new RetryAction(RetryAction.RetryDecision.FAIL, 0, "Access denied");
            } else if (e instanceof SocketException || (e instanceof IOException && !(e instanceof RemoteException))) {
                if (isIdempotentOrAtMostOnce) {
                    return new RetryAction(RetryAction.RetryDecision.FAILOVER_AND_RETRY, getFailoverOrRetrySleepTime(retries));
                } else {
    ...
    private static boolean shouldFailoverOnException(Exception e) {
        ...
        Exception unwrapped = ((RemoteException) e).unwrapRemoteException(StandbyException.class, ObserverRetryOnActiveException.class);
