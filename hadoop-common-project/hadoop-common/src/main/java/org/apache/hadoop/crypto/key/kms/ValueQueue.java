...
public class ValueQueue<E> {
    ...
    private static final int MASK = LOCK_ARRAY_SIZE == Integer.MAX_VALUE ? LOCK_ARRAY_SIZE : LOCK_ARRAY_SIZE - 1;
    ...
    public ValueQueue(final int numValues, final float lowWatermark, long expiry, int numFillerThreads, SyncGenerationPolicy policy, final QueueRefiller<E> refiller) {
        ...
        Preconditions.checkArgument(watermarkValue > 0, "(int) (\"numValues\" * \"lowWatermark\") must be > 0");
    ...
    public ValueQueue(final int numValues, final float lowWaterMark, long expiry, int numFillerThreads, QueueRefiller<E> fetcher) {
    ...
    public int getSize(String keyName) {
        ...
        try {
            ...
            Map<String, LinkedBlockingQueue<E>> map = keyQueues.getAllPresent(Arrays.asList(keyName));
