...
public final class ECSchema implements Serializable {
   ...
   */
    ...
    public String toString() {
        ...
        sb.append("Codec=" + codecName + ", ").append(NUM_DATA_UNITS_KEY + "=" + numDataUnits + ", ").append(NUM_PARITY_UNITS_KEY + "=" + numParityUnits).append((extraOptions.isEmpty() ? "" : ", "));
        int i = 0;
        for (Map.Entry<String, String> entry : extraOptions.entrySet()) {
            sb.append(entry.getKey() + "=" + entry.getValue() + (++i < extraOptions.size() ? ", " : ""));
        }
        sb.append("]");
        return sb.toString();
    }
    ...
    public boolean equals(Object o) {
        ...
        return new EqualsBuilder().append(codecName, rhs.codecName).append(extraOptions, rhs.extraOptions).append(numDataUnits, rhs.numDataUnits).append(numParityUnits, rhs.numParityUnits).isEquals();
    }
