 ...
 */
public class LoadBalancingKMSClientProvider extends KeyProvider implements CryptoExtension, KeyProviderDelegationTokenExtension.DelegationTokenExtension {
    ...
    private LoadBalancingKMSClientProvider(URI uri, KMSClientProvider[] providers, long seed, Configuration conf) {
        ...
        int maxNumRetries = conf.getInt(CommonConfigurationKeysPublic.KMS_CLIENT_FAILOVER_MAX_RETRIES_KEY, providers.length);
        int sleepBaseMillis = conf.getInt(CommonConfigurationKeysPublic.KMS_CLIENT_FAILOVER_SLEEP_BASE_MILLIS_KEY, CommonConfigurationKeysPublic.KMS_CLIENT_FAILOVER_SLEEP_BASE_MILLIS_DEFAULT);
        int sleepMaxMillis = conf.getInt(CommonConfigurationKeysPublic.KMS_CLIENT_FAILOVER_SLEEP_MAX_MILLIS_KEY, CommonConfigurationKeysPublic.KMS_CLIENT_FAILOVER_SLEEP_MAX_MILLIS_DEFAULT);
        ...
        LOG.debug("Created LoadBalancingKMSClientProvider for KMS url: {} with {} " + "providers. delegation token service: {}, canonical service: {}", uri, providers.length, dtService, canonicalService);
    }
    ...
    @Override
    public org.apache.hadoop.security.token.Token<? extends TokenIdentifier> selectDelegationToken(Credentials creds) {
        Token<? extends TokenIdentifier> token = KMSClientProvider.selectDelegationToken(creds, canonicalService);
        if (token == null) {
    ...
    private <T> T doOp(ProviderCallable<T> op, int currPos, boolean isIdempotent) throws IOException {
        ...
        for (int i = 0; ; i++, numFailovers++) {
            ...
            try {
                ...
                if (action.action == RetryAction.RetryDecision.FAIL && numFailovers >= providers.length - 1) {
    ...
    public void warmUpEncryptedKeys(String... keyNames) throws IOException {
        Preconditions.checkArgument(providers.length > 0, "No providers are configured");
        ...
        for (KMSClientProvider provider : providers) {
            try {
                ...
                e = ioe;
                LOG.error("Error warming up keys for provider with url" + "[" + provider.getKMSUrl() + "]", ioe);
            }
        }
        if (!success && e != null) {
            throw e;
    ...
    public EncryptedKeyVersion generateEncryptedKey(final String encryptionKeyName) throws IOException, GeneralSecurityException {
        try {
            return doOp(new ProviderCallable<EncryptedKeyVersion>() {
                ...
                @Override
                public EncryptedKeyVersion call(KMSClientProvider provider) throws IOException, GeneralSecurityException {
