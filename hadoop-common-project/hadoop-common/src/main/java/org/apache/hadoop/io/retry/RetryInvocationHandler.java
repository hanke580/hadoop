...
public class RetryInvocationHandler<T> implements RpcInvocationHandler {
    ...
    static class Call {
        ...
        synchronized void processRetryInfo() {
            ...
            if (retryInfo.isFailover()) {
                retryInvocationHandler.proxyDescriptor.failover(retryInfo.expectedFailoverCount, method, callId);
                counters.failovers++;
            }
            retryInfo = null;
        }
        ...
        Object invokeMethod() throws Throwable {
            if (isRpc) {
                Client.setCallIdAndRetryCount(callId, counters.retries, retryInvocationHandler.asyncCallHandler);
            }
        ...
        public String toString() {
            return getClass().getSimpleName() + "#" + callId + ": " + method.getDeclaringClass().getSimpleName() + "." + method.getName() + "(" + (args == null || args.length == 0 ? "" : Arrays.toString(args)) + ")";
        }
    }
    ...
    private static class RetryInfo {
        ...
        boolean isFailover() {
            return action != null && action.action == RetryAction.RetryDecision.FAILOVER_AND_RETRY;
        }

        boolean isFail() {
            return action != null && action.action == RetryAction.RetryDecision.FAIL;
        }
        ...
        @Override
        public String toString() {
            return "RetryInfo{" + "retryTime=" + retryTime + ", delay=" + delay + ", action=" + action + ", expectedFailoverCount=" + expectedFailoverCount + ", failException=" + failException + '}';
        }
    }
    ...
    private void log(final Method method, final boolean isFailover, final int failovers, final long delay, final Exception ex) {
        ...
        final StringBuilder b = new StringBuilder().append(ex + ", while invoking ").append(proxyDescriptor.getProxyInfo().getString(method.getName()));
        if (failovers > 0) {
