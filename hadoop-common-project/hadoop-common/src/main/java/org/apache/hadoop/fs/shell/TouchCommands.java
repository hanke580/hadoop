...
public class TouchCommands extends FsCommand {
    ...
    public static class Touch extends TouchCommands {
        ...
        public static final String USAGE = "[-" + OPTION_CHANGE_ONLY_ACCESS_TIME + "] [-" + OPTION_CHANGE_ONLY_MODIFICATION_TIME + "] [-" + OPTION_USE_TIMESTAMP + " TIMESTAMP ] [-" + OPTION_DO_NOT_CREATE_FILE + "] <path> ...";
        ...
        @Override
        protected void processOptions(LinkedList<String> args) {
            this.timestamp = StringUtils.popOptionWithArgument("-" + OPTION_USE_TIMESTAMP, args);
            CommandFormat cf = new CommandFormat(1, Integer.MAX_VALUE, OPTION_USE_TIMESTAMP, OPTION_CHANGE_ONLY_ACCESS_TIME, OPTION_CHANGE_ONLY_MODIFICATION_TIME);
            cf.parse(args);
        ...
        private void updateTime(PathData item) throws IOException {
            ...
            if (timestamp != null) {
                try {
                    ...
                    throw new IllegalArgumentException("Unable to parse the specified timestamp " + timestamp, e);
                }
            }
