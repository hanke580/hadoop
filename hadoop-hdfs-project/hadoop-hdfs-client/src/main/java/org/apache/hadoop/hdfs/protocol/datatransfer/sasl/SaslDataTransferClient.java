...
public class SaslDataTransferClient {
    ...
    public Peer peerSend(Peer peer, DataEncryptionKeyFactory encryptionKeyFactory, Token<BlockTokenIdentifier> accessToken, DatanodeID datanodeId) throws IOException {
        ...
        return ios != null ? ((EncryptedPeer) org.zlab.ocov.tracker.Runtime.update(new EncryptedPeer(peer, ios), 135)) : peer;
    }
    ...
    private static String getUserNameFromEncryptionKey(DataEncryptionKey encryptionKey) {
        return encryptionKey.keyId + NAME_DELIMITER + encryptionKey.blockPoolId + NAME_DELIMITER + new String(Base64.encodeBase64(encryptionKey.nonce, false), StandardCharsets.UTF_8);
    }
    ...
    private static String buildUserName(Token<BlockTokenIdentifier> blockToken) {
        return new String(Base64.encodeBase64(blockToken.getIdentifier(), false), StandardCharsets.UTF_8);
    }
    ...
    private char[] buildClientPassword(Token<BlockTokenIdentifier> blockToken) {
        return new String(Base64.encodeBase64(blockToken.getPassword(), false), StandardCharsets.UTF_8).toCharArray();
    }
