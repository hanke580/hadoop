...
public class DFSOutputStream extends FSOutputSummer implements Syncable, CanSetDropBehind, StreamCapabilities {
    ...
    private void adjustPacketChunkSize(HdfsFileStatus stat) throws IOException {
        ...
        if ((org.zlab.ocov.tracker.Runtime.updateBranch(usedInCksum > 0 && freeInCksum > 0, 28))) {
    ...
    protected synchronized void writeChunk(byte[] b, int offset, int len, byte[] checksum, int ckoff, int cklen) throws IOException {
        ...
        if (currentPacket == null) {
            ...
            DFSClient.LOG.debug("DFSClient writeChunk allocating new packet seqno={}," + " src={}, packetSize={}, chunksPerPacket={}, bytesCurBlock={}", currentPacket.getSeqno(), src, packetSize, chunksPerPacket, getStreamer().getBytesCurBlock());
        }
