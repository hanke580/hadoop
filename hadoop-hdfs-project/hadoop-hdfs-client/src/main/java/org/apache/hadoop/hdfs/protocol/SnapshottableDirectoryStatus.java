...
public class SnapshottableDirectoryStatus {
    ...
    public static final Comparator<SnapshottableDirectoryStatus> COMPARATOR = new Comparator<SnapshottableDirectoryStatus>() {
        ...
        public int compare(SnapshottableDirectoryStatus left, SnapshottableDirectoryStatus right) {
            ...
            return d != 0 ? d : DFSUtilClient.compareBytes(left.dirStatus.getLocalNameInBytes(), right.dirStatus.getLocalNameInBytes());
        }
    };
    ...
    public SnapshottableDirectoryStatus(long modification_time, long access_time, FsPermission permission, EnumSet<HdfsFileStatus.Flags> flags, String owner, String group, byte[] localName, long inodeId, int childrenNum, int snapshotNumber, int snapshotQuota, byte[] parentFullPath) {
        this.dirStatus = new HdfsFileStatus.Builder().isdir(true).mtime(modification_time).atime(access_time).perm(permission).flags(flags).owner(owner).group(group).path(localName).fileId(inodeId).children(childrenNum).build();
