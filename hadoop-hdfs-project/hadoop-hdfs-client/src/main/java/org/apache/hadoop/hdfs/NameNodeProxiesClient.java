...
public class NameNodeProxiesClient {
     ...
     */
    public static HAServiceProtocol createNonHAProxyWithHAServiceProtocol(InetSocketAddress address, Configuration conf) throws IOException {
        RetryPolicy timeoutPolicy = RetryPolicies.exponentialBackoffRetry(MAX_RETRIES, DELAY_MILLISECONDS, TimeUnit.MILLISECONDS);
        HAServiceProtocol proxy = new HAServiceProtocolClientSideTranslatorPB(address, conf, NetUtils.getDefaultSocketFactory(conf), Client.getRpcTimeout(conf));
        return (HAServiceProtocol) RetryProxy.create(HAServiceProtocol.class, new DefaultFailoverProxyProvider<>(HAServiceProtocol.class, proxy), timeoutPolicy);
    }
    ...
    public static ClientProtocol createProxyWithAlignmentContext(InetSocketAddress address, Configuration conf, UserGroupInformation ugi, boolean withRetries, AtomicBoolean fallbackToSimpleAuth, AlignmentContext alignmentContext) throws IOException {
        ...
        if (withRetries) {
            ...
            return ((ClientNamenodeProtocolTranslatorPB) org.zlab.ocov.tracker.Runtime.update(new ClientNamenodeProtocolTranslatorPB(proxy), 74, address, conf, ugi, withRetries, fallbackToSimpleAuth, alignmentContext));
        }
    }
}
