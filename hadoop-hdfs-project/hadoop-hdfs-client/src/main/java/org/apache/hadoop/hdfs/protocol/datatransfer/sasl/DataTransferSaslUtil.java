...
public final class DataTransferSaslUtil {
    ...
    public static char[] encryptionKeyToPassword(byte[] encryptionKey) {
        return new String(Base64.encodeBase64(encryptionKey, false), StandardCharsets.UTF_8).toCharArray();
    }
    ...
    public static SaslPropertiesResolver getSaslPropertiesResolver(Configuration conf) {
        ...
        Configuration saslPropsResolverConf = ((Configuration) org.zlab.ocov.tracker.Runtime.update(new Configuration(conf), 205));
    ...
    private static <T> T readSaslMessage(InputStream in, Function<DataTransferEncryptorMessageProto, ? extends T> handler) throws IOException {
    ...
    public static IOStreamPair createStreamPair(Configuration conf, CipherOption cipherOption, OutputStream out, InputStream in, boolean isServer) throws IOException {
        ...
        OutputStream cOut = ((CryptoOutputStream) org.zlab.ocov.tracker.Runtime.update(new CryptoOutputStream(out, codec, isServer ? outKey : inKey, isServer ? outIv : inIv), 206));
    ...
    public static void sendSaslMessage(OutputStream out, DataTransferEncryptorStatus status, byte[] payload, String message, HandshakeSecretProto handshakeSecret) throws IOException {
        sendSaslMessage(out, status, payload, message, handshakeSecret, false);
    }

    public static void sendSaslMessage(OutputStream out, DataTransferEncryptorStatus status, byte[] payload, String message, HandshakeSecretProto handshakeSecret, boolean accessTokenError) throws IOException {
        DataTransferEncryptorMessageProto.Builder builder = DataTransferEncryptorMessageProto.newBuilder();
    ...
    public static void sendSaslMessageHandshakeSecret(OutputStream out, DataTransferEncryptorStatus status, byte[] payload, String message, byte[] secret, String bpid) throws IOException {
