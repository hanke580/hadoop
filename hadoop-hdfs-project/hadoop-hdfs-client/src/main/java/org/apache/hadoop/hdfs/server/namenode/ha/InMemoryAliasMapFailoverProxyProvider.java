...
public class InMemoryAliasMapFailoverProxyProvider<T> extends ConfiguredFailoverProxyProvider<T> {
    ...
    public InMemoryAliasMapFailoverProxyProvider(Configuration conf, URI uri, Class<T> xface, HAProxyFactory<T> factory) {
        super(conf, uri, xface, factory, DFS_PROVIDED_ALIASMAP_INMEMORY_RPC_ADDRESS);
    }
}
