 ...
 */
public class WebHdfsFileSystem extends FileSystem implements DelegationTokenRenewer.Renewable, TokenAspect.TokenManagementDelegator, KeyProviderTokenIssuer {
    ...
    public static boolean isEnabled(final Configuration conf) {
        final boolean b = conf.getBoolean(HdfsClientConfigKeys.DFS_WEBHDFS_ENABLED_KEY, HdfsClientConfigKeys.DFS_WEBHDFS_ENABLED_DEFAULT);
        return b;
    }
    ...
    public int getDefaultPort() {
        return getConf().getInt(HdfsClientConfigKeys.DFS_NAMENODE_HTTP_PORT_KEY, HdfsClientConfigKeys.DFS_NAMENODE_HTTP_PORT_DEFAULT);
    }
    ...
    abstract class AbstractRunner<T> {
        ...
        protected AbstractRunner(final HttpOpParam.Op op, boolean redirected, boolean followRedirect) {
            this(op, redirected);
        ...
        protected HttpURLConnection connect(URL url) throws IOException {
            ...
            redirectHost = null;
            if (url.getProtocol().equals("http") && UserGroupInformation.isSecurityEnabled() && isTLSKrb) {
        ...
        private HttpURLConnection connect(final HttpOpParam.Op op, final URL url) throws IOException {
            ...
            if (restCsrfCustomHeader != null && !restCsrfMethodsToIgnore.contains(op.getType().name())) {
            ...
            if (checkRetry) {
                try {
                    ...
                    if (isRetry || isFailoverAndRetry) {
                        LOG.info("Retrying connect to namenode: {}. Already retried {}" + " time(s); retry policy is {}, delay {}ms.", nnAddr, retry, retryPolicy, a.delayMillis);
            ...
            return new FSDataOutputStream(new BufferedOutputStream(conn.getOutputStream(), bufferSize), statistics) {
                ...
                public void write(int b) throws IOException {
                    try {
                        ...
                        LOG.warn("Write to output stream for file '{}' failed. " + "Attempting to fetch the cause from the connection.", getFspath(), e);
    ...
    class URLRunner extends AbstractRunner<HttpURLConnection> {
        ...
        protected URLRunner(final HttpOpParam.Op op, final URL url, boolean redirected, boolean followRedirect) {
    ...
    public FSDataOutputStream createNonRecursive(final Path f, final FsPermission permission, final EnumSet<CreateFlag> flag, final int bufferSize, final short replication, final long blockSize, final Progressable progress) throws IOException {
        ...
        final HttpOpParam.Op op = PutOpParam.Op.CREATE;
        return new FsPathOutputStreamRunner(op, f, bufferSize, new PermissionParam(applyUMask(permission)), new CreateFlagParam(flag), new CreateParentParam(false), new BufferSizeParam(bufferSize), new ReplicationParam(replication), new BlockSizeParam(blockSize)).run();
    }
    ...
    class OffsetUrlOpener extends ByteRangeInputStream.URLOpener {
        ...
        protected HttpURLConnection connect(final long offset, final boolean resolved) throws IOException {
            ...
            return new URLRunner(GetOpParam.Op.OPEN, offsetUrl, resolved, true).run();
        }
    }
    ...
    public Token<?>[] addDelegationTokens(String renewer, Credentials credentials) throws IOException {
        ...
        return HdfsKMSUtil.addDelegationTokensForKeyProvider(this, renewer, credentials, getUri(), tokens);
    }
   ...
   */
    ...
    public class WebHdfsInputStream extends FSInputStream {
        ...
        InputStream createWrappedInputStream() throws IOException {
            return HdfsKMSUtil.createWrappedInputStream(this, getKeyProvider(), getFileEncryptionInfo(), getConf());
        }
    }

    enum RunnerState {

        // Connection is closed programmatically by ReadRunner
        DISCONNECTED,
        // Connection has been established by ReadRunner
        OPEN,
        // Calling code has explicitly called seek()
        SEEK,
        // Calling code has explicitly called close()
        CLOSED
    }
    ...
    protected class ReadRunner extends AbstractFsPathRunner<Integer> {
        ...
        private void getRedirectedUrl() throws IOException {
            URLRunner urlRunner = new URLRunner(GetOpParam.Op.OPEN, null, false, false) {
            ...
            if (feInfoStr != null) {
                byte[] decodedBytes = Base64.decodeBase64(feInfoStr.getBytes(StandardCharsets.UTF_8));
                feInfo = PBHelperClient.convert(FileEncryptionInfoProto.parseFrom(decodedBytes));
            }
        ...
        int read(byte[] b, int off, int len) throws IOException {
            ...
            if (runnerState == RunnerState.SEEK) {
                try {
                    ...
                    cachedConnection = new URLRunner(GetOpParam.Op.OPEN, rurl, true, false).run();
                } catch (IOException ioe) {
     ...
     */
        ...
        protected URL getUrl() throws IOException {
            ...
            if (cachedConnection == null) {
                ...
                updateURLParameters(new BufferSizeParam(bufferSize), new OffsetParam(pos));
                originalUrl = super.getUrl();
            }
            return originalUrl;
        }
