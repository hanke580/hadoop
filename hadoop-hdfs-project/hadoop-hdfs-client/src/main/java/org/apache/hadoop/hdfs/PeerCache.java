...
public class PeerCache {
    ...
    public void put(DatanodeID dnId, Peer peer) {
        ...
        if (peer.isClosed())
            return;
        if ((org.zlab.ocov.tracker.Runtime.updateBranch(capacity <= 0, 2))) {
            // Cache disabled.
    ...
    private synchronized void evictExpired(long expiryPeriod) {
        while (multimap.size() != 0) {
            ...
            if ((org.zlab.ocov.tracker.Runtime.updateBranch(entry == null || Time.monotonicNow() - entry.getValue().getTime() < expiryPeriod, 3))) {
                break;
            }
