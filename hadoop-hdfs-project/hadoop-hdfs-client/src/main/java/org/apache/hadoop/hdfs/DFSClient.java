...
public class DFSClient implements java.io.Closeable, RemotePeerFactory, DataEncryptionKeyFactory, KeyProviderTokenIssuer {
     ...
     * start.
     */
    private static volatile boolean disabledStopDeadNodeDetectorThreadForTest = false;
    ...
    public DFSClient(URI nameNodeUri, ClientProtocol rpcNamenode, Configuration conf, FileSystem.Statistics stats) throws IOException {
        ...
        this.dtpReplaceDatanodeOnFailureReplication = (short) conf.getInt(HdfsClientConfigKeys.BlockWrite.ReplaceDatanodeOnFailure.MIN_REPLICATION, HdfsClientConfigKeys.BlockWrite.ReplaceDatanodeOnFailure.MIN_REPLICATION_DEFAULT);
        LOG.debug("Sets {} to {}", HdfsClientConfigKeys.BlockWrite.ReplaceDatanodeOnFailure.MIN_REPLICATION, dtpReplaceDatanodeOnFailureReplication);
        ...
        int numResponseToDrop = conf.getInt(DFS_CLIENT_TEST_DROP_NAMENODE_RESPONSE_NUM_KEY, DFS_CLIENT_TEST_DROP_NAMENODE_RESPONSE_NUM_DEFAULT);
        ProxyAndInfo<ClientProtocol> proxyInfo = null;
        AtomicBoolean nnFallbackToSimpleAuth = new AtomicBoolean(false);
        if (numResponseToDrop > 0) {
            // This case is used for testing.
            LOG.warn("{} is set to {} , this hacked client will proactively drop responses", DFS_CLIENT_TEST_DROP_NAMENODE_RESPONSE_NUM_KEY, numResponseToDrop);
        ...
        String[] localInterfaces = conf.getTrimmedStrings(DFS_CLIENT_LOCAL_INTERFACES);
        localInterfaceAddrs = getLocalInterfaceAddrs(localInterfaces);
        if (LOG.isDebugEnabled() && 0 != localInterfaces.length) {
            LOG.debug("Using local interfaces [{}] with addresses [{}]", Joiner.on(',').join(localInterfaces), Joiner.on(',').join(localInterfaceAddrs));
        }
        ...
        Long readahead = (conf.get(DFS_CLIENT_CACHE_READAHEAD) == null) ? null : conf.getLongBytes(DFS_CLIENT_CACHE_READAHEAD, 0);
        this.serverDefaultsValidityPeriod = conf.getTimeDuration(DFS_CLIENT_SERVER_DEFAULTS_VALIDITY_PERIOD_MS_KEY, DFS_CLIENT_SERVER_DEFAULTS_VALIDITY_PERIOD_MS_DEFAULT, TimeUnit.MILLISECONDS);
        ...
        this.clientContext = ClientContext.get(conf.get(DFS_CLIENT_CONTEXT, DFS_CLIENT_CONTEXT_DEFAULT), dfsClientConf, conf);
        if (dfsClientConf.getHedgedReadThreadpoolSize() > 0) {
            this.initThreadsNumForHedgedReads(dfsClientConf.getHedgedReadThreadpoolSize());
        }
        this.initThreadsNumForStripedReads(dfsClientConf.getStripedReadThreadpoolSize());
     ...
     */
    public void putFileBeingWritten(final String key, final DFSOutputStream out) {
        synchronized (filesBeingWritten) {
        ...
        if (clientRunning && !isFilesBeingWrittenEmpty()) {
            try {
                ...
                if (elapsed > dfsClientConf.getleaseHardLimitPeriod()) {
                    LOG.warn("Failed to renew lease for {} for {} seconds (>= hard-limit ={} seconds.) " + "Closing all files being written ...", clientName, (elapsed / 1000), (dfsClientConf.getleaseHardLimitPeriod() / 1000), e);
        ...
        for (; ; ) {
            ...
            if (out != null) {
                try {
                    ...
                    LOG.error("Failed to {} file: {} with renewLeaseKey: {}", (abort ? "abort" : "close"), out.getSrc(), key, ie);
                }
            }
        }
    }
    ...
    public static class Renewer extends TokenRenewer {
        ...
        private static ClientProtocol getNNProxy(Token<DelegationTokenIdentifier> token, Configuration conf) throws IOException {
            ...
            ProxyAndInfo<ClientProtocol> info = NameNodeProxiesClient.createProxyWithClientProtocol(conf, uri, null);
    ...
    public DFSInputStream open(HdfsPathHandle fd, int buffersize, boolean verifyChecksum) throws IOException {
        ...
        try (TraceScope ignored = newPathTraceScope("newDFSInputStream", src)) {
            ...
            // check invariants in path handle
            fd.verify(s);
     ...
     * parent groups' whatever policy.
     */
    public DFSOutputStream create(String src, FsPermission permission, EnumSet<CreateFlag> flag, boolean createParent, short replication, long blockSize, Progressable progress, int buffersize, ChecksumOpt checksumOpt, InetSocketAddress[] favoredNodes, String ecPolicyName, String storagePolicy) throws IOException {
        checkOpen();
    ...
    private FileChecksum getFileChecksumInternal(String src, long length, ChecksumCombineMode combineMode) throws IOException {
        checkOpen();
        ...
        maker.compute();
        org.zlab.ocov.tracker.Runtime.update(maker, 208);
    ...
    protected Type inferChecksumTypeByReading(LocatedBlock lb, DatanodeInfo dn) throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(pair, 209);
        try {
    ...
    void reportChecksumFailure(String file, LocatedBlock[] lblocks) {
        try {
            ...
            LOG.info("Found corruption while reading {}" + ". Error repairing corrupt blocks. Bad blocks remain.", file, ie);
        }
    }
     ...
     * @throws IOException
     */
    public RemoteIterator<OpenFileEntry> listOpenFiles(EnumSet<OpenFilesType> openFilesTypes) throws IOException {
        checkOpen();
