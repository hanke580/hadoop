...
public class DFSClient implements java.io.Closeable, RemotePeerFactory, DataEncryptionKeyFactory, KeyProviderTokenIssuer {
    ...
    public DFSClient(URI nameNodeUri, ClientProtocol rpcNamenode, Configuration conf, FileSystem.Statistics stats) throws IOException {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("Sets " + HdfsClientConfigKeys.BlockWrite.ReplaceDatanodeOnFailure.MIN_REPLICATION + " to " + dtpReplaceDatanodeOnFailureReplication);
        }
        ...
        if (numResponseToDrop > 0) {
            ...
            LOG.warn(DFS_CLIENT_TEST_DROP_NAMENODE_RESPONSE_NUM_KEY + " is set to " + numResponseToDrop + ", this hacked client will proactively drop responses");
        ...
        this.serverDefaultsValidityPeriod = conf.getLong(DFS_CLIENT_SERVER_DEFAULTS_VALIDITY_PERIOD_MS_KEY, DFS_CLIENT_SERVER_DEFAULTS_VALIDITY_PERIOD_MS_DEFAULT);
   ...
   */
    ...
    public DFSOutputStream create(String src, FsPermission permission, EnumSet<CreateFlag> flag, boolean createParent, short replication, long blockSize, Progressable progress, int buffersize, ChecksumOpt checksumOpt, InetSocketAddress[] favoredNodes, String ecPolicyName) throws IOException {
        ...
        final DFSOutputStream result = DFSOutputStream.newStreamForCreate(this, src, masked, flag, createParent, replication, blockSize, progress, dfsClientConf.createChecksum(checksumOpt), getFavoredNodesStr(favoredNodes), ecPolicyName);
    ...
    public DFSOutputStream primitiveCreate(String src, FsPermission absPermission, EnumSet<CreateFlag> flag, boolean createParent, short replication, long blockSize, Progressable progress, int buffersize, ChecksumOpt checksumOpt) throws IOException {
        ...
        if (result == null) {
            ...
            result = DFSOutputStream.newStreamForCreate(this, src, absPermission, flag, createParent, replication, blockSize, progress, checksum, null, null);
        }
        beginFileLease(result.getFileId(), result);
        return result;
    }
    ...
    public boolean rename(String src, String dst) throws IOException {
        ...
        try (TraceScope ignored = newSrcDstTraceScope("rename", src, dst)) {
            ...
            throw re.unwrapRemoteException(AccessControlException.class, NSQuotaExceededException.class, DSQuotaExceededException.class, QuotaByStorageTypeExceededException.class, UnresolvedPathException.class, SnapshotAccessControlException.class);
        }
    }
    ...
    public boolean truncate(String src, long newLength) throws IOException {
        checkOpen();
        if ((org.zlab.ocov.tracker.Runtime.updateBranch(newLength, 0, "<", 43))) {
    ...
    private FileChecksum getFileChecksumInternal(String src, long length, ChecksumCombineMode combineMode) throws IOException {
        ...
        maker.compute();
        org.zlab.ocov.tracker.Runtime.update(maker, 151);
    ...
    protected Type inferChecksumTypeByReading(LocatedBlock lb, DatanodeInfo dn) throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(pair, 152);
        try {
