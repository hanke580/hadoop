...
public class StripedBlockUtil {
    ...
    public static class BlockReadStats {
        ...
        public String toString() {
            ...
            sb.append("bytesRead=").append(bytesRead).append(',').append("isShortCircuit=").append(isShortCircuit).append(',').append("networkDistance=").append(networkDistance);
            return sb.toString();
        }
    }
    ...
    public static LocatedBlock constructInternalBlock(LocatedStripedBlock bg, int idxInReturnedLocs, int cellSize, int dataBlkNum, int idxInBlockGroup) {
        ...
        try {
            left_71 = (idxInReturnedLocs);
            right_71 = (bg.getLocations().length);
        } catch (Exception e) {
            left_71 = 0L;
            right_71 = 0L;
        }
    ...
    public static long offsetInBlkToOffsetInBG(int cellSize, int dataBlkNum, long offsetInBlk, int idxInBlockGroup) {
        ...
        return // n full stripes before offset
        cellIdxInBlk * cellSize * dataBlkNum + // m full cells before offset
        idxInBlockGroup * cellSize + // partial cell
        offsetInBlk % cellSize;
    }
     ...
     */
    public static StripingChunkReadResult getNextCompletedStripedRead(CompletionService<BlockReadStats> readService, Map<Future<BlockReadStats>, Integer> futures, final long timeoutMillis) throws InterruptedException {
     ...
     */
    private static AlignedStripe[] mergeRangesForInternalBlocks(ErasureCodingPolicy ecPolicy, VerticalRange[] ranges, LocatedStripedBlock blockGroup, int cellSize) {
        int dataBlkNum = ecPolicy.getNumDataUnits();
        int parityBlkNum = ecPolicy.getNumParityUnits();
   ...
   */
    ...
    public static class StripingCell {
        ...
        StripingCell(ErasureCodingPolicy ecPolicy, int cellSize, long idxInBlkGroup, long offset) {
            ...
            this.idxInStripe = (int) (idxInBlkGroup - this.idxInInternalBlk * ecPolicy.getNumDataUnits());
            this.offset = offset;
            this.size = cellSize;
        }
        ...
        @Override
        public String toString() {
            return String.format("StripingCell(idxInBlkGroup=%d, " + "idxInInternalBlk=%d, idxInStrip=%d, offset=%d, size=%d)", idxInBlkGroup, idxInInternalBlk, idxInStripe, offset, size);
        }
    }
   ...
   */
    public static class AlignedStripe {
        ...
        public AlignedStripe(long offsetInBlock, long length, int width) {
            Preconditions.checkArgument(offsetInBlock >= 0 && length >= 0, "OffsetInBlock(%s) and length(%s) must be non-negative", offsetInBlock, length);
        ...
        @Override
        public String toString() {
            return "AlignedStripe(Offset=" + range.offsetInBlock + ", length=" + range.spanInBlock + ", fetchedChunksNum=" + fetchedChunksNum + ", missingChunksNum=" + missingChunksNum + ")";
        }
    }
   ...
   */
    ...
    public static class StripingChunkReadResult {
        ...
        public StripingChunkReadResult(int state) {
            Preconditions.checkArgument(state == TIMEOUT, "Only timeout result should return negative index.");
            this.index = -1;
            this.state = state;
            this.readStats = null;
        }
        ...
        public StripingChunkReadResult(int index, int state, BlockReadStats stats) {
            Preconditions.checkArgument(state != TIMEOUT, "Timeout result should return negative index.");
            this.index = index;
            this.state = state;
        ...
        @Override
        public String toString() {
            return "(index=" + index + ", state =" + state + ", readStats =" + readStats + ")";
        }
    }
    ...
    public static class StripeRange {
        ...
        public StripeRange(long offsetInBlock, long length) {
            Preconditions.checkArgument(offsetInBlock >= 0 && length >= 0, "Offset(%s) and length(%s) must be non-negative", offsetInBlock, length);
            this.offsetInBlock = offsetInBlock;
            this.length = length;
        }
        ...
        @Override
        public String toString() {
            return String.format("StripeRange(offsetInBlock=%d, length=%d)", offsetInBlock, length);
        }
    }
    ...
    private static long right_71;

    private static long left_71;
}
