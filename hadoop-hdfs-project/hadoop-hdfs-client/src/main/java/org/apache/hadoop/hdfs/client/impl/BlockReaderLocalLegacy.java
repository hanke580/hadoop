...
class BlockReaderLocalLegacy implements BlockReader {
                ...
                try {
                    proxy = ugi.doAs(new PrivilegedExceptionAction<ClientDatanodeProtocol>() {
                        ...
                        public ClientDatanodeProtocol run() throws Exception {
                            return DFSUtilClient.createClientDatanodeProtocolProxy(node, conf, socketTimeout, connectToDnViaHostname);
                        }
                    });
     ...
     */
    static BlockReaderLocalLegacy newBlockReader(DfsClientConf conf, UserGroupInformation userGroupInformation, Configuration configuration, String file, ExtendedBlock blk, Token<BlockTokenIdentifier> token, DatanodeInfo node, long startOffset, long length, StorageType storageType) throws IOException {
    ...
    private BlockReaderLocalLegacy(ShortCircuitConf conf, String hdfsfile, ExtendedBlock block, long startOffset, FileInputStream dataIn) throws IOException {
        this(conf, hdfsfile, block, startOffset, DataChecksum.newDataChecksum(DataChecksum.Type.NULL, 4), false, dataIn, startOffset, null);
    }

    private BlockReaderLocalLegacy(ShortCircuitConf conf, String hdfsfile, ExtendedBlock block, long startOffset, DataChecksum checksum, boolean verifyChecksum, FileInputStream dataIn, long firstChunkOffset, FileInputStream checksumIn) throws IOException {
    ...
    private synchronized int doByteBufferRead(ByteBuffer buf) throws IOException {
        ...
        try {
            this.left_87 = (dataRead);
            this.right_87 = (offsetFromChunkBoundary);
        } catch (Exception e) {
            this.left_87 = 0L;
            this.right_87 = 0L;
        }
    ...
    private long right_87;

    private long right_86;

    private long left_87;

    private long left_86;
}
