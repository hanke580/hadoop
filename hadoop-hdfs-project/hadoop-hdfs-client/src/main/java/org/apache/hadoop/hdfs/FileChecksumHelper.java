...
final class FileChecksumHelper {
    ...
    static abstract class FileChecksumComputer {
         ...
         */
        FileChecksumComputer(String src, long length, LocatedBlocks blockLocations, ClientProtocol namenode, DFSClient client, ChecksumCombineMode combineMode) throws IOException {
            this.src = src;
            this.length = length;
        ...
        FileChecksum makeCompositeCrcResult() throws IOException {
            ...
            try {
                this.left_68 = (locatedBlocks.size());
                this.right_68 = (0);
            } catch (Exception e) {
                this.left_68 = 0L;
                this.right_68 = 0L;
            ...
            CrcComposer crcComposer = CrcComposer.newCrcComposer(getCrcType(), blockSizeHint);
            ...
            for (int i = 0; i < locatedBlocks.size() - 1; ++i) {
                ...
                LOG.debug("Added blockCrc 0x{} for block index {} of size {}", Integer.toString(blockCrc, 16), i, block.getBlockSize());
            }
            ...
            long reportedLastBlockSize = blockLocations.getLastLocatedBlock().getBlockSize();
            long consumedLastBlockLength = reportedLastBlockSize;
            try {
                this.left_69 = (length - sumBlockLengths);
                this.right_69 = (reportedLastBlockSize);
            } catch (Exception e) {
                this.left_69 = 0L;
                this.right_69 = 0L;
            }
            if (length - sumBlockLengths < reportedLastBlockSize) {
                LOG.warn("Last block length {} is less than reportedLastBlockSize {}", length - sumBlockLengths, reportedLastBlockSize);
            ...
            int lastBlockCrc = CrcUtil.readInt(blockChecksumBytes, 4 * (locatedBlocks.size() - 1));
            crcComposer.update(lastBlockCrc, consumedLastBlockLength);
            LOG.debug("Added lastBlockCrc 0x{} for block index {} of size {}", Integer.toString(lastBlockCrc, 16), locatedBlocks.size() - 1, consumedLastBlockLength);
            int compositeCrc = CrcUtil.readInt(crcComposer.digest(), 0);
            return new CompositeCrcFileChecksum(compositeCrc, getCrcType(), bytesPerCRC);
        }
         ...
         */
        void extractChecksumProperties(OpBlockChecksumResponseProto checksumData, LocatedBlock locatedBlock, DatanodeInfo datanode, int blockIdx) throws IOException {
            ...
            if (blockIdx == 0) {
                ...
                if (getBlockChecksumType() == BlockChecksumType.COMPOSITE_CRC) {
                    LOG.warn("Current bytesPerCRC={} doesn't match next bpc={}, but " + "continuing anyway because we're using COMPOSITE_CRC. " + "If trying to preserve CHECKSUMTYPE, only the current " + "bytesPerCRC will be preserved.", getBytesPerCRC(), bpc);
                } else {
            ...
            try {
                this.left_70 = (getLocatedBlocks().size());
                this.right_70 = (1);
            } catch (Exception e) {
                this.left_70 = 0L;
                this.right_70 = 0L;
         ...
         */
        String populateBlockChecksumBuf(OpBlockChecksumResponseProto checksumData) throws IOException {
            ...
            switch(getBlockChecksumType()) {
                ...
                case COMPOSITE_CRC:
                    ...
                    if (returnedType != BlockChecksumType.COMPOSITE_CRC) {
                        throw new IOException(String.format("Unexpected blockChecksumType '%s', expecting COMPOSITE_CRC", returnedType));
                    }
        ...
        private long right_69;

        private long right_68;

        private long left_69;

        private long left_68;

        private long right_70;

        private long left_70;
    }
    ...
    static class ReplicatedFileChecksumComputer extends FileChecksumComputer {
        ...
        ReplicatedFileChecksumComputer(String src, long length, LocatedBlocks blockLocations, ClientProtocol namenode, DFSClient client, ChecksumCombineMode combineMode) throws IOException {
        ...
        private boolean checksumBlock(LocatedBlock locatedBlock) {
            ...
            int tmpTimeout = 3000 * datanodes.length + getClient().getConf().getSocketTimeout();
            ...
            for (int j = 0; !done && j < datanodes.length; j++) {
                try {
                    ...
                    if (blockIdx > getLastRetriedIndex()) {
                        LOG.debug("Got invalid encryption key error in response to " + "OP_BLOCK_CHECKSUM for file {} for block {} from " + "datanode {}. Will retry " + "the block once.", getSrc(), block, datanodes[j]);
                        setLastRetriedIndex(blockIdx);
                        // actually it's not done; but we'll retry
                        done = true;
        ...
        private void tryDatanode(LocatedBlock locatedBlock, DatanodeInfo datanode) throws IOException {
            ...
            try (IOStreamPair pair = getClient().connectToDN(datanode, getTimeout(), locatedBlock.getBlockToken())) {
                ...
                // get block checksum
                createSender(pair).blockChecksum(block, locatedBlock.getBlockToken(), new BlockChecksumOptions(getBlockChecksumType()));
                ...
                extractChecksumProperties(checksumData, locatedBlock, datanode, blockIdx);
                String blockChecksumForDebug = populateBlockChecksumBuf(checksumData);
                LOG.debug("got reply from {}: blockChecksum={}, blockChecksumType={}", datanode, blockChecksumForDebug, getBlockChecksumType());
            }
        }
    }
     ...
     */
    static class StripedFileNonStripedChecksumComputer extends FileChecksumComputer {

        private final ErasureCodingPolicy ecPolicy;

        private int bgIdx;

        StripedFileNonStripedChecksumComputer(String src, long length, LocatedBlocks blockLocations, ClientProtocol namenode, DFSClient client, ErasureCodingPolicy ecPolicy, ChecksumCombineMode combineMode) throws IOException {
            super(src, length, blockLocations, namenode, client, combineMode);
        ...
        private void tryDatanode(LocatedStripedBlock blockGroup, StripedBlockInfo stripedBlockInfo, DatanodeInfo datanode, long requestedNumBytes) throws IOException {
            try (IOStreamPair pair = getClient().connectToDN(datanode, getTimeout(), blockGroup.getBlockToken())) {
                ...
                createSender(pair).blockGroupChecksum(stripedBlockInfo, blockGroup.getBlockToken(), requestedNumBytes, new BlockChecksumOptions(getBlockChecksumType()));
