...
public class PBHelperClient {
     ...
     * methods instead.
     */
    private static ConcurrentHashMap<Object, ByteString> fixedByteStringCache = new ConcurrentHashMap<>();
     ...
     */
    private static LoadingCache<String, ByteString> bytestringCache = CacheBuilder.newBuilder().maximumSize(10000).build(new CacheLoader<String, ByteString>() {
    ...
    public static HdfsProtos.BlockChecksumTypeProto convert(BlockChecksumType type) {
        switch(type) {
            case MD5CRC:
    ...
    public static HdfsProtos.BlockChecksumOptionsProto convert(BlockChecksumOptions options) {
    ...
    public static BlockChecksumOptions convert(HdfsProtos.BlockChecksumOptionsProto options) {
    ...
    public static TokenProto convert(Token<?> tok) {
        TokenProto.Builder builder = TokenProto.newBuilder().setIdentifier(getByteString(tok.getIdentifier())).setPassword(getByteString(tok.getPassword())).setKindBytes(getFixedByteString(tok.getKind())).setServiceBytes(getFixedByteString(tok.getService()));
    ...
    static public Token<BlockTokenIdentifier>[] convertTokens(List<TokenProto> tokenProtos) {
    ...
    public static BlockTokenIdentifier.AccessMode convert(AccessModeProto accessModeProto) {
    ...
    public static Token<BlockTokenIdentifier> convert(TokenProto blockToken) {
        Token<BlockTokenIdentifier> token = new Token<>(blockToken.getIdentifier().toByteArray(), blockToken.getPassword().toByteArray(), new Text(blockToken.getKind()), new Text(blockToken.getService()));
        return token;
    }
    ...
    public static List<TokenProto> convert(Token<BlockTokenIdentifier>[] blockTokens) {
    ...
    public static List<AclEntry> convertAclEntry(List<AclEntryProto> aclSpec) {
        ...
        for (AclEntryProto e : aclSpec) {
            ...
            org.zlab.ocov.tracker.Runtime.update(builder, 155);
            if (e.hasName()) {
    ...
    public static OpenFilesBatchResponseProto convert(OpenFileEntry openFileEntry) {
    ...
    public static CacheDirectiveInfo convert(CacheDirectiveInfoProto proto) {
        CacheDirectiveInfo.Builder builder = ((CacheDirectiveInfo.Builder) org.zlab.ocov.tracker.Runtime.update(new CacheDirectiveInfo.Builder(), 156));
        if (proto.hasId()) {
    ...
    public static SnapshotDiffReportListing convert(SnapshotDiffReportListingProto reportProto) {
        if (reportProto == null) {
            return null;
        }
    ...
    public static DiffReportListingEntry convert(SnapshotDiffReportListingEntryProto entry) {
        if (entry == null) {
            return null;
        }
    ...
    private static EnumSet<HdfsFileStatus.Flags> convertFlags(FsPermissionProto pbp) {
        EnumSet<HdfsFileStatus.Flags> f = EnumSet.noneOf(HdfsFileStatus.Flags.class);
    ...
    public static QuotaUsage convert(QuotaUsageProto qu) {
        ...
        QuotaUsage.Builder builder = ((QuotaUsage.Builder) org.zlab.ocov.tracker.Runtime.update(new QuotaUsage.Builder(), 157));
    ...
    public static ReplicatedBlockStats convert(GetFsReplicatedBlockStatsResponseProto res) {
    ...
    public static ECBlockGroupStats convert(GetFsECBlockGroupStatsResponseProto res) {
        if (res.hasHighestPrioLowRedundancyBlocks()) {
    ...
    public static FsServerDefaults convert(FsServerDefaultsProto fs) {
        if (fs == null)
            return null;
        return new FsServerDefaults(fs.getBlockSize(), fs.getBytesPerChecksum(), fs.getWritePacketSize(), (short) fs.getReplication(), fs.getFileBufferSize(), fs.getEncryptDataTransfer(), fs.getTrashInterval(), convert(fs.getChecksumType()), fs.hasKeyProviderUri() ? fs.getKeyProviderUri() : null, (byte) fs.getPolicyId());
    }
    ...
    public static GetFsReplicatedBlockStatsResponseProto convert(ReplicatedBlockStats replicatedBlockStats) {
    ...
    public static GetFsECBlockGroupStatsResponseProto convert(ECBlockGroupStats ecBlockGroupStats) {
    ...
    public static SafeModeAction convert(SafeModeActionProto a) {
        switch(a) {
            case SAFEMODE_LEAVE:
    ...
    public static StorageReportProto convert(StorageReport r) {
        StorageReportProto.Builder builder = StorageReportProto.newBuilder().setBlockPoolUsed(r.getBlockPoolUsed()).setCapacity(r.getCapacity()).setDfsUsed(r.getDfsUsed()).setRemaining(r.getRemaining()).setStorageUuid(r.getStorage().getStorageID()).setStorage(convert(r.getStorage())).setNonDfsUsed(r.getNonDfsUsed());
        return builder.build();
    }
    ...
    public static SnapshotDiffReportListingEntryProto convert(DiffReportListingEntry entry) {
        if (entry == null) {
            return null;
        }
    ...
    public static SnapshotDiffReportListingProto convert(SnapshotDiffReportListing report) {
        if (report == null) {
            return null;
        }
    ...
    public static ZoneEncryptionInfoProto convert(CipherSuite suite, CryptoProtocolVersion version, String keyName) {
        return convert(suite, version, keyName, null);
    }

    public static ZoneEncryptionInfoProto convert(CipherSuite suite, CryptoProtocolVersion version, String keyName, ReencryptionInfoProto proto) {
    ...
    public static ReencryptionInfoProto convert(String ezkvn, Long submissionTime, boolean isCanceled, long numReencrypted, long numFailures, Long completionTime, String lastFile) {
    ...
    public static ZoneReencryptionStatus convert(ZoneReencryptionStatusProto proto) {
    ...
    public static ErasureCodingPolicyState convertECState(HdfsProtos.ErasureCodingPolicyState state) {
        return ErasureCodingPolicyState.fromValue(state.getNumber());
    }

    public static HdfsProtos.ErasureCodingPolicyState convertECState(ErasureCodingPolicyState state) {
     ...
     */
    public static ErasureCodingPolicy convertErasureCodingPolicy(ErasureCodingPolicyProto proto) {
     ...
     */
    public static ErasureCodingPolicyInfo convertErasureCodingPolicyInfo(ErasureCodingPolicyProto proto) {
    ...
    private static ErasureCodingPolicyProto.Builder createECPolicyProtoBuilder(ErasureCodingPolicy policy) {
     ...
     */
    public static ErasureCodingPolicyProto convertErasureCodingPolicy(ErasureCodingPolicy policy) {
     ...
     */
    public static ErasureCodingPolicyProto convertErasureCodingPolicy(ErasureCodingPolicyInfo info) {
    ...
    public static CodecProto convertErasureCodingCodec(String codec, String coders) {
        CodecProto.Builder builder = CodecProto.newBuilder().setCodec(codec).setCoders(coders);
        return builder.build();
    }

    public static AddErasureCodingPolicyResponseProto convertAddErasureCodingPolicyResponse(AddErasureCodingPolicyResponse response) {
    ...
    public static AddErasureCodingPolicyResponse convertAddErasureCodingPolicyResponse(AddErasureCodingPolicyResponseProto proto) {
    ...
    public static ECTopologyVerifierResult convertECTopologyVerifierResultProto(HdfsProtos.ECTopologyVerifierResultProto resp) {
        return new ECTopologyVerifierResult(resp.getIsSupported(), resp.getResultMessage());
    }

    public static HdfsProtos.ECTopologyVerifierResultProto convertECTopologyVerifierResult(ECTopologyVerifierResult resp) {
    ...
    public static EnumSet<AddBlockFlag> convertAddBlockFlags(List<AddBlockFlagProto> addBlockFlags) {
    ...
    public static List<AddBlockFlagProto> convertAddBlockFlags(EnumSet<AddBlockFlag> flags) {
    ...
    public static EnumSet<OpenFilesType> convertOpenFileTypes(List<OpenFilesTypeProto> openFilesTypeProtos) {
    ...
    public static List<OpenFilesTypeProto> convertOpenFileTypes(EnumSet<OpenFilesType> types) {
