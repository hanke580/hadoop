...
public class PBHelperClient {
    ...
    public static TokenProto convert(Token<?> tok) {
        return TokenProto.newBuilder().setIdentifier(getByteString(tok.getIdentifier())).setPassword(getByteString(tok.getPassword())).setKindBytes(getFixedByteString(tok.getKind())).setServiceBytes(getFixedByteString(tok.getService())).build();
    }
    ...
    public static LocatedBlocks convert(LocatedBlocksProto lb) {
        return new LocatedBlocks(lb.getFileLength(), lb.getUnderConstruction(), convertLocatedBlock(lb.getBlocksList()), lb.hasLastBlock() ? convert(lb.getLastBlock()) : null, lb.getIsLastBlockComplete(), lb.hasFileEncryptionInfo() ? convert(lb.getFileEncryptionInfo()) : null);
    }
    ...
    public static LocatedBlockProto convert(LocatedBlock b) {
        ...
        return builder.setB(convert(b.getBlock())).setBlockToken(convert(b.getBlockToken())).setCorrupt(b.isCorrupt()).setOffset(b.getStartOffset()).build();
    }
    ...
    public static List<AclEntry> convertAclEntry(List<AclEntryProto> aclSpec) {
        ...
        for (AclEntryProto e : aclSpec) {
            AclEntry.Builder builder = ((AclEntry.Builder) org.zlab.ocov.tracker.Runtime.update(new AclEntry.Builder(), 4));
    ...
    public static CacheDirectiveInfo convert(CacheDirectiveInfoProto proto) {
        CacheDirectiveInfo.Builder builder = ((CacheDirectiveInfo.Builder) org.zlab.ocov.tracker.Runtime.update(new CacheDirectiveInfo.Builder(), 5));
        if (proto.hasId()) {
        ...
        if (proto.hasExpiration()) {
            ...
            org.zlab.ocov.tracker.Runtime.update(builder, 6);
        }
        return builder.build();
    }
    ...
    public static QuotaUsage convert(QuotaUsageProto qu) {
        ...
        QuotaUsage.Builder builder = ((QuotaUsage.Builder) org.zlab.ocov.tracker.Runtime.update(new QuotaUsage.Builder(), 7));
    ...
    public static DirectoryListing convert(DirectoryListingProto dl) {
        ...
        return new DirectoryListing(partList.isEmpty() ? new HdfsLocatedFileStatus[0] : convert(partList.toArray(new HdfsFileStatusProto[partList.size()])), dl.getRemainingEntries());
    }
    ...
    public static LocatedBlocksProto convert(LocatedBlocks lb) {
        ...
        return builder.setFileLength(lb.getFileLength()).setUnderConstruction(lb.isUnderConstruction()).addAllBlocks(convertLocatedBlock2(lb.getLocatedBlocks())).setIsLastBlockComplete(lb.isLastBlockComplete()).build();
    }
