...
public class ObserverReadProxyProvider<T> extends AbstractNNFailoverProxyProvider<T> {
     ...
     */
    static final String AUTO_MSYNC_PERIOD_KEY_PREFIX = HdfsClientConfigKeys.Failover.PREFIX + "observer.auto-msync-period";
    ...
    public ObserverReadProxyProvider(Configuration conf, URI uri, Class<T> xface, HAProxyFactory<T> factory) {
        this(conf, uri, xface, factory, new ConfiguredFailoverProxyProvider<>(conf, uri, xface, factory));
    }
    ...
    public ObserverReadProxyProvider(Configuration conf, URI uri, Class<T> xface, HAProxyFactory<T> factory, AbstractNNFailoverProxyProvider<T> failoverProxy) {
        ...
        observerRetryPolicy = RetryPolicies.failoverOnNetworkException(RetryPolicies.TRY_ONCE_THEN_FAIL, 1);
    ...
    private class ObserverReadInvocationHandler implements RpcInvocationHandler {
            ...
            if (observerReadEnabled && shouldFindObserver() && isRead(method)) {
                ...
                for (int i = 0; i < nameNodeProxies.size(); i++) {
                    ...
                    if (currState != HAServiceState.OBSERVER) {
                        ...
                        LOG.debug("Skipping proxy {} for {} because it is in state {}", current.proxyInfo, method.getName(), currState == null ? "unreachable" : currState);
                        changeProxy(current);
                        continue;
                    }
                    LOG.debug("Attempting to service {} using proxy {}", method.getName(), current.proxyInfo);
                    try {
                        ...
                        if (e instanceof InterruptedIOException || e instanceof InterruptedException) {
                            ...
                            LOG.warn("Invocation returned interrupted exception on [{}];", current.proxyInfo, e);
                            throw e;
                        }
                        if (e instanceof RemoteException) {
                            RemoteException re = (RemoteException) e;
                            Exception unwrapped = re.unwrapRemoteException(ObserverRetryOnActiveException.class);
                            if (unwrapped instanceof ObserverRetryOnActiveException) {
                                LOG.debug("Encountered ObserverRetryOnActiveException from {}." + " Retry active namenode directly.", current.proxyInfo);
                                break;
                            }
                        }
                        RetryAction retryInfo = observerRetryPolicy.shouldRetry(e, 0, 0, method.isAnnotationPresent(Idempotent.class) || method.isAnnotationPresent(AtMostOnce.class));
                        if (retryInfo.action == RetryAction.RetryDecision.FAIL) {
                            throw e;
                        } else {
                            failedObserverCount++;
                            LOG.warn("Invocation returned exception on [{}]; {} failure(s) so far", current.proxyInfo, failedObserverCount, e);
                            changeProxy(current);
                        }
                    }
                }
                ...
                if (failedObserverCount > 0) {
                    ...
                    LOG.warn("{} observers have failed for read request {}; " + "also found {} standby, {} active, and {} unreachable. " + "Falling back to active.", failedObserverCount, method.getName(), standbyCount, activeCount, unreachableCount);
        ...
        public ConnectionId getConnectionId() {
            return RPC.getConnectionIdForProxy(observerReadEnabled ? getCurrentProxy().proxy : failoverProxy.getProxy().proxy);
        }
    }
