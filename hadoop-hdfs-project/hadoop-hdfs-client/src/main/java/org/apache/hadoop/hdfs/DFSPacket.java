...
public class DFSPacket {
    ...
    public synchronized void writeData(ByteBuffer inBuffer, int len) throws ClosedChannelException {
        checkBuffer();
     ...
     * @throws ClosedChannelException
     */
    public synchronized void writeChecksum(byte[] inarray, int off, int len) throws ClosedChannelException {
        checkBuffer();
        if (len == 0) {
            return;
        }
