...
public abstract class ServerWebApp extends Server implements ServletContextListener {
    ...
    private static final ThreadLocal<String> HOME_DIR_TL = new ThreadLocal<String>();
     ...
     */
    protected ServerWebApp(String name, String homeDir, String configDir, String logDir, String tempDir, Configuration config) {
    ...
    public ServerWebApp(String name) {
        super(name, getHomeDir(name), getDir(name, CONFIG_DIR, getHomeDir(name) + "/conf"), getDir(name, LOG_DIR, getHomeDir(name) + "/log"), getDir(name, TEMP_DIR, getHomeDir(name) + "/temp"), null);
    }
