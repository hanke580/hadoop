...
public class RouterClientProtocol implements ClientProtocol {
    ...
    RouterClientProtocol(Configuration conf, RouterRpcServer rpcServer) {
        ...
        this.allowPartialList = conf.getBoolean(RBFConfigKeys.DFS_ROUTER_ALLOW_PARTIAL_LIST, RBFConfigKeys.DFS_ROUTER_ALLOW_PARTIAL_LIST_DEFAULT);
        this.mountStatusTimeOut = conf.getTimeDuration(RBFConfigKeys.DFS_ROUTER_CLIENT_MOUNT_TIME_OUT, RBFConfigKeys.DFS_ROUTER_CLIENT_MOUNT_TIME_OUT_DEFAULT, TimeUnit.MILLISECONDS);
     ...
     */
    private static boolean isUnavailableSubclusterException(final IOException ioe) {
     ...
     */
    private List<RemoteLocation> checkFaultTolerantRetry(final RemoteMethod method, final String src, final IOException ioe, final RemoteLocation excludeLoc, final List<RemoteLocation> locations) throws IOException {
    ...
    @Override
    public BatchedEntries<OpenFileEntry> listOpenFiles(long prevId, EnumSet<OpenFilesIterator.OpenFilesType> openFilesTypes, String path) throws IOException {
        rpcServer.checkOperation(NameNode.OperationCategory.READ, false);
        return null;
    }
     ...
     */
    private RemoteParam getRenameDestinations(final List<RemoteLocation> srcLocations, final String dst) throws IOException {
     ...
     */
    private HdfsFileStatus getFileInfoAll(final List<RemoteLocation> locations, final RemoteMethod method) throws IOException {
     ...
     * @throws IOException If all the locations throw an exception.
     */
    private HdfsFileStatus getFileInfoAll(final List<RemoteLocation> locations, final RemoteMethod method, long timeOutMs) throws IOException {
    ...
    private HdfsFileStatus getMountPointStatus(String name, int childrenNum, long date) {
        ...
        long inodeId = 0;
        return new HdfsFileStatus.Builder().isdir(true).mtime(modTime).atime(accessTime).perm(permission).owner(owner).group(group).symlink(new byte[0]).path(DFSUtil.string2Bytes(name)).fileId(inodeId).children(childrenNum).flags(flags).build();
    }
