...
public class NamenodeHeartbeatService extends PeriodicService {
    ...
    protected void serviceInit(Configuration configuration) throws Exception {
        ...
        if (this.namenodeId != null && !this.namenodeId.isEmpty()) {
            this.localTarget = new NNHAServiceTarget(conf, nameserviceId, namenodeId);
            nnDesc += "-" + namenodeId;
        } else {
            ...
            if (aux != null) {
                for (int i = 0; i < aux.length(); i++) {
                    ...
                    if (name.equals("Hadoop:service=NameNode,name=FSNamesystemState")) {
                        ...
                        report.setNamesystemInfo(jsonObject.getLong("CapacityRemaining"), jsonObject.getLong("CapacityTotal"), jsonObject.getLong("FilesTotal"), jsonObject.getLong("BlocksTotal"), jsonObject.getLong("MissingBlocks"), jsonObject.getLong("PendingReplicationBlocks"), jsonObject.getLong("UnderReplicatedBlocks"), jsonObject.getLong("PendingDeletionBlocks"), jsonObject.optLong("ProvidedCapacityTotal"));
                    }
                }
            }
        } catch (Exception e) {
