...
public class AvailableSpaceResolver extends RouterResolver<String, SubclusterAvailableSpace> {
     ...
     */
    public static final String BALANCER_PREFERENCE_KEY = RBFConfigKeys.FEDERATION_ROUTER_PREFIX + "available-space-resolver.balanced-space-preference-fraction";
    ...
    public AvailableSpaceResolver(final Configuration conf, final Router routerService) {
        super(conf, routerService);
        float balancedPreference = conf.getFloat(BALANCER_PREFERENCE_KEY, BALANCER_PREFERENCE_DEFAULT);
    ...
    static final class SubclusterSpaceComparator implements Comparator<SubclusterAvailableSpace>, Serializable {
        ...
        SubclusterSpaceComparator(float balancedPreference) {
            Preconditions.checkArgument(balancedPreference <= 1 && balancedPreference >= 0, "The balancer preference value should be in the range 0.0 - 1.0");
