...
public class EncryptionZoneManager {
    ...
    private static class EncryptionZoneInt {
        ...
        public boolean equals(Object o) {
            ...
            return new EqualsBuilder().append(inodeId, b.getINodeId()).append(suite, b.getSuite()).append(version, b.getVersion()).append(keyName, b.getKeyName()).isEquals();
        }

        @Override
        public int hashCode() {
            return new HashCodeBuilder().append(inodeId).append(suite).append(version).append(keyName).toHashCode();
        }
    }
    ...
    public static final BatchedListEntries<ZoneReencryptionStatus> EMPTY_LIST = new BatchedListEntries<>(new ArrayList<ZoneReencryptionStatus>(), false);
    ...
    @VisibleForTesting
    public void pauseForTestingAfterNthCheckpoint(final String zone, final int count) throws IOException {
        INodesInPath iip;
    ...
    public EncryptionZoneManager(FSDirectory dir, Configuration conf) {
        ...
        if (getProvider() != null) {
            ...
            reencryptionThreadFactory = new ThreadFactoryBuilder().setDaemon(true).setNameFormat("reencryptionHandlerThread #%d").build();
        }
        maxListRecncryptionStatusResponses = conf.getInt(DFS_NAMENODE_LIST_REENCRYPTION_STATUS_NUM_RESPONSES_KEY, DFS_NAMENODE_LIST_REENCRYPTION_STATUS_NUM_RESPONSES_DEFAULT);
        Preconditions.checkArgument(maxListRecncryptionStatusResponses >= 0, DFS_NAMENODE_LIST_REENCRYPTION_STATUS_NUM_RESPONSES_KEY + " must be a positive integer.");
    ...
    void removeEncryptionZone(Long inodeId) {
        ...
        if (hasCreatedEncryptionZone()) {
            if (encryptionZones.remove(inodeId) == null || !getReencryptionStatus().hasRunningZone(inodeId)) {
                return;
            }
    ...
    private EncryptionZoneInt getEncryptionZoneForPath(INodesInPath iip) throws IOException {
        ...
        for (int i = iip.length() - 1; i >= 0; i--) {
            ...
            if (snapshotID == Snapshot.CURRENT_STATE_ID) {
                ...
                XAttr xAttr = FSDirXAttrOp.unprotectedGetXAttrByPrefixedName(inode, snapshotID, CRYPTO_XATTR_ENCRYPTION_ZONE);
                if (xAttr != null) {
                    try {
                        final HdfsProtos.ZoneEncryptionInfoProto ezProto = HdfsProtos.ZoneEncryptionInfoProto.parseFrom(xAttr.getValue());
                        return new EncryptionZoneInt(inode.getId(), PBHelperClient.convert(ezProto.getSuite()), PBHelperClient.convert(ezProto.getCryptoProtocolVersion()), ezProto.getKeyName());
    ...
    private EncryptionZoneInt getParentEncryptionZoneForPath(INodesInPath iip) throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(parentIIP, 105);
    ...
    EncryptionZone getEZINodeForPath(INodesInPath iip) throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(ezi, 106);
        if (ezi == null) {
            return null;
        } else {
            return new EncryptionZone(ezi.getINodeId(), getFullPathName(ezi.getINodeId()), ezi.getSuite(), ezi.getVersion(), ezi.getKeyName());
        }
    }
    ...
    void checkMoveValidity(INodesInPath srcIIP, INodesInPath dstIIP) throws IOException {
        ...
        if (srcInEZ) {
            if (!srcParentEZI.equals(dstParentEZI)) {
                ...
                sb.append(" can't be moved from encryption zone ").append(srcEZPath).append(" to encryption zone ").append(dstEZPath).append(".");
    ...
    private void checkMoveValidityForReencryption(final String pathName, final long zoneId) throws IOException {
        assert dir.hasReadLock();
     ...
     */
    private boolean pathResolvesToId(final long zoneId, final String zonePath) throws UnresolvedLinkException, AccessControlException, ParentNotDirectoryException {
        assert dir.hasReadLock();
     ...
     * @throws IOException
     */
    List<XAttr> reencryptEncryptionZone(final INodesInPath zoneIIP, final String keyVersionName) throws IOException {
        assert dir.hasWriteLock();
        if (reencryptionHandler == null) {
        ...
        final XAttr xattr = FSDirEncryptionZoneOp.updateReencryptionSubmitted(dir, zoneIIP, keyVersionName);
        xAttrs.add(xattr);
    ...
    List<XAttr> cancelReencryptEncryptionZone(final INodesInPath zoneIIP) throws IOException {
        ...
        return FSDirEncryptionZoneOp.updateReencryptionFinish(dir, zoneIIP, reencryptionStatus.getZoneStatus(zoneId));
    }
