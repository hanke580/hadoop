...
public class BlockTokenSecretManager extends SecretManager<BlockTokenIdentifier> {
    ...
    public Token<BlockTokenIdentifier> generateToken(String userId, ExtendedBlock block, EnumSet<BlockTokenIdentifier.AccessMode> modes) throws IOException {
        BlockTokenIdentifier id = ((BlockTokenIdentifier) org.zlab.ocov.tracker.Runtime.update(new BlockTokenIdentifier(userId, block.getBlockPoolId(), block.getBlockId(), modes), 15));
        if (shouldWrapQOP) {
    ...
    public void checkAccess(Token<BlockTokenIdentifier> token, String userId, ExtendedBlock block, BlockTokenIdentifier.AccessMode mode) throws InvalidToken {
        BlockTokenIdentifier id = ((BlockTokenIdentifier) org.zlab.ocov.tracker.Runtime.update(new BlockTokenIdentifier(), 16));
        try {
    ...
    protected byte[] createPassword(BlockTokenIdentifier identifier) {
        ...
        org.zlab.ocov.tracker.Runtime.update(identifier, 17);
        if (LOG.isDebugEnabled()) {
