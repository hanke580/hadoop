...
final class BlockChecksumHelper {
     ...
     */
    static abstract class BlockChecksumComputer extends AbstractBlockChecksumComputer {
        ...
        BlockChecksumComputer(DataNode datanode, ExtendedBlock block, BlockChecksumOptions blockChecksumOptions) throws IOException {
        ...
        void readHeader() throws IOException {
            ...
            long crcPerBlock = checksum.getChecksumSize() <= 0 ? 0 : (metadataIn.getLength() - BlockMetadataHeader.getHeaderSize()) / checksum.getChecksumSize();
        ...
        byte[] crcPartialBlock() throws IOException {
            ...
            if ((org.zlab.ocov.tracker.Runtime.updateBranch(partialLength > 0, 10))) {
    ...
    static class ReplicatedBlockChecksumComputer extends BlockChecksumComputer {
        ...
        ReplicatedBlockChecksumComputer(DataNode datanode, ExtendedBlock block, BlockChecksumOptions blockChecksumOptions) throws IOException {
        ...
        private void computeCompositeCrc(long stripeLength) throws IOException {
            ...
            CrcComposer crcComposer = CrcComposer.newStripedCrcComposer(getCrcType(), getBytesPerCRC(), stripeLength);
            ...
            if (partialChunkSize > 0) {
                if (isPartialBlk()) {
                    ...
                    crcComposer.update(partialChunkCrcBytes, 0, partialChunkCrcBytes.length, partialChunkSize);
                } else {
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug("block={}, getBytesPerCRC={}, crcPerBlock={}, compositeCrc={}", getBlock(), getBytesPerCRC(), getCrcPerBlock(), CrcUtil.toMultiCrcString(composedCrcs));
            }
        }
    }
    ...
    static class BlockGroupNonStripedChecksumComputer extends AbstractBlockChecksumComputer {
        ...
        BlockGroupNonStripedChecksumComputer(DataNode datanode, StripedBlockInfo stripedBlockInfo, long requestedNumBytes, BlockChecksumOptions blockChecksumOptions) throws IOException {
            super(datanode, blockChecksumOptions);
                ...
                try {
                    ...
                    if (liveBlkInfo == null) {
                        ...
                        try {
                            ...
                            String msg = String.format("Exception while reading checksum for block %s at index " + "%d in blockGroup %s", block, idx, blockGroup);
                            LOG.warn(msg, ioe);
                    ...
                    if (checksumLen >= requestedNumBytes) {
                        // done with the computation, simply return.
                        break;
                    }
                } catch (IOException e) {
                    LOG.warn("Failed to get the checksum for block {} at index {} " + "in blockGroup {}", block, idx, blockGroup, e);
                    throw e;
                }
            }
            BlockChecksumType type = getBlockChecksumOptions().getBlockChecksumType();
        ...
        private byte[] reassembleNonStripedCompositeCrc(long checksumLen) throws IOException {
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug("flatBlockChecksumData.length={}, numDataUnits={}, " + "checksumLen={}, digest={}", flatBlockChecksumData.length, numDataUnits, checksumLen, CrcUtil.toSingleCrcString(digest));
            }
            return digest;
        }
            ...
            try (IOStreamPair pair = getDatanode().connectToDN(targetDatanode, timeout, block, blockToken)) {
                ...
                switch(groupChecksumType) {
                    case MD5CRC:
                        ...
                        break;
                    case COMPOSITE_CRC:
                        childOptions = new BlockChecksumOptions(BlockChecksumType.COMPOSITE_CRC, ecPolicy.getCellSize());
                        break;
                    default:
                ...
                switch(groupChecksumType) {
                    ...
                    case COMPOSITE_CRC:
                        ...
                        if (LOG.isDebugEnabled()) {
                            LOG.debug("got reply from datanode:{} for blockIdx:{}, checksum:{}", targetDatanode, blockIdx, CrcUtil.toMultiCrcString(checksumBytes));
                        }
                        break;
                    default:
                        throw new IOException("Unknown BlockChecksumType: " + groupChecksumType);
        ...
        private void recalculateChecksum(int errBlkIndex, long blockLength) throws IOException {
            ...
            BlockChecksumType groupChecksumType = getBlockChecksumOptions().getBlockChecksumType();
            try (StripedBlockChecksumReconstructor checksumRecon = groupChecksumType == BlockChecksumType.COMPOSITE_CRC ? new StripedBlockChecksumCompositeCrcReconstructor(getDatanode().getErasureCodingWorker(), stripedReconInfo, blockChecksumBuf, blockLength) : new StripedBlockChecksumMd5CrcReconstructor(getDatanode().getErasureCodingWorker(), stripedReconInfo, blockChecksumBuf, blockLength)) {
                ...
                LOG.debug("Recalculated checksum for the block index:{}, checksum={}", errBlkIndex, checksumRecon.getDigestObject());
            }
        }

        private void setOrVerifyChecksumProperties(int blockIdx, int bpc, final long cpb, DataChecksum.Type ct) throws IOException {
            ...
            if (blockIdx == 0) {
                ...
                if (groupChecksumType == BlockChecksumType.COMPOSITE_CRC) {
                    throw new IOException(String.format("BlockChecksumType COMPOSITE_CRC doesn't support MIXED " + "underlying types; previous block was %s, next block is %s", getCrcType(), ct));
                } else {
