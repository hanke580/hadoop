...
public class NativePmemMappableBlockLoader extends PmemMappableBlockLoader {
    ...
    public MappableBlock load(long length, FileInputStream blockIn, FileInputStream metaIn, String blockFileName, ExtendedBlockId key) throws IOException {
        ...
        try {
            ...
            LOG.info("Successfully cached one replica:{} into persistent memory" + ", [cached path={}, address={}, length={}]", key, filePath, region.getAddress(), length);
        } finally {
     ...
     */
    private void verifyChecksumAndMapBlock(POSIX.PmemMappedRegion region, long length, FileInputStream metaIn, FileChannel blockChannel, String blockFileName) throws IOException {
    ...
    public MappableBlock getRecoveredMappableBlock(File cacheFile, String bpid, byte volumeIndex) throws IOException {
        NativeIO.POSIX.PmemMappedRegion region = NativeIO.POSIX.Pmem.mapBlock(cacheFile.getAbsolutePath(), cacheFile.length(), true);
        if (region == null) {
        ...
        MappableBlock mappableBlock = new NativePmemMappedBlock(region.getAddress(), region.getLength(), key);
        ...
        LOG.info("Recovering persistent memory cache for block {}, " + "path = {}, address = {}, length = {}", key, path, addr, length);
        return mappableBlock;
    }
}
