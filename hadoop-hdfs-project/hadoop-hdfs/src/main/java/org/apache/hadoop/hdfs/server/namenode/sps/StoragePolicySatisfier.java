...
public class StoragePolicySatisfier implements SPSService, Runnable {
    ...
    public void init(final Context context) {
        ...
        this.storageMovementsMonitor = new BlockStorageMovementAttemptedItems(this, storageMovementNeeded, context);
        this.spsWorkMultiplier = getSPSWorkMultiplier(getConf());
        this.blockMovementMaxRetry = getConf().getInt(DFSConfigKeys.DFS_STORAGE_POLICY_SATISFIER_MAX_RETRY_ATTEMPTS_KEY, DFSConfigKeys.DFS_STORAGE_POLICY_SATISFIER_MAX_RETRY_ATTEMPTS_DEFAULT);
    }
    ...
    public synchronized void start(StoragePolicySatisfierMode serviceMode) {
        ...
        LOG.info("Starting {} StoragePolicySatisfier.", StringUtils.toLowerCase(serviceMode.toString()));
        isRunning = true;
        ...
        while (isRunning) {
            ...
            try {
                ...
                if (!ctxt.isInSafeMode()) {
                    ...
                    if (itemInfo != null) {
                        try {
                            this.left_142 = (itemInfo.getRetryCount());
                            this.right_142 = (blockMovementMaxRetry);
                        } catch (Exception e) {
                            this.left_142 = 0L;
                            this.right_142 = 0L;
                        }
                        if (itemInfo.getRetryCount() >= blockMovementMaxRetry) {
                            LOG.info("Failed to satisfy the policy after " + blockMovementMaxRetry + " retries. Removing inode " + itemInfo.getFile() + " from the queue");
                        ...
                        if (fileStatus == null || fileStatus.isDir()) {
                            ...
                            switch(status.status) {
                                ...
                                case BLOCKS_TARGETS_PAIRED:
                                    if (LOG.isDebugEnabled()) {
                                        LOG.debug("Block analysis status:{} for the file id:{}." + " Adding to attempt monitor queue for the storage " + "movement attempt finished report", status.status, fileStatus.getFileId());
                                    }
                                ...
                                case NO_BLOCKS_TARGETS_PAIRED:
                                    if (LOG.isDebugEnabled()) {
                                        LOG.debug("Adding trackID:{} for the file id:{} back to" + " retry queue as none of the blocks found its eligible" + " targets.", trackId, fileStatus.getFileId());
                                    }
                                    retryItem = true;
                                    break;
                                case FEW_LOW_REDUNDANCY_BLOCKS:
                                    if (LOG.isDebugEnabled()) {
                                        LOG.debug("Adding trackID:{} for the file id:{} back to " + "retry queue as some of the blocks are low redundant.", trackId, fileStatus.getFileId());
                                    }
                                    retryItem = true;
                                    break;
                                case BLOCKS_FAILED_TO_MOVE:
                                    if (LOG.isDebugEnabled()) {
                                        LOG.debug("Adding trackID:{} for the file id:{} back to " + "retry queue as some of the blocks movement failed.", trackId, fileStatus.getFileId());
                                    }
                                    retryItem = true;
                                    break;
                                // Just clean Xattrs
                                ...
                                default:
                                    LOG.info("Block analysis status:{} for the file id:{}." + " So, Cleaning up the Xattrs.", status.status, fileStatus.getFileId());
                                    storageMovementNeeded.removeItemTrackInfo(itemInfo, true);
                                    break;
                            }
                        }
                ...
                try {
                    this.left_143 = (blockCount);
                    this.right_143 = ((numLiveDn * spsWorkMultiplier));
                } catch (Exception e) {
                    this.left_143 = 0L;
                    this.right_143 = 0L;
                }
                ...
                LOG.error("Exception during StoragePolicySatisfier execution - " + "will continue next cycle", e);
    ...
    private BlocksMovingAnalysis analyseBlocksStorageMovementsAndAssignToDN(HdfsLocatedFileStatus fileInfo, BlockStoragePolicy existingStoragePolicy) throws IOException {
        ...
        if (!lastBlkComplete) {
            ...
            LOG.info("File: {} is under construction. So, postpone" + " this to the next retry iteration", fileInfo.getFileId());
        ...
        if (blocks.size() == 0) {
            LOG.info("File: {} is not having any blocks." + " So, skipping the analysis.", fileInfo.getFileId());
        ...
        for (int i = 0; i < blocks.size(); i++) {
            ...
            List<StorageType> existing = new LinkedList<StorageType>(Arrays.asList(blockInfo.getStorageTypes()));
            if (!removeOverlapBetweenStorageTypes(expectedStorageTypes, existing, true)) {
                boolean blocksPaired = computeBlockMovingInfos(blockMovingInfos, blockInfo, expectedStorageTypes, existing, blockInfo.getLocations(), liveDns, ecPolicy);
                if (blocksPaired) {
     ...
     */
    private boolean computeBlockMovingInfos(List<BlockMovingInfo> blockMovingInfos, LocatedBlock blockInfo, List<StorageType> expectedStorageTypes, List<StorageType> existing, DatanodeInfo[] storages, DatanodeMap liveDns, ErasureCodingPolicy ecPolicy) {
        boolean foundMatchingTargetNodesForBlock = true;
        if (!removeOverlapBetweenStorageTypes(expectedStorageTypes, existing, true)) {
            List<StorageTypeNodePair> sourceWithStorageMap = new ArrayList<StorageTypeNodePair>();
            List<DatanodeInfo> existingBlockStorages = new ArrayList<DatanodeInfo>(Arrays.asList(storages));
            ...
            EnumMap<StorageType, List<DatanodeWithStorage.StorageDetails>> targetDns = findTargetsForExpectedStorageTypes(expectedStorageTypes, liveDns);
    ...
    private boolean findSourceAndTargetToMove(List<BlockMovingInfo> blockMovingInfos, LocatedBlock blockInfo, List<StorageTypeNodePair> sourceWithStorageList, List<StorageType> expectedTypes, EnumMap<StorageType, List<DatanodeWithStorage.StorageDetails>> targetDns, ErasureCodingPolicy ecPolicy, List<DatanodeInfo> excludeNodes) {
        ...
        for (int i = 0; i < sourceWithStorageList.size(); i++) {
            ...
            if (!expectedTypes.contains(existingTypeNodePair.storageType)) {
                StorageTypeNodePair chosenTarget = chooseTargetTypeInSameNode(blockInfo, existingTypeNodePair.dn, targetDns, expectedTypes);
                if (chosenTarget != null) {
                    if (blockInfo.isStriped()) {
                        buildStripedBlockMovingInfos(blockInfo, existingTypeNodePair.dn, existingTypeNodePair.storageType, chosenTarget.dn, chosenTarget.storageType, blockMovingInfos, ecPolicy);
                    } else {
                        buildContinuousBlockMovingInfos(blockInfo, existingTypeNodePair.dn, existingTypeNodePair.storageType, chosenTarget.dn, chosenTarget.storageType, blockMovingInfos);
                    }
        ...
        try {
            this.left_144 = (expectedTypes.size());
            this.right_144 = (0);
        } catch (Exception e) {
            this.left_144 = 0L;
            this.right_144 = 0L;
        ...
        for (int i = 0; i < sourceWithStorageList.size(); i++) {
            ...
            if (chosenTarget == null && dnCacheMgr.getCluster().isNodeGroupAware()) {
                chosenTarget = chooseTarget(blockInfo, existingTypeNodePair.dn, expectedTypes, Matcher.SAME_NODE_GROUP, targetDns, excludeNodes);
            }
            // Then, match nodes on the same rack
            if (chosenTarget == null) {
                chosenTarget = chooseTarget(blockInfo, existingTypeNodePair.dn, expectedTypes, Matcher.SAME_RACK, targetDns, excludeNodes);
            }
            if (chosenTarget == null) {
                chosenTarget = chooseTarget(blockInfo, existingTypeNodePair.dn, expectedTypes, Matcher.ANY_OTHER, targetDns, excludeNodes);
            }
            if (null != chosenTarget) {
                ...
                LOG.warn("Failed to choose target datanode for the required" + " storage types {}, block:{}, existing storage type:{}", expectedTypes, blockInfo, existingTypeNodePair.storageType);
            }
        }
        if (expectedTypes.size() > 0) {
    ...
    private void buildContinuousBlockMovingInfos(LocatedBlock blockInfo, DatanodeInfo sourceNode, StorageType sourceStorageType, DatanodeInfo targetNode, StorageType targetStorageType, List<BlockMovingInfo> blkMovingInfos) {
        Block blk = ExtendedBlock.getLocalBlock(blockInfo.getBlock());
        BlockMovingInfo blkMovingInfo = new BlockMovingInfo(blk, sourceNode, targetNode, sourceStorageType, targetStorageType);
        blkMovingInfos.add(blkMovingInfo);
    }

    private void buildStripedBlockMovingInfos(LocatedBlock blockInfo, DatanodeInfo sourceNode, StorageType sourceStorageType, DatanodeInfo targetNode, StorageType targetStorageType, List<BlockMovingInfo> blkMovingInfos, ErasureCodingPolicy ecPolicy) {
        ...
        for (int i = 0; i < indices.length; i++) {
            ...
            if (blkIndex >= 0) {
                ...
                if (sourceNode.equals(locations[i])) {
                    ...
                    long numBytes = StripedBlockUtil.getInternalBlockLength(extBlock.getNumBytes(), ecPolicy, blkIndex);
        ...
        for (StorageType t : targetTypes) {
            ...
            for (DatanodeWithStorage.StorageDetails targetNode : targetNodeStorages) {
                if (targetNode.getDatanodeInfo().equals(source)) {
                    ...
                    if (LOG.isDebugEnabled()) {
                        LOG.debug("Datanode:{} storage type:{} doesn't have sufficient " + "space:{} to move the target block size:{}", source, t, targetNode, blockInfo.getBlockSize());
                    }
                }
            }
        }
    ...
    private StorageTypeNodePair chooseTarget(LocatedBlock block, DatanodeInfo source, List<StorageType> targetTypes, Matcher matcher, EnumMap<StorageType, List<DatanodeWithStorage.StorageDetails>> locsForExpectedStorageTypes, List<DatanodeInfo> excludeNodes) {
        for (StorageType t : targetTypes) {
            ...
            if (nodesWithStorages == null || nodesWithStorages.isEmpty()) {
                // no target nodes with the required storage type.
                continue;
            }
            ...
            for (DatanodeWithStorage.StorageDetails targetNode : nodesWithStorages) {
                ...
                if (!excludeNodes.contains(target) && matcher.match(dnCacheMgr.getCluster(), source, target)) {
                    // Good target with enough space to write the given block size.
                    ...
                    if (LOG.isDebugEnabled()) {
                        LOG.debug("Datanode:{} storage type:{} doesn't have sufficient " + "space:{} to move the target block size:{}", target, t, targetNode, block.getBlockSize());
                    }
                }
            }
        }
    ...
    static final class StorageTypeNodePair {
        ...
        public String toString() {
            return new StringBuilder().append("StorageTypeNodePair(\n  ").append("DatanodeInfo: ").append(dn).append(", StorageType: ").append(storageType).toString();
        }
    }
    ...
    public static class DatanodeMap {
        ...
        private final EnumMap<StorageType, List<DatanodeWithStorage>> targetsMap = new EnumMap<StorageType, List<DatanodeWithStorage>>(StorageType.class);
    ...
    public static final class DatanodeWithStorage {
        ...
        private final EnumMap<StorageType, List<StorageDetails>> storageMap = new EnumMap<StorageType, List<StorageDetails>>(StorageType.class);
        ...
        @Override
        public String toString() {
            return new StringBuilder().append("DatanodeWithStorageInfo(\n  ").append("Datanode: ").append(datanode).append(" StorageTypeNodeMap: ").append(storageMap).append(")").toString();
        }
        ...
        final class StorageDetails {
            ...
            @Override
            public String toString() {
                return new StringBuilder().append("StorageDetails(\n  ").append("maxSize2Move: ").append(maxSize2Move).append(" scheduledSize: ").append(scheduledSize).append(")").toString();
            }
        }
    }
    ...
    public void addFileToProcess(ItemInfo trackInfo, boolean scanCompleted) {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("Added track info for inode {} to block " + "storageMovementNeeded queue", trackInfo.getFile());
        }
    }
    ...
    private static int getSPSWorkMultiplier(Configuration conf) {
        int spsWorkMultiplier = conf.getInt(DFSConfigKeys.DFS_SPS_WORK_MULTIPLIER_PER_ITERATION, DFSConfigKeys.DFS_SPS_WORK_MULTIPLIER_PER_ITERATION_DEFAULT);
        Preconditions.checkArgument((spsWorkMultiplier > 0), DFSConfigKeys.DFS_SPS_WORK_MULTIPLIER_PER_ITERATION + " = '" + spsWorkMultiplier + "' is invalid. " + "It should be a positive, non-zero integer value.");
        return spsWorkMultiplier;
    }

    private long right_143;

    private long right_142;

    private long right_144;

    private long left_142;

    private long left_144;

    private long left_143;
}
