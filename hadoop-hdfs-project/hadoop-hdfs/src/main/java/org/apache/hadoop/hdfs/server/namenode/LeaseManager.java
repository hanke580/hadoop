...
public class LeaseManager {
    ...
    // Set of: Lease
    private final NavigableSet<Lease> sortedLeases = new TreeSet<>(new Comparator<Lease>() {
    ...
    synchronized long getNumUnderConstructionBlocks() {
        ...
        for (Long id : getINodeIdWithLeases()) {
            ...
            if (inode == null) {
                ...
                LOG.warn("Failed to find inode " + id + " in getNumUnderConstructionBlocks().");
                continue;
            }
    ...
    public BatchedListEntries<OpenFileEntry> getUnderConstructionFiles(final long prevId) throws IOException {
        ...
        for (Long inodeId : inodeIds) {
            ...
            if (!inodeFile.isUnderConstruction()) {
                LOG.warn("The file " + inodeFile.getFullPathName() + " is not under construction but has lease.");
                continue;
            }
    ...
    public Set<INodesInPath> getINodeWithLeases(final INodeDirectory ancestorDir) throws IOException {
        ...
        if ((endTimeMs - startTimeMs) > 1000) {
            LOG.info("Took " + (endTimeMs - startTimeMs) + " ms to collect " + iipSet.size() + " open files with leases" + ((ancestorDir != null) ? " under " + ancestorDir.getFullPathName() : "."));
        }
        return iipSet;
    }
    ...
    synchronized void removeLease(String holder, INodeFile src) {
        ...
        if (lease != null) {
            ...
            LOG.warn("Removing non-existent lease! holder=" + holder + " src=" + src.getFullPathName());
        }
    }
    ...
    synchronized boolean checkLeases() {
        ...
        while (!sortedLeases.isEmpty() && sortedLeases.first().expiredHardLimit() && !isMaxLockHoldToReleaseLease(start)) {
            ...
            for (Long id : leaseINodeIds) {
                try {
                    ...
                    try {
                        ...
                        LOG.warn("Cannot release the path " + p + " in the lease " + leaseToCheck + ". It will be retried.", e);
                        continue;
                    }
                    if (LOG.isDebugEnabled()) {
                        if (completed) {
                            LOG.debug("Lease recovery for inode " + id + " is complete. " + "File closed.");
                        } else {
                ...
                if (isMaxLockHoldToReleaseLease(start)) {
                    LOG.debug("Breaking out of checkLeases after " + fsnamesystem.getMaxLockHoldToReleaseLeaseMs() + "ms.");
                    break;
                }
            }
