...
public class EditLogFileInputStream extends EditLogInputStream {
    ...
    public String toString() {
        return "EditLogFileInputStream{" + "log=" + log.getName() + ", firstTxId=" + firstTxId + ", lastTxId=" + lastTxId + ", isInProgress=" + isInProgress + ", maxOpSize=" + maxOpSize + ", state=" + state + ", logVersion=" + logVersion + '}';
    }
    ...
    static FSEditLogLoader.EditLogValidation scanEditLog(File file, long maxTxIdToScan, boolean verifyVersion) throws IOException {
        ...
        try {
            in = ((EditLogFileInputStream) org.zlab.ocov.tracker.Runtime.update(new EditLogFileInputStream(file), 124));
    ...
    static int readLogVersion(DataInputStream in, boolean verifyLayoutVersion) throws IOException, LogHeaderCorruptException {
        ...
        if (verifyLayoutVersion && (// future version
        logVersion < HdfsServerConstants.NAMENODE_LAYOUT_VERSION || logVersion > Storage.LAST_UPGRADABLE_LAYOUT_VERSION)) {
            // unsupported
