...
public class ReplicaInPipeline extends ReplicaInfo implements ReplicaInPipelineInterface {
    ...
    public ReplicaOutputStreams createStreams(boolean isCreate, DataChecksum requestedChecksum) throws IOException {
        ...
        RandomAccessFile metaRAF = getFileIoProvider().getRandomAccessFile(getVolume(), metaFile, "rw");
        if (!isCreate) {
            ...
            try {
                ...
                org.zlab.ocov.tracker.Runtime.update(header, 133, isCreate, requestedChecksum);
        ...
        try {
            blockOut = getFileIoProvider().getFileOutputStream(getVolume(), new RandomAccessFile(blockFile, "rw").getFD());
            crcOut = getFileIoProvider().getFileOutputStream(getVolume(), metaRAF.getFD());
            if (!isCreate) {
                blockOut.getChannel().position(blockDiskSize);
                crcOut.getChannel().position(crcDiskSize);
            }
            return new ReplicaOutputStreams(blockOut, crcOut, checksum, getVolume(), getFileIoProvider());
        } catch (IOException e) {
