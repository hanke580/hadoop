...
public class BlockStorageMovementAttemptedItems {
    ...
    //
    // minimum value
    private long minCheckTimeout = 1 * 60 * 1000;
    ...
    public BlockStorageMovementAttemptedItems(SPSService service, BlockStorageMovementNeeded unsatisfiedStorageMovementFiles, Context context) {
        ...
        long recheckTimeout = this.service.getConf().getLong(DFS_STORAGE_POLICY_SATISFIER_RECHECK_TIMEOUT_MILLIS_KEY, DFS_STORAGE_POLICY_SATISFIER_RECHECK_TIMEOUT_MILLIS_DEFAULT);
        if (recheckTimeout > 0) {
            this.minCheckTimeout = Math.min(minCheckTimeout, recheckTimeout);
        }
        this.selfRetryTimeout = this.service.getConf().getLong(DFS_STORAGE_POLICY_SATISFIER_SELF_RETRY_TIMEOUT_MILLIS_KEY, DFS_STORAGE_POLICY_SATISFIER_SELF_RETRY_TIMEOUT_MILLIS_DEFAULT);
     ...
     *          - retry count
     */
    public void add(long startPathId, long fileId, long monotonicNow, Map<Block, Set<StorageTypeNodePair>> assignedBlocks, int retryCount) {
        AttemptedItemInfo itemInfo = new AttemptedItemInfo(startPathId, fileId, monotonicNow, assignedBlocks.keySet(), retryCount);
    ...
    private void matchesReportedBlock(DatanodeInfo reportedDn, StorageType type, Block reportedBlock) {
        ...
        for (StorageTypeNodePair dn : blkLocs) {
            ...
            if (foundDn && foundType) {
                ...
                if (blkLocs.size() <= 0) {
                    ...
                    // clean-up reported block
                    scheduledBlkLocs.remove(reportedBlock);
                }
                // found
                return;
            }
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("Reported block:{} not found in attempted blocks. Datanode:{}" + ", StorageType:{}", reportedBlock, reportedDn, type);
        }
    }
        ...
        public void run() {
            while (monitorRunning) {
                try {
                    ...
                    LOG.warn("BlocksStorageMovementAttemptMonitor thread " + "received exception and exiting.", ie);
                }
            }
        }
    }
        ...
        synchronized (storageMovementAttemptedItems) {
            ...
            while (iter.hasNext()) {
                ...
                if (now > itemInfo.getLastAttemptedOrReportedTime() + selfRetryTimeout) {
                    ...
                    iter.remove();
                    LOG.info("TrackID: {} becomes timed out and moved to needed " + "retries queue for next iteration.", file);
                }
            }
        }
    }
            ...
            synchronized (storageMovementAttemptedItems) {
                ...
                while (iterator.hasNext()) {
                    ...
                    if (attemptedItemInfo.getBlocks().isEmpty()) {
                        blockStorageMovementNeeded.add(new ItemInfo(attemptedItemInfo.getStartPath(), attemptedItemInfo.getFile(), attemptedItemInfo.getRetryCount() + 1));
                        iterator.remove();
                    }
                }
            }
