...
public class CryptoAdmin extends Configured implements Tool {
    ...
    private static class CreateZoneCommand implements AdminHelper.Command {
        ...
        public String getLongUsage() {
            ...
            listing.addRow("<path>", "The path of the encryption zone to create. " + "It must be an empty directory. A trash directory is provisioned " + "under this path.");
    ...
    private static class ListZonesCommand implements AdminHelper.Command {
        ...
        public int run(Configuration conf, List<String> args) throws IOException {
            ...
            try {
                final TableListing listing = new TableListing.Builder().addField("").addField("", true).hideHeaders().build();
    ...
    private static class ReencryptZoneCommand implements AdminHelper.Command {
        ...
        public int run(Configuration conf, List<String> args) throws IOException {
            ...
            try {
                ...
                System.out.println("re-encrypt command successfully submitted for " + "zone: " + path + " action: " + action);
            } catch (IOException e) {
                System.err.println(prettifyException(e));
                return 4;
            }
    ...
    private static class ListReencryptionStatusCommand implements AdminHelper.Command {
        ...
        public int run(Configuration conf, List<String> args) throws IOException {
            HdfsAdmin admin = new HdfsAdmin(FileSystem.getDefaultUri(conf), conf);
            try {
                final TableListing listing = new TableListing.Builder().addField("Zone Name").addField("Status").addField("EZKey Version Name").addField("Submission Time").addField("Is Canceled?").addField("Completion Time").addField("Number of files re-encrypted").addField("Number of failures").addField("Last File Checkpointed").showHeaders().build();
                ...
                while (it.hasNext()) {
                    ...
                    listing.addRow(zs.getZoneName(), zs.getState().toString(), zs.getEzKeyVersionName(), Time.formatTime(zs.getSubmissionTime()), Boolean.toString(zs.isCanceled()), completion == 0 ? "N/A" : Time.formatTime(completion), Long.toString(zs.getFilesReencrypted()), Long.toString(zs.getNumReencryptionFailures()), zs.getLastCheckpointFile());
