...
class BackupJournalManager implements JournalManager {
    ...
    public EditLogOutputStream startLogSegment(long txId, int layoutVersion) throws IOException {
        EditLogBackupOutputStream stm = ((EditLogBackupOutputStream) org.zlab.ocov.tracker.Runtime.update(new EditLogBackupOutputStream(bnReg, journalInfo, layoutVersion), 113));
