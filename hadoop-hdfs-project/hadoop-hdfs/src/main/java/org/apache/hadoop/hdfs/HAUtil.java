...
public class HAUtil {
    ...
    private static final String[] HA_SPECIAL_INDEPENDENT_KEYS = new String[] { DFS_NAMENODE_RPC_ADDRESS_KEY, DFS_NAMENODE_RPC_BIND_HOST_KEY, DFS_NAMENODE_LIFELINE_RPC_ADDRESS_KEY, DFS_NAMENODE_LIFELINE_RPC_BIND_HOST_KEY, DFS_NAMENODE_SERVICE_RPC_ADDRESS_KEY, DFS_NAMENODE_SERVICE_RPC_BIND_HOST_KEY, DFS_NAMENODE_HTTP_ADDRESS_KEY, DFS_NAMENODE_HTTPS_ADDRESS_KEY, DFS_NAMENODE_HTTP_BIND_HOST_KEY, DFS_NAMENODE_HTTPS_BIND_HOST_KEY };
    ...
    public static List<String> getNameNodeIdOfOtherNodes(Configuration conf, String nsId) {
        ...
        Preconditions.checkArgument(nnIds.size() >= 2, "Expected at least 2 NameNodes in namespace '%s'. " + "Instead, got only %s (NN ids were '%s')", nsId, nnIds.size(), Joiner.on("','").join(nnIds));
    ...
    public static boolean shouldAllowStandbyReads(Configuration conf) {
        return conf.getBoolean(DFS_HA_ALLOW_STALE_READ_KEY, DFS_HA_ALLOW_STALE_READ_DEFAULT);
    }
    ...
    public static boolean useLogicalUri(Configuration conf, URI nameNodeUri) throws IOException {
        ...
        AbstractNNFailoverProxyProvider<ClientProtocol> provider = NameNodeProxiesClient.createFailoverProxyProvider(conf, nameNodeUri, ClientProtocol.class, false, null);
