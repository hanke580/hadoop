...
public class NNHAServiceTarget extends HAServiceTarget {
    ...
    public NNHAServiceTarget(Configuration conf, String nsId, String nnId) {
        ...
        if (nsId == null) {
            ...
            if (nsId == null) {
                ...
                if ((dfsNames != null) && (dfsNames.length > 1)) {
                    errorString = "Unable to determine the name service ID. " + "This is an HA configuration with multiple name services " + "configured. " + DFS_NAMESERVICES + " is set to " + Arrays.toString(dfsNames) + ". Please re-run with the -ns option.";
                }
        ...
        this.addr = NetUtils.createSocketAddr(serviceAddr, HdfsClientConfigKeys.DFS_NAMENODE_RPC_PORT_DEFAULT);
