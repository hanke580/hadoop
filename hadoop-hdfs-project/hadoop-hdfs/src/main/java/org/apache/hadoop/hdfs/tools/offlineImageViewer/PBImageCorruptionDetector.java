...
public class PBImageCorruptionDetector extends PBImageTextWriter {
    ...
    public String getEntry(String parentPath, FsImageProto.INodeSection.INode inode) {
        ...
        if (corruptionsMap.containsKey(id)) {
            ...
            entryBuilder.setCorruption(corruptionsMap.get(id)).setParentPath(parentPath).setName(inode.getName().toStringUtf8()).setNodeType(corrChecker.getTypeOfId(id));
            if (parentId != -1) {
    ...
    public void afterOutput() throws IOException {
        if (!corruptionsMap.isEmpty()) {
            ...
            for (PBImageCorruption c : corruptionsMap.values()) {
                ...
                entryBuilder.setCorruption(corruptionsMap.get(id)).setName(name).setNodeType(corrChecker.getTypeOfId(id));
                if (parentId != -1) {
                    entryBuilder.setParentId(parentId);
                }
