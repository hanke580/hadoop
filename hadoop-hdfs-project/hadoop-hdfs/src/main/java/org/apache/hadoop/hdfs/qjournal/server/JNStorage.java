...
class JNStorage extends Storage {
    ...
    protected JNStorage(Configuration conf, File logDir, StartupOption startOpt, StorageErrorReporter errorReporter) throws IOException {
        ...
        sd = new StorageDirectory(logDir, null, false, new FsPermission(conf.get(DFSConfigKeys.DFS_JOURNAL_EDITS_DIR_PERMISSION_KEY, DFSConfigKeys.DFS_JOURNAL_EDITS_DIR_PERMISSION_DEFAULT)));
    ...
    File getTemporaryEditsFile(long startTxId, long endTxId) {
        return new File(getEditsSyncDir(), String.format("%s_%019d-%019d", NNStorage.NameNodeFile.EDITS.getName(), startTxId, endTxId));
    }

    File getFinalizedEditsFile(long startTxId, long endTxId) {
        return new File(sd.getCurrentDir(), String.format("%s_%019d-%019d", NNStorage.NameNodeFile.EDITS.getName(), startTxId, endTxId));
    }
