...
public final class DirectoryWithQuotaFeature implements INode.Feature {
    ...
    QuotaCounts getQuota() {
        return ((QuotaCounts.Builder) org.zlab.ocov.tracker.Runtime.update(new QuotaCounts.Builder(), 108)).quotaCount(this.quota).build();
    }
    ...
    ContentSummaryComputationContext computeContentSummary(final INodeDirectory dir, final ContentSummaryComputationContext summary) throws AccessControlException {
    ...
    private void checkStoragespace(final INodeDirectory dir, final long computed) {
        if (-1 != quota.getStorageSpace() && usage.getStorageSpace() != computed) {
            NameNode.LOG.warn("BUG: Inconsistent storagespace for directory " + dir.getFullPathName() + ". Cached = " + usage.getStorageSpace() + " != Computed = " + computed);
        }
    }
    ...
    public QuotaCounts getSpaceAllowed() {
        return ((QuotaCounts.Builder) org.zlab.ocov.tracker.Runtime.update(new QuotaCounts.Builder(), 109)).quotaCount(quota).build();
    }
    ...
    public QuotaCounts getSpaceConsumed() {
        return ((QuotaCounts.Builder) org.zlab.ocov.tracker.Runtime.update(new QuotaCounts.Builder(), 110)).quotaCount(usage).build();
    }
