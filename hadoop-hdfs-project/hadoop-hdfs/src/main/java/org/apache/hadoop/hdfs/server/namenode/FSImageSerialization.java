...
public class FSImageSerialization {
    ...
    private static void writePermissionStatus(INodeAttributes inode, DataOutput out) throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(p, 43);
    ...
    static INodeFile readINodeUnderConstruction(DataInput in, FSNamesystem fsNamesys, int imgVersion) throws IOException {
        ...
        if ((org.zlab.ocov.tracker.Runtime.updateBranch(numBlocks > 0, 19))) {
            blk.readFields(in);
            blocks[i] = new BlockInfoContiguous(blk, blockReplication);
    ...
    public static void writeString(String str, DataOutput out) throws IOException {
        DeprecatedUTF8 ustr = TL_DATA.get().U_STR;
        ustr.set(str);
        org.zlab.ocov.tracker.Runtime.update(ustr, 44);
        ustr.write(out);
    }
    ...
    static void writeLong(long value, DataOutputStream out) throws IOException {
        LongWritable uLong = TL_DATA.get().U_LONG;
        uLong.set(value);
        org.zlab.ocov.tracker.Runtime.update(uLong, 45);
        uLong.write(out);
    }
    ...
    static void writeBoolean(boolean value, DataOutputStream out) throws IOException {
        BooleanWritable uBoolean = TL_DATA.get().U_BOOLEAN;
        uBoolean.set(value);
        org.zlab.ocov.tracker.Runtime.update(uBoolean, 46);
        uBoolean.write(out);
    }
    ...
    static void writeInt(int value, DataOutputStream out) throws IOException {
        IntWritable uInt = TL_DATA.get().U_INT;
        uInt.set(value);
        org.zlab.ocov.tracker.Runtime.update(uInt, 47);
        uInt.write(out);
    }
    ...
    static void writeShort(short value, DataOutputStream out) throws IOException {
        ShortWritable uShort = TL_DATA.get().U_SHORT;
        uShort.set(value);
        org.zlab.ocov.tracker.Runtime.update(uShort, 48);
        uShort.write(out);
    }
