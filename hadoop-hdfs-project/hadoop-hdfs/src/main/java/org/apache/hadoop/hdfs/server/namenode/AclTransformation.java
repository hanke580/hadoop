...
final class AclTransformation {
    ...
    public static List<AclEntry> mergeAclEntries(List<AclEntry> existingAcl, List<AclEntry> inAclSpec) throws AclException {
        ValidatedAclSpec aclSpec = ((ValidatedAclSpec) org.zlab.ocov.tracker.Runtime.update(new ValidatedAclSpec(inAclSpec), 106));
        ArrayList<AclEntry> aclBuilder = Lists.newArrayListWithCapacity(MAX_ENTRIES);
    ...
    private static void copyDefaultsIfNeeded(List<AclEntry> aclBuilder) {
        Collections.sort(aclBuilder, ACL_ENTRY_COMPARATOR);
        ScopedAclEntries scopedEntries = ((ScopedAclEntries) org.zlab.ocov.tracker.Runtime.update(new ScopedAclEntries(aclBuilder), 107));
        if (!scopedEntries.getDefaultEntries().isEmpty()) {
            List<AclEntry> accessEntries = scopedEntries.getAccessEntries();
            List<AclEntry> defaultEntries = scopedEntries.getDefaultEntries();
            ...
            for (AclEntryType type : EnumSet.of(USER, GROUP, OTHER)) {
                ...
                if (defaultEntryIndex < 0) {
                    ...
                    if (accessEntryIndex >= 0) {
                        copiedEntries.add(((org.apache.hadoop.fs.permission.AclEntry.Builder) org.zlab.ocov.tracker.Runtime.update(new AclEntry.Builder().setScope(DEFAULT).setType(type).setPermission(accessEntries.get(accessEntryIndex).getPermission()), 108)).build());
                    }
                }
            }
