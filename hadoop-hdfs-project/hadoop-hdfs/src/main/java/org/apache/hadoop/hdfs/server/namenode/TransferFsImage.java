...
public class TransferFsImage {
    ...
    public enum TransferResult {
        ...
        SUCCESS(HttpServletResponse.SC_OK, false), AUTHENTICATION_FAILURE(HttpServletResponse.SC_FORBIDDEN, true), NOT_ACTIVE_NAMENODE_FAILURE(HttpServletResponse.SC_EXPECTATION_FAILED, false), OLD_TRANSACTION_ID_FAILURE(HttpServletResponse.SC_CONFLICT, false), UNEXPECTED_FAILURE(-1, true);
     ...
     */
    static TransferResult uploadImageFromStorage(URL fsName, Configuration conf, NNStorage storage, NameNodeFile nnf, long txid) throws IOException {
   ...
   */
    private static void uploadImage(URL url, Configuration conf, NNStorage storage, NameNodeFile nnf, long txId, Canceler canceler) throws IOException {
        ...
        try {
            ...
            int chunkSize = (int) conf.getLongBytes(DFSConfigKeys.DFS_IMAGE_TRANSFER_CHUNKSIZE_KEY, DFSConfigKeys.DFS_IMAGE_TRANSFER_CHUNKSIZE_DEFAULT);
    ...
    private static void copyFileToStream(OutputStream out, File localfile, FileInputStream infile, DataTransferThrottler throttler, Canceler canceler) throws IOException {
        ...
        try {
            ...
            reportStr += " Sent total: " + total + " bytes. Size of last segment intended to send: " + num + " bytes.";
            if (ioe != null) {
