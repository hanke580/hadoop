...
class StripedBlockReader {
    ...
    private BlockReader createBlockReader(long offsetInBlock) {
        if ((org.zlab.ocov.tracker.Runtime.updateBranch(offsetInBlock, block.getNumBytes(), ">=", 29))) {
            return null;
        }
        Peer peer = null;
        try {
         ...
         */
            ...
            return BlockReaderRemote.newBlockReader("dummy", block, blockToken, offsetInBlock, block.getNumBytes() - offsetInBlock, true, "", peer, source, null, stripedReader.getCachingStrategy(), -1);
        } catch (IOException e) {
