...
class FSImagePreTransactionalStorageInspector extends FSImageStorageInspector {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(FSImagePreTransactionalStorageInspector.class);
   ...
   * fstime */
    ...
    List<FSImageFile> getLatestImages() throws IOException {
        ...
        FSImageFile file = new FSImageFile(latestNameSD, NNStorage.getStorageFile(latestNameSD, NameNodeFile.IMAGE), HdfsServerConstants.INVALID_TXID);
    ...
    static Iterable<EditLogInputStream> getEditLogStreams(NNStorage storage) throws IOException {
        ...
        for (File f : inspector.getLatestEditsFiles()) {
            editStreams.add(((EditLogFileInputStream) org.zlab.ocov.tracker.Runtime.update(new EditLogFileInputStream(f), 82)));
        }
        return editStreams;
    }
}
