...
public class StoragePolicyAdmin extends Configured implements Tool {
    ...
    private static class GetStoragePolicyCommand implements AdminHelper.Command {
        ...
        public int run(Configuration conf, List<String> args) throws IOException {
            ...
            try {
                ...
                System.err.println(getName() + " is not supported for filesystem " + fs.getScheme() + " on path " + path);
                return 2;
            } catch (Exception e) {
                System.err.println(AdminHelper.prettifyException(e));
                return 2;
    ...
    private static class SatisfyStoragePolicyCommand implements AdminHelper.Command {
        ...
        public int run(Configuration conf, List<String> args) throws IOException {
            ...
            try {
                ...
                System.out.println("Scheduled blocks to move based on the current" + " storage policy on " + path);
            } catch (Exception e) {
                System.err.println(AdminHelper.prettifyException(e));
                return 2;
            }
    ...
    private static class UnsetStoragePolicyCommand implements AdminHelper.Command {
        ...
        public String getLongUsage() {
            TableListing listing = AdminHelper.getOptionDescriptionListing();
            listing.addRow("<path>", "The path of the file/directory " + "from which the storage policy will be unset.");
