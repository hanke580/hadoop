...
public abstract class Command extends Configured implements Closeable {
    ...
    protected Set<String> getNodeList(String listArg) throws IOException {
        ...
        if (listArg.startsWith("file://")) {
            ...
            try {
                HostsFileReader.readFileToSet("include", Paths.get(listURL.getPath()).toString(), resultSet);
     ...
     */
    protected int parseTopNodes(final CommandLine cmd, final TextStringBuilder result) throws IllegalArgumentException {
        String outputLine = "";
        int nodes = 0;
