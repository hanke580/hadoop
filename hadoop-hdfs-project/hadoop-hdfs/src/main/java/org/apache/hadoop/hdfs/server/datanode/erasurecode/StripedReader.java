...
class StripedReader {
    ...
    StripedReader(StripedReconstructor reconstructor, DataNode datanode, Configuration conf, StripedReconstructionInfo stripedReconInfo) {
        ...
        int cellsNum = (int) ((stripedReconInfo.getBlockGroup().getNumBytes() - 1) / stripedReconInfo.getEcPolicy().getCellSize() + 1);
        ...
        // or targets.
        xmits = Math.max(minRequiredSources, stripedReconInfo.getTargets() != null ? stripedReconInfo.getTargets().length : 0);
    ...
    StripedBlockReader createReader(int idxInSources, long offsetInBlock) {
        return new StripedBlockReader(this, datanode, conf, liveIndices[idxInSources], reconstructor.getBlock(liveIndices[idxInSources]), sources[idxInSources], offsetInBlock);
    }
    ...
    int[] doReadMinimumSources(int reconstructLength, CorruptedBlocks corruptedBlocks) throws IOException {
    ...
    private int scheduleNewRead(BitSet used, int reconstructLength, CorruptedBlocks corruptedBlocks) {
        ...
        for (int i = 0; reader == null && i < readers.size(); i++) {
            if (!used.get(i)) {
                ...
                if ((org.zlab.ocov.tracker.Runtime.updateBranch(toRead, 0, ">", 28))) {
                    stripedReader.closeBlockReader();
                    org.zlab.ocov.tracker.Runtime.update(stripedReader, 122);
    ...
    private void clearFuturesAndService() {
        while (!futures.isEmpty()) {
            try {
                Future<BlockReadStats> future = readService.poll(stripedReadTimeoutInMills, TimeUnit.MILLISECONDS);
