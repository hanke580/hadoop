...
class RamDiskAsyncLazyPersistService {
    ...
    synchronized void execute(String storageId, Runnable task) {
        try {
            ...
            if (task instanceof ReplicaLazyPersistTask) {
                IOUtils.cleanupWithLogger(null, ((ReplicaLazyPersistTask) task).targetVolume);
            }
            throw re;
        }
    }
    ...
    void submitLazyPersistTask(String bpId, long blockId, long genStamp, long creationTime, ReplicaInfo replica, FsVolumeReference target) throws IOException {
        ...
        ReplicaLazyPersistTask lazyPersistTask = ((ReplicaLazyPersistTask) org.zlab.ocov.tracker.Runtime.update(new ReplicaLazyPersistTask(bpId, blockId, genStamp, creationTime, replica, target), 130));
    ...
    class ReplicaLazyPersistTask implements Runnable {
        ...
        public String toString() {
            ...
            return "LazyWriter async task of persist RamDisk block pool id:" + bpId + " block pool id: " + blockId + " with block file " + replicaInfo.getBlockURI() + " and meta file " + replicaInfo.getMetadataURI() + " to target volume " + targetVolume;
        }
