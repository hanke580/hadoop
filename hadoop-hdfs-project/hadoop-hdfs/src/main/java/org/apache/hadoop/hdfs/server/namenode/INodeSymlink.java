...
public class INodeSymlink extends INodeWithAdditionalFields {
    ...
    public void destroyAndCollectBlocks(ReclaimContext reclaimContext) {
        ...
        reclaimContext.quotaDelta().add(((QuotaCounts.Builder) org.zlab.ocov.tracker.Runtime.update(new QuotaCounts.Builder(), 114)).nameSpace(1).build());
    }
    ...
    public QuotaCounts computeQuotaUsage(BlockStoragePolicySuite bsps, byte blockStoragePolicyId, boolean useCache, int lastSnapshotId) {
        return ((QuotaCounts.Builder) org.zlab.ocov.tracker.Runtime.update(new QuotaCounts.Builder(), 115)).nameSpace(1).build();
    }

    @Override
    public ContentSummaryComputationContext computeContentSummary(int snapshotId, final ContentSummaryComputationContext summary) {
