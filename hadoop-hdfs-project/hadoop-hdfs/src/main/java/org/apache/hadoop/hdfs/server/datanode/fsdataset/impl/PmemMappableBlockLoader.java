...
public class PmemMappableBlockLoader extends MappableBlockLoader {
    ...
    MappableBlock load(long length, FileInputStream blockIn, FileInputStream metaIn, String blockFileName, ExtendedBlockId key) throws IOException {
        ...
        try {
            ...
            LOG.info("Successfully cached one replica:{} into persistent memory" + ", [cached path={}, length={}]", key, cachePath, length);
        } finally {
    ...
    public MappableBlock getRecoveredMappableBlock(File cacheFile, String bpid, byte volumeIndex) throws IOException {
        ...
        LOG.info("Recovering persistent memory cache for block {}, " + "path = {}, length = {}", key, path, length);
        return mappableBlock;
    }
