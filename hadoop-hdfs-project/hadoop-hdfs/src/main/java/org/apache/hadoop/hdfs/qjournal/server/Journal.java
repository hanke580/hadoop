...
public class Journal implements Closeable {
    ...
    private JournaledEditsCache createCache() {
        if (conf.getBoolean(DFSConfigKeys.DFS_HA_TAILEDITS_INPROGRESS_KEY, DFSConfigKeys.DFS_HA_TAILEDITS_INPROGRESS_DEFAULT)) {
    ...
    private synchronized EditLogFile scanStorageForLatestEdits() throws IOException {
        ...
        while (!files.isEmpty()) {
            ...
            if (latestLog.getLastTxId() == HdfsServerConstants.INVALID_TXID) {
                ...
                LOG.warn("Latest log " + latestLog + " has no transactions. " + "moving it aside and looking for previous log" + " ; journal id: " + journalId);
    ...
    void format(NamespaceInfo nsInfo, boolean force) throws IOException {
        ...
        LOG.info("Formatting journal id : " + journalId + " with namespace info: " + nsInfo + " and force: " + force);
    ...
    synchronized NewEpochResponseProto newEpoch(NamespaceInfo nsInfo, long epoch) throws IOException {
        ...
        if (epoch <= getLastPromisedEpoch()) {
            throw new IOException("Proposed epoch " + epoch + " <= last promise " + getLastPromisedEpoch() + " ; journal id: " + journalId);
        }
    ...
    synchronized void journal(RequestInfo reqInfo, long segmentTxId, long firstTxnId, int numTxns, byte[] records) throws IOException {
        ...
        if (curSegmentTxId != segmentTxId) {
            ...
            JournalOutOfSyncException e = new JournalOutOfSyncException("Writer out of sync: it thinks it is writing segment " + segmentTxId + " but current segment is " + curSegmentTxId + " ; journal id: " + journalId);
            abortCurSegment();
            throw e;
        }
        checkSync(nextTxId == firstTxnId, "Can't write txid " + firstTxnId + " expecting nextTxId=" + nextTxId + " ; journal id: " + journalId);
        ...
        if (milliSeconds > WARN_SYNC_MILLIS_THRESHOLD) {
            LOG.warn("Sync of transaction range " + firstTxnId + "-" + lastTxnId + " took " + milliSeconds + "ms" + " ; journal id: " + journalId);
        }
        if (isLagging) {
    ...
    private synchronized void checkRequest(RequestInfo reqInfo) throws IOException {
        ...
        if (reqInfo.getEpoch() < lastPromisedEpoch.get()) {
            throw new IOException("IPC's epoch " + reqInfo.getEpoch() + " is less than the last promised epoch " + lastPromisedEpoch.get() + " ; journal id: " + journalId);
        ...
        checkSync(reqInfo.getIpcSerialNumber() > currentEpochIpcSerial, "IPC serial %s from client %s was not higher than prior highest " + "IPC serial %s ; journal id: %s", reqInfo.getIpcSerialNumber(), Server.getRemoteIp(), currentEpochIpcSerial, journalId);
        currentEpochIpcSerial = reqInfo.getIpcSerialNumber();
        if (reqInfo.hasCommittedTxId()) {
            Preconditions.checkArgument(reqInfo.getCommittedTxId() >= committedTxnId.get(), "Client trying to move committed txid backward from " + committedTxnId.get() + " to " + reqInfo.getCommittedTxId() + " ; journal id: " + journalId);
    ...
    private synchronized void checkWriteRequest(RequestInfo reqInfo) throws IOException {
        ...
        if (reqInfo.getEpoch() != lastWriterEpoch.get()) {
            throw new IOException("IPC's epoch " + reqInfo.getEpoch() + " is not the current writer epoch  " + lastWriterEpoch.get() + " ; journal id: " + journalId);
        }
    }
    ...
    private void checkFormatted() throws JournalNotFormattedException {
        if (!isFormatted()) {
            throw new JournalNotFormattedException("Journal " + storage.getSingularStorageDir() + " not formatted" + " ; journal id: " + journalId);
        }
    }
    ...
    public synchronized void startLogSegment(RequestInfo reqInfo, long txid, int layoutVersion) throws IOException {
        ...
        checkRequest(reqInfo);
        if (curSegment != null) {
            LOG.warn("Client is requesting a new log segment " + txid + " though we are already writing " + curSegment + ". " + "Aborting the current segment in order to begin the new one." + " ; journal id: " + journalId);
        ...
        if (existing != null) {
            if (!existing.isInProgress()) {
                throw new IllegalStateException("Already have a finalized segment " + existing + " beginning at " + txid + " ; journal id: " + journalId);
            }
            ...
            if (existing.getLastTxId() != existing.getFirstTxId()) {
                throw new IllegalStateException("The log file " + existing + " seems to contain valid transactions" + " ; journal id: " + journalId);
            }
        }
        long curLastWriterEpoch = lastWriterEpoch.get();
        if (curLastWriterEpoch != reqInfo.getEpoch()) {
            LOG.info("Updating lastWriterEpoch from " + curLastWriterEpoch + " to " + reqInfo.getEpoch() + " for client " + Server.getRemoteIp() + " ; journal id: " + journalId);
    ...
    public synchronized void finalizeLogSegment(RequestInfo reqInfo, long startTxId, long endTxId) throws IOException {
        ...
        if (startTxId == curSegmentTxId) {
            ...
            checkSync(nextTxId == endTxId + 1, "Trying to finalize in-progress log segment %s to end at " + "txid %s but only written up to txid %s ; journal id: %s", startTxId, endTxId, nextTxId - 1, journalId);
        ...
        if (elf == null) {
            throw new JournalOutOfSyncException("No log file to finalize at " + "transaction ID " + startTxId + " ; journal id: " + journalId);
        }
        if (elf.isInProgress()) {
            if (needsValidation) {
                LOG.info("Validating log segment " + elf.getFile() + " about to be " + "finalized ; journal id: " + journalId);
                elf.scanLog(Long.MAX_VALUE, false);
                checkSync(elf.getLastTxId() == endTxId, "Trying to finalize in-progress log segment %s to end at " + "txid %s but log %s on disk only contains up to txid %s " + "; journal id: %s", startTxId, endTxId, elf.getFile(), elf.getLastTxId(), journalId);
            }
            fjm.finalizeLogSegment(startTxId, endTxId);
        } else {
            Preconditions.checkArgument(endTxId == elf.getLastTxId(), "Trying to re-finalize already finalized log " + elf + " with different endTxId " + endTxId + " ; journal id: " + journalId);
        }
    ...
    private void purgePaxosDecision(long segmentTxId) throws IOException {
        ...
        if (paxosFile.exists()) {
            if (!paxosFile.delete()) {
                throw new IOException("Unable to delete paxos file " + paxosFile + " ; journal id: " + journalId);
            }
        }
    }
     ...
     */
    public GetJournaledEditsResponseProto getJournaledEdits(long sinceTxId, int maxTxns) throws IOException {
        if (cache == null) {
            throw new IOException("The journal edits cache is not enabled, which " + "is a requirement to fetch journaled edits via RPC. Please enable " + "it via " + DFSConfigKeys.DFS_HA_TAILEDITS_INPROGRESS_KEY);
        }
        ...
        try {
            ...
            return GetJournaledEditsResponseProto.newBuilder().setTxnCount(txnCount).setEditLog(output.toByteString()).build();
    ...
    SegmentStateProto getSegmentInfo(long segmentTxId) throws IOException {
        ...
        if (elf.getLastTxId() == HdfsServerConstants.INVALID_TXID) {
            LOG.info("Edit log file " + elf + " appears to be empty. " + "Moving it aside..." + " ; journal id: " + journalId);
    ...
    public synchronized void acceptRecovery(RequestInfo reqInfo, SegmentStateProto segment, URL fromUrl) throws IOException {
        ...
        Preconditions.checkArgument(segment.getEndTxId() > 0 && segment.getEndTxId() >= segmentTxId, "bad recovery state for segment %s: %s ; journal id: %s", segmentTxId, TextFormat.shortDebugString(segment), journalId);
        ...
        if (currentSegment == null || currentSegment.getEndTxId() != segment.getEndTxId()) {
            if (currentSegment == null) {
                LOG.info("Synchronizing log " + TextFormat.shortDebugString(segment) + ": no current segment in place ; journal id: " + journalId);
                // Update the highest txid for lag metrics
                updateHighestWrittenTxId(Math.max(segment.getEndTxId(), highestWrittenTxId));
            } else {
                LOG.info("Synchronizing log " + TextFormat.shortDebugString(segment) + ": old segment " + TextFormat.shortDebugString(currentSegment) + " is not the right length ; journal id: " + journalId);
                ...
                if (txnRange(currentSegment).contains(committedTxnId.get()) && !txnRange(segment).contains(committedTxnId.get())) {
                    throw new AssertionError("Cannot replace segment " + TextFormat.shortDebugString(currentSegment) + " with new segment " + TextFormat.shortDebugString(segment) + ": would discard already-committed txn " + committedTxnId.get() + " ; journal id: " + journalId);
                }
                // Another paranoid check: we should not be asked to synchronize a log
                // on top of a finalized segment.
                alwaysAssert(currentSegment.getIsInProgress(), "Should never be asked to synchronize a different log on top of " + "an already-finalized segment ; journal id: " + journalId);
            ...
            LOG.info("Skipping download of log " + TextFormat.shortDebugString(segment) + ": already have up-to-date logs ; journal id: " + journalId);
        }
        ...
        LOG.info("Accepted recovery for segment " + segmentTxId + ": " + TextFormat.shortDebugString(newData) + " ; journal id: " + journalId);
    }

    private Range<Long> txnRange(SegmentStateProto seg) {
        Preconditions.checkArgument(seg.hasEndTxId(), "invalid segment: %s ; journal id: %s", seg, journalId);
    ...
    private void completeHalfDoneAcceptRecovery(PersistedRecoveryPaxosData paxosData) throws IOException {
        ...
        if (tmp.exists()) {
            ...
            LOG.info("Rolling forward previously half-completed synchronization: " + tmp + " -> " + dst + " ; journal id: " + journalId);
    ...
    synchronized boolean moveTmpSegmentToCurrent(File tmpFile, File finalFile, long endTxId) throws IOException {
        ...
        if (endTxId <= committedTxnId.get()) {
            if (!finalFile.getParentFile().exists()) {
                LOG.error(finalFile.getParentFile() + " doesn't exist. Aborting tmp " + "segment move to current directory ; journal id: " + journalId);
                return false;
            }
            ...
            if (finalFile.exists() && FileUtil.canRead(finalFile)) {
                ...
                success = false;
                LOG.warn("Unable to move edits file from " + tmpFile + " to " + finalFile + " ; journal id: " + journalId);
            }
        } else {
            success = false;
            LOG.error("The endTxId of the temporary file is not less than the " + "last committed transaction id. Aborting move to final file" + finalFile + " ; journal id: " + journalId);
        }
        return success;
    }
