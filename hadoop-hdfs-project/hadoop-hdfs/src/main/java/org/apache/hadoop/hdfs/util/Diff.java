...
public class Diff<K, E extends Diff.Element<K>> {
    ...
    private void addCreated(final E element, final int i) {
        if (i >= 0) {
            throw new AssertionError("Element already exists: element=" + element + ", created=" + created);
        }
        if (created == null) {
    ...
    private void addDeleted(final E element, final int i) {
        if (i >= 0) {
            throw new AssertionError("Element already exists: element=" + element + ", deleted=" + deleted);
        }
        if (deleted == null) {
    ...
    public void combinePosterior(final Diff<K, E> posterior, final Processor<E> deletedProcesser) {
        ...
        for (; c != null || d != null; ) {
            ...
            if (cmp < 0) {
                ...
                final UndoInfo<E> ui = ((org.apache.hadoop.hdfs.util.Diff.UndoInfo) org.zlab.ocov.tracker.Runtime.update(delete(d), 131));
                ...
                final UndoInfo<E> ui = ((org.apache.hadoop.hdfs.util.Diff.UndoInfo) org.zlab.ocov.tracker.Runtime.update(modify(d, c), 132));
                if (deletedProcesser != null) {
                    deletedProcesser.process(ui.trashed);
                }
                c = createdIterator.hasNext() ? createdIterator.next() : null;
    ...
    public String toString() {
        return getClass().getSimpleName() + "{created=" + getCreatedUnmodifiable() + ", deleted=" + getDeletedUnmodifiable() + "}";
    }
}
