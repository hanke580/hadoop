...
class ProvidedVolumeImpl extends FsVolumeImpl {
    ...
    static class ProvidedBlockPoolSlice {
        ...
        ProvidedBlockPoolSlice(String bpid, ProvidedVolumeImpl volume, Configuration conf) {
            ...
            Class<? extends BlockAliasMap> fmt = conf.getClass(DFSConfigKeys.DFS_PROVIDED_ALIASMAP_CLASS, TextFileRegionAliasMap.class, BlockAliasMap.class);
        ...
        void fetchVolumeMap(ReplicaMap volumeMap, RamDiskReplicaTracker ramDiskReplicaMap, FileSystem remoteFS) throws IOException {
            ...
            if (reader == null) {
                LOG.error("Got null reader from BlockAliasMap " + aliasMap + "; no blocks will be populated");
                return;
            }
            Path blockPrefixPath = new Path(providedVolume.getBaseURI());
            for (FileRegion region : reader) {
                if (containsBlock(providedVolume.baseURI, region.getProvidedStorageLocation().getPath().toUri())) {
                    String blockSuffix = getSuffix(blockPrefixPath, new Path(region.getProvidedStorageLocation().getPath().toUri()));
                    ...
                    ReplicaInfo newReplica = new ReplicaBuilder(ReplicaState.FINALIZED).setBlockId(region.getBlock().getBlockId()).setPathPrefix(blockPrefixPath).setPathSuffix(blockSuffix).setOffset(region.getProvidedStorageLocation().getOffset()).setLength(region.getBlock().getNumBytes()).setGenerationStamp(region.getBlock().getGenerationStamp()).setPathHandle(pathHandle).setFsVolume(providedVolume).setConf(conf).setRemoteFS(remoteFS).build();
                    ...
                    if (oldReplica == null) {
                        ...
                        LOG.warn("A block with id " + newReplica.getBlockId() + " exists locally. Skipping PROVIDED replica");
                    }
                }
            }
        }
        ...
        public void compileReport(Collection<ScanInfo> report, ReportCompiler reportCompiler) throws IOException, InterruptedException {
       ...
       */
            ...
            for (FileRegion region : reader) {
                ...
                report.add(new ScanInfo(region.getBlock().getBlockId(), providedVolume, region, region.getProvidedStorageLocation().getLength()));
            }
        }
    ...
    ProvidedVolumeImpl(FsDatasetImpl dataset, String storageID, StorageDirectory sd, FileIoProvider fileIoProvider, Configuration conf) throws IOException {
        super(dataset, storageID, sd, fileIoProvider, conf, null);
        assert getStorageLocation().getStorageType() == StorageType.PROVIDED : "Only provided storages must use ProvidedVolume";
    ...
    private static final ObjectReader READER = new ObjectMapper().reader(ProvidedBlockIteratorState.class);
    ...
    private class ProviderBlockIteratorImpl implements FsVolumeSpi.BlockIterator {
        ...
        public void load() throws IOException {
            ...
            rewind();
            LOG.trace("load({}, {}): loaded iterator {}: {}", getStorageID(), bpid, name, WRITER.writeValueAsString(state));
        }
    }
    ...
    public BlockIterator loadBlockIterator(String bpid, String name) throws IOException {
        ProviderBlockIteratorImpl iter = new ProviderBlockIteratorImpl(bpid, name, bpSlices.get(bpid).getBlockAliasMap());
        iter.load();
        return iter;
    }
