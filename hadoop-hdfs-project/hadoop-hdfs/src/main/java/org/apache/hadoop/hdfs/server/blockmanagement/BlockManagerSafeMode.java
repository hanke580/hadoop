...
class BlockManagerSafeMode {
    ...
    BlockManagerSafeMode(BlockManager blockManager, Namesystem namesystem, boolean haEnabled, Configuration conf) {
        ...
        this.extension = conf.getTimeDuration(DFS_NAMENODE_SAFEMODE_EXTENSION_KEY, DFS_NAMENODE_SAFEMODE_EXTENSION_DEFAULT, MILLISECONDS);
    ...
    synchronized void decrementSafeBlockCount(BlockInfo b) {
        ...
        if (storedBlock.isComplete() && blockManager.countNodes(b).liveReplicas() == safeReplication - 1) {
            this.blockSafe--;
            assert blockSafe >= 0;
            checkSafeMode();
        }
