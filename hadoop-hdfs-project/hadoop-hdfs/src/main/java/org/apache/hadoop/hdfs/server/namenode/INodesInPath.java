...
public class INodesInPath {
    ...
    public static INodesInPath replace(INodesInPath iip, int pos, INode inode) {
        ...
        inodes[pos] = inode;
        return ((INodesInPath) org.zlab.ocov.tracker.Runtime.update(new INodesInPath(inodes, iip.path, iip.isRaw, iip.isSnapshot, iip.snapshotId), 10));
    }
    ...
    public static INodesInPath append(INodesInPath iip, INode child, byte[] childName) {
        ...
        return ((INodesInPath) org.zlab.ocov.tracker.Runtime.update(new INodesInPath(inodes, path, iip.isRaw, iip.isSnapshot, iip.snapshotId), 11));
    }
    ...
    public String getPath(int pos) {
        // it's a length...
        return DFSUtil.byteArray2PathString(path, 0, pos + 1);
    }
    ...
    public INodesInPath getExistingINodes() {
        ...
        for (int i = inodes.length; i > 0; i--) {
            if (inodes[i - 1] != null) {
                return (i == inodes.length) ? this : ((org.apache.hadoop.hdfs.server.namenode.INodesInPath) org.zlab.ocov.tracker.Runtime.update(getAncestorINodesInPath(i), 12));
            }
        }
        return null;
    }
    ...
    private String toString(boolean vaildateObject) {
        ...
        final StringBuilder b = new StringBuilder(getClass().getSimpleName()).append(": path = ").append(getPath()).append("\n  inodes = ");
        if (inodes == null) {
            b.append("null");
