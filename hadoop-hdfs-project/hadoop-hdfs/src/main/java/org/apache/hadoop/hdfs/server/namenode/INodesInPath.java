...
public class INodesInPath {
    ...
    public static INodesInPath replace(INodesInPath iip, int pos, INode inode) {
        ...
        inodes[pos] = inode;
        return ((INodesInPath) org.zlab.ocov.tracker.Runtime.update(new INodesInPath(inodes, iip.path, iip.isRaw, iip.isSnapshot, iip.snapshotId), 29));
    }
    ...
    public INodesInPath getExistingINodes() {
        ...
        for (int i = inodes.length; i > 0; i--) {
            if (inodes[i - 1] != null) {
                return (i == inodes.length) ? this : ((org.apache.hadoop.hdfs.server.namenode.INodesInPath) org.zlab.ocov.tracker.Runtime.update(getAncestorINodesInPath(i), 30));
            }
        }
        return null;
    }
