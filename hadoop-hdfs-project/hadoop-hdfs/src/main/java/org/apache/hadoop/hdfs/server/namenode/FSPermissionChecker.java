...
public class FSPermissionChecker implements AccessControlEnforcer {
    ...
    private String toAccessControlString(INodeAttributes inodeAttrib, String path, FsAction access, boolean deniedFromAcl) {
        StringBuilder sb = new StringBuilder("Permission denied: ").append("user=").append(getUser()).append(", ").append("access=").append(access).append(", ").append("inode=\"").append(path).append("\":").append(inodeAttrib.getUserName()).append(':').append(inodeAttrib.getGroupName()).append(':').append(inodeAttrib.isDirectory() ? 'd' : '-').append(inodeAttrib.getFsPermission());
        if (deniedFromAcl) {
            sb.append("+");
        }
        return sb.toString();
    ...
    protected FSPermissionChecker(String fsOwner, String supergroup, UserGroupInformation callerUgi, INodeAttributeProvider attributeProvider, boolean useAuthorizationWithContextAPI) {
    ...
    private AccessControlEnforcer getAccessControlEnforcer() {
        return (attributeProvider != null) ? attributeProvider.getExternalAccessControlEnforcer(this) : this;
    }
    ...
    void checkPermission(INodesInPath inodesInPath, boolean doCheckOwner, FsAction ancestorAccess, FsAction parentAccess, FsAction access, FsAction subAccess, boolean ignoreEmptyDir) throws AccessControlException {
        ...
        try {
            if (this.authorizeWithContext && opType != null) {
                ...
                builder.fsOwner(fsOwner).supergroup(supergroup).callerUgi(callerUgi).inodeAttrs(inodeAttrs).inodes(inodes).pathByNameArr(components).snapshotId(snapshotId).path(path).ancestorIndex(ancestorIndex).doCheckOwner(doCheckOwner).ancestorAccess(ancestorAccess).parentAccess(parentAccess).access(access).subAccess(subAccess).ignoreEmptyDir(ignoreEmptyDir).operationName(opType).callerContext(CallerContext.getCurrent());
            ...
            if (exceptionClass.equals(AccessControlException.class) || exceptionClass.equals(TraverseAccessControlException.class)) {
                throw ace;
            }
    ...
    void checkPermission(INode inode, int snapshotId, FsAction access) throws AccessControlException {
        ...
        INodeAttributes nodeAttributes = getINodeAttrs(pathComponents, pathComponents.length - 1, inode, snapshotId);
        try {
    ...
    public void checkPermission(String fsOwner, String supergroup, UserGroupInformation callerUgi, INodeAttributes[] inodeAttrs, INode[] inodes, byte[][] components, int snapshotId, String path, int ancestorIndex, boolean doCheckOwner, FsAction ancestorAccess, FsAction parentAccess, FsAction access, FsAction subAccess, boolean ignoreEmptyDir) throws AccessControlException {
        ...
        if ((org.zlab.ocov.tracker.Runtime.updateBranch(parentAccess != null && parentAccess.implies(FsAction.WRITE) && inodeAttrs.length > 1 && last != null, 26))) {
    ...
    public void checkPermissionWithContext(INodeAttributeProvider.AuthorizationContext authzContext) throws AccessControlException {
        ...
        this.checkPermission(authzContext.getFsOwner(), authzContext.getSupergroup(), authzContext.getCallerUgi(), authzContext.getInodeAttrs(), authzContext.getInodes(), authzContext.getPathByNameArr(), authzContext.getSnapshotId(), authzContext.getPath(), authzContext.getAncestorIndex(), authzContext.isDoCheckOwner(), authzContext.getAncestorAccess(), authzContext.getParentAccess(), authzContext.getAccess(), authzContext.getSubAccess(), authzContext.isIgnoreEmptyDir());
    }
    ...
    private void checkOwner(INodeAttributes[] inodes, byte[][] components, int i) throws AccessControlException {
        ...
        throw new AccessControlException("Permission denied. user=" + getUser() + " is not the owner of inode=" + getPath(components, 0, i));
    }
                ...
                if (inodeAttr.getFsPermission().getStickyBit()) {
                    for (INode child : cList) {
                        ...
                        if (isStickyBitViolated(inodeAttr, childInodeAttr)) {
                            ...
                            throwStickyBitException(getPath(allComponents, 0, index + 1), child, getPath(allComponents, 0, index), inode);
                        }
                    }
                }
            }
    ...
    private void check(INodeAttributes[] inodes, byte[][] components, int i, FsAction access) throws AccessControlException {
        ...
        if (inode != null && !hasPermission(inode, access)) {
            throw new AccessControlException(toAccessControlString(inode, getPath(components, 0, i), access));
        }
    }
    ...
    private void checkStickyBit(INodeAttributes[] inodes, byte[][] components, int index) throws AccessControlException {
        ...
        throwStickyBitException(getPath(components, 0, index + 1), inode, getPath(components, 0, index), parent);
    }
    ...
    public void checkPermission(CachePool pool, FsAction access) throws AccessControlException {
        ...
        if (!getUser().equals(pool.getOwnerName()) && !isMemberOfGroup(pool.getGroupName()) && mode.getOtherAction().implies(access)) {
            return;
        }
    ...
    private static void checkNotSymlink(INode inode, byte[][] components, int i) throws UnresolvedPathException {
        if (inode != null && inode.isSymlink()) {
            ...
            if (LOG.isDebugEnabled()) {
                ...
                LOG.debug("UnresolvedPathException " + " path: " + path + " preceding: " + preceding + " count: " + i + " link: " + link + " target: " + target + " remainder: " + remainder);
            }
