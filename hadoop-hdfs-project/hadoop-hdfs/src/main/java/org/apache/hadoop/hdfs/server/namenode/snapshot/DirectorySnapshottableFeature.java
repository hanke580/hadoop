...
public class DirectorySnapshottableFeature extends DirectoryWithSnapshotFeature {
     ...
     */
    public Snapshot addSnapshot(INodeDirectory snapshotRoot, int id, String name, final LeaseManager leaseManager, final boolean captureOpenFiles, int maxSnapshotLimit, long now) throws SnapshotException {
        //check snapshot quota
        final int n = getNumSnapshots();
        try {
            this.left_132 = (n + 1);
            this.right_132 = (snapshotQuota);
        } catch (Exception e) {
            this.left_132 = 0L;
            this.right_132 = 0L;
        }
     ...
     *         exists.
     */
    public Snapshot removeSnapshot(INode.ReclaimContext reclaimContext, INodeDirectory snapshotRoot, String snapshotName, long now) throws SnapshotException {
    ...
    @Override
    public void computeContentSummary4Snapshot(final BlockStoragePolicySuite bsps, final ContentCounts counts) throws AccessControlException {
     ...
     */
    SnapshotDiffInfo computeDiff(final INodeDirectory snapshotRootDir, final INodeDirectory snapshotDiffScopeDir, final String from, final String to) throws SnapshotException {
     ...
     *           as a previous snapshot.
     */
    SnapshotDiffListingInfo computeDiff(final INodeDirectory snapshotRootDir, final INodeDirectory snapshotDiffScopeDir, final String from, final String to, byte[] startPath, int index, int snapshotDiffReportEntriesLimit) throws SnapshotException {
        Preconditions.checkArgument(snapshotDiffScopeDir.isDescendantOfSnapshotRoot(snapshotRootDir));
        Snapshot fromSnapshot = getSnapshotByName(snapshotRootDir, from);
        Snapshot toSnapshot = getSnapshotByName(snapshotRootDir, to);
        ...
        computeDiffRecursively(snapshotDiffScopeDir, snapshotDiffScopeDir, new ArrayList<byte[]>(), diffs, resumePath, 0, toProcess);
        return diffs;
    }
     ...
     */
    private void computeDiffRecursively(final INodeDirectory snapshotDir, INode node, List<byte[]> parentPath, SnapshotDiffInfo diffReport) {
        ...
        if (node.isDirectory()) {
            ...
            for (INode child : children) {
                ...
                if (!toProcess && child instanceof INodeReference.WithName) {
                    byte[][] renameTargetPath = findRenameTargetPath(snapshotDir, (WithName) child, laterSnapshot == null ? Snapshot.CURRENT_STATE_ID : laterSnapshot.getId());
     ...
     */
    private boolean computeDiffRecursively(final INodeDirectory snapshotDir, INode node, List<byte[]> parentPath, SnapshotDiffListingInfo diffReport, final byte[][] resume, int level, boolean processFlag) {
        ...
        if (node.isDirectory()) {
            ...
            for (INode child : children) {
                ...
                boolean toProcess = !diff.containsDeleted(name);
                if (!toProcess && child instanceof INodeReference.WithName) {
                    byte[][] renameTargetPath = findRenameTargetPath(snapshotDir, (WithName) child, Snapshot.getSnapshotId(later));
                    if (renameTargetPath != null) {
                        toProcess = true;
                    }
                }
                if (toProcess) {
                    parentPath.add(name);
                    processFlag = computeDiffRecursively(snapshotDir, child, parentPath, diffReport, resume, level, processFlag);
                    parentPath.remove(parentPath.size() - 1);
                    if (!processFlag) {
                        return false;
                    }
            ...
            INodeFile file = node.asFile();
            boolean change = file.getFileWithSnapshotFeature().changedBetweenSnapshots(file, earlier, later);
            if (change) {
     ...
     */
    public byte[][] findRenameTargetPath(final INodeDirectory snapshotRoot, INodeReference.WithName wn, final int snapshotId) {
    ...
    private long right_132;

    private long left_129;

    private long right_131;

    private long right_133;

    private long right_130;

    private long left_131;

    private long left_130;

    private long right_129;

    private long left_133;

    private long left_132;
}
