...
public class FileWithSnapshotFeature implements INode.Feature {
    ...
    public void cleanFile(INode.ReclaimContext reclaimContext, final INodeFile file, final int snapshotId, int priorSnapshotId, byte storagePolicyId) {
        final int snapshotToBeDeleted = reclaimContext.getSnapshotIdToBeDeleted(snapshotId, file);
        if (snapshotId == Snapshot.CURRENT_STATE_ID) {
            // delete the current file while the file has snapshot feature
            if (!isCurrentFileDeleted() && snapshotToBeDeleted == Snapshot.CURRENT_STATE_ID) {
            ...
            org.zlab.ocov.tracker.Runtime.update(old, 6);
    ...
    public void updateQuotaAndCollectBlocks(INode.ReclaimContext reclaimContext, INodeFile file, FileDiff removed) {
        ...
        if (removed.snapshotINode != null) {
            ...
            org.zlab.ocov.tracker.Runtime.update(oldCounts, 7);
