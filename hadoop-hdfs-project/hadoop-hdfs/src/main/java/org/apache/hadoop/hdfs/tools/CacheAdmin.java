...
public class CacheAdmin extends Configured implements Tool {
    ...
    private static class AddCachePoolCommand implements AdminHelper.Command {
        ...
        @Override
        public String getShortUsage() {
            return "[" + NAME + " <name> [-owner <owner>] " + "[-group <group>] [-mode <mode>] [-limit <limit>] " + "[-defaultReplication <defaultReplication>] [-maxTtl <maxTtl>]" + "]\n";
        }
    ...
    private static class ModifyCachePoolCommand implements AdminHelper.Command {
        ...
        @Override
        public String getShortUsage() {
            return "[" + getName() + " <name> [-owner <owner>] " + "[-group <group>] [-mode <mode>] [-limit <limit>] " + "[-defaultReplication <defaultReplication>] [-maxTtl <maxTtl>]]\n";
        }
    ...
    private static class ListCachePoolsCommand implements AdminHelper.Command {
        ...
        public int run(Configuration conf, List<String> args) throws IOException {
            ...
            DistributedFileSystem dfs = AdminHelper.getDFS(conf);
            TableListing.Builder builder = new TableListing.Builder().addField("NAME", Justification.LEFT).addField("OWNER", Justification.LEFT).addField("GROUP", Justification.LEFT).addField("MODE", Justification.LEFT).addField("LIMIT", Justification.RIGHT).addField("MAXTTL", Justification.RIGHT).addField("DEFAULT_REPLICATION", Justification.RIGHT);
            if (printStats) {
