...
class DataXceiverServer implements Runnable {
        ...
        private boolean setMaxConcurrentMovers(final int newMaxThreads, final int duration) {
            ...
            try {
                ...
                if (acquired) {
                    ...
                    LOG.warn("Could not lower thread count to {} from {}. Too busy.", newMaxThreads, this.maxThreads);
                }
                return acquired;
    ...
    DataXceiverServer(PeerServer peerServer, Configuration conf, DataNode datanode) {
        ...
        Preconditions.checkArgument(this.maxXceiverCount >= 1, DFSConfigKeys.DFS_DATANODE_MAX_RECEIVER_THREADS_KEY + " should not be less than 1.");
        ...
        long bandwidthPerSec = conf.getLongBytes(DFSConfigKeys.DFS_DATANODE_DATA_TRANSFER_BANDWIDTHPERSEC_KEY, DFSConfigKeys.DFS_DATANODE_DATA_TRANSFER_BANDWIDTHPERSEC_DEFAULT);
        if (bandwidthPerSec > 0) {
            this.transferThrottler = new DataTransferThrottler(bandwidthPerSec);
        } else {
            this.transferThrottler = null;
        }
        bandwidthPerSec = conf.getLongBytes(DFSConfigKeys.DFS_DATANODE_DATA_WRITE_BANDWIDTHPERSEC_KEY, DFSConfigKeys.DFS_DATANODE_DATA_WRITE_BANDWIDTHPERSEC_DEFAULT);
        if (bandwidthPerSec > 0) {
