...
public class GreedyPlanner implements Planner {
    ...
    public NodePlan plan(DiskBalancerDataNode node) throws Exception {
        ...
        LOG.info("Compute Plan for Node : {}:{} took {} ms", node.getDataNodeName(), node.getDataNodePort(), endTime - startTime);
        return plan;
    }
    ...
    public void balanceVolumeSet(DiskBalancerDataNode node, DiskBalancerVolumeSet vSet, NodePlan plan) throws Exception {
        ...
        LOG.info("Disk Volume set {} - Type : {} plan completed.", currentSet.getSetID(), currentSet.getVolumes().get(0).getStorageType());
    ...
    private void skipVolume(DiskBalancerVolumeSet currentSet, DiskBalancerVolume volume) {
        if (LOG.isDebugEnabled()) {
            String message = String.format("Skipping volume. Volume : %s " + "Type : %s Target " + "Number of bytes : %f lowVolume dfsUsed : %d. Skipping this " + "volume from all future balancing calls.", volume.getPath(), volume.getStorageType(), currentSet.getIdealUsed() * volume.getCapacity(), volume.getUsed());
            LOG.debug(message);
        }
        volume.setSkip(true);
    }
