...
public class FSImageFormatPBSnapshot {
        ...
        private void loadFileDiffList(InputStream in, INodeFile file, int size) throws IOException {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (pbf.hasSnapshotCopy()) {
                    ...
                    copy = new INodeFileAttributes.SnapshotCopy(pbf.getName().toByteArray(), permission, acl, fileInPb.getModificationTime(), fileInPb.getAccessTime(), replication, ecPolicyID, fileInPb.getPreferredBlockSize(), (byte) fileInPb.getStoragePolicyID(), xAttrs, PBHelperClient.convert(fileInPb.getBlockType()));
                }
                ...
                for (int j = 0, e = bpl.size(); j < e; ++j) {
                    ...
                    if (storedBlock == null) {
                        storedBlock = (BlockInfoContiguous) fsn.getBlockManager().addBlockCollectionWithCheck(new BlockInfoContiguous(blk, copy.getFileReplication()), file);
                    }
    ...
    public final static class Saver {
        ...
        private INodeReferenceSection.INodeReference.Builder buildINodeReference(final INodeReference ref, final long refIndex) throws IOException {
        ...
        public void serializeSnapshotDiffSection(OutputStream out) throws IOException {
            ...
            parent.commitSectionAndSubSection(headers, FSImageFormatProtobuf.SectionName.SNAPSHOT_DIFF, FSImageFormatProtobuf.SectionName.SNAPSHOT_DIFF_SUB);
        }
            ...
            if (sf != null) {
                ...
                for (int i = diffList.size() - 1; i >= 0; i--) {
                    ...
                    for (INode d : deleted) {
                        ...
                        final int result = previousNode == null ? -1 : previousNode.compareTo(d.getLocalNameBytes());
                        if (result == 0) {
