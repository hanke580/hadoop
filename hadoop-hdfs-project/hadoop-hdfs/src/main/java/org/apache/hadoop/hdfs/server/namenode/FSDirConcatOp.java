...
class FSDirConcatOp {
    ...
    static FileStatus concat(FSDirectory fsd, FSPermissionChecker pc, String target, String[] srcs, boolean logRetryCache) throws IOException {
        validatePath(target, srcs);
        assert srcs != null;
        NameNode.stateChangeLog.debug("DIR* NameSystem.concat: {} to {}", Arrays.toString(srcs), target);
        final INodesInPath targetIIP = fsd.resolvePath(pc, target, DirOp.WRITE);
        org.zlab.ocov.tracker.Runtime.update(targetIIP, 127);
    ...
    private static QuotaCounts computeQuotaDeltas(FSDirectory fsd, INodeFile target, INodeFile[] srcList) {
        ...
        org.zlab.ocov.tracker.Runtime.update(deltas, 128);
            ...
            if (targetRepl != srcRepl) {
                ...
                if (bsp != null) {
                    ...
                    for (StorageType t : targetTypeChosen) {
                        if (t.supportTypeQuota()) {
                            deltas.addTypeSpace(t, fileSize);
                            org.zlab.ocov.tracker.Runtime.update(deltas, 129);
                        }
                    }
                }
            }
    ...
    static void unprotectedConcat(FSDirectory fsd, INodesInPath targetIIP, INodeFile[] srcList, long timestamp) throws IOException {
        ...
        NameNode.stateChangeLog.debug("DIR* NameSystem.concat to {}", targetIIP.getPath());
