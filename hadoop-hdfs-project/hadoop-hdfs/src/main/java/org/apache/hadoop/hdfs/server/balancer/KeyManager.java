...
public class KeyManager implements Closeable, DataEncryptionKeyFactory {
    ...
    public KeyManager(String blockpoolID, NamenodeProtocol namenode, boolean encryptDataTransfer, Configuration conf) throws IOException {
        ...
        if (isBlockTokenEnabled) {
            ...
            final boolean enableProtobuf = conf.getBoolean(DFSConfigKeys.DFS_BLOCK_ACCESS_TOKEN_PROTOBUF_ENABLE, DFSConfigKeys.DFS_BLOCK_ACCESS_TOKEN_PROTOBUF_ENABLE_DEFAULT);
            this.blockTokenSecretManager = new BlockTokenSecretManager(updateInterval, tokenLifetime, blockpoolID, encryptionAlgorithm, enableProtobuf);
     ...
     */
    public Token<BlockTokenIdentifier> getAccessToken(ExtendedBlock eb, StorageType[] storageTypes, String[] storageIds) throws IOException {
        if (!isBlockTokenEnabled) {
            ...
            return blockTokenSecretManager.generateToken(null, eb, EnumSet.of(BlockTokenIdentifier.AccessMode.REPLACE, BlockTokenIdentifier.AccessMode.COPY), storageTypes, storageIds);
        }
    }
