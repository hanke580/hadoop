...
public class BlockManager implements BlockStatsMXBean {
     ...
     */
    private final Daemon markedDeleteBlockScrubberThread = new Daemon(new MarkedDeleteBlockScrubber());
    ...
    public BlockManager(final Namesystem namesystem, boolean haEnabled, final Configuration conf) throws IOException {
        ...
        this.blocksPerPostpondedRescan = (int) Math.min(Integer.MAX_VALUE, datanodeManager.getBlocksPerPostponedMisreplicatedBlocksRescan());
        this.rescannedMisreplicatedBlocks = new ArrayList<Block>(blocksPerPostpondedRescan);
        this.startupDelayBlockDeletionInMs = conf.getLong(DFSConfigKeys.DFS_NAMENODE_STARTUP_DELAY_BLOCK_DELETION_SEC_KEY, DFSConfigKeys.DFS_NAMENODE_STARTUP_DELAY_BLOCK_DELETION_SEC_DEFAULT) * 1000L;
        this.deleteBlockLockTimeMs = conf.getLong(DFSConfigKeys.DFS_NAMENODE_BLOCK_DELETION_LOCK_THRESHOLD_MS, DFSConfigKeys.DFS_NAMENODE_BLOCK_DELETION_LOCK_THRESHOLD_MS_DEFAULT);
        ...
        this.invalidateBlocks = new InvalidateBlocks(datanodeManager.getBlockInvalidateLimit(), startupDelayBlockDeletionInMs, blockIdManager);
        ...
        this.placementPolicies = new BlockPlacementPolicies(conf, datanodeManager.getFSClusterStats(), datanodeManager.getNetworkTopology(), datanodeManager.getHost2DatanodeMap());
        this.storagePolicySuite = BlockStoragePolicySuite.createDefaultSuite(conf);
        this.pendingReconstruction = new PendingReconstructionBlocks(conf.getInt(DFSConfigKeys.DFS_NAMENODE_RECONSTRUCTION_PENDING_TIMEOUT_SEC_KEY, DFSConfigKeys.DFS_NAMENODE_RECONSTRUCTION_PENDING_TIMEOUT_SEC_DEFAULT) * 1000L);
        ...
        this.redundancyRecheckIntervalMs = conf.getTimeDuration(DFSConfigKeys.DFS_NAMENODE_REDUNDANCY_INTERVAL_SECONDS_KEY, DFSConfigKeys.DFS_NAMENODE_REDUNDANCY_INTERVAL_SECONDS_DEFAULT, TimeUnit.SECONDS, TimeUnit.MILLISECONDS);
        ...
        this.maxLockHoldTime = conf.getTimeDuration(DFSConfigKeys.DFS_NAMENODE_BLOCKREPORT_MAX_LOCK_HOLD_TIME, DFSConfigKeys.DFS_NAMENODE_BLOCKREPORT_MAX_LOCK_HOLD_TIME_DEFAULT, TimeUnit.MILLISECONDS);
        ...
        this.minReplicationToBeInMaintenance = (short) initMinReplicationToBeInMaintenance(conf);
        this.replQueueResetToHeadThreshold = initReplQueueResetToHeadThreshold(conf);
        long heartbeatIntervalSecs = conf.getTimeDuration(DFSConfigKeys.DFS_HEARTBEAT_INTERVAL_KEY, DFSConfigKeys.DFS_HEARTBEAT_INTERVAL_DEFAULT, TimeUnit.SECONDS);
        ...
        int queueSize = conf.getInt(DFSConfigKeys.DFS_NAMENODE_BLOCKREPORT_QUEUE_SIZE_KEY, DFSConfigKeys.DFS_NAMENODE_BLOCKREPORT_QUEUE_SIZE_DEFAULT);
        this.blockReportThread = new BlockReportProcessingThread(queueSize);
        this.deleteCorruptReplicaImmediately = conf.getBoolean(DFS_NAMENODE_CORRUPT_BLOCK_DELETE_IMMEDIATELY_ENABLED, DFS_NAMENODE_CORRUPT_BLOCK_DELETE_IMMEDIATELY_ENABLED_DEFAULT);
        setExcessRedundancyTimeout(conf.getLong(DFS_NAMENODE_EXCESS_REDUNDANCY_TIMEOUT_SEC_KEY, DFS_NAMENODE_EXCESS_REDUNDANCY_TIMEOUT_SEC_DEAFULT));
        setExcessRedundancyTimeoutCheckLimit(conf.getLong(DFS_NAMENODE_EXCESS_REDUNDANCY_TIMEOUT_CHECK_LIMIT, DFS_NAMENODE_EXCESS_REDUNDANCY_TIMEOUT_CHECK_LIMIT_DEFAULT));
        printInitialConfigs();
    }
    ...
    private int initMaxReplication(Configuration conf) throws IOException {
        ...
        if (minReplication > maxR) {
            throw new IOException("Unexpected configuration parameters: " + DFSConfigKeys.DFS_NAMENODE_REPLICATION_MIN_KEY + " = " + minReplication + " > " + DFSConfigKeys.DFS_REPLICATION_MAX_KEY + " = " + maxR);
        }
        return maxR;
    }

    private int initMinReplicationToBeInMaintenance(Configuration conf) throws IOException {
        final int minMaintenanceR = conf.getInt(DFSConfigKeys.DFS_NAMENODE_MAINTENANCE_REPLICATION_MIN_KEY, DFSConfigKeys.DFS_NAMENODE_MAINTENANCE_REPLICATION_MIN_DEFAULT);
        if (minMaintenanceR < 0) {
            throw new IOException("Unexpected configuration parameters: " + DFSConfigKeys.DFS_NAMENODE_MAINTENANCE_REPLICATION_MIN_KEY + " = " + minMaintenanceR + " < 0");
        }
        if (minMaintenanceR > defaultReplication) {
            throw new IOException("Unexpected configuration parameters: " + DFSConfigKeys.DFS_NAMENODE_MAINTENANCE_REPLICATION_MIN_KEY + " = " + minMaintenanceR + " > " + DFSConfigKeys.DFS_REPLICATION_KEY + " = " + defaultReplication);
        }
        return minMaintenanceR;
    }

    private int initReplQueueResetToHeadThreshold(Configuration conf) {
        int threshold = conf.getInt(DFSConfigKeys.DFS_NAMENODE_REDUNDANCY_QUEUE_RESTART_ITERATIONS, DFSConfigKeys.DFS_NAMENODE_REDUNDANCY_QUEUE_RESTART_ITERATIONS_DEFAULT);
        if (threshold < 0) {
            LOG.warn("{} is set to {} and it must be >= 0. Resetting to default {}", DFSConfigKeys.DFS_NAMENODE_REDUNDANCY_QUEUE_RESTART_ITERATIONS, threshold, DFSConfigKeys.DFS_NAMENODE_REDUNDANCY_QUEUE_RESTART_ITERATIONS_DEFAULT);
            threshold = DFSConfigKeys.DFS_NAMENODE_REDUNDANCY_QUEUE_RESTART_ITERATIONS_DEFAULT;
        }
        return threshold;
    }
    ...
    private static BlockTokenSecretManager createBlockTokenSecretManager(final Configuration conf) throws IOException {
        ...
        LOG.info("{}={} min(s), {}={} min(s), {}={}", DFSConfigKeys.DFS_BLOCK_ACCESS_KEY_UPDATE_INTERVAL_KEY, updateMin, DFSConfigKeys.DFS_BLOCK_ACCESS_TOKEN_LIFETIME_KEY, lifetimeMin, DFSConfigKeys.DFS_DATA_ENCRYPTION_ALGORITHM_KEY, encryptionAlgorithm);
        String nsId = DFSUtil.getNamenodeNameServiceId(conf);
        boolean isHaEnabled = HAUtil.isHAEnabled(conf, nsId);
        boolean shouldWriteProtobufToken = conf.getBoolean(DFSConfigKeys.DFS_BLOCK_ACCESS_TOKEN_PROTOBUF_ENABLE, DFSConfigKeys.DFS_BLOCK_ACCESS_TOKEN_PROTOBUF_ENABLE_DEFAULT);
        boolean shouldWrapQOP = conf.getBoolean(DFS_NAMENODE_SEND_QOP_ENABLED, DFS_NAMENODE_SEND_QOP_ENABLED_DEFAULT);
        if (isHaEnabled) {
            ...
            return new BlockTokenSecretManager(updateMin * 60 * 1000L, lifetimeMin * 60 * 1000L, nnIndex, nnIds.size(), null, encryptionAlgorithm, shouldWriteProtobufToken, shouldWrapQOP);
        } else {
            return new BlockTokenSecretManager(updateMin * 60 * 1000L, lifetimeMin * 60 * 1000L, 0, 1, null, encryptionAlgorithm, shouldWriteProtobufToken, shouldWrapQOP);
        }
    }
    ...
    public void refreshBlockPlacementPolicy(Configuration conf) {
        BlockPlacementPolicies bpp = new BlockPlacementPolicies(conf, datanodeManager.getFSClusterStats(), datanodeManager.getNetworkTopology(), datanodeManager.getHost2DatanodeMap());
    ...
    public void metaSave(PrintWriter out) {
        // TODO: block manager read lock and NS write lock
        ...
        synchronized (neededReconstruction) {
            out.println("Metasave: Blocks waiting for reconstruction: " + neededReconstruction.getLowRedundancyBlockCount());
            ...
            for (Iterator<BlockInfo> it = neededReconstruction.iterator(neededReconstruction.QUEUE_WITH_CORRUPT_BLOCKS); it.hasNext(); ) {
                Block block = it.next();
                dumpBlockMeta(block, out);
            }
        }
        ...
        for (Block block : corruptBlocks) {
            ...
            for (DatanodeStorageInfo storage : blocksMap.getStorages(block)) {
                ...
                if (corruptNodes.contains(node)) {
                    ...
                    out.println("Block=" + block.toString() + "\tSize=" + block.getNumBytes() + "\tNode=" + node.getName() + "\tStorageID=" + storageId + "\tStorageState=" + state + "\tTotalReplicas=" + blocksMap.numNodes(block) + "\tReason=" + corruptReplicas.getCorruptReason(block, node));
                    numNodesToFind--;
                    if (numNodesToFind == 0) {
                        break;
                    }
            ...
            if (numNodesToFind > 0) {
                ...
                out.println(block.getBlockId() + " corrupt on " + StringUtils.join(",", corruptNodesList) + " but not all nodes are" + "found in its block locations");
            }
        }
        // Dump all datanodes
    ...
    private void dumpBlockMeta(Block block, PrintWriter out) {
        ...
        chooseSourceDatanodes(blockInfo, containingNodes, containingLiveReplicasNodes, numReplicas, new ArrayList<Byte>(), new ArrayList<Byte>(), new ArrayList<Byte>(), LowRedundancyBlocks.LEVEL);
    ...
    static private void ensurePositiveInt(int val, String key) {
        Preconditions.checkArgument((val > 0), key + " = '" + val + "' is invalid. " + "It should be a positive, non-zero integer value.");
    }
    ...
    public boolean commitOrCompleteLastBlock(BlockCollection bc, Block commitBlock, INodesInPath iip) throws IOException {
        ...
        int numUsableReplicas = numReplicas.liveReplicas() + numReplicas.decommissioning() + numReplicas.liveEnteringMaintenanceReplicas();
    ...
    public void addExpectedReplicasToPending(BlockInfo blk) {
        ...
        if (!blk.isStriped() || addForStriped) {
            if (expectedStorages.length - blk.numNodes() > 0) {
                ...
                pendingReconstruction.increment(blk, pendingNodes.toArray(new DatanodeStorageInfo[pendingNodes.size()]));
            }
        }
    }
    ...
    public LocatedBlock convertLastBlockToUnderConstruction(BlockCollection bc, long bytesToRemove) throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(lastBlock, 179);
        // Remove block from reconstruction queue.
        NumberReplicas replicas = countNodes(lastBlock);
        neededReconstruction.remove(lastBlock, replicas.liveReplicas(), replicas.readOnlyReplicas(), replicas.outOfServiceReplicas(), getExpectedRedundancyNum(lastBlock));
    ...
    private void createLocatedBlockList(LocatedBlockBuilder locatedBlocks, final BlockInfo[] blocks, final long offset, final long length, final AccessMode mode) throws IOException {
        int curBlk;
    ...
    private LocatedBlock createLocatedBlock(LocatedBlockBuilder locatedBlocks, final BlockInfo[] blocks, final long endPos, final AccessMode mode) throws IOException {
        int curBlk;
        long curPos = 0;
        int nrBlocks = (blocks[0].getNumBytes() == 0) ? 0 : blocks.length;
        for (curBlk = 0; curBlk < nrBlocks; curBlk++) {
    ...
    private LocatedBlock createLocatedBlock(LocatedBlockBuilder locatedBlocks, final BlockInfo blk, final long pos, final AccessMode mode) throws IOException {
        final LocatedBlock lb = createLocatedBlock(locatedBlocks, blk, pos);
        org.zlab.ocov.tracker.Runtime.update(lb, 180);
        if (mode != null) {
     ...
     */
    private LocatedBlock createLocatedBlock(LocatedBlockBuilder locatedBlocks, final BlockInfo blk, final long pos) throws IOException {
        if (!blk.isComplete()) {
            ...
            if (blk.isStriped()) {
                ...
                final DatanodeStorageInfo[] storages = uc.getExpectedStorageLocations();
                final ExtendedBlock eb = new ExtendedBlock(getBlockPoolId(), blk);
                return null == locatedBlocks ? newLocatedBlock(eb, storages, pos, false) : locatedBlocks.newLocatedBlock(eb, storages, pos, false);
            }
        }
        // get block locations
        ...
        if (numCorruptNodes != numCorruptReplicas) {
            LOG.warn("Inconsistent number of corrupt replicas for {}" + " blockMap has {} but corrupt replicas map has {}", blk, numCorruptNodes, numCorruptReplicas);
        }
        ...
        final ExtendedBlock eb = new ExtendedBlock(getBlockPoolId(), blk);
        return blockIndices == null ? null == locatedBlocks ? newLocatedBlock(eb, machines, pos, isCorrupt) : locatedBlocks.newLocatedBlock(eb, machines, pos, isCorrupt) : newLocatedStripedBlock(eb, machines, blockIndices, pos, isCorrupt);
    }

    /**
     * Create a LocatedBlocks.
     */
    public LocatedBlocks createLocatedBlocks(final BlockInfo[] blocks, final long fileSizeExcludeBlocksUnderConstruction, final boolean isFileUnderConstruction, final long offset, final long length, final boolean needBlockToken, final boolean inSnapshot, FileEncryptionInfo feInfo, ErasureCodingPolicy ecPolicy) throws IOException {
        assert namesystem.hasReadLock();
        if (blocks == null) {
            return null;
        } else if (blocks.length == 0) {
            return new LocatedBlocks(0, isFileUnderConstruction, Collections.<LocatedBlock>emptyList(), null, false, feInfo, ecPolicy);
        } else {
            if (LOG.isDebugEnabled()) {
                LOG.debug("blocks = {}", java.util.Arrays.asList(blocks));
            }
            final AccessMode mode = needBlockToken ? BlockTokenIdentifier.AccessMode.READ : null;
            LocatedBlockBuilder locatedBlocks = providedStorageMap.newLocatedBlocks(Integer.MAX_VALUE).fileLength(fileSizeExcludeBlocksUnderConstruction).lastUC(isFileUnderConstruction).encryption(feInfo).erasureCoding(ecPolicy);
            ...
            if (!inSnapshot) {
                ...
                locatedBlocks.lastBlock(createLocatedBlock(locatedBlocks, last, lastPos, mode)).lastComplete(last.isComplete());
            } else {
                locatedBlocks.lastBlock(createLocatedBlock(locatedBlocks, blocks, fileSizeExcludeBlocksUnderConstruction, mode)).lastComplete(true);
            }
        ...
        if (isBlockTokenEnabled()) {
            ...
            if (b.isStriped()) {
                ...
                for (int i = 0; i < indices.length; i++) {
                    ...
                    blockTokens[i] = blockTokenSecretManager.generateToken(NameNode.getRemoteUser().getShortUserName(), internalBlock, EnumSet.of(mode), b.getStorageTypes(), b.getStorageIDs());
                }
                sb.setBlockTokens(blockTokens);
            }
            b.setBlockToken(blockTokenSecretManager.generateToken(NameNode.getRemoteUser().getShortUserName(), b.getBlock(), EnumSet.of(mode), b.getStorageTypes(), b.getStorageIDs()));
        }
    }
     ...
     */
    public BlocksWithLocations getBlocksWithLocations(final DatanodeID datanode, final long size, final long minBlockSize, final long timeInterval, final StorageType storageType) throws UnregisteredNodeException {
        ...
        DatanodeStorageInfo[] storageInfos = Arrays.stream(node.getStorageInfos()).filter(s -> !s.areBlockContentsStale()).filter(s -> storageType == null || s.getStorageType().equals(storageType)).toArray(DatanodeStorageInfo[]::new);
        ...
        if ((org.zlab.ocov.tracker.Runtime.updateBranch(totalSize, size, "<", 41))) {
            // start from the beginning
            iter = node.getBlockIterator(0, storageInfos);
    ...
    public void findAndMarkBlockAsCorrupt(final ExtendedBlock blk, final DatanodeInfo dn, String storageID, String reason) throws IOException {
        ...
        markBlockAsCorrupt(new BlockToMarkCorrupt(reportedBlock, storedBlock, blk.getGenerationStamp(), reason, Reason.CORRUPTION_REPORTED), storage, node);
    }
    ...
    BlockReconstructionWork scheduleReconstruction(BlockInfo block, int priority) {
        ...
        final DatanodeDescriptor[] srcNodes = chooseSourceDatanodes(block, containingNodes, liveReplicaNodes, numReplicas, liveBlockIndices, liveBusyBlockIndices, excludeReconstructed, priority);
        ...
        if (block.isStriped()) {
            ...
            adjustSrcNodesAndIndices((BlockInfoStriped) block, srcNodes, liveBlockIndices, newSrcNodes, newIndices);
            ...
            return new ErasureCodingWork(getBlockPoolId(), block, bc, newSrcNodes, containingNodes, liveReplicaNodes, additionalReplRequired, priority, newIndices, busyIndices, excludeReconstructedIndices);
        } else {
    ...
    public DatanodeStorageInfo[] chooseTarget4WebHDFS(String src, DatanodeDescriptor clientnode, Set<Node> excludes, long blocksize) {
        return placementPolicies.getPolicy(CONTIGUOUS).chooseTarget(src, 1, clientnode, Collections.<DatanodeStorageInfo>emptyList(), false, excludes, blocksize, storagePolicySuite.getDefaultPolicy(), null);
    }
     ...
     */
    public DatanodeStorageInfo[] chooseTarget4NewBlock(final String src, final int numOfReplicas, final Node client, final Set<Node> excludedNodes, final long blocksize, final List<String> favoredNodes, final byte storagePolicyID, final BlockType blockType, final ErasureCodingPolicy ecPolicy, final EnumSet<AddBlockFlag> flags) throws IOException {
     ...
     */
    @VisibleForTesting
    DatanodeDescriptor[] chooseSourceDatanodes(BlockInfo block, List<DatanodeDescriptor> containingNodes, List<DatanodeStorageInfo> nodesContainingLiveReplicas, NumberReplicas numReplicas, List<Byte> liveBlockIndices, List<Byte> liveBusyBlockIndices, List<Byte> excludeReconstructed, int priority) {
        ...
        for (DatanodeStorageInfo storage : blocksMap.getStorages(block)) {
            ...
            if (isStriped) {
                ...
                countLiveAndDecommissioningReplicas(numReplicas, state, liveBitSet, decommissioningBitSet, blockIndex);
            }
            if (priority != LowRedundancyBlocks.QUEUE_HIGHEST_PRIORITY && (!node.isDecommissionInProgress() && !node.isEnteringMaintenance()) && node.getNumberOfBlocksToBeReplicated() + node.getNumberOfBlocksToBeErasureCoded() >= maxReplicationStreams) {
                if (isStriped && (state == StoredReplicaState.LIVE || state == StoredReplicaState.DECOMMISSIONING)) {
            ...
            if (node.getNumberOfBlocksToBeReplicated() + node.getNumberOfBlocksToBeErasureCoded() >= replicationStreamsHardLimit) {
                if (isStriped && (state == StoredReplicaState.LIVE || state == StoredReplicaState.DECOMMISSIONING)) {
                    liveBusyBlockIndices.add(blockIndex);
                    //HDFS-16566 ExcludeReconstructed won't be reconstructed.
                    excludeReconstructed.add(blockIndex);
    ...
    public void removeBlocksAndUpdateSafemodeTotal(BlocksMapUpdateInfo blocks) {
        ...
        if (trackBlockCounts) {
            if (LOG.isDebugEnabled()) {
                LOG.debug("Adjusting safe-mode totals for deletion." + "decreasing safeBlocks by {}, totalBlocks by {}", numRemovedSafe, numRemovedComplete);
            }
     ...
     */
    public boolean checkBlockReportLease(BlockReportContext context, final DatanodeID nodeID) throws UnregisteredNodeException {
        if (context == null) {
            return true;
        }
    ...
    public boolean processReport(final DatanodeID nodeID, final DatanodeStorage storage, final BlockListAsLongs newReport, BlockReportContext context) throws IOException {
        ...
        String fullBrLeaseId = context != null ? Long.toHexString(context.getLeaseId()) : "";
        try {
            ...
            if (namesystem.isInStartupSafeMode() && !StorageType.PROVIDED.equals(storageInfo.getStorageType()) && storageInfo.getBlockReportCount() > 0) {
                blockLog.info("BLOCK* processReport 0x{} with lease ID 0x{}: " + "discarded non-initial block report from {}" + " because namenode still in startup phase", strBlockReportId, fullBrLeaseId, nodeID);
            ...
            if (!storageInfo.hasReceivedBlockReport()) {
                ...
                blockLog.info("BLOCK* processReport 0x{} with lease ID 0x{}: Processing first " + "storage report for {} from datanode {}", strBlockReportId, fullBrLeaseId, storageInfo.getStorageID(), nodeID);
        ...
        if (blockLog.isDebugEnabled()) {
            for (Block b : invalidatedBlocks) {
                blockLog.debug("BLOCK* processReport 0x{} with lease ID 0x{}: {} on node {} size {} " + "does not belong to any file.", strBlockReportId, fullBrLeaseId, b, node, b.getNumBytes());
            }
        }
        ...
        blockLog.info("BLOCK* processReport 0x{} with lease ID 0x{}: from storage {} node {}, " + "blocks: {}, hasStaleStorage: {}, processing time: {} msecs, " + "invalidatedBlocks: {}", strBlockReportId, fullBrLeaseId, storage.getStorageID(), nodeID, newReport.getNumberOfBlocks(), node.hasStaleStorages(), (endTime - startTime), invalidatedBlocks.size());
        return !node.hasStaleStorages();
    }
    ...
    public void removeBRLeaseIfNeeded(final DatanodeID nodeID, final BlockReportContext context) throws IOException {
        namesystem.writeLock();
        DatanodeDescriptor node;
        try {
            node = datanodeManager.getDatanode(nodeID);
    ...
    void rescanPostponedMisreplicatedBlocks() {
        ...
        try {
            ...
            for (int i = 0; i < blocksPerPostpondedRescan && it.hasNext(); i++) {
                ...
                if (bi == null) {
                    if (LOG.isDebugEnabled()) {
                        LOG.debug("BLOCK* rescanPostponedMisreplicatedBlocks: " + "Postponed mis-replicated block {} no longer found " + "in block map.", b);
                    }
                    continue;
                }
            ...
            LOG.info("Rescan of postponedMisreplicatedBlocks completed in {}" + " msecs. {} blocks are left. {} blocks were removed.", (Time.monotonicNow() - startTime), endSize, (startSize - endSize));
        }
    }
    ...
    public void setExcessRedundancyTimeoutCheckLimit(long limit) {
        if (excessRedundancyTimeoutCheckLimit <= 0) {
            this.excessRedundancyTimeoutCheckLimit = DFS_NAMENODE_EXCESS_REDUNDANCY_TIMEOUT_CHECK_LIMIT_DEFAULT;
        } else {
    ...
    void processTimedOutExcessBlocks() {
        ...
        int processed = 0;
        try {
            Iterator<Map.Entry<String, LightWeightHashSet<Block>>> iter = excessRedundancyMap.getExcessRedundancyMap().entrySet().iterator();
            while (iter.hasNext() && processed < excessRedundancyTimeoutCheckLimit) {
                ...
                List<ExcessBlockInfo> sortedBlocks = blocks.stream().filter(block -> block instanceof ExcessBlockInfo).map(block -> (ExcessBlockInfo) block).sorted(Comparator.comparingLong(ExcessBlockInfo::getTimeStamp)).collect(Collectors.toList());
                for (ExcessBlockInfo excessBlockInfo : sortedBlocks) {
                    ...
                    while (iterator.hasNext()) {
                        ...
                        if (datanodeDescriptor.getDatanodeUuid().equals(datanodeUuid) && datanodeStorageInfo.getState().equals(State.NORMAL)) {
                            ...
                            if (!containsInvalidateBlock(datanodeDescriptor, b)) {
                                ...
                                LOG.debug("Excess block timeout ({}, {}) is added to invalidated.", b, datanodeDescriptor);
                            }
                            excessBlockInfo.setTimeStamp();
                            org.zlab.ocov.tracker.Runtime.update(excessBlockInfo, 181);
                            break;
                        }
                    }
                }
    ...
    Collection<Block> processReport(final DatanodeStorageInfo storageInfo, final BlockListAsLongs report) throws IOException {
        // Normal case:
        ...
        org.zlab.ocov.tracker.Runtime.update(toAdd, 183);
        org.zlab.ocov.tracker.Runtime.update(toUC, 182);
        DatanodeDescriptor node = storageInfo.getDatanodeDescriptor();
    ...
    public void markBlockReplicasAsCorrupt(Block oldBlock, BlockInfo block, long oldGenerationStamp, long oldNumBytes, DatanodeStorageInfo[] newStorages) throws IOException {
        ...
        if (block.getGenerationStamp() != oldGenerationStamp) {
            b = new BlockToMarkCorrupt(oldBlock, block, oldGenerationStamp, "genstamp does not match " + oldGenerationStamp + " : " + block.getGenerationStamp(), Reason.GENSTAMP_MISMATCH);
        } else if (block.getNumBytes() != oldNumBytes) {
            b = new BlockToMarkCorrupt(oldBlock, block, "length does not match " + oldNumBytes + " : " + block.getNumBytes(), Reason.SIZE_MISMATCH);
        } else {
            return;
        }
        for (DatanodeStorageInfo storage : getStorages(block)) {
            ...
            if (isCorrupt) {
                if (blockLog.isDebugEnabled()) {
                    blockLog.debug("BLOCK* markBlockReplicasAsCorrupt: mark block replica" + " {} on {} as corrupt because the dn is not in the new committed " + "storage list.", b, storage.getDatanodeDescriptor());
                }
     ...
     * @throws IOException
     */
    void processFirstBlockReport(final DatanodeStorageInfo storageInfo, final BlockListAsLongs report) throws IOException {
        if (report == null)
            return;
        assert (namesystem.hasWriteLock());
        ...
        for (BlockReportReplica iblk : report) {
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug("Initial report of block {} on {} size {} replicaState = {}", iblk.getBlockName(), storageInfo.getDatanodeDescriptor(), iblk.getNumBytes(), reportedState);
            }
            ...
            org.zlab.ocov.tracker.Runtime.update(c, 184);
            if (c != null) {
    ...
    private void reportDiff(DatanodeStorageInfo storageInfo, BlockListAsLongs newReport, // add to DatanodeDescriptor
    ...
    private void queueReportedBlock(DatanodeStorageInfo storageInfo, Block block, ReplicaState reportedState, String reason) {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("Queueing reported block {} in state {}" + " from datanode {} for later processing because {}.", block, reportedState, storageInfo.getDatanodeDescriptor(), reason);
        }
    ...
    private void processQueuedMessages(Iterable<ReportedBlockInfo> rbis) throws IOException {
        ...
        for (ReportedBlockInfo rbi : rbis) {
            ...
            if (rbi.getReportedState() == null) {
                ...
                queueReportedBlock(rbi.getStorageInfo(), rbi.getBlock(), rbi.getReportedState(), QUEUE_REASON_FUTURE_GENSTAMP);
            } else {
                isPreviousMessageProcessed = processAndHandleReportedBlock(rbi.getStorageInfo(), rbi.getBlock(), rbi.getReportedState(), null);
            }
        }
    }
    ...
    public void processAllPendingDNMessages() throws IOException {
        ...
        if (count > 0) {
            LOG.info("Processing {} messages from DataNodes " + "that were previously queued during standby state", count);
        }
     ...
     */
    private BlockToMarkCorrupt checkReplicaCorrupt(Block reported, ReplicaState reportedState, BlockInfo storedBlock, BlockUCState ucState, DatanodeDescriptor dn) {
        switch(reportedState) {
                ...
                switch(ucState) {
                    ...
                    case COMMITTED:
                        ...
                        if (storedBlock.isStriped()) {
                            ...
                            blockMapSize = getInternalBlockLength(stripedBlock.getNumBytes(), stripedBlock.getCellSize(), stripedBlock.getDataBlockNum(), reportedBlkIdx);
                        ...
                        if (wrongSize) {
                            return new BlockToMarkCorrupt(new Block(reported), storedBlock, "block is " + ucState + " and reported length " + reported.getNumBytes() + " does not match " + "length in block map " + blockMapSize, Reason.SIZE_MISMATCH);
                        } else {
                            // not corrupt
                            return null;
                        }
            ...
            case RWR:
                ...
                if (!storedBlock.isComplete()) {
                    ...
                    if (storedBlock.getGenerationStamp() > reported.getGenerationStamp()) {
                        return new BlockToMarkCorrupt(new Block(reported), storedBlock, reportedGS, "reported " + reportedState + " replica with genstamp " + reportedGS + " does not match Stored block's genstamp in block map " + storedBlock.getGenerationStamp(), Reason.GENSTAMP_MISMATCH);
                    }
                    // not corrupt
                    return null;
                    ...
                    if (reportedState == ReplicaState.RBW) {
                        ...
                        LOG.info("Received an RBW replica for {} on {}: ignoring it, since " + "it is complete with the same genstamp", storedBlock, dn);
                        return null;
                    } else {
    ...
    private boolean isBlockUnderConstruction(BlockInfo storedBlock, BlockUCState ucState, ReplicaState reportedState) {
        switch(reportedState) {
            case FINALIZED:
                switch(ucState) {
                    case UNDER_CONSTRUCTION:
    ...
    private Block addStoredBlock(final BlockInfo block, final Block reportedBlock, DatanodeStorageInfo storageInfo, DatanodeDescriptor delNodeHint, boolean logEveryBlock) throws IOException {
        ...
        if (storedBlock == null || storedBlock.isDeleted()) {
            ...
            blockLog.debug("BLOCK* addStoredBlock: {} on {} size {} but it does not belong to any file", reportedBlock, node, reportedBlock.getNumBytes());
        ...
        if (result == AddBlockResult.ADDED) {
            ...
            if (logEveryBlock) {
                blockLog.info("BLOCK* addStoredBlock: {} is added to {} (size={})", node, reportedBlock, reportedBlock.getNumBytes());
            }
        } else if (result == AddBlockResult.REPLACED) {
            curReplicaDelta = 0;
            blockLog.warn("BLOCK* addStoredBlock: block {} moved to storageType " + "{} on node {} storageId {}, reportedBlock is {}", reportedBlock, storageInfo.getStorageType(), node, storageInfo.getStorageID(), reportedBlock);
        } else {
            ...
            curReplicaDelta = 0;
            if (blockLog.isDebugEnabled()) {
                blockLog.debug("BLOCK* addStoredBlock: Redundant addStoredBlock request" + " received for {} on node {} size {}", reportedBlock, node, reportedBlock.getNumBytes());
            }
        }
        // Now check for completion of blocks and safe block count
   ...
   */
    private void processMisReplicatesAsync() throws InterruptedException {
        ...
        while (namesystem.isRunning() && !Thread.currentThread().isInterrupted()) {
            ...
            try {
                ...
                if (totalBlocks > 0) {
                    // here avoid metrics appear as NaN.
                    reconstructionQueuesInitProgress = Math.min((float) totalProcessed / totalBlocks, 1.0f);
                }
                ...
                LOG.info("Reconstruction queues initialisation progress: {}, total number of blocks " + "processed: {}/{}", reconstructionQueuesInitProgress, totalProcessed, totalBlocks);
    ...
    public int processMisReplicatedBlocks(List<BlockInfo> blocks) {
        ...
        try {
            while (isPopulatingReplQueues() && namesystem.isRunning() && !Thread.currentThread().isInterrupted() && iter.hasNext()) {
                ...
                try {
                    while (iter.hasNext() && processed < limit) {
                        ...
                        processed++;
                        LOG.debug("BLOCK* processMisReplicatedBlocks: Re-scanned block {}, result is {}", blk, r);
                    }
                } finally {
    ...
    private MisReplicationResult processMisReplicatedBlock(BlockInfo block) {
        ...
        if (shouldProcessExtraRedundancy(num, expectedRedundancy)) {
            ...
            if (!processExtraRedundancyBlockWithoutPostpone(block, expectedRedundancy, null, null)) {
    ...
    private void processExtraRedundancyBlock(final BlockInfo block, final short replication, final DatanodeDescriptor addedNode, DatanodeDescriptor delNodeHint) {
        if (!processExtraRedundancyBlockWithoutPostpone(block, replication, addedNode, delNodeHint)) {
            postponeBlock(block);
        }
    }
     ...
     */
    private boolean processExtraRedundancyBlockWithoutPostpone(final BlockInfo block, final short replication, final DatanodeDescriptor addedNode, DatanodeDescriptor delNodeHint) {
        assert namesystem.hasWriteLock();
        ...
        if (hasStaleStorage) {
            LOG.trace("BLOCK* processExtraRedundancyBlockWithoutPostpone: Postponing {}" + " since storages {} does not yet have up-to-date information.", block, staleStorages);
            return false;
        }
        return true;
    }
     ...
     */
    private void chooseExcessRedundancyContiguous(final Collection<DatanodeStorageInfo> nonExcess, BlockInfo storedBlock, short replication, DatanodeDescriptor addedNode, DatanodeDescriptor delNodeHint, List<StorageType> excessTypes) {
    ...
    private void chooseExcessRedundancyStriped(BlockCollection bc, final Collection<DatanodeStorageInfo> nonExcess, BlockInfo storedBlock, DatanodeDescriptor delNodeHint) {
        ...
        if (duplicated.isEmpty()) {
            LOG.debug("Found no duplicated internal blocks for {}. Maybe it's " + "because there are stale storages.", storedBlock);
            return;
        }
        ...
        for (int targetIndex = duplicated.nextSetBit(0); targetIndex >= 0; targetIndex = duplicated.nextSetBit(targetIndex + 1)) {
            ...
            if (candidates.size() > 1) {
                ...
                Preconditions.checkArgument(candidates.containsAll(replicasToDelete), "The EC replicas to be deleted are not in the candidate list");
    ...
    private void processChosenExcessRedundancy(final Collection<DatanodeStorageInfo> nonExcess, final DatanodeStorageInfo chosen, BlockInfo storedBlock) {
     ...
     * @throws IOException
     */
    private boolean processAndHandleReportedBlock(DatanodeStorageInfo storageInfo, Block block, ReplicaState reportedState, DatanodeDescriptor delHintNode) throws IOException {
        ...
        LOG.debug("Reported block {} on {} size {} replicaState = {}", block, node, block.getNumBytes(), reportedState);
        if (shouldPostponeBlocksFromFuture && isGenStampInFuture(block)) {
            queueReportedBlock(storageInfo, block, reportedState, QUEUE_REASON_FUTURE_GENSTAMP);
            return false;
        }
        processReportedBlock(storageInfo, block, reportedState, toAdd, toInvalidate, toCorrupt, toUC);
        org.zlab.ocov.tracker.Runtime.update(toAdd, 186);
        org.zlab.ocov.tracker.Runtime.update(toUC, 185);
        ...
        for (BlockInfoToAdd b : toAdd) {
            addStoredBlock(b.stored, b.reported, storageInfo, delHintNode, numBlocksLogged < maxNumBlocksToLog);
            numBlocksLogged++;
        }
        if (numBlocksLogged > maxNumBlocksToLog) {
            blockLog.debug("BLOCK* addBlock: logged info for {} of {} reported.", maxNumBlocksToLog, numBlocksLogged);
        }
        for (Block b : toInvalidate) {
            blockLog.debug("BLOCK* addBlock: block {} on node {} size {} does not belong to any file", b, node, b.getNumBytes());
            addToInvalidates(b, node);
        }
        for (BlockToMarkCorrupt b : toCorrupt) {
            markBlockAsCorrupt(b, storageInfo, node);
    ...
    private void countReplicasForStripedBlock(NumberReplicas counters, BlockInfoStriped block, Collection<DatanodeDescriptor> nodesCorrupt, boolean inStartupSafeMode) {
        ...
        for (StorageAndBlockIndex si : block.getStorageAndIndexInfos()) {
            ...
            countLiveAndDecommissioningReplicas(counters, state, liveBitSet, decommissioningBitSet, si.getBlockIndex());
        }
    }
     ...
     */
    private void countLiveAndDecommissioningReplicas(NumberReplicas counters, StoredReplicaState state, BitSet liveBitSet, BitSet decommissioningBitSet, byte blockIndex) {
        if (state == StoredReplicaState.LIVE) {
    ...
    boolean isNodeHealthyForDecommissionOrMaintenance(DatanodeDescriptor node) {
        ...
        if (pendingReconstructionBlocksCount == 0 && lowRedundancyBlocksCount == 0) {
            LOG.info("Node {} is dead and there are no low redundancy" + " blocks or blocks pending reconstruction. Safe to decommission or" + " put in maintenance.", node);
            return true;
        }
    ...
    public void checkRedundancy(BlockCollection bc) {
        for (BlockInfo block : bc.getBlocks()) {
            ...
            if (!hasEnoughEffectiveReplicas(block, n, pending)) {
                neededReconstruction.add(block, n.liveReplicas() + pending, n.readOnlyReplicas(), n.outOfServiceReplicas(), expected);
        ...
        try {
            ...
            try {
                ...
                if (dnDescriptor == null) {
                    LOG.warn("DataNode {} cannot be found with UUID {}" + ", removing block invalidation work.", dn, dn.getDatanodeUuid());
    ...
    boolean isNeededReconstructionForMaintenance(BlockInfo storedBlock, NumberReplicas numberReplicas) {
        return storedBlock.isComplete() && (numberReplicas.liveReplicas() < getMinMaintenanceStorageNum(storedBlock) || !isPlacementPolicySatisfied(storedBlock));
    }
    ...
    public short getExpectedLiveRedundancyNum(BlockInfo block, NumberReplicas numberReplicas) {
        ...
        return (short) Math.max(expectedRedundancy - numberReplicas.maintenanceReplicas(), getMinMaintenanceStorageNum(block));
    }
    ...
    public BlockInfo addBlockCollectionWithCheck(BlockInfo block, BlockCollection bc) {
        if (!hasNonEcBlockUsingStripedID && !block.isStriped() && BlockIdManager.isStripedBlockID(block.getBlockId())) {
            ...
            while (namesystem.isRunning() && !Thread.currentThread().isInterrupted()) {
                if (!markedDeleteQueue.isEmpty() || checkToDeleteIterator()) {
                    try {
                        ...
                        long startTime = Time.monotonicNow();
                        remove(startTime);
                        while (!isSleep && !markedDeleteQueue.isEmpty() && !Thread.currentThread().isInterrupted()) {
                        ...
                        LOG.warn("MarkedDeleteBlockScrubber encountered an exception" + " during the block deletion process, " + " the deletion of the block will retry in {} millisecond.", deleteBlockUnlockIntervalTimeMs, e);
                    }
                }
                if (isSleep) {
                    LOG.debug("Clear markedDeleteQueue over {} millisecond to release the write lock", deleteBlockLockTimeMs);
                }
                try {
    ...
    public static LocatedBlock newLocatedBlock(ExtendedBlock eb, BlockInfo info, DatanodeStorageInfo[] locs, long offset) throws IOException {
        ...
        if (info.isStriped()) {
            lb = newLocatedStripedBlock(eb, locs, info.getUnderConstructionFeature().getBlockIndices(), offset, false);
        } else {
    ...
    public void addBLocksToMarkedDeleteQueue(List<BlockInfo> blockInfos) {
        ...
        NameNode.getNameNodeMetrics().incrPendingDeleteBlocksCount(blockInfos.size());
    }
    ...
    public boolean createSPSManager(final Configuration conf, final String spsMode) {
        ...
        boolean storagePolicyEnabled = conf.getBoolean(DFSConfigKeys.DFS_STORAGE_POLICY_ENABLED_KEY, DFSConfigKeys.DFS_STORAGE_POLICY_ENABLED_DEFAULT);
        String modeVal = spsMode;
        if (org.apache.commons.lang3.StringUtils.isBlank(modeVal)) {
            modeVal = conf.get(DFSConfigKeys.DFS_STORAGE_POLICY_SATISFIER_MODE_KEY, DFSConfigKeys.DFS_STORAGE_POLICY_SATISFIER_MODE_DEFAULT);
        }
    ...
    public void setMinBlocksForWrite(int minBlocksForWrite) {
        ensurePositiveInt(minBlocksForWrite, DFS_NAMENODE_BLOCKPLACEMENTPOLICY_MIN_BLOCKS_FOR_WRITE_KEY);
