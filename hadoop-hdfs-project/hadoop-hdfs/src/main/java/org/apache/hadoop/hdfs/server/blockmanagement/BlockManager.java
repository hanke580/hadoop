...
public class BlockManager implements BlockStatsMXBean {
    ...
    public BlockManager(final Namesystem namesystem, boolean haEnabled, final Configuration conf) throws IOException {
        ...
        blocksPerPostpondedRescan = (int) Math.min(Integer.MAX_VALUE, datanodeManager.getBlocksPerPostponedMisreplicatedBlocksRescan());
        ...
        invalidateBlocks = new InvalidateBlocks(datanodeManager.getBlockInvalidateLimit(), startupDelayBlockDeletionInMs, blockIdManager);
        ...
        placementPolicies = new BlockPlacementPolicies(conf, datanodeManager.getFSClusterStats(), datanodeManager.getNetworkTopology(), datanodeManager.getHost2DatanodeMap());
        storagePolicySuite = BlockStoragePolicySuite.createDefaultSuite();
        pendingReconstruction = new PendingReconstructionBlocks(conf.getInt(DFSConfigKeys.DFS_NAMENODE_RECONSTRUCTION_PENDING_TIMEOUT_SEC_KEY, DFSConfigKeys.DFS_NAMENODE_RECONSTRUCTION_PENDING_TIMEOUT_SEC_DEFAULT) * 1000L);
        ...
        replQueueResetToHeadThreshold = conf.getInt(DFSConfigKeys.DFS_NAMENODE_REDUNDANCY_QUEUE_RESTART_ITERATIONS, DFSConfigKeys.DFS_NAMENODE_REDUNDANCY_QUEUE_RESTART_ITERATIONS_DEFAULT);
        if (replQueueResetToHeadThreshold < 0) {
            LOG.warn("{} is set to {} and it must be >= 0. Resetting to default {}", DFSConfigKeys.DFS_NAMENODE_REDUNDANCY_QUEUE_RESTART_ITERATIONS, replQueueResetToHeadThreshold, DFSConfigKeys.DFS_NAMENODE_REDUNDANCY_QUEUE_RESTART_ITERATIONS_DEFAULT);
            replQueueResetToHeadThreshold = DFSConfigKeys.DFS_NAMENODE_REDUNDANCY_QUEUE_RESTART_ITERATIONS_DEFAULT;
        }
    ...
    private List<DatanodeStorageInfo> getValidLocations(BlockInfo block) {
        ...
        for (DatanodeStorageInfo storage : blocksMap.getStorages(block)) {
            ...
            org.zlab.ocov.tracker.Runtime.update(b, 137);
    ...
    private LocatedBlock createLocatedBlock(LocatedBlockBuilder locatedBlocks, final BlockInfo blk, final long pos, final AccessMode mode) throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(lb, 138);
        if (mode != null) {
    ...
    public void setBlockToken(final LocatedBlock b, final AccessMode mode) throws IOException {
        if (isBlockTokenEnabled()) {
            ...
            if (b.isStriped()) {
                ...
                ExtendedBlock internalBlock = ((ExtendedBlock) org.zlab.ocov.tracker.Runtime.update(new ExtendedBlock(b.getBlock()), 139));
     ...
     */
    public BlocksWithLocations getBlocksWithLocations(final DatanodeID datanode, final long size, final long minBlockSize) throws UnregisteredNodeException {
        ...
        // skip stale storage
        DatanodeStorageInfo[] storageInfos = Arrays.stream(node.getStorageInfos()).filter(s -> !s.areBlockContentsStale()).toArray(DatanodeStorageInfo[]::new);
        ...
        if ((org.zlab.ocov.tracker.Runtime.updateBranch(totalSize, size, "<", 38))) {
   ...
   */
    ...
    private boolean processAndHandleReportedBlock(DatanodeStorageInfo storageInfo, Block block, ReplicaState reportedState, DatanodeDescriptor delHintNode) throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(toAdd, 141);
        org.zlab.ocov.tracker.Runtime.update(toUC, 140);
