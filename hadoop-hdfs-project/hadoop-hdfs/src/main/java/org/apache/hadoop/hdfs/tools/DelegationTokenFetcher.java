...
public class DelegationTokenFetcher {
    ...
    private static FileSystem getFileSystem(Configuration conf, String url) throws IOException {
        ...
        URI fsUri = URI.create(url.replaceFirst("^http://", WebHdfsConstants.WEBHDFS_SCHEME + "://").replaceFirst("^https://", WebHdfsConstants.SWEBHDFS_SCHEME + "://"));
        return FileSystem.get(fsUri, conf);
    }

    @VisibleForTesting
    static void cancelTokens(final Configuration conf, final Path tokenFile) throws IOException, InterruptedException {
    ...
    @VisibleForTesting
    static void saveDelegationToken(Configuration conf, FileSystem fs, final String renewer, final Path tokenFile) throws IOException {
    ...
    @VisibleForTesting
    public static String printTokensToString(final Configuration conf, final Path tokenFile, final boolean verbose) throws IOException {
    ...
    static void printTokens(final Configuration conf, final Path tokenFile, final boolean verbose) throws IOException {
