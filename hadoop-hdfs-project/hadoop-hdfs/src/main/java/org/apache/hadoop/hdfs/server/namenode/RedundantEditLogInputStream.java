...
class RedundantEditLogInputStream extends EditLogInputStream {
    ...
    private final LogThrottlingHelper fastForwardLoggingHelper = new LogThrottlingHelper(FAST_FORWARD_LOGGING_INTERVAL_MS);
    ...
    RedundantEditLogInputStream(Collection<EditLogInputStream> streams, long startTxId) {
        this.curIdx = 0;
        this.prevTxId = (startTxId == HdfsServerConstants.INVALID_TXID) ? HdfsServerConstants.INVALID_TXID : (startTxId - 1);
        ...
        for (EditLogInputStream s : streams) {
            Preconditions.checkArgument(s.getFirstTxId() != HdfsServerConstants.INVALID_TXID, "invalid first txid in stream: %s", s);
            Preconditions.checkArgument(s.getLastTxId() != HdfsServerConstants.INVALID_TXID, "invalid last txid in stream: %s", s);
            if (first == null) {
                first = s;
            } else {
                    ...
                    try {
                        if (prevTxId != HdfsServerConstants.INVALID_TXID) {
                            ...
                            if (logAction.shouldLog()) {
                                LOG.info("Fast-forwarding stream '" + streams[curIdx].getName() + "' to transaction ID " + (prevTxId + 1) + LogThrottlingHelper.getLogSupressionMessage(logAction));
                            }
