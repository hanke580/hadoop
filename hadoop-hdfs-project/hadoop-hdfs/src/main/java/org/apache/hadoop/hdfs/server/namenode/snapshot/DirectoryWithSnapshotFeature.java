...
public class DirectoryWithSnapshotFeature implements INode.Feature {
    ...
    static class ChildrenDiff extends Diff<byte[], INode> {
         ...
         */
        private void destroyCreatedList(INode.ReclaimContext reclaimContext, final INodeDirectory currentINode) {
    ...
    public static class DirectoryDiff extends AbstractINodeDiff<INodeDirectory, INodeDirectoryAttributes, DirectoryDiff> {
        ...
        @Override
        void combinePosteriorAndCollectBlocks(final INode.ReclaimContext reclaimContext, final INodeDirectory currentDir, final DirectoryDiff posterior) {
        ...
        while (!queue.isEmpty()) {
            ...
            if (topNode instanceof INodeReference.WithName) {
                ...
                for (INode child : dir.getChildrenList(prior)) {
                    if (priorChildrenDiff != null && priorChildrenDiff.getDeleted(child.getLocalNameBytes()) != null) {
                        continue;
                    }
                    queue.addLast(child);
                }
    ...
    public boolean addChild(INodeDirectory parent, INode inode, boolean setModTime, int latestSnapshotId) {
        ...
        try {
            ...
            org.zlab.ocov.tracker.Runtime.update(inode, 117);
        } finally {
            if (!added) {
    ...
    public QuotaCounts computeQuotaUsage4CurrentDirectory(BlockStoragePolicySuite bsps, byte storagePolicyId) {
        final QuotaCounts counts = ((QuotaCounts.Builder) org.zlab.ocov.tracker.Runtime.update(new QuotaCounts.Builder(), 119)).build();
        org.zlab.ocov.tracker.Runtime.update(counts, 118);
    ...
    boolean computeDiffBetweenSnapshots(Snapshot fromSnapshot, Snapshot toSnapshot, ChildrenDiff diff, INodeDirectory currentINode) {
        ...
        List<DirectoryDiff> difflist = diffs.getDiffListBetweenSnapshots(earlierDiffIndex, laterDiffIndex, currentINode);
            ...
            if (prior != NO_SNAPSHOT_ID) {
                ...
                if (priorDiff != null && priorDiff.getSnapshotId() == prior) {
                    ...
                    if (priorCreated != null) {
                        ...
                        final boolean destroy = currentINode.isLastReference() && currentINode.getDiffs().getLastSnapshotId() == prior && !currentINode.isInCurrentState();
                        // we only check the node originally in prior's created list
                        for (INode cNode : new ArrayList<>(priorDiff.diff.getCreatedUnmodifiable())) {
