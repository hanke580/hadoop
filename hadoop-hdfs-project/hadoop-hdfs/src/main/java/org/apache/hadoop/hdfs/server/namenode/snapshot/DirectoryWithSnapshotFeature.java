...
public class DirectoryWithSnapshotFeature implements INode.Feature {
    ...
    public boolean addChild(INodeDirectory parent, INode inode, boolean setModTime, int latestSnapshotId) {
        ...
        try {
            ...
            org.zlab.ocov.tracker.Runtime.update(inode, 8);
        } finally {
            if (!added) {
    ...
    public QuotaCounts computeQuotaUsage4CurrentDirectory(BlockStoragePolicySuite bsps, byte storagePolicyId) {
        final QuotaCounts counts = ((QuotaCounts.Builder) org.zlab.ocov.tracker.Runtime.update(new QuotaCounts.Builder(), 10)).build();
        org.zlab.ocov.tracker.Runtime.update(counts, 9);
