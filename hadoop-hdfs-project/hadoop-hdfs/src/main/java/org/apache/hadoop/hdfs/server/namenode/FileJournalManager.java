...
public class FileJournalManager implements JournalManager {
    ...
    synchronized public void selectInputStreams(Collection<EditLogInputStream> streams, long fromTxnId, boolean inProgressOk) throws IOException {
        selectInputStreams(streams, fromTxnId, inProgressOk, false);
    }

    @Override
    synchronized public void selectInputStreams(Collection<EditLogInputStream> streams, long fromTxId, boolean inProgressOk, boolean onlyDurableTxns) throws IOException {
    ...
    static void internal$addStreamsToCollectionFromFiles(Collection<EditLogFile> elfs, Collection<EditLogInputStream> streams, long fromTxId, long maxTxIdToScan, boolean inProgressOk) {
    ...
    public static class EditLogFile {
        ...
        EditLogFile(File file, long firstTxId, long lastTxId, boolean isInProgress) {
            assert (lastTxId == HdfsServerConstants.INVALID_TXID && isInProgress) || (lastTxId != HdfsServerConstants.INVALID_TXID && lastTxId >= firstTxId);
        ...
        public void scanLog(long maxTxIdToScan, boolean verifyVersion) throws IOException {
            EditLogValidation val = EditLogFileInputStream.scanEditLog(file, maxTxIdToScan, verifyVersion);
    ...
    static void addStreamsToCollectionFromFiles(Collection<EditLogFile> elfs, Collection<EditLogInputStream> streams, long fromTxId, long maxTxIdToScan, boolean inProgressOk) {
        if (!(elfs.size() == streams.size())) {
            org.zlab.dinv.runtimechecker.Runtime.addViolation(2410);
        }
        internal$addStreamsToCollectionFromFiles(elfs, streams, fromTxId, maxTxIdToScan, inProgressOk);
        if (!(elfs.size() == streams.size())) {
            org.zlab.dinv.runtimechecker.Runtime.addViolation(2411);
        }
    }
}
