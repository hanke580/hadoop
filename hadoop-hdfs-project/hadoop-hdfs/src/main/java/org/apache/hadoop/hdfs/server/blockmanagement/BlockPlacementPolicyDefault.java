...
public class BlockPlacementPolicyDefault extends BlockPlacementPolicy {
    ...
    private static final ThreadLocal<HashMap<NodeNotChosenReason, Integer>> CHOOSE_RANDOM_REASONS = new ThreadLocal<HashMap<NodeNotChosenReason, Integer>>() {
    ...
    private enum NodeNotChosenReason {
        ...
        NOT_IN_SERVICE("the node isn't in service"), NODE_STALE("the node is stale"), NODE_TOO_BUSY("the node is too busy"), TOO_MANY_NODES_ON_RACK("the rack has too many chosen nodes"), NOT_ENOUGH_STORAGE_SPACE("no enough storage space to place the block");
    ...
    public void initialize(Configuration conf, FSClusterStats stats, NetworkTopology clusterMap, Host2NodesMap host2datanodeMap) {
        this.considerLoad = conf.getBoolean(DFSConfigKeys.DFS_NAMENODE_REPLICATION_CONSIDERLOAD_KEY, DFSConfigKeys.DFS_NAMENODE_REPLICATION_CONSIDERLOAD_DEFAULT);
    ...
    protected DatanodeStorageInfo chooseLocalStorage(Node localMachine, Set<Node> excludedNodes, long blocksize, int maxNodesPerRack, List<DatanodeStorageInfo> results, boolean avoidStaleNodes, EnumMap<StorageType, Integer> storageTypes) throws NotEnoughReplicasException {
        ...
        if (preferLocalNode && localMachine instanceof DatanodeDescriptor && clusterMap.contains(localMachine)) {
            ...
            excludedNodes.add(localMachine) && isGoodDatanode(localDatanode, maxNodesPerRack, false, results, avoidStaleNodes)) {
