...
public class JournalNode implements Tool, Configurable, JournalNodeMXBean {
    ...
    synchronized Journal getOrCreateJournal(String jid, String nameServiceId, StartupOption startOpt) throws IOException {
        ...
        if (journal == null) {
            ...
            if (conf.getBoolean(DFSConfigKeys.DFS_JOURNALNODE_ENABLE_SYNC_KEY, DFSConfigKeys.DFS_JOURNALNODE_ENABLE_SYNC_DEFAULT)) {
                startSyncer(journal, jid, nameServiceId);
            }
        } else if (journalSyncersById.get(jid) != null && !journalSyncersById.get(jid).isJournalSyncerStarted() && !journalsById.get(jid).getTriedJournalSyncerStartedwithnsId() && nameServiceId != null) {
            startSyncer(journal, jid, nameServiceId);
        }
        return journal;
    }
    ...
    public void setConf(Configuration conf) {
        ...
        if (nameserviceIds.size() < 2) {
            ...
            if (journalNodeDir == null) {
                journalNodeDir = conf.get(DFSConfigKeys.DFS_JOURNALNODE_EDITS_DIR_KEY, DFSConfigKeys.DFS_JOURNALNODE_EDITS_DIR_DEFAULT);
            }
            localDir.add(new File(journalNodeDir.trim()));
        }
        if (this.tracer == null) {
            this.tracer = new Tracer.Builder("JournalNode").conf(TraceUtils.wrapHadoopConf("journalnode.htrace", conf)).build();
        }
    }
    ...
    private File getLogDir(String jid, String nameServiceId) throws IOException {
        ...
        if (dir == null) {
            dir = conf.get(DFSConfigKeys.DFS_JOURNALNODE_EDITS_DIR_KEY, DFSConfigKeys.DFS_JOURNALNODE_EDITS_DIR_DEFAULT);
        }
    ...
    public List<String> getClusterIds() {
        return journalsById.values().stream().map(j -> j.getStorage().getClusterID()).filter(cid -> !Strings.isNullOrEmpty(cid)).distinct().collect(Collectors.toList());
    }
    ...
    public Boolean canRollBack(String journalId, StorageInfo storage, StorageInfo prevStorage, int targetLayoutVersion, String nameServiceId) throws IOException {
        return getOrCreateJournal(journalId, nameServiceId, StartupOption.ROLLBACK).canRollBack(storage, prevStorage, targetLayoutVersion);
    }
