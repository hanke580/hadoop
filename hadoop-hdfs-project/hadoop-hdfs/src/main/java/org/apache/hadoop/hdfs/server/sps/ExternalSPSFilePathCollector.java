...
public class ExternalSPSFilePathCollector implements FileCollector {
    ...
    public ExternalSPSFilePathCollector(SPSService service) {
        ...
        this.maxQueueLimitToScan = service.getConf().getInt(DFSConfigKeys.DFS_STORAGE_POLICY_SATISFIER_QUEUE_LIMIT_KEY, DFSConfigKeys.DFS_STORAGE_POLICY_SATISFIER_QUEUE_LIMIT_DEFAULT);
        try {
    ...
    private long processPath(Long startID, String childPath) {
        // to be satisfied file counter
        long pendingWorkCount = 0;
        for (byte[] lastReturnedName = HdfsFileStatus.EMPTY_NAME; ; ) {
            ...
            for (HdfsFileStatus child : children.getPartialListing()) {
                if (child.isFile()) {
                    ...
                    // increment to be satisfied file count
                    pendingWorkCount++;
                } else {
    ...
    public int remainingCapacity() {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("SPS processing Q -> maximum capacity:{}, current size:{}," + " remaining size:{}", maxQueueLimitToScan, size, remainingSize);
        }
