...
public class IPCLoggerChannel implements AsyncLogger {
    ...
    protected ExecutorService createParallelExecutor() {
        int numThreads = conf.getInt(DFSConfigKeys.DFS_QJOURNAL_PARALLEL_READ_NUM_THREADS_KEY, DFSConfigKeys.DFS_QJOURNAL_PARALLEL_READ_NUM_THREADS_DEFAULT);
        return new ThreadPoolExecutor(1, numThreads, 60L, TimeUnit.SECONDS, new LinkedBlockingQueue<>(), new ThreadFactoryBuilder().setDaemon(true).setNameFormat("Logger channel (from parallel executor) to " + addr).setUncaughtExceptionHandler(UncaughtExceptionHandlers.systemExit()).build());
    }
    ...
    private synchronized void reserveQueueSpace(int size) throws LoggerTooFarBehindException {
        ...
        if (queuedEditsSizeBytes + size > queueSizeLimitBytes && queuedEditsSizeBytes > 0) {
            QuorumJournalManager.LOG.warn("Pending edits to " + IPCLoggerChannel.this + " is going to exceed limit size: " + queueSizeLimitBytes + ", current queued edits size: " + queuedEditsSizeBytes + ", will silently drop " + size + " bytes of edits!");
    ...
    public ListenableFuture<GetJournaledEditsResponseProto> getJournaledEdits(long fromTxnId, int maxTransactions) {
        return parallelExecutor.submit(new Callable<GetJournaledEditsResponseProto>() {
    ...
    public ListenableFuture<RemoteEditLogManifest> getEditLogManifest(final long fromTxnId, final boolean inProgressOk) {
        return parallelExecutor.submit(new Callable<RemoteEditLogManifest>() {
            ...
            public RemoteEditLogManifest call() throws IOException {
                GetEditLogManifestResponseProto ret = getProxy().getEditLogManifest(journalId, nameServiceId, fromTxnId, inProgressOk);
    ...
    public ListenableFuture<Boolean> canRollBack(final StorageInfo storage, final StorageInfo prevStorage, final int targetLayoutVersion) {
        return singleThreadExecutor.submit(new Callable<Boolean>() {
            ...
            @Override
            public Boolean call() throws IOException {
                return getProxy().canRollBack(journalId, nameServiceId, storage, prevStorage, targetLayoutVersion);
            }
        });
    }
