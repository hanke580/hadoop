...
public abstract class INode implements INodeAttributes, Diff.Element<byte[]> {
    ...
    public QuotaCounts getQuotaCounts() {
        return ((QuotaCounts.Builder) org.zlab.ocov.tracker.Runtime.update(new QuotaCounts.Builder(), 104)).nameSpace(HdfsConstants.QUOTA_RESET).storageSpace(HdfsConstants.QUOTA_RESET).typeSpaces(HdfsConstants.QUOTA_RESET).build();
    }
