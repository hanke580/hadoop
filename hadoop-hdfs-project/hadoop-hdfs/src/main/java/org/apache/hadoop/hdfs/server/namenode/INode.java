...
public abstract class INode implements INodeAttributes, Diff.Element<byte[]> {
    ...
    public boolean isInCurrentState() {
        ...
        if (parentDir == null) {
            // this inode is only referenced in snapshots
            return false;
        }
        if (!parentDir.isInCurrentState()) {
            return false;
        }
        final INode child = parentDir.getChild(getLocalNameBytes(), Snapshot.CURRENT_STATE_ID);
        if (this == child) {
            return true;
        }
        return child != null && child.isReference() && this.equals(child.asReference().getReferredINode());
    }
    ...
    public final ContentSummary computeContentSummary(BlockStoragePolicySuite bsps) throws AccessControlException {
        return computeAndConvertContentSummary(Snapshot.CURRENT_STATE_ID, new ContentSummaryComputationContext(bsps));
    }

    /**
     * Compute {@link ContentSummary}.
     */
    public final ContentSummary computeAndConvertContentSummary(int snapshotId, ContentSummaryComputationContext summary) throws AccessControlException {
     ...
     */
    public abstract ContentSummaryComputationContext computeContentSummary(int snapshotId, ContentSummaryComputationContext summary) throws AccessControlException;
    ...
    public QuotaCounts getQuotaCounts() {
        return ((QuotaCounts.Builder) org.zlab.ocov.tracker.Runtime.update(new QuotaCounts.Builder(), 91)).nameSpace(HdfsConstants.QUOTA_RESET).storageSpace(HdfsConstants.QUOTA_RESET).typeSpaces(HdfsConstants.QUOTA_RESET).build();
    }
    ...
    public String toDetailString() {
        return toString() + "(" + getId() + ", " + getObjectString() + ", " + getParentString() + ")";
    }
    ...
    public final StringBuilder dumpParentINodes() {
        final StringBuilder b = parent == null ? new StringBuilder() : parent.dumpParentINodes().append("\n  ");
    ...
    public static class ReclaimContext {
         ...
         */
        public ReclaimContext(BlockStoragePolicySuite bsps, BlocksMapUpdateInfo collectedBlocks, List<INode> removedINodes, List<Long> removedUCFiles) {
            this.bsps = bsps;
        ...
        public void setSnapshotToBeDeleted(Snapshot snapshot) {
            this.snapshotToBeDeleted = Objects.requireNonNull(snapshot, "snapshot == null");
        }
        ...
        public int getSnapshotIdToBeDeleted(int snapshotId, INode inode) {
            ...
            if (snapshotId != snapshotIdToBeDeleted) {
                LOG.warn("Snapshot changed: current = {}, original = {}, inode: {}", Snapshot.getSnapshotString(snapshotId), snapshotToBeDeleted, inode.toDetailString());
            }
        ...
        public ReclaimContext getCopy() {
            final ReclaimContext that = new ReclaimContext(bsps, collectedBlocks, removedINodes, removedUCFiles);
