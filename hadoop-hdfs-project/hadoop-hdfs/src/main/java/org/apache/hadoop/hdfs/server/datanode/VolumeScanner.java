...
public class VolumeScanner extends Thread {
    ...
    public void printStats(StringBuilder p) {
        p.append(String.format("Block scanner information for volume %s with base" + " path %s%n", volume.getStorageID(), volume.getBasePath()));
        synchronized (stats) {
    ...
    private long runLoop(ExtendedBlock suspectBlock) {
        ...
        try {
            ...
            if (curBlockIter != null) {
                ...
                if ((org.zlab.ocov.tracker.Runtime.updateBranch(saveDelta, conf.cursorSaveMs, ">=", 10))) {
