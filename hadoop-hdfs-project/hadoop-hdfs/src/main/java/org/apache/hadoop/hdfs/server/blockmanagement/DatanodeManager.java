...
public class DatanodeManager {
    ...
    DatanodeManager(final BlockManager blockManager, final Namesystem namesystem, final Configuration conf) throws IOException {
        ...
        this.useDfsNetworkTopology = conf.getBoolean(DFSConfigKeys.DFS_USE_DFS_NETWORK_TOPOLOGY_KEY, DFSConfigKeys.DFS_USE_DFS_NETWORK_TOPOLOGY_DEFAULT);
        ...
        this.dataNodeDiskStatsEnabled = Util.isDiskStatsEnabled(conf.getInt(DFSConfigKeys.DFS_DATANODE_FILEIO_PROFILING_SAMPLING_PERCENTAGE_KEY, DFSConfigKeys.DFS_DATANODE_FILEIO_PROFILING_SAMPLING_PERCENTAGE_DEFAULT));
        final Timer timer = new Timer();
        final boolean dataNodePeerStatsEnabledVal = conf.getBoolean(DFSConfigKeys.DFS_DATANODE_PEER_STATS_ENABLED_KEY, DFSConfigKeys.DFS_DATANODE_PEER_STATS_ENABLED_DEFAULT);
        this.maxSlowPeerReportNodes = conf.getInt(DFSConfigKeys.DFS_NAMENODE_MAX_SLOWPEER_COLLECT_NODES_KEY, DFSConfigKeys.DFS_NAMENODE_MAX_SLOWPEER_COLLECT_NODES_DEFAULT);
        this.slowPeerCollectionInterval = conf.getTimeDuration(DFSConfigKeys.DFS_NAMENODE_SLOWPEER_COLLECT_INTERVAL_KEY, DFSConfigKeys.DFS_NAMENODE_SLOWPEER_COLLECT_INTERVAL_DEFAULT, TimeUnit.MILLISECONDS);
        ...
        this.hostConfigManager = ReflectionUtils.newInstance(conf.getClass(DFSConfigKeys.DFS_NAMENODE_HOSTS_PROVIDER_CLASSNAME_KEY, HostFileManager.class, HostConfigManager.class), conf);
        try {
        ...
        final int configuredBlockInvalidateLimit = conf.getInt(DFSConfigKeys.DFS_BLOCK_INVALIDATE_LIMIT_KEY, DFSConfigKeys.DFS_BLOCK_INVALIDATE_LIMIT_DEFAULT);
        ...
        this.avoidSlowDataNodesForRead = conf.getBoolean(DFSConfigKeys.DFS_NAMENODE_AVOID_SLOW_DATANODE_FOR_READ_KEY, DFSConfigKeys.DFS_NAMENODE_AVOID_SLOW_DATANODE_FOR_READ_DEFAULT);
        this.readConsiderLoad = conf.getBoolean(DFSConfigKeys.DFS_NAMENODE_READ_CONSIDERLOAD_KEY, DFSConfigKeys.DFS_NAMENODE_READ_CONSIDERLOAD_DEFAULT);
     ...
     */
    public void initSlowPeerTracker(Configuration conf, Timer timer, boolean dataNodePeerStatsEnabled) {
        this.slowPeerTracker = dataNodePeerStatsEnabled ? new SlowPeerTracker(conf, timer) : new SlowPeerDisabledTracker(conf, timer);
    ...
    public void restartSlowPeerCollector(long interval) {
        Preconditions.checkNotNull(slowPeerCollectorDaemon, "slowPeerCollectorDaemon thread is null, not support restart");
        stopSlowPeerCollector();
    ...
    private boolean isInactive(DatanodeInfo datanode) {
        return datanode.isDecommissioned() || datanode.isDecommissionInProgress() || datanode.isEnteringMaintenance() || (avoidStaleDataNodesForRead && datanode.isStale(staleInterval));
    }
    ...
    public void sortLocatedBlocks(final String targetHost, final List<LocatedBlock> locatedBlocks) {
        Comparator<DatanodeInfo> comparator = avoidStaleDataNodesForRead || avoidSlowDataNodesForRead ? new DFSUtil.StaleAndSlowComparator(avoidStaleDataNodesForRead, staleInterval, avoidSlowDataNodesForRead, slowNodesUuidSet) : new DFSUtil.ServiceComparator();
        // sort located block
     ...
     * @param comparator dn comparator
     */
    private void sortLocatedBlock(final LocatedBlock lb, String targetHost, Comparator<DatanodeInfo> comparator) {
        ...
        while (lastActiveIndex > 0 && (isSlowNode(di[lastActiveIndex].getDatanodeUuid()) || isInactive(di[lastActiveIndex]))) {
            --lastActiveIndex;
        }
        int activeLen = lastActiveIndex + 1;
        if (nonDatanodeReader) {
            networktopology.sortByDistanceUsingNetworkLocation(client, lb.getLocations(), activeLen, createSecondaryNodeSorter());
        } else {
            networktopology.sortByDistance(client, lb.getLocations(), activeLen, createSecondaryNodeSorter());
        }
    ...
    private Consumer<List<DatanodeInfoWithStorage>> createSecondaryNodeSorter() {
        ...
        if (readConsiderStorageType) {
            Comparator<DatanodeInfoWithStorage> comp = Comparator.comparing(DatanodeInfoWithStorage::getStorageType);
            secondarySort = list -> Collections.sort(list, comp);
        }
        if (readConsiderLoad) {
            Comparator<DatanodeInfoWithStorage> comp = Comparator.comparingInt(DatanodeInfo::getXceiverCount);
            secondarySort = list -> Collections.sort(list, comp);
        }
        ...
        if (node == null) {
            ...
            if (rackNodes != null) {
                ...
                if (node == null && !rackNodes.isEmpty()) {
                    node = (DatanodeDescriptor) (rackNodes.get(ThreadLocalRandom.current().nextInt(rackNodes.size())));
                }
            }
    ...
    public DatanodeStorageInfo[] getDatanodeStorageInfos(DatanodeID[] datanodeID, String[] storageIDs, String format, Object... args) throws UnregisteredNodeException {
        ...
        if (datanodeID.length != storageIDs.length) {
            ...
            final String err = (storageIDs.length == 0 ? "Missing storageIDs: It is likely that the HDFS client," + " who made this call, is running in an older version of Hadoop" + "(pre-2.0.0-alpha)  which does not support storageIDs." : "Length mismatched: storageIDs.length=" + storageIDs.length + " != ") + " datanodeID.length=" + datanodeID.length;
    ...
    void removeDeadDatanode(final DatanodeID nodeID, boolean removeBlocksFromBlockMap) {
        ...
        if (d != null && isDatanodeDead(d)) {
            NameNode.stateChangeLog.info("BLOCK* removeDeadDatanode: lost heartbeat from " + d + ", removeBlocksFromBlockMap " + removeBlocksFromBlockMap);
    ...
    void addDatanode(final DatanodeDescriptor node) {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("{}.addDatanode: node {} is added to datanodeMap.", getClass().getSimpleName(), node);
        }
    }
    ...
    private void wipeDatanode(final DatanodeID node) {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("{}.wipeDatanode({}): storage {} is removed from datanodeMap.", getClass().getSimpleName(), node, key);
        }
    }
    ...
    public void registerDatanode(DatanodeRegistration nodeReg) throws DisallowedDatanodeException, UnresolvedTopologyException {
         ...
         */
            ...
            DatanodeDescriptor nodeDescr = ((DatanodeDescriptor) org.zlab.ocov.tracker.Runtime.update(new DatanodeDescriptor(nodeReg, NetworkTopology.DEFAULT_RACK), 7));
            boolean success = false;
            try {
                // resolve network location
                if (this.rejectUnresolvedTopologyDN) {
    ...
    public boolean shouldAvoidStaleDataNodesForWrite() {
        ...
        return avoidStaleDataNodesForWrite && numStaleNodes > 0 && (numStaleNodes <= heartbeatManager.getLiveDatanodeCount() * ratioUseStaleDataNodesForWrite);
    }
    ...
    public List<DatanodeDescriptor> getDatanodeListForReport(final DatanodeReportType type) {
        ...
        final boolean listEnteringMaintenanceNodes = type == DatanodeReportType.ALL || type == DatanodeReportType.ENTERING_MAINTENANCE;
        final boolean listInMaintenanceNodes = type == DatanodeReportType.ALL || type == DatanodeReportType.IN_MAINTENANCE;
        ...
        synchronized (this) {
            ...
            for (DatanodeDescriptor dn : datanodeMap.values()) {
                ...
                if (((listLiveNodes && !isDead) || (listDeadNodes && isDead) || (listDecommissioningNodes && isDecommissioning) || (listEnteringMaintenanceNodes && isEnteringMaintenance) || (listInMaintenanceNodes && isInMaintenance)) && hostConfigManager.isIncluded(dn)) {
                    nodes.add(dn);
                }
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("getDatanodeListForReport with includedNodes = {}, excludedNodes = {}" + ", foundNodes = {}, nodes = {}.", hostConfigManager.getIncludes(), hostConfigManager.getExcludes(), foundNodes, nodes);
        }
        return nodes;
    }
    ...
    public List<DatanodeDescriptor> getAllSlowDataNodes() {
        if (slowPeerTracker == null) {
            LOG.debug("{} is disabled. Try enabling it first to capture slow peer outliers.", DFSConfigKeys.DFS_DATANODE_PEER_STATS_ENABLED_KEY);
    ...
    private BlockRecoveryCommand getBlockRecoveryCommand(String blockPoolId, DatanodeDescriptor nodeinfo) throws IOException {
        ...
        for (BlockInfo b : blocks) {
            ...
            if (truncateRecovery) {
                ...
                if (b.isStriped()) {
                    rBlock = new RecoveringStripedBlock(rBlock, uc.getBlockIndicesForSpecifiedStorages(storageIdx), ((BlockInfoStriped) b).getErasureCodingPolicy());
                }
            }
    ...
    public DatanodeCommand[] handleHeartbeat(DatanodeRegistration nodeReg, StorageReport[] reports, final String blockPoolId, long cacheCapacity, long cacheUsed, int xceiverCount, int xmitsInProgress, int failedVolumes, VolumeFailureSummary volumeFailureSummary, @Nonnull SlowPeerReports slowPeers, @Nonnull SlowDiskReports slowDisks) throws IOException {
        ...
        if (totalBlocks > 0) {
            ...
            List<BlockTargetPair> pendingReplicationList = nodeinfo.getReplicationCommand(numReplicationTasks);
            List<BlockTargetPair> pendingECReplicatedList = nodeinfo.getECReplicatedCommand(numEcReplicatedTasks);
            ...
            List<BlockECReconstructionInfo> pendingECList = nodeinfo.getErasureCodeCommand(numECReconstructedTasks);
     ...
     */
    public void handleLifeline(DatanodeRegistration nodeReg, StorageReport[] reports, long cacheCapacity, long cacheUsed, int xceiverCount, int failedVolumes, VolumeFailureSummary volumeFailureSummary) throws IOException {
    ...
    public void setBlockInvalidateLimit(int configuredBlockInvalidateLimit) {
        ...
        LOG.info("{} : configured={}, counted={}, effected={}", DFSConfigKeys.DFS_BLOCK_INVALIDATE_LIMIT_KEY, configuredBlockInvalidateLimit, countedBlockInvalidateLimit, this.blockInvalidateLimit);
    }
    ...
    public Set<String> getSlowPeersUuidSet() {
        ...
        datanodeDescriptors.forEach(datanodeDescriptor -> slowPeersUuidSet.add(datanodeDescriptor.getDatanodeUuid()));
    ...
    public DatanodeStorageReport[] getDatanodeStorageReport(DatanodeReportType type) {
        ...
        for (int i = 0; i < reports.length; i++) {
            ...
            reports[i] = new DatanodeStorageReport(new DatanodeInfoBuilder().setFrom(d).setNumBlocks(d.numBlocks()).build(), d.getStorageReports());
        }
        return reports;
    }
