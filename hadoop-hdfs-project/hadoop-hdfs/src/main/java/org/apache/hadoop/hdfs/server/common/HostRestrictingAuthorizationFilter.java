   ...
   */
    private boolean matchRule(String user, String remoteIp, String path) {
        ...
        for (Rule rule : rules) {
            ...
            LOG.trace("Evaluating rule, subnet: {}, path: {}", subnet != null ? subnet.getCidrSignature() : "*", rulePath);
            try {
                if ((subnet == null || subnet.isInRange(remoteIp)) && FilenameUtils.directoryContains(rulePath, path)) {
                    LOG.debug("Found matching rule, subnet: {}, path: {}; returned true", rule.getSubnet() != null ? subnet.getCidrSignature() : null, rulePath);
                    return true;
                }
            } catch (IOException e) {
   ...
   */
    private void loadRuleMap(String ruleString) throws IllegalArgumentException {
        if (ruleString == null || ruleString.equals("")) {
            ...
            Map<Integer, List<String[]>> splits = rule_split.splitAsStream(ruleString).map(x -> comma_split.split(x, 3)).collect(Collectors.groupingBy(x -> x.length));
            ...
            if (!splits.keySet().equals(Collections.singleton(3))) {
                ...
                String bad_lines = rule_split.splitAsStream(ruleString).filter(x -> comma_split.split(x, 3).length != 3).collect(Collectors.joining("\n"));
   ...
   */
    public void handleInteraction(HttpInteraction interaction) throws IOException, ServletException {
        ...
        if (!uri.startsWith(WebHdfsFileSystem.PATH_PREFIX)) {
            ...
            interaction.sendError(HttpServletResponse.SC_NOT_FOUND, "The request URI must start with " + WebHdfsFileSystem.PATH_PREFIX);
            return;
        }
        ...
        if (!interaction.isCommitted() && authenticatedQuery) {
            ...
            if (authenticatedQuery && !(matchRule("*", address, path) || matchRule(user, address, path))) {
                LOG.trace("Rejecting interaction; no rule found");
                interaction.sendError(HttpServletResponse.SC_FORBIDDEN, "WebHDFS is configured write-only for " + user + "@" + address + " for file: " + path);
                return;
            }
        }
