...
public abstract class MD5FileUtils {
    ...
    private static Matcher readStoredMd5(File md5File) throws IOException {
        BufferedReader reader = new BufferedReader(new InputStreamReader(Files.newInputStream(md5File.toPath()), StandardCharsets.UTF_8));
        String md5Line;
        try {
    ...
    private static void saveMD5File(File dataFile, String digestString) throws IOException {
        ...
        AtomicFileOutputStream afos = ((AtomicFileOutputStream) org.zlab.ocov.tracker.Runtime.update(new AtomicFileOutputStream(md5File), 129));
