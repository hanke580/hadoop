...
public class ReplicaBuilder {
    ...
    private LocalReplica buildRUR() throws IllegalArgumentException {
        ...
        if (fromReplica.getState() == ReplicaState.RUR) {
            return ((ReplicaUnderRecovery) org.zlab.ocov.tracker.Runtime.update(new ReplicaUnderRecovery((ReplicaUnderRecovery) fromReplica), 166));
        } else {
            return ((ReplicaUnderRecovery) org.zlab.ocov.tracker.Runtime.update(new ReplicaUnderRecovery(fromReplica, recoveryId), 167));
        }
    }
