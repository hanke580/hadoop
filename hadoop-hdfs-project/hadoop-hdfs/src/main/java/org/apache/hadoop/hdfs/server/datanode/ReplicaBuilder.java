...
public class ReplicaBuilder {
    ...
    private LocalReplica buildFinalizedReplica() throws IllegalArgumentException {
        if (null != fromReplica && fromReplica.getState() == ReplicaState.FINALIZED) {
            ...
            if (null != block) {
                ...
                return new FinalizedReplica(blockId, length, genStamp, volume, directoryUsed, lastPartialChunkChecksum);
            }
        }
    }
    ...
    private LocalReplica buildRUR() throws IllegalArgumentException {
        ...
        if (fromReplica.getState() == ReplicaState.RUR) {
            return ((ReplicaUnderRecovery) org.zlab.ocov.tracker.Runtime.update(new ReplicaUnderRecovery((ReplicaUnderRecovery) fromReplica), 125));
        } else {
            return ((ReplicaUnderRecovery) org.zlab.ocov.tracker.Runtime.update(new ReplicaUnderRecovery(fromReplica, recoveryId), 126));
        }
    }
    ...
    private ProvidedReplica buildProvidedFinalizedReplica() throws IllegalArgumentException {
        ...
        if (fileRegion == null && uri == null && (pathPrefix == null || pathSuffix == null)) {
            throw new IllegalArgumentException("Trying to construct a provided replica on " + volume + " without enough information");
        }
        if (fileRegion == null) {
            if (uri != null) {
                info = new FinalizedProvidedReplica(blockId, uri, offset, length, genStamp, pathHandle, volume, conf, remoteFS);
            } else {
                info = new FinalizedProvidedReplica(blockId, pathPrefix, pathSuffix, offset, length, genStamp, pathHandle, volume, conf, remoteFS);
            }
        } else {
    ...
    private ProvidedReplica buildProvidedReplica() throws IllegalArgumentException {
        ...
        switch(this.state) {
            ...
            case TEMPORARY:
            default:
                throw new IllegalArgumentException("Unknown replica state " + state + " for PROVIDED replica");
        }
        return info;
    }
