...
public class FsVolumeImpl implements FsVolumeSpi {
    ...
    FsVolumeImpl(FsDatasetImpl dataset, String storageID, StorageDirectory sd, FileIoProvider fileIoProvider, Configuration conf) throws IOException {
        // outside tests, usage created in ReservedSpaceCalculator.Builder
        this(dataset, storageID, sd, fileIoProvider, conf, null);
    }

    FsVolumeImpl(FsDatasetImpl dataset, String storageID, StorageDirectory sd, FileIoProvider fileIoProvider, Configuration conf, DF usage) throws IOException {
        ...
        this.usage = usage;
        if (this.usage != null) {
            reserved = new ReservedSpaceCalculator.Builder(conf).setUsage(this.usage).setStorageType(storageType).build();
    ...
    boolean checkClosed() {
        if (this.reference.getReferenceCount() > 0) {
            FsDatasetImpl.LOG.debug("The reference count for {} is {}, wait to be 0.", this, reference.getReferenceCount());
            return false;
        }
        return true;
    }
    ...
    private class BlockIteratorImpl implements FsVolumeSpi.BlockIterator {
        ...
        private String getNextSubDir(String prev, File dir) throws IOException {
            List<String> children = fileIoProvider.listDirectory(FsVolumeImpl.this, dir, SubdirFilter.INSTANCE);
            cache = null;
            cacheMs = 0;
            if (children.isEmpty()) {
        ...
        private List<String> getSubdirEntries() throws IOException {
            ...
            List<String> entries = fileIoProvider.listDirectory(FsVolumeImpl.this, dir, BlockFileFilter.INSTANCE);
            if (entries.isEmpty()) {
                entries = null;
        ...
        private File getBlockFile(String bpid, ExtendedBlock blk) throws IOException {
            return new File(DatanodeUtil.idToBlockDir(getFinalizedDir(bpid), blk.getBlockId()).toString() + "/" + blk.getBlockName());
        }
        ...
        public void save() throws IOException {
            ...
            try (BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(fileIoProvider.getFileOutputStream(FsVolumeImpl.this, getTempSaveFile()), "UTF-8"))) {
            ...
            fileIoProvider.move(FsVolumeImpl.this, getTempSaveFile().toPath(), getSaveFile().toPath(), StandardCopyOption.ATOMIC_MOVE);
    ...
    ReplicaInfo addFinalizedBlock(String bpid, Block b, ReplicaInfo replicaInfo, long bytesReserved) throws IOException {
        ...
        return new ReplicaBuilder(ReplicaState.FINALIZED).setBlock(replicaInfo).setFsVolume(this).setDirectoryToUse(dest.getParentFile()).setLastPartialChunkChecksum(checksum).build();
    }
    ...
    boolean isBPDirEmpty(String bpid) throws IOException {
        ...
        if (fileIoProvider.exists(this, finalizedDir) && !DatanodeUtil.dirNoFilesRecursive(this, finalizedDir, fileIoProvider)) {
            return false;
        }
    ...
    void deleteBPDirectories(String bpid, boolean force) throws IOException {
        ...
        if (force) {
            ...
            if (!DatanodeUtil.dirNoFilesRecursive(this, finalizedDir, fileIoProvider) || !fileIoProvider.fullyDelete(this, finalizedDir)) {
                throw new IOException("Failed to delete " + finalizedDir);
            }
            if (lazypersistDir.exists() && ((!DatanodeUtil.dirNoFilesRecursive(this, lazypersistDir, fileIoProvider) || !fileIoProvider.fullyDelete(this, lazypersistDir)))) {
    ...
    private File[] copyReplicaWithNewBlockIdAndGS(ReplicaInfo replicaInfo, String bpid, long newBlkId, long newGS) throws IOException {
        ...
        return FsDatasetImpl.copyBlockFiles(replicaInfo, dstMetaFile, dstBlockFile, true, DFSUtilClient.getSmallBufferSize(conf), conf);
    }
    ...
    public enum BlockDirFilter implements FilenameFilter {
        ...
        @Override
        public boolean accept(File dir, String name) {
            return name.startsWith(DataStorage.BLOCK_SUBDIR_PREFIX) || name.startsWith(DataStorage.STORAGE_DIR_FINALIZED) || name.startsWith(Block.BLOCK_FILE_PREFIX);
        }
    }

    private void compileReport(File bpFinalizedDir, File dir, Collection<ScanInfo> report, ReportCompiler reportCompiler) throws InterruptedException {
     ...
     */
        for (int i = 0; i < fileNames.size(); i++) {
            ...
            report.add(new ScanInfo(blockId, dir, blockFile.getName(), metaFile == null ? null : metaFile.getName(), this));
        }
    }
    ...
    public ReplicaInfo activateSavedReplica(String bpid, ReplicaInfo replicaInfo, RamDiskReplica replicaState) throws IOException {
