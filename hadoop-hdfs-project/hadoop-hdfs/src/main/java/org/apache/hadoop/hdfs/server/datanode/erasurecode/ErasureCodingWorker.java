...
public final class ErasureCodingWorker {
    ...
    public ErasureCodingWorker(Configuration conf, DataNode datanode) {
        ...
        this.conf = conf;
        this.xmitWeight = conf.getFloat(DFSConfigKeys.DFS_DN_EC_RECONSTRUCTION_XMITS_WEIGHT_KEY, DFSConfigKeys.DFS_DN_EC_RECONSTRUCTION_XMITS_WEIGHT_DEFAULT);
        Preconditions.checkArgument(this.xmitWeight >= 0, "Invalid value configured for " + DFSConfigKeys.DFS_DN_EC_RECONSTRUCTION_XMITS_WEIGHT_KEY + ", it can not be negative value (" + this.xmitWeight + ").");
    ...
    private void initializeStripedReadThreadPool() {
        ...
        stripedReadPool = new ThreadPoolExecutor(0, Integer.MAX_VALUE, 60, TimeUnit.SECONDS, new SynchronousQueue<>(), new Daemon.DaemonFactory() {
    ...
    private void initializeStripedBlkReconstructionThreadPool(int numThreads) {
        ...
        stripedReconstructionPool = DFSUtilClient.getThreadPoolExecutor(numThreads, numThreads, 60, new LinkedBlockingQueue<>(), "StripedBlockReconstruction-", false);
