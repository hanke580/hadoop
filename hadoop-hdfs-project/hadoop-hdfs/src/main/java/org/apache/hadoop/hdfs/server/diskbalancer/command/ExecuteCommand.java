...
public class ExecuteCommand extends Command {
    ...
    public ExecuteCommand(Configuration conf) {
        ...
        addValidCommandParameters(DiskBalancerCLI.SKIPDATECHECK, "skips the date check and force execute the plan");
    }
    ...
    public void execute(CommandLine cmd) throws Exception {
        ...
        if (cmd.hasOption(DiskBalancerCLI.SKIPDATECHECK)) {
            skipDateCheck = true;
            LOG.warn("Skipping date check on this plan. This could mean we are " + "executing an old plan and may not be the right plan for this " + "data node.");
        }
     ...
     * @throws IOException
     */
    private void submitPlan(final String planFile, final String planData, boolean skipDateCheck) throws IOException {
        ...
        try {
            dataNode.submitDiskBalancerPlan(planHash, DiskBalancerCLI.PLAN_VERSION, planFile, planData, skipDateCheck);
