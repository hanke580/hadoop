...
public enum XAttrFormat implements LongBitFormat.Enum {
    ...
    RESERVED(null, 5), NS_EXT(RESERVED.BITS, 1), NAME(NS_EXT.BITS, 24), NS(NAME.BITS, 2);
    ...
    static XAttr toXAttr(int record, byte[] value, SerialNumberManager.StringTable stringTable) {
        ...
        return new XAttr.Builder().setNameSpace(getNamespace(record)).setName(name).setValue(value).build();
    }
