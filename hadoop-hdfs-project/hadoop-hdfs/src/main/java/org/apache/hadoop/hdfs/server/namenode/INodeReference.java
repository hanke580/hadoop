...
public abstract class INodeReference extends INode {
    ...
    public void destroyAndCollectBlocks(ReclaimContext reclaimContext) {
        if ((org.zlab.ocov.tracker.Runtime.updateBranch(removeReference(this) <= 0, 28))) {
    ...
    public static class WithName extends INodeReference {
        ...
        String getNameDetails() {
            return getClass().getSimpleName() + "[" + getLocalName() + ", lastSnapshot=" + lastSnapshotId + "]";
        }
        ...
        public final ContentSummaryComputationContext computeContentSummary(int snapshotId, ContentSummaryComputationContext summary) throws AccessControlException {
            Preconditions.checkState(snapshotId == Snapshot.CURRENT_STATE_ID || this.lastSnapshotId >= snapshotId);
        ...
        public void cleanSubtree(ReclaimContext reclaimContext, final int snapshot, int prior) {
            ...
            if ((org.zlab.ocov.tracker.Runtime.updateBranch(snapshot < lastSnapshotId, 29))) {
        ...
        public void destroyAndCollectBlocks(ReclaimContext reclaimContext) {
            ...
            if ((org.zlab.ocov.tracker.Runtime.updateBranch(removeReference(this) <= 0, 30))) {
    ...
    public static class DstReference extends INodeReference {
        ...
        String getDstDetails() {
            return getClass().getSimpleName() + "[" + getLocalName() + ", dstSnapshot=" + dstSnapshotId + "]";
        }
        ...
        private void shouldDestroy(ReclaimContext context) {
            ...
            if (snapshotToBeDeleted == Snapshot.CURRENT_STATE_ID || snapshotToBeDeleted > dstSnapshotId) {
                return;
            }
            LOG.warn("Try to destroy a DstReference with dstSnapshotId = {}" + " >= snapshotToBeDeleted = {}", dstSnapshotId, snapshotToBeDeleted);
        ...
        public void destroyAndCollectBlocks(ReclaimContext reclaimContext) {
            ...
            reclaimContext.quotaDelta().add(computeQuotaUsage(reclaimContext.bsps));
            ReclaimContext newCtx = reclaimContext.getCopy();
            if ((org.zlab.ocov.tracker.Runtime.updateBranch(removeReference(this) <= 0, 31))) {
