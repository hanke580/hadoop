...
class JournaledEditsCache {
    ...
    JournaledEditsCache(Configuration conf) {
        capacity = conf.getInt(DFSConfigKeys.DFS_JOURNALNODE_EDIT_CACHE_SIZE_KEY, DFSConfigKeys.DFS_JOURNALNODE_EDIT_CACHE_SIZE_DEFAULT);
        if (capacity > 0.9 * Runtime.getRuntime().maxMemory()) {
            Journal.LOG.warn(String.format("Cache capacity is set at %d bytes but " + "maximum JVM memory is only %d bytes. It is recommended that you " + "decrease the cache size or increase the heap size.", capacity, Runtime.getRuntime().maxMemory()));
        }
        Journal.LOG.info("Enabling the journaled edits cache with a capacity " + "of bytes: " + capacity);
    ...
    int retrieveEdits(long requestedStartTxn, int maxTxns, List<ByteBuffer> outputBuffers) throws IOException {
        ...
        try (AutoCloseableLock l = readLock.acquire()) {
            ...
            Iterator<Map.Entry<Long, byte[]>> incrBuffIter = dataMap.tailMap(dataMap.floorKey(requestedStartTxn), true).entrySet().iterator();
            ...
            (txnCount < maxTxns) && (incrBuffIter.hasNext() || prevBuf != null)) {
                ...
                if (prevBuf != null) {
                    // True except for the first loop iteration
    ...
    void storeEdits(byte[] inputData, long newStartTxn, long newEndTxn, int newLayoutVersion) {
        if (newStartTxn < 0 || newEndTxn < newStartTxn) {
            Journal.LOG.error(String.format("Attempted to cache data of length %d " + "with newStartTxn %d and newEndTxn %d", inputData.length, newStartTxn, newEndTxn));
            return;
        }
        try (AutoCloseableLock l = writeLock.acquire()) {
            if (newLayoutVersion != layoutVersion) {
                try {
                    ...
                    Journal.LOG.error(String.format("Unable to save new edits [%d, %d] " + "due to exception when updating to new layout version %d", newStartTxn, newEndTxn, newLayoutVersion), ioe);
                    return;
                }
                ...
                Journal.LOG.error(String.format("Edits cache is out of sync; " + "looked for next txn id at %d but got start txn id for " + "cache put request at %d. Reinitializing at new request.", highestTxnId + 1, newStartTxn));
                initialize(newStartTxn);
            }
    ...
    private int findTransactionPosition(byte[] buf, long txnId) throws IOException {
        ...
        FSEditLogOp.Reader reader = FSEditLogOp.Reader.create(new DataInputStream(tracker), tracker, layoutVersion);
        long previousPos = 0;
    ...
    private void updateLayoutVersion(int newLayoutVersion, long newStartTxn) throws IOException {
        ...
        if (layoutVersion != INVALID_LAYOUT_VERSION) {
            logMsg.append("; previous version was ").append(layoutVersion).append("; old entries will be cleared.");
        }
    ...
    private CacheMissException getCacheMissException(long requestedTxnId) {
        if (lowestTxnId == INVALID_TXN_ID) {
            return new CacheMissException(0, "Cache is empty; either it was never " + "written to or the last write overflowed the cache capacity.");
        } else if (requestedTxnId < initialTxnId) {
            return new CacheMissException(initialTxnId - requestedTxnId, "Cache started at txn ID %d but requested txns starting at %d.", initialTxnId, requestedTxnId);
        } else {
            return new CacheMissException(lowestTxnId - requestedTxnId, "Oldest txn ID available in the cache is %d, but requested txns " + "starting at %d. The cache size (%s) may need to be increased " + "to hold more transactions (currently %d bytes containing %d " + "transactions)", lowestTxnId, requestedTxnId, DFSConfigKeys.DFS_JOURNALNODE_EDIT_CACHE_SIZE_KEY, capacity, highestTxnId - lowestTxnId + 1);
        }
    }
