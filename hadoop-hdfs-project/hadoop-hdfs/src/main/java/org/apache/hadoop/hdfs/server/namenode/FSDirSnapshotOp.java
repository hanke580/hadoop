...
class FSDirSnapshotOp {
    ...
    public static final Logger LOG = LoggerFactory.getLogger(FSDirSnapshotOp.class);
     ...
     *                      rebuilding.
     */
    static String createSnapshot(FSDirectory fsd, FSPermissionChecker pc, SnapshotManager snapshotManager, String snapshotRoot, String snapshotName, boolean logRetryCache) throws IOException {
        final INodesInPath iip = fsd.resolvePath(pc, snapshotRoot, DirOp.WRITE);
        org.zlab.ocov.tracker.Runtime.update(iip, 100);
        ...
        fsd.writeLock();
        try {
            snapshotPath = snapshotManager.createSnapshot(fsd.getFSNamesystem().getLeaseManager(), iip, snapshotRoot, snapshotName, now);
        } finally {
            fsd.writeUnlock();
        }
    ...
    static void renameSnapshot(FSDirectory fsd, FSPermissionChecker pc, SnapshotManager snapshotManager, String path, String snapshotOldName, String snapshotNewName, boolean logRetryCache) throws IOException {
        ...
        LOG.info("Snapshot renamed from {} to {} for SnapshotRoot {}", snapshotOldName, snapshotNewName, path);
    }

    static SnapshottableDirectoryStatus[] getSnapshottableDirListing(FSDirectory fsd, FSPermissionChecker pc, SnapshotManager snapshotManager) throws IOException {
        fsd.readLock();
        try {
    ...
    static SnapshotStatus[] getSnapshotListing(FSDirectory fsd, FSPermissionChecker pc, SnapshotManager snapshotManager, String path) throws IOException {
        fsd.readLock();
        try {
    ...
    static SnapshotDiffReport getSnapshotDiffReport(FSDirectory fsd, FSPermissionChecker pc, SnapshotManager snapshotManager, String path, String fromSnapshot, String toSnapshot) throws IOException {
        SnapshotDiffReport diffs;
        fsd.readLock();
        try {
            INodesInPath iip = fsd.resolvePath(pc, path, DirOp.READ);
            org.zlab.ocov.tracker.Runtime.update(iip, 101);
            if (fsd.isPermissionEnabled()) {
    ...
    static SnapshotDiffReportListing getSnapshotDiffReportListing(FSDirectory fsd, FSPermissionChecker pc, SnapshotManager snapshotManager, String path, String fromSnapshot, String toSnapshot, byte[] startPath, int index, int snapshotDiffReportLimit) throws IOException {
        SnapshotDiffReportListing diffs;
        fsd.readLock();
        try {
            INodesInPath iip = fsd.resolvePath(pc, path, DirOp.READ);
            org.zlab.ocov.tracker.Runtime.update(iip, 102);
            if (fsd.isPermissionEnabled()) {
                checkSubtreeReadPermission(fsd, pc, path, fromSnapshot);
                checkSubtreeReadPermission(fsd, pc, path, toSnapshot);
            }
     ...
     * @throws IOException
     */
    static INode.BlocksMapUpdateInfo deleteSnapshot(FSDirectory fsd, FSPermissionChecker pc, SnapshotManager snapshotManager, String snapshotRoot, String snapshotName, boolean logRetryCache) throws IOException {
        final INodesInPath iip = fsd.resolvePath(pc, snapshotRoot, DirOp.WRITE);
        if (fsd.isPermissionEnabled()) {
            fsd.checkOwner(pc, iip);
        }
        ...
        final long now = Time.now();
        final INode.BlocksMapUpdateInfo collectedBlocks = deleteSnapshot(fsd, snapshotManager, iip, snapshotName, now, snapshotRoot, logRetryCache);
        LOG.info("Snapshot {} deleted for SnapshotRoot {}", snapshotName, snapshotName);
        return collectedBlocks;
    }

    static INode.BlocksMapUpdateInfo deleteSnapshot(FSDirectory fsd, SnapshotManager snapshotManager, INodesInPath iip, String snapshotName, long now, String snapshotRoot, boolean logRetryCache) throws IOException {
