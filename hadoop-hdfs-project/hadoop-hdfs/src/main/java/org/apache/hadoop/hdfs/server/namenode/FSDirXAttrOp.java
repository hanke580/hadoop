...
public class FSDirXAttrOp {
     ...
     * @throws IOException
     */
    static FileStatus setXAttr(FSDirectory fsd, FSPermissionChecker pc, String src, XAttr xAttr, EnumSet<XAttrSetFlag> flag, boolean logRetryCache) throws IOException {
    ...
    static List<XAttr> getXAttrs(FSDirectory fsd, FSPermissionChecker pc, final String srcArg, List<XAttr> xAttrs) throws IOException {
        String src = srcArg;
        checkXAttrsConfigFlag(fsd);
        ...
        for (XAttr xAttr : xAttrs) {
            ...
            for (XAttr a : filteredAll) {
                if (xAttr.getNameSpace() == a.getNameSpace() && xAttr.getName().equals(a.getName())) {
                    toGet.add(a);
                    org.zlab.ocov.tracker.Runtime.update(toGet, 121);
                    foundIt = true;
                    break;
                }
            }
    ...
    static List<XAttr> listXAttrs(FSDirectory fsd, FSPermissionChecker pc, String src) throws IOException {
     ...
     * @throws IOException
     */
    static FileStatus removeXAttr(FSDirectory fsd, FSPermissionChecker pc, String src, XAttr xAttr, boolean logRetryCache) throws IOException {
        FSDirXAttrOp.checkXAttrsConfigFlag(fsd);
        XAttrPermissionFilter.checkPermissionForApi(pc, xAttr, FSDirectory.isReservedRawName(src));
        List<XAttr> xAttrs = Lists.newArrayListWithCapacity(1);
        xAttrs.add(xAttr);
     ...
     */
    static List<XAttr> unprotectedRemoveXAttrs(FSDirectory fsd, final INodesInPath iip, final List<XAttr> toRemove) throws IOException {
    ...
    public static INode unprotectedSetXAttrs(FSDirectory fsd, final INodesInPath iip, final List<XAttr> xAttrs, final EnumSet<XAttrSetFlag> flag) throws IOException {
        assert fsd.hasWriteLock();
        INode inode = FSDirectory.resolveLastINode(iip);
        List<XAttr> existingXAttrs = XAttrStorage.readINodeXAttrs(inode);
       ...
       */
            if (CRYPTO_XATTR_FILE_ENCRYPTION_INFO.equals(xaName)) {
                HdfsProtos.PerFileEncryptionInfoProto fileProto = HdfsProtos.PerFileEncryptionInfoProto.parseFrom(xattr.getValue());
                String keyVersionName = fileProto.getEzKeyVersionName();
                String zoneKeyName = fsd.ezManager.getKeyName(iip);
                if (zoneKeyName == null) {
                    throw new IOException("Cannot add raw feInfo XAttr to a file in a " + "non-encryption zone");
                }
                if (!KeyProviderCryptoExtension.getBaseName(keyVersionName).equals(zoneKeyName)) {
                    throw new IllegalArgumentException(String.format("KeyVersion '%s' does not belong to the key '%s'", keyVersionName, zoneKeyName));
                }
            }
            if (CRYPTO_XATTR_ENCRYPTION_ZONE.equals(xaName)) {
                ...
                if (ezProto.hasReencryptionProto()) {
                    ...
                    fsd.ezManager.getReencryptionStatus().updateZoneStatus(inode.getId(), iip.getPath(), reProto);
                }
            }
            ...
            if (xaName.equals(XATTR_SNAPSHOT_DELETED) && !(inode.isDirectory() && inode.getParent().isSnapshottable())) {
                throw new IOException("Can only set '" + XATTR_SNAPSHOT_DELETED + "' on a snapshot root.");
            }
        }
    ...
    static XAttr getXAttrByPrefixedName(FSDirectory fsd, INodesInPath iip, String prefixedName) throws IOException {
        fsd.readLock();
        try {
            return XAttrStorage.readINodeXAttrByPrefixedName(iip.getLastINode(), iip.getPathSnapshotId(), prefixedName);
        } finally {
            fsd.readUnlock();
        }
    }

    static XAttr unprotectedGetXAttrByPrefixedName(INode inode, int snapshotId, String prefixedName) throws IOException {
