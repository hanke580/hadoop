...
public class TextFileRegionAliasMap extends BlockAliasMap<FileRegion> implements Configurable {
    ...
    TextWriter createWriter(Path file, CompressionCodec codec, String delim, Configuration cfg) throws IOException {
        ...
        java.io.Writer out = new BufferedWriter(new OutputStreamWriter((null == codec) ? tmp : codec.createOutputStream(tmp), "UTF-8"));
    ...
    public static class ReaderOptions implements TextReader.Options, Configurable {
        ...
        public void setConf(Configuration conf) {
            this.conf = conf;
            String tmpfile = conf.get(DFSConfigKeys.DFS_PROVIDED_ALIASMAP_TEXT_READ_FILE, DFSConfigKeys.DFS_PROVIDED_ALIASMAP_TEXT_READ_FILE_DEFAULT);
            file = new Path(tmpfile);
            delim = conf.get(DFSConfigKeys.DFS_PROVIDED_ALIASMAP_TEXT_DELIMITER, DFSConfigKeys.DFS_PROVIDED_ALIASMAP_TEXT_DELIMITER_DEFAULT);
