...
class OfflineImageReconstructor {
    ...
    private void loadNodeChildren(Node parent, String expected, String... terminators) throws IOException {
        ...
        if (LOG.isTraceEnabled()) {
            LOG.trace("loadNodeChildren(expected=" + expected + ", terminators=[" + StringUtils.join(",", terminators) + "]):" + parent.dump());
        }
    }
            ...
            while (true) {
                ...
                if (options != null) {
                    while (true) {
                        ...
                        schemaBuilder.addOptions(HdfsProtos.ECSchemaOptionEntryProto.newBuilder().setKey(key).setValue(value).build());
                    }
                }
    ...
    private INodeSection.INodeDirectory.Builder createINodeDirectoryBuilder(Node node) throws IOException {
    ...
    void recordSectionLength(String sectionNamePb) throws IOException {
        ...
        fileSummaryBld.addSections(FileSummary.Section.newBuilder().setName(sectionNamePb).setLength(curPos - curSectionStartOffset).setOffset(curSectionStartOffset));
        //}
    ...
    private void readVersion() throws IOException {
        try {
            ...
            // fsimage.
            throw new IOException("No <version> section found at the top of " + "the fsimage XML.  This XML file is too old to be processed " + "by oiv.", e);
        }
