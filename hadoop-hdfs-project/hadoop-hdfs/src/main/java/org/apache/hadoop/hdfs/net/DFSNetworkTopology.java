...
public class DFSNetworkTopology extends NetworkTopology {
    ...
    public static DFSNetworkTopology getInstance(Configuration conf) {
        DFSNetworkTopology nt = ReflectionUtils.newInstance(conf.getClass(DFSConfigKeys.DFS_NET_TOPOLOGY_IMPL_KEY, DFSConfigKeys.DFS_NET_TOPOLOGY_IMPL_DEFAULT, DFSNetworkTopology.class), conf);
     ...
     */
    public Node chooseRandomWithStorageType(final String scope, final Collection<Node> excludedNodes, StorageType type) {
     ...
     * @return the chosen node
     */
    public Node chooseRandomWithStorageTypeTwoTrial(final String scope, final Collection<Node> excludedNodes, StorageType type) {
        netlock.readLock().lock();
        try {
            String searchScope;
            ...
            if (dnDescriptor.hasStorageType(type)) {
                ...
                LOG.debug("First trial failed, node has no type {}, " + "making second trial carrying this type", type);
    ...
    Node chooseRandomWithStorageType(final String scope, String excludedScope, final Collection<Node> excludedNodes, StorageType type) {
        ...
        Node chosen = chooseRandomWithStorageTypeAndExcludeRoot(root, excludeRoot, type, excludedNodes);
     ...
     * scope
     */
    private Node chooseRandomWithStorageTypeAndExcludeRoot(DFSTopologyNodeImpl root, Node excludeRoot, StorageType type, Collection<Node> excludedNodes) {
        Node chosenNode;
        if (root.isRack()) {
            // children are datanode descriptor
            ArrayList<Node> candidates = new ArrayList<>();
            for (Node node : root.getChildren()) {
                if (node.equals(excludeRoot) || (excludedNodes != null && excludedNodes.contains(node))) {
                    continue;
                }
            ...
            ArrayList<DFSTopologyNodeImpl> candidates = getEligibleChildren(root, excludeRoot, type, excludedNodes);
            if (candidates.size() == 0) {
                return null;
            }
            ...
            chosenNode = chooseRandomWithStorageTypeAndExcludeRoot(nextRoot, excludeRoot, type, excludedNodes);
        }
        return chosenNode;
    }
     ...
     */
    private ArrayList<DFSTopologyNodeImpl> getEligibleChildren(DFSTopologyNodeImpl root, Node excludeRoot, StorageType type, Collection<Node> excludedNodes) {
        ...
        for (Node node : root.getChildren()) {
            ...
            if (excludedNodes != null) {
                for (Node excludedNode : excludedNodes) {
                    if (excludeRoot != null && isNodeInScope(excludedNode, NodeBase.getPath(excludeRoot))) {
                        continue;
                    }
                    if (isNodeInScope(excludedNode, NodeBase.getPath(node))) {
                        if (excludedNode instanceof DatanodeDescriptor) {
                            storageCount -= ((DatanodeDescriptor) excludedNode).hasStorageType(type) ? 1 : 0;
                        } else if (excludedNode instanceof DFSTopologyNodeImpl) {
                            storageCount -= ((DFSTopologyNodeImpl) excludedNode).getSubtreeStorageCount(type);
                        }
                    }
                }
            }
