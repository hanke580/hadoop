...
class GlobalStateIdContext implements AlignmentContext {
    ...
    public long receiveRequestState(RpcRequestHeaderProto header, long clientWaitTime) throws IOException {
        ...
        if (FSNamesystem.LOG.isTraceEnabled()) {
            FSNamesystem.LOG.trace("Client State ID= " + clientStateId + " and Server State ID= " + serverStateId);
        }
        if (clientStateId > serverStateId && HAServiceState.ACTIVE.equals(namesystem.getState())) {
            FSNamesystem.LOG.warn("The client stateId: " + clientStateId + " is greater than the server stateId: " + serverStateId + " This is unexpected. " + "Resetting client stateId to server stateId");
