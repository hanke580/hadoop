...
class FsVolumeList {
    ...
    private final Map<StorageLocation, VolumeFailureInfo> volumeFailureInfos = Collections.synchronizedMap(new TreeMap<StorageLocation, VolumeFailureInfo>());
    ...
    private FsVolumeReference chooseVolume(List<FsVolumeImpl> list, long blockSize, String storageId) throws IOException {
        while (true) {
     ...
     */
    FsVolumeReference getNextVolume(StorageType storageType, String storageId, long blockSize) throws IOException {
    ...
    void getAllVolumesMap(final String bpid, final ReplicaMap volumeMap, final RamDiskReplicaTracker ramDiskReplicaMap) throws IOException {
        ...
        final Map<FsVolumeSpi, IOException> unhealthyDataDirs = new ConcurrentHashMap<FsVolumeSpi, IOException>();
        ...
        FsDatasetImpl.LOG.info("Total time to add all replicas to map for block pool " + bpid + ": " + totalTimeTaken + "ms");
    ...
    private void addVolumeFailureInfo(FsVolumeImpl vol) {
        addVolumeFailureInfo(new VolumeFailureInfo(vol.getStorageLocation(), Time.now(), vol.getCapacity()));
    }
