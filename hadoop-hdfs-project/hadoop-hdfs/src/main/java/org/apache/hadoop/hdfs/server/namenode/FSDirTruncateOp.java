...
final class FSDirTruncateOp {
    ...
    static Block prepareFileForTruncate(FSNamesystem fsn, INodesInPath iip, String leaseHolder, String clientMachine, long lastBlockDelta, Block newBlock) throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(file, 56);
        ...
        if (newBlock == null) {
            newBlock = (shouldCopyOnTruncate) ? fsn.createNewBlock() : ((Block) org.zlab.ocov.tracker.Runtime.update(new Block(oldBlock.getBlockId(), oldBlock.getNumBytes(), fsn.nextGenerationStamp(fsn.getBlockManager().isLegacyBlock(oldBlock))), 57));
        }
