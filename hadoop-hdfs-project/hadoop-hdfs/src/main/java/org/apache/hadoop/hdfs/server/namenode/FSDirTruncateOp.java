...
final class FSDirTruncateOp {
    ...
    static TruncateResult truncate(final FSNamesystem fsn, final String srcArg, final long newLength, final String clientName, final String clientMachine, final long mtime, final BlocksMapUpdateInfo toRemoveBlocks, final FSPermissionChecker pc) throws IOException, UnresolvedLinkException {
        ...
        try {
            ...
            org.zlab.ocov.tracker.Runtime.update(iip, 88);
            src = iip.getPath();
            ...
            if (last != null && last.getBlockUCState() == BlockUCState.UNDER_RECOVERY) {
                ...
                if (truncatedBlock != null) {
                    ...
                    if (newLength == truncateLength) {
                        ...
                        throw new AlreadyBeingCreatedException(RecoverLeaseOp.TRUNCATE_FILE.getExceptionMessage(src, clientName, clientMachine, src + " is being truncated."));
                    }
                }
            }
            ...
            final QuotaCounts delta = ((QuotaCounts.Builder) org.zlab.ocov.tracker.Runtime.update(new QuotaCounts.Builder(), 89)).build();
    ...
    static void unprotectedTruncate(final FSNamesystem fsn, final INodesInPath iip, final String clientName, final String clientMachine, final long newLength, final long mtime, final Block truncateBlock) throws UnresolvedLinkException, QuotaExceededException, SnapshotAccessControlException, IOException {
        ...
        BlocksMapUpdateInfo collectedBlocks = ((BlocksMapUpdateInfo) org.zlab.ocov.tracker.Runtime.update(new BlocksMapUpdateInfo(), 90));
        boolean onBlockBoundary = unprotectedTruncate(fsn, iip, newLength, collectedBlocks, mtime, null);
        org.zlab.ocov.tracker.Runtime.update(collectedBlocks, 91);
        if (!onBlockBoundary) {
    ...
    static Block prepareFileForTruncate(FSNamesystem fsn, INodesInPath iip, String leaseHolder, String clientMachine, long lastBlockDelta, Block newBlock) throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(file, 92);
        ...
        if (shouldRecoverNow) {
            truncatedBlockUC.getUnderConstructionFeature().initializeBlockRecovery(truncatedBlockUC, newBlock.getGenerationStamp(), true);
        }
        return newBlock;
    }
     ...
     */
    private static boolean unprotectedTruncate(FSNamesystem fsn, INodesInPath iip, long newLength, BlocksMapUpdateInfo collectedBlocks, long mtime, QuotaCounts delta) throws IOException {
        assert fsn.hasWriteLock();
        INodeFile file = iip.getLastINode().asFile();
