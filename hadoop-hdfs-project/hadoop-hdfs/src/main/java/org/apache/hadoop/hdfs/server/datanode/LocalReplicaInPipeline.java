    ...
    public void waitForMinLength(long minLength, long time, TimeUnit unit) throws IOException {
        ...
        try {
            while (bytesOnDisk < minLength) {
                if (nanos <= 0L) {
                    throw new IOException(String.format("Need %d bytes, but only %d bytes available", minLength, bytesOnDisk));
                }
    ...
    public ReplicaOutputStreams createStreams(boolean isCreate, DataChecksum requestedChecksum) throws IOException {
        ...
        if (!isCreate) {
            ...
            try {
                ...
                if ((org.zlab.ocov.tracker.Runtime.updateBranch(blockDiskSize > 0 && (blockDiskSize > blockFile.length() || crcDiskSize > metaFile.length()), 3))) {
