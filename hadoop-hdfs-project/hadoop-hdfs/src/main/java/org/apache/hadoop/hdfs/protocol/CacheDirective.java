...
public final class CacheDirective implements IntrusiveCollection.Element {
    ...
    public CacheDirectiveInfo toInfo() {
        return ((CacheDirectiveInfo.Builder) org.zlab.ocov.tracker.Runtime.update(new CacheDirectiveInfo.Builder(), 143)).setId(id).setPath(new Path(path)).setReplication(replication).setPool(pool.getPoolName()).setExpiration(CacheDirectiveInfo.Expiration.newAbsolute(expiryTime)).build();
    }
