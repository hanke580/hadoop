...
public class FSDirAttrOp {
    ...
    static FileStatus setPermission(FSDirectory fsd, FSPermissionChecker pc, final String src, FsPermission permission) throws IOException {
    ...
    static FileStatus setOwner(FSDirectory fsd, FSPermissionChecker pc, String src, String username, String group) throws IOException {
        if (FSDirectory.isExactReservedName(src)) {
            throw new InvalidPathException(src);
        }
        INodesInPath iip;
        ...
        try {
            ...
            if ((username != null && !pc.getUser().equals(username)) || (group != null && !pc.isMemberOfGroup(group))) {
                try {
                    // check if the user is superuser
                    pc.checkSuperuserPrivilege(iip.getPath());
                } catch (AccessControlException e) {
                    if (username != null && !pc.getUser().equals(username)) {
                        throw new AccessControlException("User " + pc.getUser() + " is not a super user (non-super user cannot change owner).");
                    }
    ...
    static FileStatus setTimes(FSDirectory fsd, FSPermissionChecker pc, String src, long mtime, long atime) throws IOException {
        INodesInPath iip;
        fsd.writeLock();
        try {
            iip = fsd.resolvePath(pc, src, DirOp.WRITE);
    ...
    static boolean setReplication(FSDirectory fsd, FSPermissionChecker pc, BlockManager bm, String src, final short replication) throws IOException {
    ...
    static FileStatus unsetStoragePolicy(FSDirectory fsd, FSPermissionChecker pc, BlockManager bm, String src) throws IOException {
        return setStoragePolicy(fsd, pc, bm, src, HdfsConstants.BLOCK_STORAGE_POLICY_ID_UNSPECIFIED);
    }

    static FileStatus setStoragePolicy(FSDirectory fsd, FSPermissionChecker pc, BlockManager bm, String src, final String policyName) throws IOException {
    ...
    static FileStatus setStoragePolicy(FSDirectory fsd, FSPermissionChecker pc, BlockManager bm, String src, final byte policyId) throws IOException {
        INodesInPath iip;
        fsd.writeLock();
        try {
            iip = fsd.resolvePath(pc, src, DirOp.WRITE);
    ...
    static BlockStoragePolicy getStoragePolicy(FSDirectory fsd, FSPermissionChecker pc, BlockManager bm, String path) throws IOException {
        fsd.readLock();
        try {
    ...
    static long getPreferredBlockSize(FSDirectory fsd, FSPermissionChecker pc, String src) throws IOException {
        fsd.readLock();
        try {
     ...
     */
    static void setQuota(FSDirectory fsd, FSPermissionChecker pc, String src, long nsQuota, long ssQuota, StorageType type, boolean allowOwner) throws IOException {
        fsd.writeLock();
        try {
            INodesInPath iip = fsd.resolvePath(pc, src, DirOp.WRITE);
            org.zlab.ocov.tracker.Runtime.update(iip, 131);
            ...
            if (fsd.isPermissionEnabled() && !pc.isSuperUser() && allowOwner) {
                try {
                    ...
                    throw new AccessControlException("Access denied for user " + pc.getUser() + ". Superuser or owner of parent folder privilege is required");
                }
            }
            INodeDirectory changed = unprotectedSetQuota(fsd, iip, nsQuota, ssQuota, type);
            if (changed != null) {
                final QuotaCounts q = changed.getQuotaCounts();
                org.zlab.ocov.tracker.Runtime.update(q, 132);
                if (type == null) {
    ...
    static boolean unprotectedSetPermission(FSDirectory fsd, INodesInPath iip, FsPermission permissions) throws FileNotFoundException, UnresolvedLinkException, QuotaExceededException, SnapshotAccessControlException {
    ...
    static boolean unprotectedSetOwner(FSDirectory fsd, INodesInPath iip, String username, String groupname) throws FileNotFoundException, UnresolvedLinkException, QuotaExceededException, SnapshotAccessControlException {
        assert fsd.hasWriteLock();
        final INode inode = FSDirectory.resolveLastINode(iip);
        long oldPerm = inode.getPermissionLong();
        if (username != null) {
    ...
    static boolean setTimes(FSDirectory fsd, INodesInPath iip, long mtime, long atime, boolean force) throws FileNotFoundException {
        fsd.writeLock();
        try {
     ...
     */
    static INodeDirectory unprotectedSetQuota(FSDirectory fsd, INodesInPath iip, long nsQuota, long ssQuota, StorageType type) throws FileNotFoundException, PathIsNotDirectoryException, QuotaExceededException, UnresolvedLinkException, SnapshotAccessControlException, UnsupportedActionException {
        assert fsd.hasWriteLock();
        // sanity check
    ...
    static BlockInfo[] unprotectedSetReplication(FSDirectory fsd, INodesInPath iip, short replication) throws QuotaExceededException, UnresolvedLinkException, SnapshotAccessControlException, UnsupportedActionException {
        ...
        if (oldBR != -1 && FSDirectory.LOG.isDebugEnabled()) {
            if (oldBR > targetReplication) {
                FSDirectory.LOG.debug("Decreasing replication from {} to {} for {}", oldBR, targetReplication, iip.getPath());
            } else if (oldBR < targetReplication) {
                FSDirectory.LOG.debug("Increasing replication from {} to {} for {}", oldBR, targetReplication, iip.getPath());
            } else {
    ...
    static boolean unprotectedSetTimes(FSDirectory fsd, INodesInPath iip, long mtime, long atime, boolean force) throws FileNotFoundException {
        assert fsd.hasWriteLock();
        ...
        if (atime >= 0 && (status || force || atime > inode.getAccessTime() + fsd.getAccessTimePrecision())) {
            inode.setAccessTime(atime, latest, fsd.getFSNamesystem().getSnapshotManager().getSkipCaptureAccessTimeOnlyChange());
            status = true;
        }
        return status;
    }
