...
public class FSDirAttrOp {
    ...
    static FileStatus setStoragePolicy(FSDirectory fsd, FSPermissionChecker pc, BlockManager bm, String src, final byte policyId) throws IOException {
        ...
        try {
            iip = fsd.resolvePath(pc, src, DirOp.WRITE);
            org.zlab.ocov.tracker.Runtime.update(iip, 86);
            if (fsd.isPermissionEnabled()) {
                fsd.checkPathAccess(pc, iip, FsAction.WRITE);
            }
    ...
    static void setQuota(FSDirectory fsd, FSPermissionChecker pc, String src, long nsQuota, long ssQuota, StorageType type, boolean allowOwner) throws IOException {
        ...
        try {
            ...
            org.zlab.ocov.tracker.Runtime.update(iip, 87);
    ...
    static void unprotectedSetPermission(FSDirectory fsd, INodesInPath iip, FsPermission permissions) throws FileNotFoundException, UnresolvedLinkException, QuotaExceededException, SnapshotAccessControlException {
    ...
    static void unprotectedSetOwner(FSDirectory fsd, INodesInPath iip, String username, String groupname) throws FileNotFoundException, UnresolvedLinkException, QuotaExceededException, SnapshotAccessControlException {
        assert fsd.hasWriteLock();
        final INode inode = FSDirectory.resolveLastINode(iip);
        if (username != null) {
