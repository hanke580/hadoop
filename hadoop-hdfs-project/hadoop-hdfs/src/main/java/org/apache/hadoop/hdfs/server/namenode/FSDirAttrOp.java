...
public class FSDirAttrOp {
    ...
    static FileStatus setPermission(FSDirectory fsd, final String src, FsPermission permission) throws IOException {
    ...
    static FileStatus setOwner(FSDirectory fsd, String src, String username, String group) throws IOException {
        if (FSDirectory.isExactReservedName(src)) {
            throw new InvalidPathException(src);
        }
        FSPermissionChecker pc = fsd.getPermissionChecker();
    ...
    static FileStatus setTimes(FSDirectory fsd, String src, long mtime, long atime) throws IOException {
        FSPermissionChecker pc = fsd.getPermissionChecker();
        INodesInPath iip;
        fsd.writeLock();
        try {
    ...
    static FileStatus unsetStoragePolicy(FSDirectory fsd, BlockManager bm, String src) throws IOException {
        return setStoragePolicy(fsd, bm, src, HdfsConstants.BLOCK_STORAGE_POLICY_ID_UNSPECIFIED, "unset");
    }

    static FileStatus setStoragePolicy(FSDirectory fsd, BlockManager bm, String src, final String policyName) throws IOException {
    ...
    static FileStatus setStoragePolicy(FSDirectory fsd, BlockManager bm, String src, final byte policyId, final String operation) throws IOException {
    ...
    static void setQuota(FSDirectory fsd, String src, long nsQuota, long ssQuota, StorageType type) throws IOException {
        ...
        try {
            ...
            if (changed != null) {
                ...
                org.zlab.ocov.tracker.Runtime.update(q, 51);
                if (type == null) {
    ...
    static BlockInfo[] unprotectedSetReplication(FSDirectory fsd, INodesInPath iip, short replication) throws QuotaExceededException, UnresolvedLinkException, SnapshotAccessControlException {
        assert fsd.hasWriteLock();
