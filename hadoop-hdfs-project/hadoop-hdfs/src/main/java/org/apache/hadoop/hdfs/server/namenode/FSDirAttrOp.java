...
public class FSDirAttrOp {
    ...
    static FileStatus unsetStoragePolicy(FSDirectory fsd, FSPermissionChecker pc, BlockManager bm, String src) throws IOException {
        return setStoragePolicy(fsd, pc, bm, src, HdfsConstants.BLOCK_STORAGE_POLICY_ID_UNSPECIFIED, "unset");
    }
    ...
    static FileStatus setStoragePolicy(FSDirectory fsd, FSPermissionChecker pc, BlockManager bm, String src, final byte policyId, final String operation) throws IOException {
        ...
        try {
            iip = fsd.resolvePath(pc, src, DirOp.WRITE);
            org.zlab.ocov.tracker.Runtime.update(iip, 91);
            if (fsd.isPermissionEnabled()) {
                fsd.checkPathAccess(pc, iip, FsAction.WRITE);
            }
     ...
     */
    static void setQuota(FSDirectory fsd, FSPermissionChecker pc, String src, long nsQuota, long ssQuota, StorageType type) throws IOException {
        if (fsd.isPermissionEnabled()) {
    ...
    static boolean unprotectedSetTimes(FSDirectory fsd, INodesInPath iip, long mtime, long atime, boolean force) throws QuotaExceededException {
        ...
        if (atime != -1 && (status || force || atime > inode.getAccessTime() + fsd.getAccessTimePrecision())) {
