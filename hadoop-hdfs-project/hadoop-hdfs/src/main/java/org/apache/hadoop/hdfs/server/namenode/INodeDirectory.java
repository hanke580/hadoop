...
public class INodeDirectory extends INodeWithAdditionalFields implements INodeDirectoryAttributes {
    ...
    void setQuota(BlockStoragePolicySuite bsps, long nsQuota, long ssQuota, StorageType type) {
        ...
        if (quota != null) {
            ...
            DirectoryWithQuotaFeature.Builder builder = ((DirectoryWithQuotaFeature.Builder) org.zlab.ocov.tracker.Runtime.update(new DirectoryWithQuotaFeature.Builder(), 126)).nameSpaceQuota(nsQuota);
            if (type != null) {
     ...
     */
    public Snapshot addSnapshot(int id, String name, final LeaseManager leaseManager, final boolean captureOpenFiles, int maxSnapshotLimit, long mtime) throws SnapshotException {
        return getDirectorySnapshottableFeature().addSnapshot(this, id, name, leaseManager, captureOpenFiles, maxSnapshotLimit, mtime);
    }
     ...
     */
    public Snapshot removeSnapshot(ReclaimContext reclaimContext, String snapshotName, long mtime) throws SnapshotException {
        return getDirectorySnapshottableFeature().removeSnapshot(reclaimContext, this, snapshotName, mtime);
    }
    ...
    public void removeSnapshottableFeature() {
        ...
        this.removeFeature(s);
        if ((org.zlab.ocov.tracker.Runtime.updateBranch(s.getDiffs().asList().size() > 0, 38))) {
    ...
    public QuotaCounts computeQuotaUsage(BlockStoragePolicySuite bsps, byte blockStoragePolicyId, boolean useCache, int lastSnapshotId) {
        ...
        QuotaCounts counts = ((QuotaCounts.Builder) org.zlab.ocov.tracker.Runtime.update(new QuotaCounts.Builder(), 128)).build();
        org.zlab.ocov.tracker.Runtime.update(counts, 127);
    ...
    public void destroyAndCollectBlocks(ReclaimContext reclaimContext) {
        reclaimContext.quotaDelta().add(((QuotaCounts.Builder) org.zlab.ocov.tracker.Runtime.update(new QuotaCounts.Builder(), 129)).nameSpace(1).build());
        final DirectoryWithSnapshotFeature sf = getDirectoryWithSnapshotFeature();
        if (sf != null) {
