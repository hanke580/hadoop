...
public class INodeDirectory extends INodeWithAdditionalFields implements INodeDirectoryAttributes {
    ...
    void setQuota(BlockStoragePolicySuite bsps, long nsQuota, long ssQuota, StorageType type) {
        ...
        if (quota != null) {
            ...
            DirectoryWithQuotaFeature.Builder builder = ((DirectoryWithQuotaFeature.Builder) org.zlab.ocov.tracker.Runtime.update(new DirectoryWithQuotaFeature.Builder(), 17)).nameSpaceQuota(nsQuota);
            if (type != null) {
    ...
    public Snapshot addSnapshot(int id, String name, final LeaseManager leaseManager, final boolean captureOpenFiles, int maxSnapshotLimit) throws SnapshotException {
        return getDirectorySnapshottableFeature().addSnapshot(this, id, name, leaseManager, captureOpenFiles, maxSnapshotLimit);
    }

    public Snapshot removeSnapshot(ReclaimContext reclaimContext, String snapshotName) throws SnapshotException {
        return getDirectorySnapshottableFeature().removeSnapshot(reclaimContext, this, snapshotName);
    }
    ...
    public void removeSnapshottableFeature() {
        ...
        this.removeFeature(s);
        if ((org.zlab.ocov.tracker.Runtime.updateBranch(s.getDiffs().asList().size(), 0, ">", 8))) {
    ...
    public boolean addChild(INode node, final boolean setModTime, final int latestSnapshotId) {
        ...
        addChild(node, low);
        org.zlab.ocov.tracker.Runtime.update(node, 18);
        if (setModTime) {
    ...
    public QuotaCounts computeQuotaUsage(BlockStoragePolicySuite bsps, byte blockStoragePolicyId, boolean useCache, int lastSnapshotId) {
        ...
        QuotaCounts counts = ((QuotaCounts.Builder) org.zlab.ocov.tracker.Runtime.update(new QuotaCounts.Builder(), 20)).build();
        org.zlab.ocov.tracker.Runtime.update(counts, 19);
    ...
    public void undoRename4DstParent(final BlockStoragePolicySuite bsps, final INode deletedChild, int latestSnapshotId) {
        ...
        org.zlab.ocov.tracker.Runtime.update(deletedChild, 21);
    ...
    public void destroyAndCollectBlocks(ReclaimContext reclaimContext) {
        reclaimContext.quotaDelta().add(((QuotaCounts.Builder) org.zlab.ocov.tracker.Runtime.update(new QuotaCounts.Builder(), 22)).nameSpace(1).build());
        final DirectoryWithSnapshotFeature sf = getDirectoryWithSnapshotFeature();
        if (sf != null) {
