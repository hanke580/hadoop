...
public class INodeDirectory extends INodeWithAdditionalFields implements INodeDirectoryAttributes {
    ...
    void setQuota(BlockStoragePolicySuite bsps, long nsQuota, long ssQuota, StorageType type) {
        ...
        if (quota != null) {
            ...
            DirectoryWithQuotaFeature.Builder builder = ((DirectoryWithQuotaFeature.Builder) org.zlab.ocov.tracker.Runtime.update(new DirectoryWithQuotaFeature.Builder(), 55)).nameSpaceQuota(nsQuota);
            if (type != null) {
    ...
    public QuotaCounts computeQuotaUsage(BlockStoragePolicySuite bsps, byte blockStoragePolicyId, boolean useCache, int lastSnapshotId) {
        ...
        QuotaCounts counts = ((QuotaCounts.Builder) org.zlab.ocov.tracker.Runtime.update(new QuotaCounts.Builder(), 57)).build();
        org.zlab.ocov.tracker.Runtime.update(counts, 56);
    ...
    public void destroyAndCollectBlocks(ReclaimContext reclaimContext) {
        reclaimContext.quotaDelta().add(((QuotaCounts.Builder) org.zlab.ocov.tracker.Runtime.update(new QuotaCounts.Builder(), 58)).nameSpace(1).build());
        final DirectoryWithSnapshotFeature sf = getDirectoryWithSnapshotFeature();
        if (sf != null) {
