...
class BPServiceActor implements Runnable {
    ...
    BPServiceActor(String serviceId, String nnId, InetSocketAddress nnAddr, InetSocketAddress lifelineNnAddr, BPOfferService bpos) {
        ...
        scheduler = new Scheduler(dnConf.heartBeatInterval, dnConf.getLifelineIntervalMs(), dnConf.blockReportInterval, dnConf.outliersReportIntervalMs);
        ...
        ibrExecutorService = Executors.newSingleThreadExecutor(new ThreadFactoryBuilder().setDaemon(true).setNameFormat("ibr-executor-%d").build());
    }
    ...
    List<DatanodeCommand> blockReport(long fullBrLeaseId) throws IOException {
        ...
        try {
            if (totalBlockCount < dnConf.blockReportSplitThreshold) {
                ...
                DatanodeCommand cmd = bpNamenode.blockReport(bpRegistration, bpos.getBlockPoolId(), reports, new BlockReportContext(1, 0, reportId, fullBrLeaseId));
                ...
                for (int r = 0; r < reports.length; r++) {
                    ...
                    DatanodeCommand cmd = bpNamenode.blockReport(bpRegistration, bpos.getBlockPoolId(), singleReport, new BlockReportContext(reports.length, r, reportId, fullBrLeaseId));
            ...
            LOG.info((success ? "S" : "Uns") + "uccessfully sent block report 0x" + Long.toHexString(reportId) + " with lease ID 0x" + Long.toHexString(fullBrLeaseId) + " to namenode: " + nnAddr + ",  containing " + reports.length + " storage report(s), of which we sent " + numReportsSent + "." + " The reports had " + totalBlockCount + " total blocks and used " + numRPCs + " RPC(s). This took " + brCreateCost + " msecs to generate and " + brSendCost + " msecs for RPC and NN processing." + " Got back " + ((nCmds == 0) ? "no commands" : ((nCmds == 1) ? "one command: " + cmds.get(0) : (nCmds + " commands: " + Joiner.on("; ").join(cmds)))) + ".");
        }
    ...
    HeartbeatResponse sendHeartBeat(boolean requestBlockReportLease) throws IOException {
        ...
        final SlowPeerReports slowPeers = outliersReportDue && dnConf.peerStatsEnabled && dn.getPeerMetrics() != null ? SlowPeerReports.create(dn.getPeerMetrics().getOutliers()) : SlowPeerReports.EMPTY_REPORT;
        final SlowDiskReports slowDisks = outliersReportDue && dnConf.diskStatsEnabled && dn.getDiskMetrics() != null ? SlowDiskReports.create(dn.getDiskMetrics().getDiskOutliersStats()) : SlowDiskReports.EMPTY_REPORT;
        HeartbeatResponse response = bpNamenode.sendHeartbeat(bpRegistration, reports, dn.getFSDataset().getCacheCapacity(), dn.getFSDataset().getCacheUsed(), dn.getXmitsInProgress(), dn.getActiveTransferThreadCount(), numFailedVolumes, volumeFailureSummary, requestBlockReportLease, slowPeers, slowDisks);
    ...
    private void offerService() throws Exception {
        LOG.info("For namenode " + nnAddr + " using" + " BLOCKREPORT_INTERVAL of " + dnConf.blockReportInterval + "msecs" + " CACHEREPORT_INTERVAL of " + dnConf.cacheReportInterval + "msecs" + " Initial delay: " + dnConf.initialBlockReportDelayMs + "msecs" + "; heartBeatInterval=" + dnConf.heartBeatInterval + (lifelineSender != null ? "; lifelineIntervalMs=" + dnConf.getLifelineIntervalMs() : ""));
        //
        ...
        while (shouldRun()) {
            try {
                ...
                LOG.debug("BP offer service run start time: {}, sendHeartbeat: {}", startTime, sendHeartbeat);
                ...
                if (sendHeartbeat) {
                    ...
                    if (!dn.areHeartbeatsDisabledForTests()) {
                        LOG.debug("Before sending heartbeat to namenode {}, the state of the namenode known" + " to datanode so far is {}", this.getNameNodeAddress(), state);
                        ...
                        dn.getMetrics().addHeartbeat(scheduler.monotonicNow() - startTime, getRpcMetricSuffix());
                        ...
                        if (state != stateFromResp) {
                            LOG.info("After receiving heartbeat response, updating state of namenode {} to {}", this.getNameNodeAddress(), stateFromResp);
                        }
                        state = stateFromResp;
                ...
                if (sendHeartbeat) {
                    dn.getMetrics().addHeartbeatTotal(scheduler.monotonicNow() - startTime, getRpcMetricSuffix());
                }
    ...
    void triggerBlockReport(BlockReportOptions options) {
        if (options.isIncremental()) {
            LOG.info(bpos.toString() + ": scheduling an incremental block report " + "to namenode: " + nnAddr + ".");
            ibrManager.triggerIBR(true);
        } else {
            LOG.info(bpos.toString() + ": scheduling a full block report " + "to namenode: " + nnAddr + ".");
            synchronized (ibrManager) {
                scheduler.forceFullBlockReportNow();
                ibrManager.notifyAll();
            }
    ...
    private void processQueueMessages() {
        ...
        while (!duplicateQueue.isEmpty()) {
            ...
            try {
                LOG.debug("BPServiceActor ( {} ) processing queued messages. Action item: {}", this, actionItem);
    ...
    private final class LifelineSender implements Runnable, Closeable {
        ...
        public void start() {
            ...
            lifelineThread.setUncaughtExceptionHandler(new Thread.UncaughtExceptionHandler() {
    ...
    static class Scheduler {
        ...
        long scheduleBlockReport(long delay, boolean isRegistration) {
            if (delay > 0) {
                ...
                // Numerical overflow is possible here and is okay.
                nextBlockReportTime.getAndSet(monotonicNow() + ThreadLocalRandom.current().nextInt((int) (delay)));
            } else {
                // send at next heartbeat
                nextBlockReportTime.getAndSet(monotonicNow());
            }
            // reset future BRs for
            resetBlockReportTime = isRegistration;
        ...
        void scheduleNextBlockReport() {
            ...
            if (resetBlockReportTime) {
                nextBlockReportTime.getAndSet(monotonicNow() + ThreadLocalRandom.current().nextInt((int) (blockReportIntervalMs)));
         ...
         *      on the next day.
         */
                long factor = (monotonicNow() - nextBlockReportTime.get() + blockReportIntervalMs) / blockReportIntervalMs;
                if (factor != 0) {
        ...
        void setBlockReportIntervalMs(long intervalMs) {
            Preconditions.checkArgument(intervalMs > 0, DFS_BLOCKREPORT_INTERVAL_MSEC_KEY + " should be larger than 0");
            this.blockReportIntervalMs = intervalMs;
        }

        void setOutliersReportIntervalMs(long intervalMs) {
            Preconditions.checkArgument(intervalMs > 0, DFS_DATANODE_OUTLIERS_REPORT_INTERVAL_KEY + " should be larger than 0");
        ...
        private boolean processCommand(DatanodeCommand[] cmds) {
            if (cmds != null) {
                ...
                if (processCommandsMs > dnConf.getProcessCommandsThresholdMs()) {
                    LOG.warn("Took {} ms to process {} commands from NN", processCommandsMs, cmds.length);
                }
            }
            return true;
        }
