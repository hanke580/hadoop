...
class FSDirStatAndListingOp {
    ...
    static DirectoryListing getListingInt(FSDirectory fsd, final String srcArg, byte[] startAfter, boolean needLocation) throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(iip, 79);
    ...
    static HdfsFileStatus getFileInfo(FSDirectory fsd, String srcArg, boolean resolveLink) throws IOException {
        ...
        if (pc.isSuperUser()) {
            ...
            try {
                ...
                org.zlab.ocov.tracker.Runtime.update(iip, 80);
    ...
    static GetBlockLocationsResult getBlockLocations(FSDirectory fsd, FSPermissionChecker pc, String src, long offset, long length, boolean needBlockToken) throws IOException {
        ...
        try {
            ...
            final LocatedBlocks blocks = bm.createLocatedBlocks(inode.getBlocks(iip.getPathSnapshotId()), fileSize, isUc, offset, length, needBlockToken, iip.isSnapshot(), feInfo);
            final long now = now();
    ...
    private static byte getStoragePolicyID(byte inodePolicy, byte parentPolicy) {
        return inodePolicy != HdfsConstants.BLOCK_STORAGE_POLICY_ID_UNSPECIFIED ? inodePolicy : parentPolicy;
    }
    ...
    private static DirectoryListing getSnapshotsListing(FSDirectory fsd, INodesInPath iip, byte[] startAfter) throws IOException {
        ...
        return ((DirectoryListing) org.zlab.ocov.tracker.Runtime.update(new DirectoryListing(listing, snapshots.size() - skipSize - numOfListing), 81));
    }
    ...
    static HdfsFileStatus getFileInfo(FSDirectory fsd, INodesInPath iip, boolean includeStoragePolicy) throws IOException {
        ...
        try {
            ...
            byte policy = (includeStoragePolicy && !node.isSymlink()) ? node.getStoragePolicyID() : HdfsConstants.BLOCK_STORAGE_POLICY_ID_UNSPECIFIED;
    ...
    private static HdfsFileStatus createFileStatus(FSDirectory fsd, INodesInPath iip, INode child, byte storagePolicy, boolean needLocation) throws IOException {
        ...
        if (node.isFile()) {
            ...
            if (needLocation) {
                ...
                loc = fsd.getBlockManager().createLocatedBlocks(fileNode.getBlocks(snapshot), fileSize, isUc, 0L, size, false, inSnapshot, feInfo);
                if (loc == null) {
        ...
        return createFileStatus(size, node.isDirectory(), replication, blocksize, node.getModificationTime(snapshot), node.getAccessTime(snapshot), getPermissionForFileStatus(nodeAttrs, isEncrypted), nodeAttrs.getUserName(), nodeAttrs.getGroupName(), node.isSymlink() ? node.asSymlink().getSymlink() : null, name, node.getId(), childrenNum, feInfo, storagePolicy, loc);
    }

    private static HdfsFileStatus createFileStatus(long length, boolean isdir, int replication, long blocksize, long mtime, long atime, FsPermission permission, String owner, String group, byte[] symlink, byte[] path, long fileId, int childrenNum, FileEncryptionInfo feInfo, byte storagePolicy, LocatedBlocks locations) {
        if (locations == null) {
            return new HdfsFileStatus(length, isdir, replication, blocksize, mtime, atime, permission, owner, group, symlink, path, fileId, childrenNum, feInfo, storagePolicy);
        } else {
            return new HdfsLocatedFileStatus(length, isdir, replication, blocksize, mtime, atime, permission, owner, group, symlink, path, fileId, locations, childrenNum, feInfo, storagePolicy);
        }
    }
    ...
    private static FsPermission getPermissionForFileStatus(INodeAttributes node, boolean isEncrypted) {
        ...
        org.zlab.ocov.tracker.Runtime.update(perm, 82);
    ...
    private static ContentSummary getContentSummaryInt(FSDirectory fsd, INodesInPath iip) throws IOException {
        ...
        try {
            ...
            if (targetNode == null) {
                ...
                ContentSummaryComputationContext cscc = new ContentSummaryComputationContext(fsd, fsd.getFSNamesystem(), fsd.getContentCountLimit(), fsd.getContentSleepMicroSec(), fsd.getPermissionChecker());
        ...
        try {
            ...
            if (targetNode.isDirectory()) {
                ...
                if (feature != null) {
                    ...
                    org.zlab.ocov.tracker.Runtime.update(counts, 83);
                    QuotaCounts quotas = feature.getQuota();
                    org.zlab.ocov.tracker.Runtime.update(quotas, 84);
                    usage = ((QuotaUsage.Builder) org.zlab.ocov.tracker.Runtime.update(new QuotaUsage.Builder(), 85)).fileAndDirectoryCount(counts.getNameSpace()).quota(quotas.getNameSpace()).spaceConsumed(counts.getStorageSpace()).spaceQuota(quotas.getStorageSpace()).typeConsumed(counts.getTypeSpaces().asArray()).typeQuota(quotas.getTypeSpaces().asArray()).build();
                }
            }
            return usage;
        } finally {
    ...
    static class GetBlockLocationsResult {
        ...
        private GetBlockLocationsResult(boolean updateAccessTime, LocatedBlocks blocks, INodesInPath iip) {
