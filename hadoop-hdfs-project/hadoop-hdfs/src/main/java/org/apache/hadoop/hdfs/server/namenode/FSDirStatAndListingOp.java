...
class FSDirStatAndListingOp {
    ...
    static DirectoryListing getListingInt(FSDirectory fsd, FSPermissionChecker pc, final String srcArg, byte[] startAfter, boolean needLocation) throws IOException {
        final INodesInPath iip = fsd.resolvePath(pc, srcArg, DirOp.READ);
        org.zlab.ocov.tracker.Runtime.update(iip, 95);
    ...
    static HdfsFileStatus getFileInfo(FSDirectory fsd, FSPermissionChecker pc, String srcArg, boolean resolveLink, boolean needLocation, boolean needBlockToken) throws IOException {
        ...
        if (pc.isSuperUser()) {
            ...
            try {
                ...
                org.zlab.ocov.tracker.Runtime.update(iip, 96);
     ...
     */
    private static DirectoryListing getListing(FSDirectory fsd, INodesInPath iip, byte[] startAfter, boolean needLocation) throws IOException {
        ...
        try {
            ...
            if (!targetNode.isDirectory()) {
                ...
                // target INode
                return new DirectoryListing(new HdfsFileStatus[] { createFileStatus(fsd, iip, null, parentStoragePolicy, needLocation, false) }, 0);
            }
            ...
            for (int i = 0; i < numOfListing && locationBudget > 0; i++) {
                ...
                byte childStoragePolicy = !child.isSymlink() ? getStoragePolicyID(child.getLocalStoragePolicyID(), parentStoragePolicy) : parentStoragePolicy;
                ...
                if (listing[i] instanceof HdfsLocatedFileStatus) {
                    ...
                    if (blks != null) {
                        ...
                        if (ecPolicy != null && !ecPolicy.isReplicationPolicy()) {
                            ...
                            // Approximate #locations with locatedBlockCount() *
                            // replicationFactor.
                            locationBudget -= blks.locatedBlockCount() * listing[i].getReplication();
                        }
                    }
                }
            }
    ...
    private static DirectoryListing getSnapshotsListing(FSDirectory fsd, INodesInPath iip, byte[] startAfter) throws IOException {
        ...
        for (int i = 0; i < numOfListing; i++) {
            ...
            listing[i] = createFileStatus(fsd, iip, sRoot, HdfsConstants.BLOCK_STORAGE_POLICY_ID_UNSPECIFIED, false, false);
        }
    ...
    static HdfsFileStatus createFileStatusForEditLog(FSDirectory fsd, INodesInPath iip) throws IOException {
        return createFileStatus(fsd, iip, null, HdfsConstants.BLOCK_STORAGE_POLICY_ID_UNSPECIFIED, false, false);
    }
    ...
    private static HdfsFileStatus createFileStatus(FSDirectory fsd, INodesInPath iip, INode child, byte storagePolicy, boolean needLocation, boolean needBlockToken) throws IOException {
        ...
        EnumSet<HdfsFileStatus.Flags> flags = DFSUtil.getFlags(isEncrypted, isErasureCoded, isSnapShottable, hasAcl);
        return createFileStatus(size, node.isDirectory(), replication, blocksize, node.getModificationTime(snapshot), node.getAccessTime(snapshot), nodeAttrs.getFsPermission(), flags, nodeAttrs.getUserName(), nodeAttrs.getGroupName(), node.isSymlink() ? node.asSymlink().getSymlink() : null, name, node.getId(), childrenNum, feInfo, storagePolicy, ecPolicy, loc);
    }
        ...
        try {
            ...
            if (targetNode.isDirectory()) {
                ...
                if (feature != null) {
                    ...
                    org.zlab.ocov.tracker.Runtime.update(counts, 97);
                    QuotaCounts quotas = feature.getQuota();
                    org.zlab.ocov.tracker.Runtime.update(quotas, 98);
                    usage = ((QuotaUsage.Builder) org.zlab.ocov.tracker.Runtime.update(new QuotaUsage.Builder(), 99)).fileAndDirectoryCount(counts.getNameSpace()).quota(quotas.getNameSpace()).spaceConsumed(counts.getStorageSpace()).spaceQuota(quotas.getStorageSpace()).typeConsumed(counts.getTypeSpaces().asArray()).typeQuota(quotas.getTypeSpaces().asArray()).build();
                }
            }
            return usage;
        } finally {
