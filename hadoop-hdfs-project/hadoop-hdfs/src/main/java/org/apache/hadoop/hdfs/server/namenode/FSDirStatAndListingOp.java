...
class FSDirStatAndListingOp {
    ...
    static DirectoryListing getListingInt(FSDirectory fsd, FSPermissionChecker pc, final String srcArg, byte[] startAfter, boolean needLocation) throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(iip, 66);
    ...
    static HdfsFileStatus getFileInfo(FSDirectory fsd, FSPermissionChecker pc, String srcArg, boolean resolveLink, boolean needLocation, boolean needBlockToken) throws IOException {
        ...
        if (pc.isSuperUser()) {
            ...
            try {
                ...
                org.zlab.ocov.tracker.Runtime.update(iip, 67);
        ...
        try {
            ...
            if (targetNode.isDirectory()) {
                ...
                if (feature != null) {
                    ...
                    org.zlab.ocov.tracker.Runtime.update(quotas, 68);
                    usage = ((QuotaUsage.Builder) org.zlab.ocov.tracker.Runtime.update(new QuotaUsage.Builder(), 69)).fileAndDirectoryCount(counts.getNameSpace()).quota(quotas.getNameSpace()).spaceConsumed(counts.getStorageSpace()).spaceQuota(quotas.getStorageSpace()).typeConsumed(counts.getTypeSpaces().asArray()).typeQuota(quotas.getTypeSpaces().asArray()).build();
                }
            }
            return usage;
        } finally {
