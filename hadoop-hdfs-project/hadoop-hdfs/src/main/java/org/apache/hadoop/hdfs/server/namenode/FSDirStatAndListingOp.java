...
class FSDirStatAndListingOp {
    ...
    static DirectoryListing getListingInt(FSDirectory fsd, final String srcArg, byte[] startAfter, boolean needLocation) throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(iip, 50);
    ...
    private static DirectoryListing getSnapshotsListing(FSDirectory fsd, INodesInPath iip, byte[] startAfter) throws IOException {
        ...
        return ((DirectoryListing) org.zlab.ocov.tracker.Runtime.update(new DirectoryListing(listing, snapshots.size() - skipSize - numOfListing), 52));
    }
    ...
    private static FsPermission getPermissionForFileStatus(INodeAttributes node, boolean isEncrypted) {
        ...
        org.zlab.ocov.tracker.Runtime.update(perm, 53);
        ...
        try {
            ...
            if (targetNode.isDirectory()) {
                ...
                if (feature != null) {
                    ...
                    org.zlab.ocov.tracker.Runtime.update(counts, 54);
                    QuotaCounts quotas = feature.getQuota();
                    org.zlab.ocov.tracker.Runtime.update(quotas, 55);
                    usage = ((QuotaUsage.Builder) org.zlab.ocov.tracker.Runtime.update(new QuotaUsage.Builder(), 56)).fileAndDirectoryCount(counts.getNameSpace()).quota(quotas.getNameSpace()).spaceConsumed(counts.getStorageSpace()).spaceQuota(quotas.getStorageSpace()).typeConsumed(counts.getTypeSpaces().asArray()).typeQuota(quotas.getTypeSpaces().asArray()).build();
                }
            }
            return usage;
        } finally {
