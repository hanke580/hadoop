...
public class CacheManager {
    ...
    CacheManager(FSNamesystem namesystem, Configuration conf, BlockManager blockManager) {
        ...
        this.enabled = conf.getBoolean(DFS_NAMENODE_CACHING_ENABLED_KEY, DFS_NAMENODE_CACHING_ENABLED_DEFAULT);
        ...
        this.cachedBlocks = enabled ? new LightWeightGSet<CachedBlock, CachedBlock>(LightWeightGSet.computeCapacity(cachedBlocksPercent, "cachedBlocks")) : new LightWeightGSet<>(0);
        this.isCheckLockTimeEnable = conf.getBoolean(DFS_NAMENODE_CRM_CHECKLOCKTIME_ENABLE, DFS_NAMENODE_CRM_CHECKLOCKTIME_DEFAULT);
        this.maxLockTimeMs = conf.getLong(DFS_NAMENODE_CRM_MAXLOCKTIME_MS, DFS_NAMENODE_CRM_MAXLOCKTIME_MS_DEFAULT);
        this.sleepTimeMs = conf.getLong(DFS_NAMENODE_CRM_SLEEP_TIME_MS, DFS_NAMENODE_CRM_SLEEP_TIME_MS_DEFAULT);
    }
    ...
    CacheDirectiveInfo addDirectiveFromEditLog(CacheDirectiveInfo directive) throws InvalidRequestException {
        ...
        CacheDirective entry = ((CacheDirective) org.zlab.ocov.tracker.Runtime.update(new CacheDirective(directive), 146));
    ...
    public CacheDirectiveInfo addDirective(CacheDirectiveInfo info, FSPermissionChecker pc, EnumSet<CacheFlag> flags) throws IOException {
        ...
        try {
            ...
            directive = ((CacheDirective) org.zlab.ocov.tracker.Runtime.update(new CacheDirective(id, path, replication, expiryTime), 147));
    ...
    public BatchedListEntries<CachePoolEntry> listCachePools(FSPermissionChecker pc, String prevKey) {
        ...
        for (Entry<String, CachePool> cur : tailMap.entrySet()) {
            if ((org.zlab.ocov.tracker.Runtime.updateBranch(numListed++, maxListCachePoolsResponses, ">=", 27))) {
    ...
    public void setCachedLocations(LocatedBlocks locations) {
        ...
        if ((org.zlab.ocov.tracker.Runtime.updateBranch(cachedBlocks.size(), 0, ">", 28))) {
    ...
    @SuppressFBWarnings(value = "EC_UNRELATED_TYPES", justification = "HDFS-15255 Asked Wei-Chiu and Pifta to review this" + " warning and we all agree the code is OK and the warning is not " + "needed")
    ...
    public final void processCacheReport(final DatanodeID datanodeID, final List<Long> blockIds) throws IOException {
        if (!enabled) {
            LOG.debug("Ignoring cache report from {} because {} = false. number of blocks: {}", datanodeID, DFS_NAMENODE_CACHING_ENABLED_KEY, blockIds.size());
            return;
        }
