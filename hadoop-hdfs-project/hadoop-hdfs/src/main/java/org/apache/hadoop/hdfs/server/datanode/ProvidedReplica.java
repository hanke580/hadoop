...
public abstract class ProvidedReplica extends ReplicaInfo {
    ...
    public InputStream getDataInputStream(long seekOffset) throws IOException {
        if (remoteFS != null) {
            ...
            try {
                if (pathHandle != null) {
                    ins = remoteFS.open(pathHandle, conf.getInt(IO_FILE_BUFFER_SIZE_KEY, IO_FILE_BUFFER_SIZE_DEFAULT));
                } else {
    ...
    public int compareWith(ScanInfo info) {
        if (info.getFileRegion().equals(new FileRegion(this.getBlockId(), new Path(getRemoteURI()), fileOffset, this.getNumBytes(), this.getGenerationStamp()))) {
            return 0;
        } else {
