...
public class StorageLocation implements Checkable<StorageLocation.CheckContext, VolumeCheckResult>, Comparable<StorageLocation> {
    ...
    public boolean matchesStorageDirectory(StorageDirectory sd, String bpid) throws IOException {
        if (sd.getStorageLocation().getStorageType() == StorageType.PROVIDED && storageType == StorageType.PROVIDED) {
        ...
        return this.getBpURI(bpid, Storage.STORAGE_DIR_CURRENT).normalize().equals(sd.getRoot().toURI().normalize());
    }
