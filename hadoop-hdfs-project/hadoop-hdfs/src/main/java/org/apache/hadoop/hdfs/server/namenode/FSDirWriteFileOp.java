...
class FSDirWriteFileOp {
    ...
    static LocatedBlock makeLocatedBlock(FSNamesystem fsn, BlockInfo blk, DatanodeStorageInfo[] locs, long offset) throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(lBlk, 54);
    ...
    static LocatedBlock storeAllocatedBlock(FSNamesystem fsn, String src, long fileId, String clientName, ExtendedBlock previous, DatanodeStorageInfo[] targets) throws IOException {
        ...
        FileState fileState = analyzeFileState(fsn, iip, fileId, clientName, previous, onRetryBlock);
        org.zlab.ocov.tracker.Runtime.update(fileState, 55);
        final INodeFile pendingFile = fileState.inode;
        src = fileState.path;
    ...
    static INodeFile addFileForEditLog(FSDirectory fsd, long id, INodesInPath existing, byte[] localName, PermissionStatus permissions, List<AclEntry> aclEntries, List<XAttr> xAttrs, short replication, long modificationTime, long atime, long preferredBlockSize, boolean underConstruction, String clientName, String clientMachine, byte storagePolicyId, byte ecPolicyID) {
        ...
        try {
            ...
            if (underConstruction) {
                ...
                org.zlab.ocov.tracker.Runtime.update(newNode, 56);
    ...
    private static INodesInPath addFile(FSDirectory fsd, INodesInPath existing, byte[] localName, PermissionStatus permissions, short replication, long preferredBlockSize, String clientName, String clientMachine, boolean shouldReplicate, String ecPolicyName, String storagePolicy) throws IOException {
        ...
        try {
            ...
            org.zlab.ocov.tracker.Runtime.update(newNode, 57);
            newNode.setLocalName(localName);
            newNode.toUnderConstruction(clientName, clientMachine);
            newiip = fsd.addINode(existing, newNode, permissions.getPermission());
            org.zlab.ocov.tracker.Runtime.update(newiip, 58);
        } finally {
            fsd.writeUnlock();
        }
        if (newiip == null) {
