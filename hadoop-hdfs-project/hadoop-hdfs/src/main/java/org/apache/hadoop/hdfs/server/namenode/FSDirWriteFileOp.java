...
class FSDirWriteFileOp {
    ...
    static ValidateAddBlockResult validateAddBlock(FSNamesystem fsn, FSPermissionChecker pc, String src, long fileId, String clientName, ExtendedBlock previous, LocatedBlock[] onRetryBlock) throws IOException {
        ...
        return new ValidateAddBlockResult(blockSize, numTargets, storagePolicyID, clientMachine, blockType, ecPolicy);
    }
    ...
    static LocatedBlock makeLocatedBlock(FSNamesystem fsn, BlockInfo blk, DatanodeStorageInfo[] locs, long offset) throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(lBlk, 57);
    ...
    static LocatedBlock storeAllocatedBlock(FSNamesystem fsn, String src, long fileId, String clientName, ExtendedBlock previous, DatanodeStorageInfo[] targets) throws IOException {
        ...
        FileState fileState = analyzeFileState(fsn, iip, fileId, clientName, previous, onRetryBlock);
        org.zlab.ocov.tracker.Runtime.update(fileState, 58);
        final INodeFile pendingFile = fileState.inode;
        src = fileState.path;
        if (onRetryBlock[0] != null) {
            if ((org.zlab.ocov.tracker.Runtime.updateBranch(onRetryBlock[0].getLocations().length, 0, ">", 21))) {
        ...
        org.zlab.ocov.tracker.Runtime.update(newBlock, 59);
    ...
    static DatanodeStorageInfo[] chooseTargetForNewBlock(BlockManager bm, String src, DatanodeInfo[] excludedNodes, String[] favoredNodes, EnumSet<AddBlockFlag> flags, ValidateAddBlockResult r) throws IOException {
        ...
        boolean ignoreClientLocality = (flags != null && flags.contains(AddBlockFlag.IGNORE_CLIENT_LOCALITY));
        ...
        Set<Node> excludedNodesSet = (excludedNodes == null) ? new HashSet<>() : new HashSet<>(Arrays.asList(excludedNodes));
        List<String> favoredNodesList = (favoredNodes == null) ? Collections.emptyList() : Arrays.asList(favoredNodes);
    ...
    static HdfsFileStatus startFile(FSNamesystem fsn, INodesInPath iip, PermissionStatus permissions, String holder, String clientMachine, EnumSet<CreateFlag> flag, boolean createParent, short replication, long blockSize, FileEncryptionInfo feInfo, INode.BlocksMapUpdateInfo toRemoveBlocks, boolean shouldReplicate, String ecPolicyName, boolean logRetryEntry) throws IOException {
        ...
        if (iip.getLastINode() != null) {
            if (overwrite) {
                ...
                if ((org.zlab.ocov.tracker.Runtime.updateBranch(ret, 0, ">=", 22))) {
    ...
    static INodeFile addFileForEditLog(FSDirectory fsd, long id, INodesInPath existing, byte[] localName, PermissionStatus permissions, List<AclEntry> aclEntries, List<XAttr> xAttrs, short replication, long modificationTime, long atime, long preferredBlockSize, boolean underConstruction, String clientName, String clientMachine, byte storagePolicyId, byte ecPolicyID) {
        ...
        try {
            ...
            if (isStriped) {
                ...
                if (ecPolicy == null) {
                    throw new IOException(String.format("Cannot find erasure coding policy for new file %s/%s, " + "ecPolicyID=%d", existing.getPath(), Arrays.toString(localName), ecPolicyID));
                }
            }
     ...
     */
    private static INodesInPath addFile(FSDirectory fsd, INodesInPath existing, byte[] localName, PermissionStatus permissions, short replication, long preferredBlockSize, String clientName, String clientMachine, boolean shouldReplicate, String ecPolicyName) throws IOException {
        Preconditions.checkNotNull(existing);
        long modTime = now();
        INodesInPath newiip;
        fsd.writeLock();
    ...
    static boolean completeFile(FSNamesystem fsn, FSPermissionChecker pc, final String srcArg, String holder, ExtendedBlock last, long fileId) throws IOException {
        String src = srcArg;
        if (NameNode.stateChangeLog.isDebugEnabled()) {
    ...
    private static boolean completeFileInternal(FSNamesystem fsn, INodesInPath iip, String holder, Block last, long fileId) throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(pendingFile, 60);
        return true;
    }

    private static INodeFile newINodeFile(long id, PermissionStatus permissions, long mtime, long atime, Short replication, Byte ecPolicyID, long preferredBlockSize, byte storagePolicyId, BlockType blockType) {
        return new INodeFile(id, null, permissions, mtime, atime, BlockInfo.EMPTY_ARRAY, replication, ecPolicyID, preferredBlockSize, storagePolicyId, blockType);
    }

    private static INodeFile newINodeFile(long id, PermissionStatus permissions, long mtime, long atime, Short replication, Byte ecPolicyID, long preferredBlockSize, BlockType blockType) {
        return newINodeFile(id, permissions, mtime, atime, replication, ecPolicyID, preferredBlockSize, (byte) 0, blockType);
    }
     ...
     */
    static void saveAllocatedBlock(FSNamesystem fsn, String src, INodesInPath inodesInPath, Block newBlock, DatanodeStorageInfo[] targets, BlockType blockType) throws IOException {
        assert fsn.hasWriteLock();
    ...
    static class ValidateAddBlockResult {
        ...
        ValidateAddBlockResult(long blockSize, int numTargets, byte storagePolicyID, String clientMachine, BlockType blockType, ErasureCodingPolicy ecPolicy) {
            ...
            if (blockType == BlockType.STRIPED) {
                Preconditions.checkArgument(ecPolicy != null, "ecPolicy is not specified for striped block");
            }
        }
    }
}
