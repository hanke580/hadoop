...
public final class AclStorage {
    ...
    public static List<AclEntry> readINodeLogicalAcl(INode inode) {
        ...
        org.zlab.ocov.tracker.Runtime.update(perm, 152);
        ...
        if (!accessEntries.isEmpty()) {
            ...
            existingAcl.add(((org.apache.hadoop.fs.permission.AclEntry.Builder) org.zlab.ocov.tracker.Runtime.update(new AclEntry.Builder().setScope(AclEntryScope.ACCESS).setType(AclEntryType.USER).setPermission(perm.getUserAction()), 153)).build());
            ...
            existingAcl.add(((org.apache.hadoop.fs.permission.AclEntry.Builder) org.zlab.ocov.tracker.Runtime.update(new AclEntry.Builder().setScope(AclEntryScope.ACCESS).setType(AclEntryType.MASK).setPermission(perm.getGroupAction()), 154)).build());
            // Add other entry implied from other permission bits.
            existingAcl.add(((org.apache.hadoop.fs.permission.AclEntry.Builder) org.zlab.ocov.tracker.Runtime.update(new AclEntry.Builder().setScope(AclEntryScope.ACCESS).setType(AclEntryType.OTHER).setPermission(perm.getOtherAction()), 155)).build());
        } else {
