...
public final class AclStorage {
    ...
    public static boolean copyINodeDefaultAcl(INode child) {
        ...
        if (!AclUtil.isMinimalAcl(accessEntries) || !defaultEntries.isEmpty()) {
            ...
            org.zlab.ocov.tracker.Runtime.update(child, 109);
    ...
    public static List<AclEntry> readINodeLogicalAcl(INode inode) {
        ...
        org.zlab.ocov.tracker.Runtime.update(perm, 110);
        ...
        if (!accessEntries.isEmpty()) {
            ...
            existingAcl.add(((org.apache.hadoop.fs.permission.AclEntry.Builder) org.zlab.ocov.tracker.Runtime.update(new AclEntry.Builder().setScope(AclEntryScope.ACCESS).setType(AclEntryType.USER).setPermission(perm.getUserAction()), 111)).build());
            ...
            existingAcl.add(((org.apache.hadoop.fs.permission.AclEntry.Builder) org.zlab.ocov.tracker.Runtime.update(new AclEntry.Builder().setScope(AclEntryScope.ACCESS).setType(AclEntryType.MASK).setPermission(perm.getGroupAction()), 112)).build());
            // Add other entry implied from other permission bits.
            existingAcl.add(((org.apache.hadoop.fs.permission.AclEntry.Builder) org.zlab.ocov.tracker.Runtime.update(new AclEntry.Builder().setScope(AclEntryScope.ACCESS).setType(AclEntryType.OTHER).setPermission(perm.getOtherAction()), 113)).build());
        } else {
