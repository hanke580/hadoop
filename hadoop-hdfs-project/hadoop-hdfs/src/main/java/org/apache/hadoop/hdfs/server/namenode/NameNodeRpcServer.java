...
public class NameNodeRpcServer implements NamenodeProtocols {
    ...
    public DirectoryListing getListing(String src, byte[] startAfter, boolean needLocation) throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(files, 30);
        if (files != null) {
    ...
    public EventBatchList getEditsFromTxid(final long txid) throws IOException {
        ...
        namesystem.checkOperation(OperationCategory.READ);
        namesystem.checkSuperuserPrivilege();
        final int maxEventsPerRPC = nn.getConf().getInt(DFSConfigKeys.DFS_NAMENODE_INOTIFY_MAX_EVENTS_PER_RPC_KEY, DFSConfigKeys.DFS_NAMENODE_INOTIFY_MAX_EVENTS_PER_RPC_DEFAULT);
