...
public class NameNodeRpcServer implements NamenodeProtocols {
    ...
    public NameNodeRpcServer(Configuration conf, NameNode nn) throws IOException {
        ...
        BlockingService lifelineProtoPbService = DatanodeLifelineProtocolService.newReflectiveBlockingService(lifelineProtoPbTranslator);
        ...
        if (serviceRpcAddr != null) {
            ...
            DFSUtil.addInternalPBProtocol(conf, HAServiceProtocolPB.class, haPbService, serviceRpcServer);
            DFSUtil.addInternalPBProtocol(conf, ReconfigurationProtocolPB.class, reconfigurationPbService, serviceRpcServer);
            DFSUtil.addInternalPBProtocol(conf, NamenodeProtocolPB.class, NNPbService, serviceRpcServer);
            DFSUtil.addInternalPBProtocol(conf, DatanodeProtocolPB.class, dnProtoPbService, serviceRpcServer);
            DFSUtil.addInternalPBProtocol(conf, RefreshAuthorizationPolicyProtocolPB.class, refreshAuthService, serviceRpcServer);
            DFSUtil.addInternalPBProtocol(conf, RefreshUserMappingsProtocolPB.class, refreshUserMappingService, serviceRpcServer);
            // We support Refreshing call queue here in case the client RPC queue is full
            DFSUtil.addInternalPBProtocol(conf, RefreshCallQueueProtocolPB.class, refreshCallQueueService, serviceRpcServer);
            DFSUtil.addInternalPBProtocol(conf, GenericRefreshProtocolPB.class, genericRefreshService, serviceRpcServer);
            DFSUtil.addInternalPBProtocol(conf, GetUserMappingsProtocolPB.class, getUserMappingService, serviceRpcServer);
        ...
        if (lifelineRpcAddr != null) {
            ...
            if (lifelineHandlerCount <= 0) {
                float lifelineHandlerRatio = conf.getFloat(DFS_NAMENODE_LIFELINE_HANDLER_RATIO_KEY, DFS_NAMENODE_LIFELINE_HANDLER_RATIO_DEFAULT);
            ...
            DFSUtil.addInternalPBProtocol(conf, DatanodeLifelineProtocolPB.class, lifelineProtoPbService, lifelineRpcServer);
            // Update the address with the correct port
        ...
        boolean enableStateContext = conf.getBoolean(DFS_NAMENODE_STATE_CONTEXT_ENABLED_KEY, DFS_NAMENODE_STATE_CONTEXT_ENABLED_DEFAULT);
        ...
        // Add all the RPC protocols that the namenode implements
        DFSUtil.addInternalPBProtocol(conf, HAServiceProtocolPB.class, haPbService, clientRpcServer);
        DFSUtil.addInternalPBProtocol(conf, ReconfigurationProtocolPB.class, reconfigurationPbService, clientRpcServer);
        DFSUtil.addInternalPBProtocol(conf, NamenodeProtocolPB.class, NNPbService, clientRpcServer);
        DFSUtil.addInternalPBProtocol(conf, DatanodeProtocolPB.class, dnProtoPbService, clientRpcServer);
        DFSUtil.addInternalPBProtocol(conf, RefreshAuthorizationPolicyProtocolPB.class, refreshAuthService, clientRpcServer);
        DFSUtil.addInternalPBProtocol(conf, RefreshUserMappingsProtocolPB.class, refreshUserMappingService, clientRpcServer);
        DFSUtil.addInternalPBProtocol(conf, RefreshCallQueueProtocolPB.class, refreshCallQueueService, clientRpcServer);
        DFSUtil.addInternalPBProtocol(conf, GenericRefreshProtocolPB.class, genericRefreshService, clientRpcServer);
        DFSUtil.addInternalPBProtocol(conf, GetUserMappingsProtocolPB.class, getUserMappingService, clientRpcServer);
        ...
        defaultECPolicyName = conf.getTrimmed(DFSConfigKeys.DFS_NAMENODE_EC_SYSTEM_DEFAULT_POLICY, DFSConfigKeys.DFS_NAMENODE_EC_SYSTEM_DEFAULT_POLICY_DEFAULT);
        ...
        clientRpcServer.addSuppressedLoggingExceptions(StandbyException.class, UnresolvedPathException.class);
    ...
    private CacheEntryWithPayload getCacheEntryWithPayload(Object payload) {
        Pair<byte[], Integer> clientInfo = NameNode.getClientIdAndCallId(this.ipProxyUsers);
        return RetryCache.waitForCompletion(retryCache, payload, clientInfo.getLeft(), clientInfo.getRight());
    }
    ...
    public HdfsFileStatus create(String src, FsPermission masked, String clientName, EnumSetWritable<CreateFlag> flag, boolean createParent, short replication, long blockSize, CryptoProtocolVersion[] supportedVersions, String ecPolicyName, String storagePolicy) throws IOException {
        ...
        stateChangeLog.debug("*DIR* NameNode.create: file {} for {} at {}.", src, clientName, clientMachine);
        if (!checkPathLength(src)) {
            throw new IOException("create: Pathname too long.  Limit " + MAX_PATH_LENGTH + " characters, " + MAX_PATH_DEPTH + " levels.");
        }
        namesystem.checkOperation(OperationCategory.WRITE);
        CacheEntryWithPayload cacheEntry = getCacheEntryWithPayload(null);
        org.zlab.ocov.tracker.Runtime.update(cacheEntry, 18);
        if (cacheEntry != null && cacheEntry.isSuccess()) {
        ...
        try {
            ...
            status = namesystem.startFile(src, perm, clientName, clientMachine, flag.get(), createParent, replication, blockSize, supportedVersions, ecPolicyName, storagePolicy, cacheEntry != null);
        } finally {
    ...
    public LastBlockWithStatus append(String src, String clientName, EnumSetWritable<CreateFlag> flag) throws IOException {
        checkNNStartup();
        String clientMachine = getClientMachine();
        stateChangeLog.debug("*DIR* NameNode.append: file {} for {} at {}.", src, clientName, clientMachine);
        namesystem.checkOperation(OperationCategory.WRITE);
        CacheEntryWithPayload cacheEntry = getCacheEntryWithPayload(null);
        if (cacheEntry != null && cacheEntry.isSuccess()) {
    ...
    public void setStoragePolicy(String src, String policyName) throws IOException {
        checkNNStartup();
        stateChangeLog.debug("*DIR* NameNode.setStoragePolicy for path: {}, policyName: {}", src, policyName);
    ...
    public LocatedBlock getAdditionalDatanode(final String src, final long fileId, final ExtendedBlock blk, final DatanodeInfo[] existings, final String[] existingStorageIDs, final DatanodeInfo[] excludes, final int numAdditionalNodes, final String clientName) throws IOException {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("getAdditionalDatanode: src={}, fileId={}, blk={}, existings={}, excludes={}" + ", numAdditionalNodes={}, clientName={}", src, fileId, blk, Arrays.asList(existings), Arrays.asList(excludes), numAdditionalNodes, clientName);
        }
        ...
        LocatedBlock locatedBlock = namesystem.getAdditionalDatanode(src, fileId, blk, existings, existingStorageIDs, excludeSet, numAdditionalNodes, clientName);
        return locatedBlock;
    }
    ...
    public void concat(String trg, String[] src) throws IOException {
        ...
        if (stateChangeLog.isDebugEnabled()) {
            stateChangeLog.debug("*DIR* NameNode.concat: src path {} to target path {}", Arrays.toString(src), trg);
        }
        namesystem.checkOperation(OperationCategory.WRITE);
        CacheEntry cacheEntry = getCacheEntry();
        if (cacheEntry != null && cacheEntry.isSuccess()) {
    ...
    public void renewLease(String clientName, List<String> namespaces) throws IOException {
        if (namespaces != null && namespaces.size() > 0) {
            LOG.warn("namespaces({}) should be null or empty " + "on NameNode side, please check it.", namespaces);
            throw new IOException("namespaces(" + namespaces + ") should be null or empty");
        }
        checkNNStartup();
    ...
    public BatchedDirectoryListing getBatchedListing(String[] srcs, byte[] startAfter, boolean needLocation) throws IOException {
        checkNNStartup();
        BatchedDirectoryListing batchedListing = namesystem.getBatchedListing(srcs, startAfter, needLocation);
    ...
    // ClientProtocol
    @Override
    public BatchedEntries<OpenFileEntry> listOpenFiles(long prevId, EnumSet<OpenFilesType> openFilesTypes, String path) throws IOException {
        checkNNStartup();
    ...
    public void setBalancerBandwidth(long bandwidth) throws IOException {
        if (bandwidth > HdfsServerConstants.MAX_BANDWIDTH_PER_DATANODE) {
            throw new IllegalArgumentException("Bandwidth should not exceed maximum limit " + HdfsServerConstants.MAX_BANDWIDTH_PER_DATANODE + " bytes per second");
        }
        checkNNStartup();
    ...
    public HeartbeatResponse sendHeartbeat(DatanodeRegistration nodeReg, StorageReport[] report, long dnCacheCapacity, long dnCacheUsed, int xmitsInProgress, int xceiverCount, int failedVolumes, VolumeFailureSummary volumeFailureSummary, boolean requestFullBlockReportLease, @Nonnull SlowPeerReports slowPeers, @Nonnull SlowDiskReports slowDisks) throws IOException {
        ...
        return namesystem.handleHeartbeat(nodeReg, report, dnCacheCapacity, dnCacheUsed, xceiverCount, xmitsInProgress, failedVolumes, volumeFailureSummary, requestFullBlockReportLease, slowPeers, slowDisks);
    }
    ...
    public DatanodeCommand blockReport(final DatanodeRegistration nodeReg, String poolId, final StorageBlockReport[] reports, final BlockReportContext context) throws IOException {
        checkNNStartup();
        verifyRequest(nodeReg);
        blockStateChangeLog.debug("*BLOCK* NameNode.blockReport: from {}, reports.length={}.", nodeReg, reports.length);
        ...
        try {
            ...
            LOG.warn("Datanode {} is attempting to report but not register yet.", nodeReg);
        ...
        if (nn.getFSImage().isUpgradeFinalized() && !namesystem.isRollingUpgrade() && nn.isActiveState() && noStaleStorages) {
    ...
    public DatanodeCommand cacheReport(DatanodeRegistration nodeReg, String poolId, List<Long> blockIds) throws IOException {
        checkNNStartup();
        verifyRequest(nodeReg);
        blockStateChangeLog.debug("*BLOCK* NameNode.cacheReport: from {} {} blocks", nodeReg, blockIds.size());
    ...
    public void blockReceivedAndDeleted(final DatanodeRegistration nodeReg, String poolId, StorageReceivedDeletedBlocks[] receivedAndDeletedBlocks) throws IOException {
        ...
        blockStateChangeLog.debug("*BLOCK* NameNode.blockReceivedAndDeleted: from {} {} blocks.", nodeReg, receivedAndDeletedBlocks.length);
        final BlockManager bm = namesystem.getBlockManager();
    ...
    public void sendLifeline(DatanodeRegistration nodeReg, StorageReport[] report, long dnCacheCapacity, long dnCacheUsed, int xmitsInProgress, int xceiverCount, int failedVolumes, VolumeFailureSummary volumeFailureSummary) throws IOException {
        checkNNStartup();
        verifyRequest(nodeReg);
        namesystem.handleLifeline(nodeReg, report, dnCacheCapacity, dnCacheUsed, xceiverCount, xmitsInProgress, failedVolumes, volumeFailureSummary);
    }
    ...
    public synchronized void transitionToStandby(StateChangeRequestInfo req) throws ServiceFailedException, AccessControlException, IOException {
        ...
        if (nn.getState().equals(NameNode.OBSERVER_STATE.toString()) && req.getSource() == RequestSource.REQUEST_BY_ZKFC) {
    ...
    // HAServiceProtocol
    @Override
    public synchronized void transitionToObserver(StateChangeRequestInfo req) throws ServiceFailedException, AccessControlException, IOException {
        checkNNStartup();
        nn.checkHaStateChange(req);
    ...
    public String createSnapshot(String snapshotRoot, String snapshotName) throws IOException {
        checkNNStartup();
        LOG.debug("*DIR* NameNode.createSnapshot: Path {} and SnapshotName {}", snapshotRoot, snapshotName);
    ...
    public void deleteSnapshot(String snapshotRoot, String snapshotName) throws IOException {
        checkNNStartup();
        LOG.debug("*DIR* NameNode.deleteSnapshot: Path {} and SnapshotName {}", snapshotRoot, snapshotName);
    ...
    @Override
    public // Client Protocol
    void disallowSnapshot(String snapshot) throws IOException {
        checkNNStartup();
    ...
    public SnapshotStatus[] getSnapshotListing(String snapshotRoot) throws IOException {
        checkNNStartup();
        SnapshotStatus[] status = namesystem.getSnapshotListing(snapshotRoot);
    ...
    public SnapshotDiffReportListing getSnapshotDiffReportListing(String snapshotRoot, String earlierSnapshotName, String laterSnapshotName, byte[] startPath, int index) throws IOException {
        checkNNStartup();
        SnapshotDiffReportListing report = namesystem.getSnapshotDiffReportListing(snapshotRoot, earlierSnapshotName, laterSnapshotName, startPath, index);
        metrics.incrSnapshotDiffReportOps();
        return report;
    }
    ...
    // ClientProtocol
    @Override
    public void reencryptEncryptionZone(final String zone, final ReencryptAction action) throws IOException {
        checkNNStartup();
        namesystem.checkOperation(OperationCategory.WRITE);
        final CacheEntry cacheEntry = getCacheEntry();
        if (cacheEntry != null && cacheEntry.isSuccess()) {
    ...
    // ClientProtocol
    @Override
    public BatchedEntries<ZoneReencryptionStatus> listReencryptionStatus(final long prevId) throws IOException {
        checkNNStartup();
    ...
    public Long getNextSPSPath() throws IOException {
        ...
        StoragePolicySatisfierMode spsMode = (spsMgr != null ? spsMgr.getMode() : StoragePolicySatisfierMode.NONE);
        if (spsMode != StoragePolicySatisfierMode.EXTERNAL) {
            if (LOG.isDebugEnabled()) {
                LOG.debug("SPS service mode is {}, so external SPS service is " + "not allowed to fetch the path Ids", spsMode);
            }
            throw new IOException("SPS service mode is " + spsMode + ", so " + "external SPS service is not allowed to fetch the path Ids");
        }
