...
public class DataStorage extends Storage {
    ...
    private static boolean createStorageID(StorageDirectory sd, int lv, Configuration conf) {
        ...
        final boolean haveValidStorageId = DataNodeLayoutVersion.supports(LayoutVersion.Feature.ADD_DATANODE_AND_STORAGE_UUIDS, lv) && DatanodeStorage.isValidStorageId(sd.getStorageUuid());
     ...
     */
    public static boolean createStorageID(StorageDirectory sd, boolean regenerateStorageIds, Configuration conf) {
        final String oldStorageID = sd.getStorageUuid();
        if (sd.getStorageLocation() != null && sd.getStorageLocation().getStorageType() == StorageType.PROVIDED) {
            // Only one provided storage id is supported.
            // TODO support multiple provided storage ids
            sd.setStorageUuid(conf.get(DFSConfigKeys.DFS_PROVIDER_STORAGEUUID, DFSConfigKeys.DFS_PROVIDER_STORAGEUUID_DEFAULT));
            return false;
        }
    ...
    private StorageDirectory loadStorageDirectory(DataNode datanode, NamespaceInfo nsInfo, StorageLocation location, StartupOption startOpt, List<Callable<StorageDirectory>> callables) throws IOException {
        ...
        try {
            ...
            switch(curState) {
                ...
                case // format
                NOT_FORMATTED:
                    LOG.info("Storage directory with location {} is not formatted for " + "namespace {}. Formatting...", location, nsInfo.getNamespaceID());
    ...
    static int getParallelVolumeLoadThreadsNum(int dataDirs, Configuration conf) {
        ...
        LOG.info("Using {} threads to upgrade data directories ({}={}, " + "dataDirs={})", min, key, n, dataDirs);
        return min;
    }
     ...
     */
    @VisibleForTesting
    synchronized List<StorageDirectory> addStorageLocations(DataNode datanode, NamespaceInfo nsInfo, Collection<StorageLocation> dataDirs, StartupOption startOpt) throws IOException {
    ...
    private List<StorageLocation> loadDataStorage(DataNode datanode, NamespaceInfo nsInfo, Collection<StorageLocation> dataDirs, StartupOption startOpt, ExecutorService executor) throws IOException {
    ...
    private List<StorageDirectory> loadBlockPoolSliceStorage(DataNode datanode, NamespaceInfo nsInfo, Collection<StorageLocation> dataDirs, StartupOption startOpt, ExecutorService executor) throws IOException {
     ...
     */
    synchronized void removeVolumes(final Collection<StorageLocation> storageLocations) throws IOException {
    ...
    void format(StorageDirectory sd, NamespaceInfo nsInfo, String newDatanodeUuid, Configuration conf) throws IOException {
        // create directory
        sd.clearDirectory();
  ...
  */
   ...
   */
     ...
     */
    private boolean doTransition(StorageDirectory sd, NamespaceInfo nsInfo, StartupOption startOpt, List<Callable<StorageDirectory>> callables, Configuration conf) throws IOException {
        if (sd.getStorageLocation().getStorageType() == StorageType.PROVIDED) {
            createStorageID(sd, layoutVersion, conf);
            // regular start up for PROVIDED storage directories
            return false;
        }
        ...
        if (this.layoutVersion > HdfsServerConstants.DATANODE_LAYOUT_VERSION) {
            ...
            // doUgrade already has written properties
            return true;
        }
        ...
        // failed.
        throw new IOException("BUG: The stored LV = " + this.getLayoutVersion() + " is newer than the supported LV = " + HdfsServerConstants.DATANODE_LAYOUT_VERSION);
    }
     ...
     * @param sd  storage directory
     */
    void doUpgradePreFederation(final StorageDirectory sd, final NamespaceInfo nsInfo, final List<Callable<StorageDirectory>> callables, final Configuration conf) throws IOException {
    ...
    private void doUpgrade(final StorageDirectory sd, final NamespaceInfo nsInfo, final File prevDir, final File tmpDir, final File bbwDir, final File toDir, final int oldLV, Configuration conf) throws IOException {
    ...
    void upgradeProperties(StorageDirectory sd, Configuration conf) throws IOException {
        createStorageID(sd, layoutVersion, conf);
        LOG.info("Updating layout version from {} to {} for storage {}", layoutVersion, HdfsServerConstants.DATANODE_LAYOUT_VERSION, sd.getRoot());
    ...
    void doRollback(StorageDirectory sd, NamespaceInfo nsInfo) throws IOException {
        ...
        if (!prevDir.exists()) {
            if (DataNodeLayoutVersion.supports(LayoutVersion.Feature.FEDERATION, HdfsServerConstants.DATANODE_LAYOUT_VERSION)) {
                ...
                writeProperties(sd);
                LOG.info("Layout version rolled back to {} for storage {}", HdfsServerConstants.DATANODE_LAYOUT_VERSION, sd.getRoot());
            }
            return;
        }
   ...
   */
    ...
    private static void linkBlocks(File from, File to, int oldLV, HardLink hl, Configuration conf) throws IOException {
        ...
        linkBlocksHelper(from, to, hl, upgradeToIdBasedLayout, to, idBasedLayoutSingleLinks, pathCache);
        // Detect and remove duplicate entries.
        final ArrayList<LinkArgs> duplicates = findDuplicateEntries(idBasedLayoutSingleLinks);
        if (!duplicates.isEmpty()) {
            LOG.error("There are {} duplicate block " + "entries within the same volume.", duplicates.size());
    ...
    static ArrayList<LinkArgs> findDuplicateEntries(ArrayList<LinkArgs> all) {
        ...
        Collections.sort(all, new Comparator<LinkArgs>() {
            ...
            public int compare(LinkArgs a, LinkArgs b) {
                return ComparisonChain.start().compare(a.blockFile(), b.blockFile()).compare(a.src(), b.src()).compare(a.dst(), b.dst()).result();
            }
        });
    ...
    private static void removeDuplicateEntries(ArrayList<LinkArgs> all, ArrayList<LinkArgs> duplicates) {
        ...
        for (LinkArgs duplicate : duplicates) {
            ...
            long prevGenstamp = Block.getGenerationStamp(prevHighest.get(0).blockFile());
    ...
    static void linkBlocksHelper(File from, File to, HardLink hl, boolean upgradeToIdBasedLayout, File blockRoot, List<LinkArgs> idBasedLayoutSingleLinks, Map<File, File> pathCache) throws IOException {
        if (!from.exists()) {
            return;
        }
        if (!from.isDirectory()) {
        ...
        if (blockNames.length > 0) {
            if (upgradeToIdBasedLayout) {
                for (String blockName : blockNames) {
                     ...
                     */
                    File cachedDest = pathCache.computeIfAbsent(blockLocation, k -> blockLocation);
                    idBasedLayoutSingleLinks.add(new LinkArgs(from, cachedDest, blockName));
                    hl.linkStats.countSingleLinks++;
                }
            } else {
        ...
        if (otherNames != null) {
            for (int i = 0; i < otherNames.length; i++) {
                linkBlocksHelper(new File(from, otherNames[i]), new File(to, otherNames[i]), hl, upgradeToIdBasedLayout, blockRoot, idBasedLayoutSingleLinks, pathCache);
            }
        }
    }
