...
public class FsImageValidation {
    ...
    static class Util {
        ...
        static void setLogLevel(Class<?> clazz, Level level) {
            ...
            if (log instanceof Log4JLogger) {
                ...
                LOG.info("setLogLevel {} to {}, getEffectiveLevel() = {}", clazz.getName(), level, logger.getEffectiveLevel());
            } else {
    ...
    private FSNamesystem loadImage(Configuration conf) throws IOException {
        final TimerTask checkProgress = new TimerTask() {
            ...
            @Override
            public void run() {
                final double percent = NameNode.getStartupProgress().createView().getPercentComplete(Phase.LOADING_FSIMAGE);
                LOG.info(String.format("%s Progress: %.1f%% (%s)", Phase.LOADING_FSIMAGE, 100 * percent, Util.memoryInfo()));
            }
        };
        ...
        if (fsImageFile.isDirectory()) {
            ...
            final FSImage fsImage = ((FSImage) org.zlab.ocov.tracker.Runtime.update(new FSImage(conf), 117));
            namesystem = ((FSNamesystem) org.zlab.ocov.tracker.Runtime.update(new FSNamesystem(conf, fsImage, true), 118));
            ...
            final FSImage fsImage = ((FSImage) org.zlab.ocov.tracker.Runtime.update(new FSImage(conf), 119));
            namesystem = ((FSNamesystem) org.zlab.ocov.tracker.Runtime.update(new FSNamesystem(conf, fsImage, true), 120));
        ...
        t.cancel();
        Cli.println("Loaded %s %s successfully in %s", FS_IMAGE, fsImageFile, StringUtils.formatTime(now() - loadStart));
        return namesystem;
    }
    ...
    static class INodeMapValidation {
        ...
        static void run(FSDirectory fsdir, AtomicInteger errorCount) {
            ...
            println("%s ended successfully: %d error(s) found.", INodeMapValidation.class.getSimpleName(), errorCount.get() - initErrorCount);
        }
    }
    ...
    public static void validate(File path, AtomicInteger errorCount) throws Exception {
        if (path.isFile()) {
            ...
            final File[] images = path.listFiles(Util.newFilenameFilter(NameNodeFile.IMAGE));
