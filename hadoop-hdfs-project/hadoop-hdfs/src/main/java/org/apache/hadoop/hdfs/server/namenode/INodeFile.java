...
public class INodeFile extends INodeWithAdditionalFields implements INodeFileAttributes, BlockCollection {
    ...
    enum HeaderFormat {
        ...
        PREFERRED_BLOCK_SIZE(null, 48, 1), BLOCK_LAYOUT_AND_REDUNDANCY(PREFERRED_BLOCK_SIZE.BITS, HeaderFormat.LAYOUT_BIT_WIDTH + 11, 0), STORAGE_POLICY_ID(BLOCK_LAYOUT_AND_REDUNDANCY.BITS, BlockStoragePolicySuite.ID_BIT_LENGTH, 0);
         ...
         */
        static long getBlockLayoutRedundancy(BlockType blockType, Short replication, Byte erasureCodingPolicyID) {
            ...
            switch(blockType) {
                case STRIPED:
                    ...
                    if (null == ErasureCodingPolicyManager.getInstance().getByID(erasureCodingPolicyID)) {
                        throw new IllegalArgumentException(String.format("Could not find EC policy with ID 0x%02x", erasureCodingPolicyID));
                    }
                ...
                case CONTIGUOUS:
                    if (erasureCodingPolicyID != REPLICATION_POLICY_ID) {
                        throw new IllegalArgumentException(String.format("Illegal EC policy 0x%02x for CONTIGUOUS block type", erasureCodingPolicyID));
                    }
    ...
    INodeFile(long id, byte[] name, PermissionStatus permissions, long mtime, long atime, BlockInfo[] blklist, short replication, long preferredBlockSize) {
        this(id, name, permissions, mtime, atime, blklist, replication, null, preferredBlockSize, (byte) 0, CONTIGUOUS);
    }

    INodeFile(long id, byte[] name, PermissionStatus permissions, long mtime, long atime, BlockInfo[] blklist, Short replication, Byte ecPolicyID, long preferredBlockSize, byte storagePolicyID, BlockType blockType) {
        super(id, name, permissions, mtime, atime);
        final long layoutRedundancy = HeaderFormat.getBlockLayoutRedundancy(blockType, replication, ecPolicyID);
    ...
    private void assertAllBlocksComplete(int numCommittedAllowed, short minReplication) {
        for (int i = 0; i < blocks.length; i++) {
            ...
            if (err != null) {
                throw new IllegalStateException(String.format("Unexpected block state: " + "%s, file=%s (%s), blocks=%s (i=%s)", err, this, getClass().getSimpleName(), Arrays.asList(blocks), i));
            }
        }
    }
    ...
    static String checkBlockComplete(BlockInfo[] blocks, int i, int numCommittedAllowed, short minReplication) {
        ...
        if (b.isStriped()) {
            ...
            if (b.getUnderConstructionFeature().getNumExpectedLocations() != blkStriped.getRealTotalBlockNum()) {
                return b + " is a striped block in " + state + " with less then " + "required number of blocks.";
            }
        } else {
    ...
    public final QuotaCounts computeQuotaUsage(BlockStoragePolicySuite bsps, byte blockStoragePolicyId, boolean useCache, int lastSnapshotId) {
        final QuotaCounts counts = ((QuotaCounts.Builder) org.zlab.ocov.tracker.Runtime.update(new QuotaCounts.Builder(), 61)).nameSpace(1).build();
        org.zlab.ocov.tracker.Runtime.update(counts, 62);
    ...
    public final QuotaCounts storagespaceConsumedStriped() {
        ...
        org.zlab.ocov.tracker.Runtime.update(counts, 63);
        for (BlockInfo b : blocks) {
    ...
    public final QuotaCounts storagespaceConsumedContiguous(BlockStoragePolicy bsp) {
        QuotaCounts counts = new QuotaCounts.Builder().build();
        org.zlab.ocov.tracker.Runtime.update(counts, 64);
    ...
    public void dumpINodeFile(PrintWriter out, StringBuilder prefix, final int snapshotId) {
    ...
    public long collectBlocksBeyondMax(final long max, final BlocksMapUpdateInfo collectedBlocks, Set<BlockInfo> toRetain) {
        ...
        if ((org.zlab.ocov.tracker.Runtime.updateBranch(n >= oldBlocks.length, 22))) {
            return size;
        }
