...
public class FSEditLogLoader {
        ...
        try {
            while (true) {
                try {
                    ...
                    try {
                        op = in.readOp();
                        org.zlab.ocov.tracker.Runtime.update(op, 71);
                        if (op == null) {
                            break;
                        }
                    } catch (Throwable e) {
    ...
    private void addNewBlock(AddBlockOp op, INodeFile file, ErasureCodingPolicy ecPolicy) throws IOException {
        ...
        if (isStriped) {
            newBlockInfo = ((BlockInfoStriped) org.zlab.ocov.tracker.Runtime.update(new BlockInfoStriped(newBlock, ecPolicy), 72));
        } else {
        ...
        if (newBlocks.length < oldBlocks.length) {
            ...
            for (int i = oldBlocks.length; i < newBlocks.length; i++) {
                ...
                if (!op.shouldCompleteLastBlock()) {
                    ...
                    if (isStriped) {
                        newBI = ((BlockInfoStriped) org.zlab.ocov.tracker.Runtime.update(new BlockInfoStriped(newBlock, ecPolicy), 73));
                    } else {
                    ...
                    if (isStriped) {
                        newBI = ((BlockInfoStriped) org.zlab.ocov.tracker.Runtime.update(new BlockInfoStriped(newBlock, ecPolicy), 74));
                    } else {
