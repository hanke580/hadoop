...
public class FSEditLogLoader {
    ...
    @VisibleForTesting
    static final LogThrottlingHelper LOAD_EDITS_LOG_HELPER = new LogThrottlingHelper(LOAD_EDIT_LOG_INTERVAL_MS);
    ...
    long loadFSEdits(EditLogInputStream edits, long expectedStartingTxId, long maxTxnsToRead, StartupOption startOpt, MetaRecoveryContext recovery) throws IOException {
        ...
        try {
            ...
            LogAction postLogAction = LOAD_EDITS_LOG_HELPER.record("post", endTime, numEdits, edits.length(), endTime - startTime);
        ...
        try {
            while (true) {
                try {
                    ...
                    try {
                        op = in.readOp();
                        org.zlab.ocov.tracker.Runtime.update(op, 43);
                        if (op == null) {
                            break;
                        }
                    } catch (Throwable e) {
    ...
    private void addNewBlock(AddBlockOp op, INodeFile file, ErasureCodingPolicy ecPolicy) throws IOException {
        ...
        if (isStriped) {
            newBlockInfo = ((BlockInfoStriped) org.zlab.ocov.tracker.Runtime.update(new BlockInfoStriped(newBlock, ecPolicy), 44));
        } else {
        ...
        if (newBlocks.length < oldBlocks.length) {
            ...
            for (int i = oldBlocks.length; i < newBlocks.length; i++) {
                ...
                if (!op.shouldCompleteLastBlock()) {
                    ...
                    if (isStriped) {
                        newBI = ((BlockInfoStriped) org.zlab.ocov.tracker.Runtime.update(new BlockInfoStriped(newBlock, ecPolicy), 45));
                    } else {
                    ...
                    if (isStriped) {
                        newBI = ((BlockInfoStriped) org.zlab.ocov.tracker.Runtime.update(new BlockInfoStriped(newBlock, ecPolicy), 46));
                    } else {
