...
public class FSEditLogLoader {
    ...
    private final LogThrottlingHelper loadEditsLogHelper = new LogThrottlingHelper(LOAD_EDIT_LOG_INTERVAL_MS);
     ...
     * along.
     */
    long loadFSEdits(EditLogInputStream edits, long expectedStartingTxId, long maxTxnsToRead, StartupOption startOpt, MetaRecoveryContext recovery) throws IOException {
        ...
        try {
            ...
            if (preLogAction.shouldLog()) {
                FSImage.LOG.info("Start loading edits file " + edits.getName() + " maxTxnsToRead = " + maxTxnsToRead + LogThrottlingHelper.getLogSupressionMessage(preLogAction));
            }
            long numEdits = loadEditRecords(edits, false, expectedStartingTxId, maxTxnsToRead, startOpt, recovery);
            long endTime = timer.monotonicNow();
            LogAction postLogAction = loadEditsLogHelper.record("post", endTime, numEdits, edits.length(), endTime - startTime);
            if (postLogAction.shouldLog()) {
                FSImage.LOG.info("Loaded {} edits file(s) (the last named {}) of " + "total size {}, total edits {}, total load time {} ms", postLogAction.getCount(), edits.getName(), postLogAction.getStats(1).getSum(), postLogAction.getStats(0).getSum(), postLogAction.getStats(2).getSum());
            }
            return numEdits;
        } finally {
            edits.close();
    ...
    long loadEditRecords(EditLogInputStream in, boolean closeOnExit, long expectedStartingTxId, StartupOption startOpt, MetaRecoveryContext recovery) throws IOException {
        return loadEditRecords(in, closeOnExit, expectedStartingTxId, Long.MAX_VALUE, startOpt, recovery);
    }

    long loadEditRecords(EditLogInputStream in, boolean closeOnExit, long expectedStartingTxId, long maxTxnsToRead, StartupOption startOpt, MetaRecoveryContext recovery) throws IOException {
        ...
        try {
            while (true) {
                try {
                    ...
                    if (op.hasTransactionId()) {
                        try {
                            this.left_136 = (op.getTransactionId());
                            this.right_136 = (expectedTxId);
                        } catch (Exception e) {
                            this.left_136 = 0L;
                            this.right_136 = 0L;
                        }
                    ...
                    try {
                        ...
                        try {
                            this.left_137 = (lastInodeId);
                            this.right_137 = (inodeId);
                        } catch (Exception e) {
                            this.left_137 = 0L;
                            this.right_137 = 0L;
                        }
                    ...
                    if (op.hasTransactionId()) {
                        ...
                        try {
                            this.left_138 = (now - lastLogTime);
                            this.right_138 = (REPLAY_TRANSACTION_LOG_INTERVAL);
                        } catch (Exception e) {
                            this.left_138 = 0L;
                            this.right_138 = 0L;
                        }
                    ...
                    totalEdits++;
                    try {
                        this.left_139 = (numEdits);
                        this.right_139 = (maxTxnsToRead);
                    } catch (Exception e) {
                        this.left_139 = 0L;
                        this.right_139 = 0L;
                    }
            ...
            assert numEdits <= maxTxnsToRead || numEdits == 1 : "should read at least one txn, but not more than the configured max";
        }
        return numEdits;
    }
            ...
            case OP_CREATE_SNAPSHOT:
                {
                    ...
                    String path = fsNamesys.getSnapshotManager().createSnapshot(fsDir.getFSNamesystem().getLeaseManager(), iip, snapshotRoot, createSnapshotOp.snapshotName, createSnapshotOp.mtime);
                    if (toAddRetryCache) {
            ...
            case OP_DELETE_SNAPSHOT:
                {
                    ...
                    INodesInPath iip = fsDir.getINodesInPath(snapshotRoot, DirOp.WRITE);
                    fsNamesys.getSnapshotManager().deleteSnapshot(iip, deleteSnapshotOp.snapshotName, new INode.ReclaimContext(fsNamesys.dir.getBlockStoragePolicySuite(), collectedBlocks, removedINodes, null), deleteSnapshotOp.mtime);
            ...
            case OP_RENAME_SNAPSHOT:
                {
                    ...
                    INodesInPath iip = fsDir.getINodesInPath(snapshotRoot, DirOp.WRITE);
                    fsNamesys.getSnapshotManager().renameSnapshot(iip, snapshotRoot, renameSnapshotOp.snapshotOldName, renameSnapshotOp.snapshotNewName, renameSnapshotOp.mtime);
                    if (toAddRetryCache) {
    ...
    private static String formatEditLogReplayError(EditLogInputStream in, long[] recentOpcodeOffsets, long txid) {
        ...
        sb.append("Error replaying edit log at offset " + in.getPosition()).append(".  Expected transaction ID was ").append(txid);
    ...
    private void updateBlocks(FSDirectory fsDir, BlockListUpdatingOp op, INodesInPath iip, INodeFile file, ErasureCodingPolicy ecPolicy) throws IOException {
        ...
        for (int i = 0; i < oldBlocks.length && i < newBlocks.length; i++) {
            ...
            fsNamesys.getBlockManager().getBlockIdManager().setGenerationStampIfGreater(newGenerationStamp);
    ...
    private void check203UpgradeFailure(int logVersion, Throwable e) throws IOException {
        ...
        // with the upgrade.
        if (Storage.is203LayoutVersion(logVersion) && logVersion != HdfsServerConstants.NAMENODE_LAYOUT_VERSION) {
        ...
        while (true) {
            ...
            try {
                ...
                if (in.getPosition() <= lastPos) {
                    FSImage.LOG.warn("After resync, the position, {} is not greater " + "than the previous position {}. Skipping remainder of this log.", in.getPosition(), lastPos);
                    break;
                }
                continue;
            }
    ...
    private long right_139;

    private long right_136;

    private long left_137;

    private long left_136;

    private long right_138;

    private long left_139;

    private long right_137;

    private long left_138;
}
