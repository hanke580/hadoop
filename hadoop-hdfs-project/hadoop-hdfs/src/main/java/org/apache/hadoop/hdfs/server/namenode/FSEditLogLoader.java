    ...
    long loadFSEdits(EditLogInputStream edits, long expectedStartingTxId, StartupOption startOpt, MetaRecoveryContext recovery) throws IOException {
        ...
        try {
            ...
            if (preLogAction.shouldLog()) {
                FSImage.LOG.info("Start loading edits file " + edits.getName() + LogThrottlingHelper.getLogSupressionMessage(preLogAction));
            }
            ...
            if (postLogAction.shouldLog()) {
                String msg = "Loaded " + postLogAction.getCount() + " edits file(s) " + "(the last named " + edits.getName() + ") of " + "total size " + postLogAction.getStats(1).getSum() + ", total edits " + postLogAction.getStats(0).getSum() + ", total load time " + postLogAction.getStats(2).getSum() + " ms";
        ...
        try {
            while (true) {
                try {
                    ...
                    try {
                        op = in.readOp();
                        org.zlab.ocov.tracker.Runtime.update(op, 53);
                        if (op == null) {
                            break;
                        }
                    } catch (Throwable e) {
            ...
            case OP_ALLOCATE_BLOCK_ID:
                {
                    ...
                    blockManager.getBlockIdManager().setLastAllocatedBlockId(allocateBlockIdOp.blockId);
                    break;
                }
