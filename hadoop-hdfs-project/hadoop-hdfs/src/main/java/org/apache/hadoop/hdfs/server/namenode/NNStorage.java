...
public class NNStorage extends Storage implements Closeable, StorageErrorReporter {
     ...
     */
    final protected List<StorageDirectory> removedStorageDirs = new CopyOnWriteArrayList<>();
    ...
    public boolean isPreUpgradableLayout(StorageDirectory sd) throws IOException {
        ...
        try {
            ...
            oldFile.close();
            oldFile = null;
            if ((org.zlab.ocov.tracker.Runtime.updateBranch(oldVersion < LAST_PRE_UPGRADE_LAYOUT_VERSION, 38))) {
                return false;
            }
        } finally {
    ...
    void attemptRestoreRemovedStorage() {
        ...
        synchronized (this.restorationLock) {
            ...
            for (StorageDirectory sd : this.removedStorageDirs) {
                ...
                LOG.info("currently disabled dir {}; type={} ;canwrite={}", root.getAbsolutePath(), sd.getStorageDirType(), FileUtil.canWrite(root));
    ...
    synchronized void setStorageDirectories(Collection<URI> fsNameDirs, Collection<URI> fsEditsDirs, Collection<URI> sharedEditsDirs) throws IOException {
        ...
        for (URI dirName : fsNameDirs) {
            ...
            if (dirName.getScheme().compareTo("file") == 0) {
                ...
                StorageDirectory sd = new StorageDirectory(new File(dirName.getPath()), dirType, sharedEditsDirs.contains(dirName), new FsPermission(conf.get(DFSConfigKeys.DFS_NAMENODE_NAME_DIR_PERMISSION_KEY, DFSConfigKeys.DFS_NAMENODE_NAME_DIR_PERMISSION_DEFAULT)));
                this.addStorageDir(sd);
            }
        }
        ...
        for (URI dirName : fsEditsDirs) {
            ...
            // URI is of type file://
            if (dirName.getScheme().compareTo("file") == 0) {
                this.addStorageDir(new StorageDirectory(new File(dirName.getPath()), NameNodeDirType.EDITS, sharedEditsDirs.contains(dirName), new FsPermission(conf.get(DFSConfigKeys.DFS_NAMENODE_NAME_DIR_PERMISSION_KEY, DFSConfigKeys.DFS_NAMENODE_NAME_DIR_PERMISSION_DEFAULT))));
            }
        }
    }
    ...
    public void format(NamespaceInfo nsInfo, boolean isRollingUpgrade) throws IOException {
        Preconditions.checkArgument(nsInfo.getLayoutVersion() == 0 || nsInfo.getLayoutVersion() == getServiceLayoutVersion() || isRollingUpgrade, "Bad layout version: %s", nsInfo.getLayoutVersion());
    ...
    void readProperties(StorageDirectory sd, StartupOption startupOption) throws IOException {
        ...
        if (HdfsServerConstants.RollingUpgradeStartupOption.ROLLBACK.matches(startupOption)) {
            ...
            props.setProperty("layoutVersion", Integer.toString(getServiceLayoutVersion()));
        }
    ...
    String getDeprecatedProperty(String prop) {
        assert getLayoutVersion() > getServiceLayoutVersion() : "getDeprecatedProperty should only be done when loading " + "storage from past versions during upgrade.";
    ...
    public static File getTemporaryEditsFile(StorageDirectory sd, long startTxId, long endTxId, long timestamp) {
        ...
        if (startOpt == StartupOption.UPGRADE || startOpt == StartupOption.UPGRADEONLY) {
            ...
            if (!NameNodeLayoutVersion.supports(LayoutVersion.Feature.FEDERATION, layoutVersion)) {
                ...
                if (startOpt.getClusterId() != null && !startOpt.getClusterId().equals(getClusterID())) {
                    LOG.warn("Clusterid mismatch - current clusterid: {}, Ignoring " + "given clusterid: {}", getClusterID(), startOpt.getClusterId());
                }
            }
    ...
    public String determineClusterId() {
        ...
        while (sdit.hasNext()) {
            ...
            try {
                ...
                LOG.info("current cluster id for sd={};lv={};" + "cid={}", sd.getCurrentDir(), layoutVersion, cid);
        ...
        for (StorageDirectory sd : getStorageDirs()) {
            try {
                ...
                if (getStorageDirs().isEmpty()) {
                    throw new IOException("All the storage failed while writing " + "properties to VERSION file");
                }
            }
        }
    }
