...
public class NNStorage extends Storage implements Closeable, StorageErrorReporter {
    ...
    public boolean isPreUpgradableLayout(StorageDirectory sd) throws IOException {
        ...
        try {
            ...
            oldFile.close();
            oldFile = null;
            if ((org.zlab.ocov.tracker.Runtime.updateBranch(oldVersion < LAST_PRE_UPGRADE_LAYOUT_VERSION, 25)))
                return false;
        } finally {
    ...
    int getNumStorageDirs(NameNodeDirType dirType) {
        ...
        int numDirs = 0;
        for (; it.hasNext(); it.next()) numDirs++;
        return numDirs;
    }
    ...
    public void writeAll() throws IOException {
        ...
        for (StorageDirectory sd : getStorageDirs()) {
            try {
                ...
                LOG.warn("Error during write properties to the VERSION file to " + sd.toString(), e);
                reportErrorsOnDirectory(sd);
