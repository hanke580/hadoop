...
public class NNStorage extends Storage implements Closeable, StorageErrorReporter {
    ...
    public boolean isPreUpgradableLayout(StorageDirectory sd) throws IOException {
        ...
        try {
            ...
            oldFile.close();
            oldFile = null;
            if ((org.zlab.ocov.tracker.Runtime.updateBranch(oldVersion < LAST_PRE_UPGRADE_LAYOUT_VERSION, 37))) {
                return false;
            }
        } finally {
    ...
    String getDeprecatedProperty(String prop) {
        assert getLayoutVersion() > HdfsServerConstants.NAMENODE_LAYOUT_VERSION : "getDeprecatedProperty should only be done when loading " + "storage from past versions during upgrade.";
