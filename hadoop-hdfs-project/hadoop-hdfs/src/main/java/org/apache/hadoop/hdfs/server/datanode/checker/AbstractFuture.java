...
public abstract class AbstractFuture<V> implements ListenableFuture<V> {
    ...
    static {
        ...
        try {
            ...
            try {
                helper = new SafeAtomicHelper(newUpdater(Waiter.class, Thread.class, "thread"), newUpdater(Waiter.class, Waiter.class, "next"), newUpdater(AbstractFuture.class, Waiter.class, "waiters"), newUpdater(AbstractFuture.class, Listener.class, "listeners"), newUpdater(AbstractFuture.class, Object.class, "value"));
    ...
    private void removeWaiter(Waiter node) {
        ...
        restart: while (true) {
            ...
            while (curr != null) {
                succ = curr.next;
                if (curr.thread != null) {
                    // we aren't unlinking this node, update
                    // pred.
                    pred = curr;
                } else if (pred != null) {
                    // We are unlinking this node and it has a
                    // predecessor.
                    pred.next = succ;
                    if (pred.thread == null) {
                        // We raced with another node that
                    ...
                    if (ATOMIC_HELPER.casWaiters(this, oldHead, node)) {
                        while (true) {
                            ...
                            if (remainingNanos < SPIN_THRESHOLD_NANOS) {
                                ...
                                // this thread.
                                removeWaiter(node);
                                // jump down to the busy wait loop
