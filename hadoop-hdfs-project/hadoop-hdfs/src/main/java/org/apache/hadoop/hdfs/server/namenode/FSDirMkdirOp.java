...
class FSDirMkdirOp {
    ...
    static FileStatus mkdirs(FSNamesystem fsn, FSPermissionChecker pc, String src, PermissionStatus permissions, boolean createParent) throws IOException {
        ...
        try {
            ...
            org.zlab.ocov.tracker.Runtime.update(iip, 117);
    ...
    private static INodesInPath createParentDirectories(FSDirectory fsd, INodesInPath iip, PermissionStatus perm, boolean inheritPerms) throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(existing, 118);
        int missing = iip.length() - existing.length();
        if (missing == 0) {
            // full path exists, return parents.
            existing = iip.getParentINodesInPath();
        } else if (missing > 1) {
            // need to create at least one ancestor dir.
            // Ensure that the user can traversal the path by adding implicit
            // u+wx permission to all ancestor directories.
    ...
    static void mkdirForEditLog(FSDirectory fsd, long inodeId, String src, PermissionStatus permissions, List<AclEntry> aclEntries, long timestamp) throws QuotaExceededException, UnresolvedLinkException, AclException, FileAlreadyExistsException, ParentNotDirectoryException, AccessControlException {
        ...
        org.zlab.ocov.tracker.Runtime.update(iip, 119);
        final byte[] localName = iip.getLastLocalName();
        final INodesInPath existing = iip.getParentINodesInPath();
        org.zlab.ocov.tracker.Runtime.update(existing, 120);
    ...
    private static PermissionStatus addImplicitUwx(PermissionStatus parentPerm, PermissionStatus perm) {
        ...
        if (p.getUnmasked() == null) {
            ancestorPerm = new FsPermission(p.getUserAction().or(FsAction.WRITE_EXECUTE), p.getGroupAction(), p.getOtherAction());
        } else {
            ancestorPerm = FsCreateModes.create(new FsPermission(p.getUserAction().or(FsAction.WRITE_EXECUTE), p.getGroupAction(), p.getOtherAction()), p.getUnmasked());
        }
    ...
    private static INodesInPath unprotectedMkdir(FSDirectory fsd, long inodeId, INodesInPath parent, byte[] name, PermissionStatus permission, List<AclEntry> aclEntries, long timestamp) throws QuotaExceededException, AclException, FileAlreadyExistsException {
        ...
        final INodeDirectory dir = ((INodeDirectory) org.zlab.ocov.tracker.Runtime.update(new INodeDirectory(inodeId, name, permission, timestamp), 121));
