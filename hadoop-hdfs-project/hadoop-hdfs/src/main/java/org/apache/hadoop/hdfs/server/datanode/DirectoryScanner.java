...
public class DirectoryScanner implements Runnable {
    ...
    private static final String START_MESSAGE_WITH_THROTTLE = START_MESSAGE + " and throttle limit of %dms/s";
    ...
    void start() {
        shouldRun = true;
        long offset = ThreadLocalRandom.current().nextInt((int) (scanPeriodMsecs / MILLIS_PER_SECOND)) * //msec
        MILLIS_PER_SECOND;
        long firstScanTime = Time.now() + offset;
        String logMsg;
        if (throttleLimitMsPerSec < MILLIS_PER_SECOND) {
            logMsg = String.format(START_MESSAGE_WITH_THROTTLE, FastDateFormat.getInstance().format(firstScanTime), scanPeriodMsecs, throttleLimitMsPerSec);
        } else {
            logMsg = String.format(START_MESSAGE, FastDateFormat.getInstance().format(firstScanTime), scanPeriodMsecs);
        }
        LOG.info(logMsg);
    ...
    public void run() {
        try {
            org.zlab.ocov.tracker.Runtime.update(this, 128);
            if (!shouldRun) {
    ...
    public class ReportCompiler implements Callable<ScanInfoPerBlockPool> {
        ...
        public void throttle() throws InterruptedException {
            ...
            if ((throttleLimitMsPerSec < 1000) && (throttleTimer.now(TimeUnit.MILLISECONDS) > throttleLimitMsPerSec)) {
