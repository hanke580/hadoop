...
public final class ErasureCodingPolicyManager {
    ...
    private boolean userDefinedAllowed = DFSConfigKeys.DFS_NAMENODE_EC_POLICIES_USERPOLICIES_ALLOWED_KEY_DEFAULT;

    // Supported storage policies for striped EC files
    private static final byte[] SUITABLE_STORAGE_POLICIES_FOR_EC_STRIPED_MODE = new byte[] { HdfsConstants.HOT_STORAGE_POLICY_ID, HdfsConstants.COLD_STORAGE_POLICY_ID, HdfsConstants.ALLSSD_STORAGE_POLICY_ID };
     ...
     */
        ...
        updatePolicies();
        maxCellSize = conf.getInt(DFSConfigKeys.DFS_NAMENODE_EC_POLICIES_MAX_CELLSIZE_KEY, DFSConfigKeys.DFS_NAMENODE_EC_POLICIES_MAX_CELLSIZE_DEFAULT);
        userDefinedAllowed = conf.getBoolean(DFSConfigKeys.DFS_NAMENODE_EC_POLICIES_USERPOLICIES_ALLOWED_KEY, DFSConfigKeys.DFS_NAMENODE_EC_POLICIES_USERPOLICIES_ALLOWED_KEY_DEFAULT);
    }
    ...
    public synchronized ErasureCodingPolicy addPolicy(ErasureCodingPolicy policy) {
        ...
        try {
            this.left_146 = (policy.getCellSize());
            this.right_146 = (maxCellSize);
        } catch (Exception e) {
            this.left_146 = 0L;
            this.right_146 = 0L;
        }
        if (policy.getCellSize() > maxCellSize) {
            throw new HadoopIllegalArgumentException("Cell size " + policy.getCellSize() + " should not exceed maximum " + maxCellSize + " bytes");
        }
        String assignedNewName = ErasureCodingPolicy.composePolicyName(policy.getSchema(), policy.getCellSize());
        ...
        if (getCurrentMaxPolicyID() == ErasureCodeConstants.MAX_POLICY_ID) {
            throw new HadoopIllegalArgumentException("Adding erasure coding " + "policy failed because the number of policies stored in the " + "system already reached the threshold, which is " + ErasureCodeConstants.MAX_POLICY_ID);
        }
        policy = new ErasureCodingPolicy(assignedNewName, policy.getSchema(), policy.getCellSize(), getNextAvailablePolicyID());
    ...
    private byte getNextAvailablePolicyID() {
        ...
        return nextPolicyID > ErasureCodeConstants.USER_DEFINED_POLICY_START_ID ? nextPolicyID : ErasureCodeConstants.USER_DEFINED_POLICY_START_ID;
    }
    ...
    public synchronized void removePolicy(String name) {
        ...
        allPersistedPolicies.put(ecPolicy.getId(), createPolicyInfo(ecPolicy, ErasureCodingPolicyState.REMOVED));
        /*
    ...
    public synchronized boolean disablePolicy(String name) {
        ...
        if (enabledPoliciesByName.containsKey(name)) {
            ...
            allPersistedPolicies.put(info.getPolicy().getId(), createPolicyInfo(info.getPolicy(), ErasureCodingPolicyState.DISABLED));
            return true;
        }
        return false;
    }
    ...
    public synchronized boolean enablePolicy(String name) {
        ...
        if (enabledPoliciesByName.containsKey(name)) {
            if (defaultPolicyName.equals(name)) {
                allPersistedPolicies.put(info.getPolicy().getId(), createPolicyInfo(info.getPolicy(), ErasureCodingPolicyState.ENABLED));
                return true;
            }
            return false;
        }
        ...
        enabledPolicies = enabledPoliciesByName.values().toArray(new ErasureCodingPolicy[0]);
        allPersistedPolicies.put(ecPolicy.getId(), createPolicyInfo(info.getPolicy(), ErasureCodingPolicyState.ENABLED));
     ...
     */
    public synchronized void loadPolicies(List<ErasureCodingPolicyInfo> ecPolicies, Configuration conf) throws IOException {
    ...
    private void enableDefaultPolicy(Configuration conf) throws IOException {
        defaultPolicyName = conf.getTrimmed(DFSConfigKeys.DFS_NAMENODE_EC_SYSTEM_DEFAULT_POLICY, DFSConfigKeys.DFS_NAMENODE_EC_SYSTEM_DEFAULT_POLICY_DEFAULT);
        if (!defaultPolicyName.isEmpty()) {
            final ErasureCodingPolicyInfo info = policiesByName.get(defaultPolicyName);
            if (info == null) {
                String names = policiesByName.values().stream().map((pi) -> pi.getPolicy().getName()).collect(Collectors.joining(", "));
                String msg = String.format("EC policy '%s' specified at %s is not a " + "valid policy. Please choose from list of available " + "policies: [%s]", defaultPolicyName, DFSConfigKeys.DFS_NAMENODE_EC_SYSTEM_DEFAULT_POLICY, names);
    ...
    private ErasureCodingPolicyInfo createPolicyInfo(ErasureCodingPolicy p, ErasureCodingPolicyState s) {
        ErasureCodingPolicyInfo policyInfo = new ErasureCodingPolicyInfo(p);
        policyInfo.setState(s);
        return policyInfo;
    }

    private long left_146;

    private long right_146;
}
