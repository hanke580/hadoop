...
abstract class AbstractINodeDiffList<N extends INode, A extends INodeAttributes, D extends AbstractINodeDiff<N, A, D>> implements Iterable<D> {
    ...
    public final List<D> asList() {
        return diffs != null ? Collections.unmodifiableList(diffs) : Collections.<D>emptyList();
    }
     ...
     */
    public final void deleteSnapshotDiff(INode.ReclaimContext reclaimContext, final int snapshot, final int prior, final N currentINode) {
        if (diffs == null) {
            return;
        }
    ...
    public String toString() {
        if (diffs != null) {
            final StringBuilder b = new StringBuilder(getClass().getSimpleName()).append("@").append(Integer.toHexString(hashCode())).append(": ");
            b.append("[");
            for (D d : diffs) {
                b.append(d).append(", ");
            }
