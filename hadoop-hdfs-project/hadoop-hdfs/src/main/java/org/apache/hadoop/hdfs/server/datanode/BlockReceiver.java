...
class BlockReceiver implements Closeable {
    ...
    void flushOrSync(boolean isSync) throws IOException {
        ...
        if (duration > datanodeSlowLogThresholdMs && LOG.isWarnEnabled()) {
            LOG.warn("Slow flushOrSync took " + duration + "ms (threshold=" + datanodeSlowLogThresholdMs + "ms), isSync:" + isSync + ", flushTotalNanos=" + flushTotalNanos + "ns, volume=" + getVolumeBaseUri() + ", blockId=" + replicaInfo.getBlockId());
        }
    }
    ...
    private int receivePacket() throws IOException {
        ...
        if (mirrorOut != null && !mirrorError) {
            try {
                ...
                if (duration > datanodeSlowLogThresholdMs && LOG.isWarnEnabled()) {
                    LOG.warn("Slow BlockReceiver write packet to mirror took " + duration + "ms (threshold=" + datanodeSlowLogThresholdMs + "ms), " + "downstream DNs=" + Arrays.toString(downstreamDNs) + ", blockId=" + replicaInfo.getBlockId());
                }
            } catch (IOException e) {
        ...
        if (lastPacketInBlock || len == 0) {
            ...
            try {
                ...
                if (onDiskLen < offsetInBlock) {
                    ...
                    long duration = Time.monotonicNow() - begin;
                    if (duration > datanodeSlowLogThresholdMs && LOG.isWarnEnabled()) {
                        LOG.warn("Slow BlockReceiver write data to disk cost:" + duration + "ms (threshold=" + datanodeSlowLogThresholdMs + "ms), " + "volume=" + getVolumeBaseUri() + ", blockId=" + replicaInfo.getBlockId());
                    }
    ...
    private void manageWriterOsCache(long offsetInBlock) {
        try {
            if (streams.getOutFd() != null && offsetInBlock > lastCacheManagementOffset + CACHE_DROP_LAG_BYTES) {
                ...
                long duration = Time.monotonicNow() - begin;
                if (duration > datanodeSlowLogThresholdMs && LOG.isWarnEnabled()) {
                    LOG.warn("Slow manageWriterOsCache took " + duration + "ms (threshold=" + datanodeSlowLogThresholdMs + "ms), volume=" + getVolumeBaseUri() + ", blockId=" + replicaInfo.getBlockId());
                }
            }
        } catch (Throwable t) {
    ...
    private Checksum computePartialChunkCrc(long blkoff, long ckoff) throws IOException {
        ...
        final Checksum partialCrc = ((org.apache.hadoop.util.DataChecksum) org.zlab.ocov.tracker.Runtime.update(DataChecksum.newDataChecksum(diskChecksum.getChecksumType(), diskChecksum.getBytesPerChecksum()), 7));
    ...
    class PacketResponder implements Runnable, Closeable {
        ...
        private void sendAckUpstreamUnprotected(PipelineAck ack, long seqno, long totalAckTimeNanos, long offsetInBlock, int myHeader) throws IOException {
            ...
            if (duration > datanodeSlowLogThresholdMs) {
                LOG.warn("Slow PacketResponder send ack to upstream took " + duration + "ms (threshold=" + datanodeSlowLogThresholdMs + "ms), " + myString + ", replyAck=" + replyAck + ", downstream DNs=" + Arrays.toString(downstreamDNs) + ", blockId=" + replicaInfo.getBlockId());
