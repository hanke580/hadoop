...
class BlockReceiver implements Closeable {
    ...
    BlockReceiver(final ExtendedBlock block, final StorageType storageType, final DataInputStream in, final String inAddr, final String myAddr, final BlockConstructionStage stage, final long newGs, final long minBytesRcvd, final long maxBytesRcvd, final String clientname, final DatanodeInfo srcDataNode, final DataNode datanode, DataChecksum requestedChecksum, CachingStrategy cachingStrategy, final boolean allowLazyPersist, final boolean pinning, final String storageId) throws IOException {
        try {
            ...
            if (isDatanode) {
                ...
                switch(stage) {
                    case PIPELINE_SETUP_CREATE:
                        replicaHandler = datanode.data.createRbw(storageType, storageId, block, allowLazyPersist, newGs);
                        if (newGs != 0L) {
                    ...
                    case TRANSFER_FINALIZED:
                        ...
                        replicaHandler = datanode.data.createTemporary(storageType, storageId, block, isTransfer);
                        break;
                    default:
            ...
            this.checksumOut = new DataOutputStream(new BufferedOutputStream(streams.getChecksumOut(), DFSUtilClient.getSmallBufferSize(datanode.getConf())));
            // write data chunk header if creating a new replica
            if (isCreate) {
                BlockMetadataHeader.writeHeader(checksumOut, diskChecksum);
            }
        } catch (ReplicaAlreadyExistsException | ReplicaNotFoundException | DiskOutOfSpaceException e) {
            throw e;
            ...
            DataNode.LOG.warn("IOException in BlockReceiver constructor :" + ioe.getMessage() + (cause == null ? "" : ". Cause is "), cause);
            if (cause != null) {
                ioe = cause;
    ...
    public void releaseAnyRemainingReservedSpace() {
        if (replicaInfo != null) {
            if (replicaInfo.getReplicaInfo().getBytesReserved() > 0) {
                LOG.warn("Block {} has not released the reserved bytes. " + "Releasing {} bytes as part of close.", replicaInfo.getBlockId(), replicaInfo.getReplicaInfo().getBytesReserved());
                replicaInfo.releaseAllBytesReserved();
            }
        }
    }
    ...
    boolean packetSentInTime() {
        ...
        if (!allowedIdleTime) {
            LOG.warn("A packet was last sent {}ms ago. Maximum idle time: {}ms.", diff, this.maxSendIdleTime);
        }
    ...
    void flushOrSync(boolean isSync, long seqno) throws IOException {
        ...
        if (duration > datanodeSlowLogThresholdMs && LOG.isWarnEnabled()) {
            ...
            LOG.warn("Slow flushOrSync took " + duration + "ms (threshold=" + datanodeSlowLogThresholdMs + "ms), isSync:" + isSync + ", flushTotalNanos=" + flushTotalNanos + "ns, volume=" + getVolumeBaseUri() + ", blockId=" + replicaInfo.getBlockId() + ", seqno=" + seqno);
        }
    }
    ...
    private int receivePacket() throws IOException {
        ...
        LOG.debug("Receiving one packet for block {} seqno:{} header:{} ", block, seqno, header);
            ...
            try {
                ...
                if (duration > datanodeSlowLogThresholdMs) {
                    ...
                    if (LOG.isWarnEnabled()) {
                        LOG.warn("Slow BlockReceiver write packet to mirror took {}ms " + "(threshold={}ms), downstream DNs={}, blockId={}, seqno={}", duration, datanodeSlowLogThresholdMs, Arrays.toString(downstreamDNs), replicaInfo.getBlockId(), seqno);
                    }
                }
            } catch (IOException e) {
            ...
            try {
                ...
                if (onDiskLen < offsetInBlock) {
                    ...
                    if (duration > datanodeSlowLogThresholdMs) {
                        ...
                        if (LOG.isWarnEnabled()) {
                            LOG.warn("Slow BlockReceiver write data to disk cost: {}ms " + "(threshold={}ms), volume={}, blockId={}, seqno={}", duration, datanodeSlowLogThresholdMs, getVolumeBaseUri(), replicaInfo.getBlockId(), seqno);
                        }
                    }
    ...
    private void manageWriterOsCache(long offsetInBlock, long seqno) {
        try {
            if (streams.getOutFd() != null && offsetInBlock > lastCacheManagementOffset + CACHE_DROP_LAG_BYTES) {
                long begin = Time.monotonicNow();
                //
                ...
                if (syncBehindWrites) {
                    if (syncBehindWritesInBackground) {
                        this.datanode.getFSDataset().submitBackgroundSyncFileRangeRequest(block, streams, lastCacheManagementOffset, offsetInBlock - lastCacheManagementOffset, SYNC_FILE_RANGE_WRITE);
                    } else {
                        streams.syncFileRangeIfPossible(lastCacheManagementOffset, offsetInBlock - lastCacheManagementOffset, SYNC_FILE_RANGE_WRITE);
                    }
                }
                //
                ...
                if (duration > datanodeSlowLogThresholdMs) {
                    ...
                    if (LOG.isWarnEnabled()) {
                        LOG.warn("Slow manageWriterOsCache took {}ms " + "(threshold={}ms), volume={}, blockId={}, seqno={}", duration, datanodeSlowLogThresholdMs, getVolumeBaseUri(), replicaInfo.getBlockId(), seqno);
                    }
                }
            }
        } catch (Throwable t) {
    ...
    private Checksum computePartialChunkCrc(long blkoff, long ckoff) throws IOException {
        ...
        final Checksum partialCrc = ((org.apache.hadoop.util.DataChecksum) org.zlab.ocov.tracker.Runtime.update(DataChecksum.newDataChecksum(diskChecksum.getChecksumType(), diskChecksum.getBytesPerChecksum()), 175));
    ...
    class PacketResponder implements Runnable, Closeable {
        ...
        void sendOOBResponse(final Status ackStatus) throws IOException, InterruptedException {
            ...
            try {
                sendAckUpstreamUnprotected(null, PipelineAck.UNKOWN_SEQNO, 0L, 0L, PipelineAck.combineHeader(datanode.getECN(), ackStatus, datanode.getSLOWByBlockPoolId(block.getBlockPoolId())));
            } finally {
        ...
        public void run() {
                    ...
                    try {
                        if (type != PacketResponderType.LAST_IN_PIPELINE && !mirrorError) {
                            ...
                            if (oobStatus != null) {
                                ...
                                sendAckUpstream(ack, PipelineAck.UNKOWN_SEQNO, 0L, 0L, PipelineAck.combineHeader(datanode.getECN(), Status.SUCCESS, datanode.getSLOWByBlockPoolId(block.getBlockPoolId())));
                                continue;
                            }
                            seqno = ack.getSeqno();
                        }
             ...
             */
                    ...
                    sendAckUpstream(ack, expected, totalAckTimeNanos, (pkt != null ? pkt.offsetInBlock : 0), PipelineAck.combineHeader(datanode.getECN(), myStatus, datanode.getSLOWByBlockPoolId(block.getBlockPoolId())));
                    if (pkt != null) {
        ...
        private void finalizeBlock(long startTime) throws IOException {
            ...
            if (CLIENT_TRACE_LOG.isInfoEnabled() && isClient) {
                ...
                CLIENT_TRACE_LOG.info(String.format(DN_CLIENTTRACE_FORMAT, inAddr, myAddr, replicaInfo.getVolume(), block.getNumBytes(), "HDFS_WRITE", clientname, offset, dnR.getDatanodeUuid(), block, endTime - startTime));
            } else {
                LOG.info("Received " + block + " on volume " + replicaInfo.getVolume() + " size " + block.getNumBytes() + " from " + inAddr);
            }
        }
        ...
        private void sendAckUpstreamUnprotected(PipelineAck ack, long seqno, long totalAckTimeNanos, long offsetInBlock, int myHeader) throws IOException {
            ...
            if (ack == null) {
                ...
                // ack read error
                int h = PipelineAck.combineHeader(datanode.getECN(), Status.SUCCESS, datanode.getSLOWByBlockPoolId(block.getBlockPoolId()));
            ...
            if (duration > datanodeSlowLogThresholdMs) {
                ...
                LOG.warn("Slow PacketResponder send ack to upstream took " + duration + "ms (threshold=" + datanodeSlowLogThresholdMs + "ms), " + myString + ", replyAck=" + replyAck + ", downstream DNs=" + Arrays.toString(downstreamDNs) + ", blockId=" + replicaInfo.getBlockId() + ", seqno=" + seqno);
