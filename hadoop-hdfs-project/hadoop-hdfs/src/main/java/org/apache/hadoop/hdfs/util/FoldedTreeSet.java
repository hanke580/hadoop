...
public class FoldedTreeSet<E> implements SortedSet<E> {
    ...
    private static class Node<E> {
        ...
        public void addEntryAt(E entry, int index) {
            assert !isFull();
            if (leftIndex == 0 || ((rightIndex != Node.NODE_SIZE - 1) && (rightIndex - index <= index - leftIndex))) {
                rightIndex++;
    ...
    private static final class TreeSetIterator<E> implements Iterator<E> {
        ...
        public void remove() {
            ...
            if (lastNode.size == 1) {
                ...
                try {
                    this.left_107 = (oldRIndex);
                    this.right_107 = (lastNode.rightIndex);
                } catch (Exception e) {
                    this.left_107 = 0L;
                    this.right_107 = 0L;
                }
        ...
        private long left_107;

        private long right_107;
    }
    ...
    private static int compare(Object lookup, Object stored, Comparator cmp) {
        return cmp != null ? cmp.compare(lookup, stored) : ((Comparable<Object>) lookup).compareTo(stored);
    }
    ...
    public Object[] toArray() {
        ...
        if (!isEmpty()) {
            int pos = 0;
            for (Node<E> node = root.getLeftMostNode(); node != null; pos += node.size, node = node.next) {
    ...
    public boolean addSortedLast(E entry) {
        if (isEmpty()) {
            ...
            Node<E> node = root.getRightMostNode();
            try {
                this.left_104 = (compare(node.entries[node.rightIndex], entry, comparator));
                this.right_104 = (0);
            } catch (Exception e) {
                this.left_104 = 0L;
                this.right_104 = 0L;
    ...
    public double fillRatio() {
        if (nodeCount > 1) {
            ...
            return (size + (Node.NODE_SIZE - root.getRightMostNode().size)) / (double) (nodeCount * Node.NODE_SIZE);
        }
        return 1.0;
    }
    ...
    private long right_101;

    private long right_106;

    private long left_102;

    private long left_101;

    private long right_103;

    private long left_104;

    private long right_102;

    private long left_103;

    private long right_105;

    private long left_106;
