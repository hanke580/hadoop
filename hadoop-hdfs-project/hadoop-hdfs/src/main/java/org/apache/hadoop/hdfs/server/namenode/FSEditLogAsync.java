...
class FSEditLogAsync extends FSEditLog implements Runnable {
    ...
    private boolean internal$isSyncThreadAlive29() {
        synchronized (syncThreadLock) {
            return syncThread != null && syncThread.isAlive();
        }
    }

    private void internal$startSyncThread30() {
        synchronized (syncThreadLock) {
    ...
    void internal$openForWrite31(int layoutVersion) throws IOException {
        try {
            startSyncThread();
    ...
    void internal$logEdit32(final FSEditLogOp op) {
    ...
    public void internal$logSync33() {
    ...
    private void internal$enqueueEdit34(Edit edit) {
        if (LOG.isDebugEnabled()) {
        ...
        try {
            ...
            if (!editPendingQ.offer(edit)) {
                ...
                if (Thread.holdsLock(this)) {
                    ...
                    try {
                        do {
                            // will be notified by next logSync.
                            this.wait(1000);
    ...
    private Edit internal$getEditInstance35(FSEditLogOp op) {
        final Edit edit;
    ...
    private static class SyncEdit extends Edit {
        ...
        public void internal$logSyncNotify28(RuntimeException ex) {
            synchronized (lock) {
                done = true;
                syncEx = ex;
                lock.notifyAll();
    ...
    private boolean isSyncThreadAlive() {
        try {
            if (!(this.editPendingQ.size() == this.syncWaitQ.size())) {
                org.zlab.dinv.runtimechecker.Runtime.addViolation(455);
            }
        } catch (Exception e) {
        }
        boolean returnValue;
        returnValue = internal$isSyncThreadAlive29();
        try {
            if (!(this.editPendingQ.size() == this.syncWaitQ.size())) {
    ...
    void openForWrite(int layoutVersion) throws IOException {
        ...
        internal$openForWrite31(layoutVersion);
        try {
            if (!(this.editPendingQ.size() == this.syncWaitQ.size())) {
    ...
    void logEdit(final FSEditLogOp op) {
        ...
        try {
            if (!(this.editPendingQ.size() == org.zlab.dinv.runtimechecker.Quant.size(op.rpcClientId))) {
                org.zlab.dinv.runtimechecker.Runtime.addViolation(462);
            }
        } catch (Exception e) {
        }
        internal$logEdit32(op);
        try {
            if (!(this.editPendingQ.size() == this.syncWaitQ.size())) {
    ...
    private void enqueueEdit(Edit edit) {
        ...
        try {
            if (!(this.editPendingQ.size() == org.zlab.dinv.runtimechecker.Quant.size(edit.op.rpcClientId))) {
                org.zlab.dinv.runtimechecker.Runtime.addViolation(468);
            }
        } catch (Exception e) {
        }
        internal$enqueueEdit34(edit);
        try {
            if (!(this.syncWaitQ.size() == org.zlab.dinv.runtimechecker.Quant.size(edit.op.rpcClientId))) {
    ...
    private Edit getEditInstance(FSEditLogOp op) {
        ...
        Edit returnValue;
        returnValue = internal$getEditInstance35(op);
        try {
            if (!(this.editPendingQ.size() == this.syncWaitQ.size())) {
