...
class FSEditLogAsync extends FSEditLog implements Runnable {
    ...
    private boolean internal$isSyncThreadAlive() {
        synchronized (syncThreadLock) {
            return syncThread != null && syncThread.isAlive();
        }
    }

    private void internal$startSyncThread() {
        synchronized (syncThreadLock) {
    ...
    void internal$openForWrite(int layoutVersion) throws IOException {
        try {
            startSyncThread();
    ...
    void internal$logEdit(final FSEditLogOp op) {
    ...
    private void internal$enqueueEdit(Edit edit) {
        if (LOG.isDebugEnabled()) {
    ...
    private Edit internal$getEditInstance(FSEditLogOp op) {
        final Edit edit;
    ...
    private static class SyncEdit extends Edit {
        ...
        public void internal$logSyncNotify(RuntimeException ex) {
            synchronized (lock) {
                done = true;
                syncEx = ex;
                lock.notifyAll();
        ...
        public void logSyncNotify(RuntimeException ex) {
            if (!(this.syncEx == ex)) {
                org.zlab.dinv.runtimechecker.Runtime.addViolation(2953);
            }
            internal$logSyncNotify(ex);
        }
    }
    ...
    private boolean isSyncThreadAlive() {
        if (!(this.editPendingQ.size() == this.syncWaitQ.size())) {
            org.zlab.dinv.runtimechecker.Runtime.addViolation(2954);
        }
        boolean returnValue;
        returnValue = internal$isSyncThreadAlive();
        if (!(this.editPendingQ.size() == this.syncWaitQ.size())) {
            org.zlab.dinv.runtimechecker.Runtime.addViolation(2955);
        }
        return returnValue;
    }
    ...
    private void startSyncThread() {
        if (!(this.editPendingQ.size() == this.syncWaitQ.size())) {
            org.zlab.dinv.runtimechecker.Runtime.addViolation(2956);
        }
        internal$startSyncThread();
        if (!(this.editPendingQ.size() == this.syncWaitQ.size())) {
            org.zlab.dinv.runtimechecker.Runtime.addViolation(2957);
        }
    }
    ...
    void openForWrite(int layoutVersion) throws IOException {
        if (!(this.editPendingQ.size() == this.syncWaitQ.size())) {
            org.zlab.dinv.runtimechecker.Runtime.addViolation(2958);
        }
        internal$openForWrite(layoutVersion);
        if (!(this.editPendingQ.size() == this.syncWaitQ.size())) {
            org.zlab.dinv.runtimechecker.Runtime.addViolation(2959);
        }
    }
    ...
    void logEdit(final FSEditLogOp op) {
        if (!(this.editPendingQ.size() == this.syncWaitQ.size())) {
            org.zlab.dinv.runtimechecker.Runtime.addViolation(2960);
        }
        if (!(this.editPendingQ.size() == org.zlab.dinv.runtimechecker.Quant.size(op.rpcClientId))) {
            org.zlab.dinv.runtimechecker.Runtime.addViolation(2961);
        }
        internal$logEdit(op);
        if (!(this.editPendingQ.size() == this.syncWaitQ.size())) {
            org.zlab.dinv.runtimechecker.Runtime.addViolation(2962);
        }
        if (!(this.editPendingQ.size() == org.zlab.dinv.runtimechecker.Quant.size(op.rpcClientId))) {
            org.zlab.dinv.runtimechecker.Runtime.addViolation(2963);
        }
    }
    ...
    public void logSync() {
        if (!(this.editPendingQ.size() == this.syncWaitQ.size())) {
            org.zlab.dinv.runtimechecker.Runtime.addViolation(2964);
        }
        internal$logSync();
        if (!(this.editPendingQ.size() == this.syncWaitQ.size())) {
            org.zlab.dinv.runtimechecker.Runtime.addViolation(2965);
        }
    }

    private void enqueueEdit(Edit edit) {
        if (!(this.editPendingQ.size() == this.syncWaitQ.size())) {
            org.zlab.dinv.runtimechecker.Runtime.addViolation(2966);
        }
        if (!(this.editPendingQ.size() == org.zlab.dinv.runtimechecker.Quant.size(edit.op.rpcClientId))) {
            org.zlab.dinv.runtimechecker.Runtime.addViolation(2967);
        }
        internal$enqueueEdit(edit);
        if (!(this.syncWaitQ.size() == org.zlab.dinv.runtimechecker.Quant.size(edit.op.rpcClientId))) {
            org.zlab.dinv.runtimechecker.Runtime.addViolation(2968);
        }
    }

    private Edit getEditInstance(FSEditLogOp op) {
        if (!(this.editPendingQ.size() == this.syncWaitQ.size())) {
            org.zlab.dinv.runtimechecker.Runtime.addViolation(2969);
        }
        if (!(this.editPendingQ.size() == org.zlab.dinv.runtimechecker.Quant.size(op.rpcClientId))) {
            org.zlab.dinv.runtimechecker.Runtime.addViolation(2970);
        }
        Edit returnValue;
        returnValue = internal$getEditInstance(op);
        if (!(this.editPendingQ.size() == this.syncWaitQ.size())) {
            org.zlab.dinv.runtimechecker.Runtime.addViolation(2971);
        }
        if (!(this.editPendingQ.size() == org.zlab.dinv.runtimechecker.Quant.size(op.rpcClientId))) {
            org.zlab.dinv.runtimechecker.Runtime.addViolation(2972);
        }
        return returnValue;
    }
}
