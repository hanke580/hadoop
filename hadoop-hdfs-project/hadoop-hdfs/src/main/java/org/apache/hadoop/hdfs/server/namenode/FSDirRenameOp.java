...
class FSDirRenameOp {
    ...
    static RenameResult renameToInt(FSDirectory fsd, FSPermissionChecker pc, final String src, final String dst, boolean logRetryCache) throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(srcIIP, 53);
        INodesInPath dstIIP = fsd.resolvePath(pc, dst, DirOp.CREATE_LINK);
        org.zlab.ocov.tracker.Runtime.update(dstIIP, 54);
    ...
    private static void verifyQuotaForRename(FSDirectory fsd, INodesInPath src, INodesInPath dst) throws QuotaExceededException {
        ...
        final QuotaCounts delta = src.getLastINode().computeQuotaUsage(bsps, dstParent.getStoragePolicyID(), false, Snapshot.CURRENT_STATE_ID);
    ...
    static INodesInPath renameForEditLog(FSDirectory fsd, String src, String dst, long timestamp) throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(srcIIP, 55);
        INodesInPath dstIIP = fsd.getINodesInPath(dst, DirOp.WRITE_LINK);
        org.zlab.ocov.tracker.Runtime.update(dstIIP, 56);
    ...
    static INodesInPath unprotectedRenameTo(FSDirectory fsd, final INodesInPath srcIIP, final INodesInPath dstIIP, long timestamp) throws IOException {
        ...
        RenameOperation tx = ((RenameOperation) org.zlab.ocov.tracker.Runtime.update(new RenameOperation(fsd, srcIIP, dstIIP), 57));
        boolean added = false;
    ...
    static RenameResult renameTo(FSDirectory fsd, FSPermissionChecker pc, String src, String dst, BlocksMapUpdateInfo collectedBlocks, boolean logRetryCache, Options.Rename... options) throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(srcIIP, 58);
        final INodesInPath dstIIP = fsd.resolvePath(pc, dst, DirOp.CREATE_LINK);
        org.zlab.ocov.tracker.Runtime.update(dstIIP, 59);
    ...
    static void renameForEditLog(FSDirectory fsd, String src, String dst, long timestamp, Options.Rename... options) throws IOException {
        BlocksMapUpdateInfo collectedBlocks = new BlocksMapUpdateInfo();
        final INodesInPath srcIIP = fsd.getINodesInPath(src, DirOp.WRITE_LINK);
        org.zlab.ocov.tracker.Runtime.update(srcIIP, 60);
        final INodesInPath dstIIP = fsd.getINodesInPath(dst, DirOp.WRITE_LINK);
        org.zlab.ocov.tracker.Runtime.update(dstIIP, 61);
    ...
    static RenameResult unprotectedRenameTo(FSDirectory fsd, final INodesInPath srcIIP, final INodesInPath dstIIP, long timestamp, BlocksMapUpdateInfo collectedBlocks, Options.Rename... options) throws IOException {
        ...
        verifyFsLimitsForRename(fsd, srcIIP, dstIIP);
        verifyQuotaForRename(fsd, srcIIP, dstIIP);
        RenameOperation tx = ((RenameOperation) org.zlab.ocov.tracker.Runtime.update(new RenameOperation(fsd, srcIIP, dstIIP), 62));
        ...
        try {
            if (dstInode != null) {
                ...
                org.zlab.ocov.tracker.Runtime.update(tx, 63);
                if (removedNum != -1) {
                    undoRemoveDst = true;
                }
            }
            ...
            if (renamedIIP != null) {
                ...
                if (undoRemoveDst) {
                    undoRemoveDst = false;
                    if ((org.zlab.ocov.tracker.Runtime.updateBranch(removedNum, 0, ">", 18))) {
            ...
            if (undoRemoveSrc) {
                tx.restoreSource();
                org.zlab.ocov.tracker.Runtime.update(tx, 64);
            }
            if (undoRemoveDst) {
