...
class FSDirRenameOp {
    ...
    @Deprecated
    static RenameResult renameToInt(FSDirectory fsd, FSPermissionChecker pc, final String src, final String dst, boolean logRetryCache) throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(srcIIP, 104);
        INodesInPath dstIIP = fsd.resolvePath(pc, dst, DirOp.CREATE_LINK);
        org.zlab.ocov.tracker.Runtime.update(dstIIP, 105);
    ...
    private static void verifyQuotaForRename(FSDirectory fsd, INodesInPath src, INodesInPath dst) throws QuotaExceededException {
        ...
        final byte storagePolicyID = isSrcSetSp ? src.getLastINode().getLocalStoragePolicyID() : dstParent.getStoragePolicyID();
        final QuotaCounts delta = src.getLastINode().computeQuotaUsage(bsps, storagePolicyID, false, Snapshot.CURRENT_STATE_ID);
    ...
    static INodesInPath renameForEditLog(FSDirectory fsd, String src, String dst, long timestamp) throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(srcIIP, 106);
        INodesInPath dstIIP = fsd.getINodesInPath(dst, DirOp.WRITE_LINK);
        org.zlab.ocov.tracker.Runtime.update(dstIIP, 107);
     ...
     */
    @Deprecated
    static INodesInPath unprotectedRenameTo(FSDirectory fsd, final INodesInPath srcIIP, final INodesInPath dstIIP, long timestamp) throws IOException {
        ...
        RenameOperation tx = ((RenameOperation) org.zlab.ocov.tracker.Runtime.update(new RenameOperation(fsd, srcIIP, dstIIP), 108));
        boolean added = false;
     ...
     */
    static RenameResult renameToInt(FSDirectory fsd, FSPermissionChecker pc, final String srcArg, final String dstArg, boolean logRetryCache, Options.Rename... options) throws IOException {
        String src = srcArg;
        String dst = dstArg;
        if (NameNode.stateChangeLog.isDebugEnabled()) {
            NameNode.stateChangeLog.debug("DIR* NameSystem.renameTo: with options={} {} to {}", Arrays.toString(options), src, dst);
        }
     ...
     */
    static RenameResult renameTo(FSDirectory fsd, FSPermissionChecker pc, String src, String dst, BlocksMapUpdateInfo collectedBlocks, boolean logRetryCache, Options.Rename... options) throws IOException {
        final INodesInPath srcIIP = fsd.resolvePath(pc, src, DirOp.WRITE_LINK);
        org.zlab.ocov.tracker.Runtime.update(srcIIP, 109);
        final INodesInPath dstIIP = fsd.resolvePath(pc, dst, DirOp.CREATE_LINK);
        org.zlab.ocov.tracker.Runtime.update(dstIIP, 110);
    ...
    static void renameForEditLog(FSDirectory fsd, String src, String dst, long timestamp, Options.Rename... options) throws IOException {
        BlocksMapUpdateInfo collectedBlocks = ((BlocksMapUpdateInfo) org.zlab.ocov.tracker.Runtime.update(new BlocksMapUpdateInfo(), 111));
        final INodesInPath srcIIP = fsd.getINodesInPath(src, DirOp.WRITE_LINK);
        org.zlab.ocov.tracker.Runtime.update(srcIIP, 112);
        final INodesInPath dstIIP = fsd.getINodesInPath(dst, DirOp.WRITE_LINK);
        org.zlab.ocov.tracker.Runtime.update(dstIIP, 113);
    ...
    static RenameResult unprotectedRenameTo(FSDirectory fsd, final INodesInPath srcIIP, final INodesInPath dstIIP, long timestamp, BlocksMapUpdateInfo collectedBlocks, Options.Rename... options) throws IOException {
        ...
        verifyFsLimitsForRename(fsd, srcIIP, dstIIP);
        verifyQuotaForRename(fsd, srcIIP, dstIIP);
        RenameOperation tx = ((RenameOperation) org.zlab.ocov.tracker.Runtime.update(new RenameOperation(fsd, srcIIP, dstIIP), 114));
        ...
        try {
            if (dstInode != null) {
                ...
                org.zlab.ocov.tracker.Runtime.update(tx, 115);
                if (removedNum != -1) {
                    undoRemoveDst = true;
                }
            }
            ...
            if (renamedIIP != null) {
                ...
                if (undoRemoveDst) {
                    undoRemoveDst = false;
                    if ((org.zlab.ocov.tracker.Runtime.updateBranch(removedNum, 0, ">", 25))) {
            ...
            if (undoRemoveSrc) {
                tx.restoreSource();
                org.zlab.ocov.tracker.Runtime.update(tx, 116);
            }
            if (undoRemoveDst) {
    ...
    private static void checkUnderSameSnapshottableRoot(FSDirectory fsd, INodesInPath srcIIP, INodesInPath dstIIP) throws IOException {
        ...
        if (snapshotManager.isSnapshotDeletionOrdered() && fsd.getFSNamesystem().isSnapshotTrashRootEnabled()) {
            INodeDirectory srcRoot = snapshotManager.getSnapshottableAncestorDir(srcIIP);
            INodeDirectory dstRoot = snapshotManager.getSnapshottableAncestorDir(dstIIP);
    ...
    static class RenameResult {
        ...
        RenameResult(boolean success, FileStatus auditStat, boolean filesDeleted, BlocksMapUpdateInfo collectedBlocks) {
