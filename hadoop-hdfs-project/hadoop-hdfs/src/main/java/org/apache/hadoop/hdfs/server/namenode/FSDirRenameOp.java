...
class FSDirRenameOp {
    ...
    @Deprecated
    static RenameResult renameToInt(FSDirectory fsd, final String src, final String dst, boolean logRetryCache) throws IOException {
    ...
    static INodesInPath unprotectedRenameTo(FSDirectory fsd, final INodesInPath srcIIP, final INodesInPath dstIIP, long timestamp) throws IOException {
        ...
        RenameOperation tx = ((RenameOperation) org.zlab.ocov.tracker.Runtime.update(new RenameOperation(fsd, srcIIP, dstIIP), 53, fsd, srcIIP, dstIIP, timestamp));
        boolean added = false;
    ...
    static RenameResult unprotectedRenameTo(FSDirectory fsd, final INodesInPath srcIIP, final INodesInPath dstIIP, long timestamp, BlocksMapUpdateInfo collectedBlocks, Options.Rename... options) throws IOException {
        ...
        verifyFsLimitsForRename(fsd, srcIIP, dstIIP);
        verifyQuotaForRename(fsd, srcIIP, dstIIP);
        RenameOperation tx = ((RenameOperation) org.zlab.ocov.tracker.Runtime.update(new RenameOperation(fsd, srcIIP, dstIIP), 52, fsd, srcIIP, dstIIP, timestamp, collectedBlocks, options));
        ...
        try {
            ...
            if (renamedIIP != null) {
                ...
                if (undoRemoveDst) {
                    undoRemoveDst = false;
                    if ((org.zlab.ocov.tracker.Runtime.updateBranch(removedNum, 0, ">", 23))) {
    ...
    private static void validateRenameSource(FSDirectory fsd, INodesInPath srcIIP) throws IOException {
        String error;
        final INode srcInode = srcIIP.getLastINode();
        // validate source
        if (srcInode == null) {
    ...
    private static class RenameOperation {
        ...
        RenameOperation(FSDirectory fsd, INodesInPath srcIIP, INodesInPath dstIIP) throws QuotaExceededException {
            this.fsd = fsd;
            this.srcIIP = srcIIP;
            this.dstIIP = dstIIP;
