...
public class BlockScanner {
    ...
    static class Conf {
        ...
        Conf(Configuration conf) {
            ...
            this.skipRecentAccessed = conf.getBoolean(DFS_BLOCK_SCANNER_SKIP_RECENT_ACCESSED, DFS_BLOCK_SCANNER_SKIP_RECENT_ACCESSED_DEFAULT);
            if (allowUnitTestSettings) {
    ...
    public BlockScanner(DataNode datanode, Configuration conf) {
        ...
        setJoinVolumeScannersTimeOutMs(conf.getLong(DFS_BLOCK_SCANNER_VOLUME_JOIN_TIMEOUT_MSEC_KEY, DFS_BLOCK_SCANNER_VOLUME_JOIN_TIMEOUT_MSEC_DEFAULT));
    ...
    public synchronized void removeAllVolumeScanners() {
        ...
        for (Entry<String, VolumeScanner> entry : scanners.entrySet()) {
            Uninterruptibles.joinUninterruptibly(entry.getValue(), getJoinVolumeScannersTimeOutMs(), TimeUnit.MILLISECONDS);
        }
        scanners.clear();
    }
    ...
    synchronized void markSuspectBlock(String storageId, ExtendedBlock block) {
        if (!isEnabled()) {
            LOG.debug("Not scanning suspicious block {} on {}, because the block " + "scanner is disabled.", block, storageId);
            return;
        }
