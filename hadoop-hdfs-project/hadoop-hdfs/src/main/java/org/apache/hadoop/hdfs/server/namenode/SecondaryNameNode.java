...
public class SecondaryNameNode implements Runnable, SecondaryNameNodeInfoMXBean {
    ...
    private void initialize(final Configuration conf, CommandLineOpts commandLineOpts) throws IOException {
        ...
        namesystem = ((FSNamesystem) org.zlab.ocov.tracker.Runtime.update(new FSNamesystem(conf, checkpointImage, true), 4));
    ...
    void doMerge(CheckpointSignature sig, RemoteEditLogManifest manifest, boolean loadImage, FSImage dstImage, FSNamesystem dstNamesystem) throws IOException {
        ...
        if (loadImage) {
            ...
            try {
                ...
                org.zlab.ocov.tracker.Runtime.update(dstImage, 5);
            } finally {
