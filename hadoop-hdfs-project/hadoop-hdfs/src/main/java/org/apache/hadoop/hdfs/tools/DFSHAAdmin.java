...
public class DFSHAAdmin extends HAAdmin {
    ...
    private final static Map<String, UsageInfo> USAGE_DFS_ONLY = ImmutableMap.<String, UsageInfo>builder().put("-transitionToObserver", new UsageInfo("<serviceId>", "Transitions the service into Observer state")).put("-failover", new UsageInfo("[--" + FORCEFENCE + "] [--" + FORCEACTIVE + "] " + "<serviceId> <serviceId>", "Failover from the first service to the second.\n" + "Unconditionally fence services if the --" + FORCEFENCE + " option is used.\n" + "Try to failover to the target service " + "even if it is not ready if the " + "--" + FORCEACTIVE + " option is used.")).build();

    private final static Map<String, UsageInfo> USAGE_DFS_MERGED = ImmutableSortedMap.<String, UsageInfo>naturalOrder().putAll(USAGE).putAll(USAGE_DFS_ONLY).build();
    ...
    protected HAServiceTarget resolveTarget(String nnId) {
        ...
        return ((NNHAServiceTarget) org.zlab.ocov.tracker.Runtime.update(new NNHAServiceTarget(conf, nameserviceId, nnId), 5));
    }
    ...
    */
    ...
    protected Collection<String> getTargetIds(String namenodeToActivate) {
        return DFSUtilClient.getNameNodeIds(getConf(), (nameserviceId != null) ? nameserviceId : DFSUtil.getNamenodeNameServiceId(getConf()));
    }
    ...
    private int transitionToObserver(final CommandLine cmd) throws IOException {
    ...
    private int failover(CommandLine cmd) throws IOException, ServiceFailedException {
        ...
        fromNode.setTransitionTargetHAStatus(HAServiceProtocol.HAServiceState.STANDBY);
        toNode.setTransitionTargetHAStatus(HAServiceProtocol.HAServiceState.ACTIVE);
