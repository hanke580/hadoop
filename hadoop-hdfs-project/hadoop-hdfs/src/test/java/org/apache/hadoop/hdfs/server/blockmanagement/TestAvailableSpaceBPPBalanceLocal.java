...
public class TestAvailableSpaceBPPBalanceLocal {
    ...
    public static void setupCluster() throws Exception {
        ...
        conf.setBoolean(DFS_NAMENODE_AVAILABLE_SPACE_BLOCK_PLACEMENT_POLICY_BALANCE_LOCAL_NODE_KEY, true);
    ...
    protected static void setupDataNodeCapacity() {
        for (int i = 0; i < NODES_PER_RACK * NUM_RACKS; i++) {
            if ((i % 2) == 0) {
                // remaining 100%
                updateHeartbeatWithUsage(dataNodes[i], 4 * HdfsServerConstants.MIN_BLOCKS_FOR_WRITE * BLOCK_SIZE, 0L, 4 * HdfsServerConstants.MIN_BLOCKS_FOR_WRITE * BLOCK_SIZE, 0L, 0L, 0L, 0, 0);
            } else {
                // remaining 25%
                updateHeartbeatWithUsage(dataNodes[i], 4 * HdfsServerConstants.MIN_BLOCKS_FOR_WRITE * BLOCK_SIZE, 3 * HdfsServerConstants.MIN_BLOCKS_FOR_WRITE * BLOCK_SIZE, HdfsServerConstants.MIN_BLOCKS_FOR_WRITE * BLOCK_SIZE, 0L, 0L, 0L, 0, 0);
            }
        }
    }
    ...
    public void testChooseLocalNode() {
        ...
        for (int i = 0; i < CHOOSE_TIMES; i++) {
            DatanodeStorageInfo[] targets = namenode.getNamesystem().getBlockManager().getBlockPlacementPolicy().chooseTarget(FILE, 1, localNode, new ArrayList<DatanodeStorageInfo>(), false, null, BLOCK_SIZE, TestBlockStoragePolicy.DEFAULT_STORAGE_POLICY, null);
