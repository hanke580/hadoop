...
public class NameNodeAdapter {
    ...
    public static HdfsFileStatus getFileInfo(NameNode namenode, String src, boolean resolveLink, boolean needLocation, boolean needBlockToken) throws AccessControlException, UnresolvedLinkException, StandbyException, IOException {
        final FSPermissionChecker pc = namenode.getNamesystem().getPermissionChecker();
        ...
        try {
            return FSDirStatAndListingOp.getFileInfo(namenode.getNamesystem().getFSDirectory(), pc, src, resolveLink, needLocation, needBlockToken);
        } finally {
    ...
    public static DelegationTokenSecretManager getDtSecretManager(final FSNamesystem ns) {
    ...
    public static HeartbeatResponse sendHeartBeat(DatanodeRegistration nodeReg, DatanodeDescriptor dd, FSNamesystem namesystem) throws IOException {
        return namesystem.handleHeartbeat(nodeReg, BlockManagerTestUtil.getStorageReportsForDatanode(dd), dd.getCacheCapacity(), dd.getCacheRemaining(), 0, 0, 0, null, true, SlowPeerReports.EMPTY_REPORT, SlowDiskReports.EMPTY_REPORT);
    }

    public static boolean setReplication(final FSNamesystem ns, final String src, final short replication) throws IOException {
    ...
    public static Lease getLeaseForPath(NameNode nn, String path) {
        ...
        try {
            ...
            // unresolvable path, ex. parent dir is a file
            return null;
        }
     ...
     */
    public static DatanodeDescriptor getDatanode(final FSNamesystem ns, DatanodeID id) throws IOException {
        ns.readLock();
        try {
    ...
    public static long getGenerationStamp(final FSNamesystem fsn) throws IOException {
    ...
    public static long getImpendingGenerationStamp(final FSNamesystem fsn) {
        return fsn.getBlockManager().getBlockIdManager().getImpendingGenerationStamp();
    }

    public static BlockInfo addBlockNoJournal(final FSNamesystem fsn, final String src, final DatanodeStorageInfo[] targets) throws IOException {
        fsn.writeLock();
        try {
            ...
            FSDirWriteFileOp.saveAllocatedBlock(fsn, src, inodesInPath, newBlock, targets, BlockType.CONTIGUOUS);
    ...
    public static void persistBlocks(final FSNamesystem fsn, final String src, final INodeFile file) throws IOException {
        fsn.writeLock();
        try {
    ...
    public static BlockInfo getStoredBlock(final FSNamesystem fsn, final Block b) {
    ...
    public static FSNamesystem spyOnNamesystem(NameNode nn) {
        ...
        try {
            FieldUtils.writeDeclaredField((NameNodeRpcServer) nn.getRpcServer(), "namesystem", fsnSpy, true);
            FieldUtils.writeDeclaredField(fsnSpy.getBlockManager(), "namesystem", fsnSpy, true);
            FieldUtils.writeDeclaredField(fsnSpy.getLeaseManager(), "fsnamesystem", fsnSpy, true);
            FieldUtils.writeDeclaredField(fsnSpy.getBlockManager().getDatanodeManager(), "namesystem", fsnSpy, true);
            FieldUtils.writeDeclaredField(BlockManagerTestUtil.getHeartbeatManager(fsnSpy.getBlockManager()), "namesystem", fsnSpy, true);
    ...
    public static FSEditLogOp createMkdirOp(String path) {
        MkdirOp op = MkdirOp.getInstance(new FSEditLogOp.OpInstanceCache()).setPath(path).setTimestamp(0).setPermissionStatus(new PermissionStatus("testuser", "testgroup", FsPermission.getDefault()));
        return op;
    }
