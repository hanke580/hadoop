...
public class UpgradeUtilities {
     ...
     */
    public static long checksumContents(NodeType nodeType, File dir, boolean recursive) throws IOException {
        CRC32 checksum = new CRC32();
        checksumContentsHelper(nodeType, dir, checksum, recursive);
        return checksum.getValue();
    }

    public static void checksumContentsHelper(NodeType nodeType, File dir, CRC32 checksum, boolean recursive) throws IOException {
        if (!dir.isDirectory()) {
        ...
        for (int i = 0; i < list.length; i++) {
            ...
            if (nodeType == DATA_NODE && (list[i].getName().equals("VERSION") || list[i].getName().equals("dfsUsed") || list[i].getName().equals("replicas"))) {
                continue;
            }
     ...
     * @param bpid Block pool Id
     */
    public static void createDataNodeVersionFile(File[] parent, StorageInfo version, String bpid, Configuration conf) throws IOException {
     ...
     */
    public static void createDataNodeVersionFile(File[] parent, StorageInfo version, String bpid, String bpidToWrite, Configuration conf) throws IOException {
