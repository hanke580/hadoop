...
abstract public class ReadStripedFileWithDecodingHelper {
    ...
    protected static final ErasureCodingPolicy EC_POLICY = StripedFileTestUtil.getDefaultECPolicy();

    protected static final short NUM_DATA_UNITS = (short) EC_POLICY.getNumDataUnits();

    protected static final short NUM_PARITY_UNITS = (short) EC_POLICY.getNumParityUnits();
    ...
    protected static final int[] FILE_LENGTHS = { BLOCK_GROUP_SIZE - 123, BLOCK_GROUP_SIZE + 123 };
    ...
    public static MiniDFSCluster initializeCluster() throws IOException {
        ...
        MiniDFSCluster myCluster = new MiniDFSCluster.Builder(conf).numDataNodes(NUM_DATANODES).build();
    ...
    public static Collection<Object[]> getParameters() {
        ...
        for (int fileLength : FILE_LENGTHS) {
            for (int dataDelNum = 1; dataDelNum <= NUM_PARITY_UNITS; dataDelNum++) {
                for (int parityDelNum = 0; (dataDelNum + parityDelNum) <= NUM_PARITY_UNITS; parityDelNum++) {
    ...
    public static void verifyRead(DistributedFileSystem dfs, Path testPath, int length, byte[] expected) throws IOException {
        ...
        StripedFileTestUtil.verifyStatefulRead(dfs, testPath, length, expected, buffer);
        LOG.info("verifyRead verifyStatefulRead2 on path {}", testPath);
        StripedFileTestUtil.verifyStatefulRead(dfs, testPath, length, expected, ByteBuffer.allocate(length + 100));
        LOG.info("verifyRead verifySeek on path {}", testPath);
        StripedFileTestUtil.verifySeek(dfs, testPath, length, EC_POLICY, BLOCK_GROUP_SIZE);
    }
    ...
    public static void testReadWithDNFailure(MiniDFSCluster cluster, DistributedFileSystem dfs, int fileLength, int dnFailureNum) throws Exception {
        ...
        BlockLocation[] locs = dfs.getFileBlockLocations(testPath, CELL_SIZE * 5, CELL_SIZE);
    ...
    public static void testReadWithBlockCorrupted(MiniDFSCluster cluster, DistributedFileSystem dfs, String src, int fileNumBytes, int dataBlkDelNum, int parityBlkDelNum, boolean deleteBlockFile) throws IOException {
        ...
        Assert.assertTrue("The sum of dataBlkDelNum and parityBlkDelNum " + "should be between 1 ~ " + NUM_PARITY_UNITS, recoverBlkNum <= NUM_PARITY_UNITS);
        ...
        corruptBlocks(cluster, dfs, srcPath, dataBlkDelNum, parityBlkDelNum, deleteBlockFile);
    ...
    public static LocatedBlocks getLocatedBlocks(DistributedFileSystem dfs, Path filePath) throws IOException {
        return dfs.getClient().getLocatedBlocks(filePath.toString(), 0, Long.MAX_VALUE);
    }
}
