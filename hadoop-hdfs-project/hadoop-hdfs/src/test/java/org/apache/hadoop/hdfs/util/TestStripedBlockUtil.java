...
public class TestStripedBlockUtil {
    ...
    private byte[][] createInternalBlkBuffers(int bgSize) {
        ...
        for (int i = 0; i < dataBlocks + parityBlocks; i++) {
            int bufSize = (int) getInternalBlockLength(bgSize, cellSize, dataBlocks, i);
    ...
    public void testLocatedStripedBlockType() {
        LocatedStripedBlock lsb = new LocatedStripedBlock(null, null, null, null, null, 0, false, null);
    ...
    private void verifyInternalBlocks(int numBytesInGroup, int[] expected) {
        for (int i = 1; i < groupSize; i++) {
            assertEquals(expected[i], getInternalBlockLength(numBytesInGroup, cellSize, dataBlocks, i));
        }
    }
