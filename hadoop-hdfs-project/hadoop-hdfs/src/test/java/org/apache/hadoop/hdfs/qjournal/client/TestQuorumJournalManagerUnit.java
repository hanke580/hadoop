...
public class TestQuorumJournalManagerUnit {
    ...
    public void testSetOutputBufferCapacityTooLarge() throws Exception {
        qjm.setOutputBufferCapacity(CommonConfigurationKeys.IPC_MAXIMUM_DATA_LENGTH_DEFAULT + 1);
    }
    ...
    public void testFSEditLogAutoSyncToQuorumStream() throws Exception {
        ...
        // sets up to the mocks for startLogSegment
        createLogSegment();
        for (int logIdx = 0; logIdx < 3; logIdx++) {
            futureReturns(null).when(spyLoggers.get(logIdx)).sendEdits(anyLong(), anyLong(), anyInt(), any());
        }
        PermissionStatus permStat = PermissionStatus.createImmutable("user", "group", FsPermission.getDefault());
        INode fakeInode = FSImageTestUtil.createEmptyInodeFile(1, "foo", permStat, 1, 1, (short) 1, 1);
        ...
        FSEditLog editLog = FSImageTestUtil.createEditLogWithJournalManager(conf, mockStorage, URI.create(mockQjmEdits), qjm);
        editLog.initJournalsForWrite();
        editLog.startLogSegment(1, false, NameNodeLayoutVersion.CURRENT_LAYOUT_VERSION);
        ...
        for (int i = 0; i < 3; i++) {
            Mockito.verify(spyLoggers.get(i), times(1)).sendEdits(eq(1L), eq(1L), anyInt(), any());
        }
    }
    ...
    public void testReadRpcInputStreams() throws Exception {
        for (int jn = 0; jn < 3; jn++) {
            futureReturns(getJournaledEditsReponse(1, 3)).when(spyLoggers.get(jn)).getJournaledEdits(1, QuorumJournalManager.QJM_RPC_MAX_TXNS_DEFAULT);
        }
    ...
    public void testReadRpcMismatchedInputStreams() throws Exception {
        for (int jn = 0; jn < 3; jn++) {
            futureReturns(getJournaledEditsReponse(1, jn + 1)).when(spyLoggers.get(jn)).getJournaledEdits(1, QuorumJournalManager.QJM_RPC_MAX_TXNS_DEFAULT);
        }
        List<EditLogInputStream> streams = Lists.newArrayList();
        qjm.selectInputStreams(streams, 1, true, true);
        assertEquals(1, streams.size());
    ...
    public void testReadRpcInputStreamsOneSlow() throws Exception {
        ...
        Mockito.doReturn(SettableFuture.create()).when(spyLoggers.get(2)).getJournaledEdits(1, QuorumJournalManager.QJM_RPC_MAX_TXNS_DEFAULT);
        List<EditLogInputStream> streams = Lists.newArrayList();
        qjm.selectInputStreams(streams, 1, true, true);
        assertEquals(1, streams.size());
    ...
    public void testReadRpcInputStreamsOneException() throws Exception {
        ...
        futureThrows(new IOException()).when(spyLoggers.get(2)).getJournaledEdits(1, QuorumJournalManager.QJM_RPC_MAX_TXNS_DEFAULT);
        List<EditLogInputStream> streams = Lists.newArrayList();
        qjm.selectInputStreams(streams, 1, true, true);
        assertEquals(1, streams.size());
        verifyEdits(streams, 1, 1);
    ...
    public void testReadRpcInputStreamsNoNewEdits() throws Exception {
        for (int jn = 0; jn < 3; jn++) {
            futureReturns(GetJournaledEditsResponseProto.newBuilder().setTxnCount(0).setEditLog(ByteString.EMPTY).build()).when(spyLoggers.get(jn)).getJournaledEdits(1, QuorumJournalManager.QJM_RPC_MAX_TXNS_DEFAULT);
        }
        List<EditLogInputStream> streams = Lists.newArrayList();
        qjm.selectInputStreams(streams, 1, true, true);
    ...
    private GetJournaledEditsResponseProto getJournaledEditsReponse(int startTxn, int numTxns) throws Exception {
        ByteArrayOutputStream byteStream = new ByteArrayOutputStream();
        EditLogFileOutputStream.writeHeader(NameNodeLayoutVersion.CURRENT_LAYOUT_VERSION, new DataOutputStream(byteStream));
        byteStream.write(createTxnData(startTxn, numTxns));
        return GetJournaledEditsResponseProto.newBuilder().setTxnCount(numTxns).setEditLog(ByteString.copyFrom(byteStream.toByteArray())).build();
    }
