...
public class TestNNStorageRetentionManager {
    ...
    public void testRetainExtraLogsLimitedSegments() throws IOException {
        ...
        tc.addLog("/foo2/current/" + getInProgressEditsFileName(101) + ".trash", true);
        ...
        tc.addLog("/foo2/current/" + getInProgressEditsFileName(276) + ".trash", false);
   ...
   */
    ...
    private void runTest(TestCaseDescription tc) throws IOException {
        ...
        ArgumentCaptor<EditLogFile> staleLogsCaptor = ArgumentCaptor.forClass(EditLogFile.class);
        ...
        Mockito.verify(mockPurger, Mockito.atLeast(0)).markStale(staleLogsCaptor.capture());
        ...
        Assert.assertEquals("Image file check.", Joiner.on(",").join(filesToPaths(tc.expectedPurgedImages)), Joiner.on(",").join(capturedPaths));
        capturedPaths.clear();
        ...
        Assert.assertEquals("Check old edits are removed.", Joiner.on(",").join(filesToPaths(tc.expectedPurgedLogs)), Joiner.on(",").join(capturedPaths));
        capturedPaths.clear();
        // Check in progress edits to keep are marked as stale
        for (EditLogFile captured : staleLogsCaptor.getAllValues()) {
            capturedPaths.add(fileToPath(captured.getFile()));
        }
        Assert.assertEquals("Check unnecessary but kept edits are marked stale", Joiner.on(",").join(filesToPaths(tc.expectedStaleLogs)), Joiner.on(",").join(capturedPaths));
    }
    ...
    private class TestCaseDescription {
        ...
        private File findLastInProgressEdit(FakeRoot root) {
            Pattern p = Pattern.compile(NameNodeFile.EDITS_INPROGRESS.getName() + "_(\\d+)");
            ToLongFunction<File> fileNameToTxId = f -> {
                Matcher m = p.matcher(f.getName());
                return m.matches() ? Long.parseLong(m.group(1)) : HdfsServerConstants.INVALID_TXID;
            };
        ...
        public FSEditLog mockEditLog(StoragePurger purger) throws IOException {
            ...
            for (FakeRoot root : dirRoots.values()) {
                if (!root.type.isOfType(NameNodeDirType.EDITS))
                    continue;
