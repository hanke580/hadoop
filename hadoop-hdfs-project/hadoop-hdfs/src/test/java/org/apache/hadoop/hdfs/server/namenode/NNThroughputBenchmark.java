...
public class NNThroughputBenchmark implements Tool {
    ...
    static void setNameNodeLoggingLevel(Level logLevel) {
        ...
        GenericTestUtils.setLogLevel(LogManager.getLogger(NetworkTopology.class.getName()), logLevel);
        GenericTestUtils.setLogLevel(LogManager.getLogger(Groups.class.getName()), logLevel);
    }
    ...
    abstract class OperationStatsBase {
        ...
        protected int numOpsRequired = 0;
        ...
        protected int ugcRefreshCount = 0;
        ...
        private boolean isInProgress() {
            for (StatsDaemon d : daemons) if (d.isInProgress())
                return true;
            return false;
        }
    ...
    class CreateFileStats extends OperationStatsBase {
        ...
        void generateInputs(int[] opsPerThread) throws IOException {
            ...
            for (int idx = 0; idx < numThreads; idx++) {
                ...
                for (int jdx = 0; jdx < threadOps; jdx++) fileNames[idx][jdx] = nameGenerator.getNextFileName("ThroughputBench");
            }
        }

        void dummyActionNoSynch(int daemonId, int fileIdx) {
            for (int i = 0; i < 2000; i++) fileNames[daemonId][fileIdx].contains("" + i);
        }
        ...
        long executeOp(int daemonId, int inputIdx, String clientName) throws IOException {
            ...
            clientProto.create(fileNames[daemonId][inputIdx], FsPermission.getDefault(), clientName, new EnumSetWritable<CreateFlag>(EnumSet.of(CreateFlag.CREATE, CreateFlag.OVERWRITE)), true, replication, BLOCK_SIZE, CryptoProtocolVersion.supported(), null, null);
            long end = Time.now();
    ...
    class MkdirsStats extends OperationStatsBase {
        ...
        void generateInputs(int[] opsPerThread) throws IOException {
            ...
            for (int idx = 0; idx < numThreads; idx++) {
                ...
                for (int jdx = 0; jdx < threadOps; jdx++) dirPaths[idx][jdx] = nameGenerator.getNextFileName("ThroughputBench");
            }
        }
        ...
        long executeOp(int daemonId, int inputIdx, String clientName) throws IOException {
            long start = Time.now();
            clientProto.mkdirs(dirPaths[daemonId][inputIdx], FsPermission.getDefault(), true);
            long end = Time.now();
            return end - start;
        }
    ...
    class FileStatusStats extends OpenFileStats {
        ...
        static final String OP_FILE_STATUS_USAGE = "-op " + OP_FILE_STATUS_NAME + OP_USAGE_ARGS;
    ...
    class RenameFileStats extends OpenFileStats {
        ...
        long executeOp(int daemonId, int inputIdx, String ignore) throws IOException {
            long start = Time.now();
            clientProto.rename(fileNames[daemonId][inputIdx], destNames[daemonId][inputIdx]);
            long end = Time.now();
            return end - start;
        }
    }
    ...
    private static class TinyDatanode implements Comparable<String> {
        ...
        void sendHeartbeat() throws IOException {
            ...
            DatanodeCommand[] cmds = dataNodeProto.sendHeartbeat(dnRegistration, rep, 0L, 0L, 0, 0, 0, null, true, SlowPeerReports.EMPTY_REPORT, SlowDiskReports.EMPTY_REPORT).getCommands();
            if (cmds != null) {
        ...
        int replicateBlocks() throws IOException {
            // register datanode
            StorageReport[] rep = { new StorageReport(storage, false, DF_CAPACITY, DF_USED, DF_CAPACITY - DF_USED, DF_USED, 0) };
            DatanodeCommand[] cmds = dataNodeProto.sendHeartbeat(dnRegistration, rep, 0L, 0L, 0, 0, 0, null, true, SlowPeerReports.EMPTY_REPORT, SlowDiskReports.EMPTY_REPORT).getCommands();
            if (cmds != null) {
                for (DatanodeCommand cmd : cmds) {
        ...
        private int transferBlocks(Block[] blocks, DatanodeInfo[][] xferTargets, String[][] targetStorageIDs) throws IOException {
            for (int i = 0; i < blocks.length; i++) {
                ...
                for (int t = 0; t < blockTargets.length; t++) {
                    ...
                    StorageReceivedDeletedBlocks[] report = { new StorageReceivedDeletedBlocks(new DatanodeStorage(targetStorageID), rdBlocks) };
    ...
    class BlockReportStats extends OperationStatsBase {
        ...
        private ExtendedBlock addBlocks(String fileName, String clientName) throws IOException {
            ...
            for (int jdx = 0; jdx < blocksPerFile; jdx++) {
                ...
                for (DatanodeInfo dnInfo : loc.getLocations()) {
                    ...
                    StorageReceivedDeletedBlocks[] report = { new StorageReceivedDeletedBlocks(new DatanodeStorage(datanodes[dnIdx].storage.getStorageID()), rdBlocks) };
        ...
        long executeOp(int daemonId, int inputIdx, String ignore) throws IOException {
            ...
            long start = Time.now();
            StorageBlockReport[] report = { new StorageBlockReport(dn.storage, dn.getBlockReportList()) };
            dataNodeProto.blockReport(dn.dnRegistration, bpid, report, new BlockReportContext(1, 0, System.nanoTime(), 0L, true));
            long end = Time.now();
            return end - start;
        }
    ...
    class ReplicationStats extends OperationStatsBase {
        ...
        ReplicationStats(List<String> args) {
            ...
            String[] blkReportArgs = { "-op", "blockReport", "-datanodes", String.valueOf(numDatanodes), "-blocksPerReport", String.valueOf(totalBlocks * replication / numDatanodes), "-blocksPerFile", String.valueOf(numDatanodes) };
    ...
    public int run(String[] aArgs) throws Exception {
        ...
        try {
            ...
            if (nnUri.getScheme() == null || nnUri.getScheme().equals("file")) {
                ...
                dataNodeProto = new DatanodeProtocolClientSideTranslatorPB(nnAddr, config);
                refreshUserMappingsProto = DFSTestUtil.getRefreshUserMappingsProtocolProxy(config, nnAddr);
                getBlockPoolId(dfs);
            }
            // run each benchmark
