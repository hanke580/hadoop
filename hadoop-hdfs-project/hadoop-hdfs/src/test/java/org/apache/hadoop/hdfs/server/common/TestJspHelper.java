...
public class TestJspHelper {
    ...
    public void testGetUgiDuringStartup() throws Exception {
        ...
        Mockito.doCallRealMethod().when(mockNN).verifyToken(Mockito.any(), Mockito.any());
        when(context.getAttribute("name.node")).thenReturn(mockNN);
        LambdaTestUtils.intercept(RetriableException.class, "Namenode is in startup mode", () -> JspHelper.getUGI(context, request, conf));
    }
