...
public class WebHdfsTestUtil {
    ...
    public static HttpURLConnection openConnection(URL url, Configuration conf) throws IOException {
        URLConnectionFactory connectionFactory = URLConnectionFactory.newDefaultURLConnectionFactory(60000, 60000, conf);
        return (HttpURLConnection) connectionFactory.openConnection(url);
    }

    public static int sendRequest(final HttpURLConnection conn) throws IOException {
        conn.connect();
        return conn.getResponseCode();
    }

    public static Map<?, ?> getAndParseResponse(final HttpURLConnection conn) throws IOException {
        return WebHdfsFileSystem.jsonParse(conn, false);
    }

    public static Token<DelegationTokenIdentifier> convertJsonToDelegationToken(Map<?, ?> json) throws IOException {
