...
public class SimulatedFSDataset implements FsDatasetSpi<FsVolumeSpi> {
    ...
    // 1 terabyte
    public static final long DEFAULT_CAPACITY = 2L << 40;
    ...
    static {
        DataChecksum checksum = DataChecksum.newDataChecksum(DataChecksum.Type.NULL, 16 * 1024);
    ...
    private class BInfo implements ReplicaInPipeline {
        ...
        @Override
        synchronized public ReplicaOutputStreams createStreams(boolean isCreate, DataChecksum requestedChecksum) throws IOException {
            if (finalized) {
                throw new IOException("Trying to write to a finalized replica " + theBlock);
            } else {
                SimulatedOutputStream crcStream = new SimulatedOutputStream();
                return new ReplicaOutputStreams(oStream, crcStream, requestedChecksum, getStorage(theBlock).getVolume(), fileIoProvider);
            }
        }
    ...
    static class SimulatedStorage {
        ...
        SimulatedStorage(long cap, DatanodeStorage.State state, FileIoProvider fileIoProvider, Configuration conf) {
            capacity = cap;
            dnStorage = new DatanodeStorage("SimulatedStorage-" + DatanodeStorage.generateUuid(), state, StorageType.DEFAULT);
            DataNodeVolumeMetrics volumeMetrics = DataNodeVolumeMetrics.create(conf, dnStorage.getStorageID());
        ...
        synchronized StorageReport getStorageReport(String bpid) {
            return new StorageReport(dnStorage, false, getCapacity(), getUsed(), getFree(), map.get(bpid).getUsed(), 0L);
        }
    ...
    static class SimulatedVolume implements FsVolumeSpi {
        ...
        SimulatedVolume(final SimulatedStorage storage, final FileIoProvider fileIoProvider, final DataNodeVolumeMetrics metrics) {
    ...
    public SimulatedFSDataset(DataNode datanode, DataStorage storage, Configuration conf) {
        ...
        for (int i = 0; i < storageCount; i++) {
            this.storages.add(new SimulatedStorage(conf.getLong(CONFIG_PROPERTY_CAPACITY, DEFAULT_CAPACITY), conf.getEnum(CONFIG_PROPERTY_STATE, DEFAULT_STATE), fileIoProvider, conf));
        }
    }
    ...
    // {@link FsDatasetSpi}
    @Override
    public void checkBlock(ExtendedBlock b, long minLength, ReplicaState state) throws ReplicaNotFoundException, UnexpectedReplicaStateException {
