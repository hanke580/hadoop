...
public class TestDFSUtil {
    ...
    public void checkNameServiceId(Configuration conf, String addr, String expectedNameServiceId) {
        ...
        String nameserviceId = DFSUtil.getNameServiceIdFromAddress(conf, s, DFS_NAMENODE_SERVICE_RPC_ADDRESS_KEY, DFS_NAMENODE_RPC_ADDRESS_KEY);
    ...
    public void testGetNamenodeWebAddr() {
        HdfsConfiguration conf = new HdfsConfiguration();
        conf.set(DFSUtil.addKeySuffixes(DFS_NAMENODE_HTTPS_ADDRESS_KEY, "ns1", "nn1"), NS1_NN1_HTTPS_ADDR);
        conf.set(DFSUtil.addKeySuffixes(DFS_NAMENODE_HTTP_ADDRESS_KEY, "ns1", "nn1"), NS1_NN1_HTTP_ADDR);
        conf.set(DFS_HTTP_POLICY_KEY, HttpConfig.Policy.HTTPS_ONLY.name());
        String httpsOnlyWebAddr = DFSUtil.getNamenodeWebAddr(conf, "ns1", "nn1");
        assertEquals(NS1_NN1_HTTPS_ADDR, httpsOnlyWebAddr);
        conf.set(DFS_HTTP_POLICY_KEY, HttpConfig.Policy.HTTP_ONLY.name());
        String httpOnlyWebAddr = DFSUtil.getNamenodeWebAddr(conf, "ns1", "nn1");
        assertEquals(NS1_NN1_HTTP_ADDR, httpOnlyWebAddr);
        conf.set(DFS_HTTP_POLICY_KEY, HttpConfig.Policy.HTTP_AND_HTTPS.name());
        String httpAndHttpsWebAddr = DFSUtil.getNamenodeWebAddr(conf, "ns1", "nn1");
    ...
    private static Configuration createWebHDFSHAConfiguration(String logicalHostName, String nnaddr1, String nnaddr2) {
        ...
        conf.set(HdfsClientConfigKeys.Failover.PROXY_PROVIDER_KEY_PREFIX + "." + logicalHostName, ConfiguredFailoverProxyProvider.class.getName());
        return conf;
    }
    ...
    public void testDurationToString() throws Exception {
        ...
        assertEquals("001:01:01:01.000", DFSUtil.durationToString(((24 * 60 * 60) + (60 * 60) + (60) + 1) * 1000));
        assertEquals("000:23:59:59.999", DFSUtil.durationToString(((23 * 60 * 60) + (59 * 60) + (59)) * 1000 + 999));
        assertEquals("-001:01:01:01.000", DFSUtil.durationToString(-((24 * 60 * 60) + (60 * 60) + (60) + 1) * 1000));
        assertEquals("-000:23:59:59.574", DFSUtil.durationToString(-(((23 * 60 * 60) + (59 * 60) + (59)) * 1000 + 574)));
    }
    ...
    public void testFileIdPath() throws Throwable {
        ...
        for (int i = 0; i < 100; ++i) {
            ...
            assertEquals(new Path(prefix + inode), DFSUtilClient.makePathFromFileId(inode));
        }
    }
    ...
    public void testErrorMessageForInvalidNameservice() throws Exception {
        ...
        String expectedErrorMessage = "Incorrect configuration: namenode address " + DFS_NAMENODE_SERVICE_RPC_ADDRESS_KEY + ".[ns1, ns2]" + " or " + DFS_NAMENODE_RPC_ADDRESS_KEY + ".[ns1, ns2]" + " is not configured.";
        LambdaTestUtils.intercept(IOException.class, expectedErrorMessage, () -> DFSUtil.getNNServiceRpcAddressesForCluster(conf));
    }
}
