...
public class AppendTestUtil {
     ...
     */
    public static FileSystem createHdfsWithDifferentUsername(final Configuration conf) throws IOException, InterruptedException {
    ...
    public static FSDataOutputStream createFile(FileSystem fileSys, Path name, int repl) throws IOException {
        return fileSys.create(name, true, fileSys.getConf().getInt(CommonConfigurationKeys.IO_FILE_BUFFER_SIZE_KEY, 4096), (short) repl, BLOCK_SIZE);
    }

    public static void checkFullFile(FileSystem fs, Path file, int len, final byte[] compareContent) throws IOException {
     ...
     *  @throws IOException an exception might be thrown
     */
    public static void checkFullFile(FileSystem fs, Path name, int len, final byte[] compareContent, String message) throws IOException {
    ...
    private static void checkData(final byte[] actual, int from, final byte[] expected, String message) {
