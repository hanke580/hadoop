...
public class ErasureCodeBenchmarkThroughput extends Configured implements Tool {
    ...
    private void setReadThreadPoolSize(int numClients) {
        ...
        getConf().setInt(HdfsClientConfigKeys.StripedRead.THREADPOOL_SIZE_KEY, numThread);
    }
    ...
    private void setUpDir() throws IOException {
        ...
        if (!dfs.exists(repPath)) {
            ...
            Preconditions.checkArgument(dfs.getClient().getErasureCodingPolicy(repPath.toString()) == null);
        }
        if (!dfs.exists(ecPath)) {
            dfs.mkdirs(ecPath);
            dfs.getClient().setErasureCodingPolicy(ecPath.toString(), ecPolicy.getName());
        } else {
            Preconditions.checkArgument(dfs.getClient().getErasureCodingPolicy(ecPath.toString()).equals(ecPolicy));
        }
    }
    ...
    private class ReadCallable extends CallableBase {
        ...
        public Long call() throws Exception {
            ...
            long dataSize = dataSizeMB * 1024 * 1024L;
            Preconditions.checkArgument(bytesRead == dataSize, "Specified data size: " + dataSize + ", actually read " + bytesRead);
            return bytesRead;
        }
    }
